/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/CloudKit.framework/CloudKit
 */

@interface CKAsset : NSObject <CKDDownloadAssetPreparation, CKDUploadAssetPreparation, CKRecordValue, HMBModelObjectCoder, NSSecureCoding> {
    NSString * _UUID;
    long long  _arrayIndex;
    NSDictionary * _assetChunkerOptions;
    NSData * _assetContent;
    NSData * _assetKey;
    CKAssetReference * _assetReference;
    CKAssetRereferenceInfo * _assetRereferenceInfo;
    CKAssetTransferOptions * _assetTransferOptions;
    NSString * _authToken;
    NSData * _boundaryKey;
    NSData * _clearAssetKey;
    NSData * _constructedAssetDownloadParameters;
    NSURL * _constructedAssetDownloadURL;
    NSString * _constructedAssetDownloadURLTemplate;
    unsigned long long  _constructedAssetEstimatedSize;
    NSURL * _contentBaseURL;
    NSNumber * _deviceID;
    NSString * _downloadBaseURL;
    CKAssetDownloadPreauthorization * _downloadPreauthorization;
    unsigned long long  _downloadTokenExpiration;
    NSDate * _downloadURLExpiration;
    NSString * _downloadURLTemplate;
    bool  _downloaded;
    bool  _exposeAssetKey;
    NSFileHandle * _fileHandle;
    NSNumber * _fileID;
    NSURL * _fileURL;
    NSNumber * _generationCountToSave;
    bool  _hasSize;
    NSString * _itemTypeHint;
    CKMediaItemMaker * _mediaItemMaker;
    CKMergeableDeltaID * _mergeableDeltaID;
    NSString * _owner;
    unsigned long long  _paddedFileSize;
    unsigned long long  _reReferenceItemID;
    NSURL * _realPathURL;
    CKRecord * _record;
    CKRecordID * _recordID;
    NSString * _recordKey;
    NSData * _referenceSignature;
    NSString * _requestor;
    bool  _shouldReadAssetContentUsingClientProxy;
    bool  _shouldReadRawEncryptedData;
    NSData * _signature;
    unsigned long long  _size;
    long long  _storageGroupingPolicy;
    long long  _uploadRank;
    NSString * _uploadReceipt;
    double  _uploadReceiptExpiration;
    bool  _uploaded;
    NSData * _verificationKey;
    bool  _wasCached;
    NSData * _wrappedAssetKey;
}

@property (copy) NSString *UUID;
@property long long arrayIndex;
@property (copy) NSDictionary *assetChunkerOptions;
@property (copy) NSData *assetContent;
@property (copy) NSData *assetKey;
@property (copy) CKAssetReference *assetReference;
@property (retain) CKAssetRereferenceInfo *assetRereferenceInfo;
@property (copy) CKAssetTransferOptions *assetTransferOptions;
@property (copy) NSString *authToken;
@property (copy) NSData *boundaryKey;
@property (copy) NSData *clearAssetKey;
@property (copy) NSData *constructedAssetDownloadParameters;
@property (copy) NSURL *constructedAssetDownloadURL;
@property (copy) NSString *constructedAssetDownloadURLTemplate;
@property unsigned long long constructedAssetEstimatedSize;
@property (copy) NSURL *contentBaseURL;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (nonatomic, readonly, copy) NSNumber *deviceID;
@property (copy) NSString *downloadBaseURL;
@property (retain) CKAssetDownloadPreauthorization *downloadPreauthorization;
@property unsigned long long downloadTokenExpiration;
@property (copy) NSDate *downloadURLExpiration;
@property (copy) NSString *downloadURLTemplate;
@property bool downloaded;
@property bool exposeAssetKey;
@property (retain) NSFileHandle *fileHandle;
@property (nonatomic, readonly, copy) NSNumber *fileID;
@property (copy) NSURL *fileURL;
@property (copy) NSNumber *generationCountToSave;
@property bool hasSize;
@property (readonly) unsigned long long hash;
@property (readonly) bool isConstructedAsset;
@property (copy) NSString *itemTypeHint;
@property (retain) CKMediaItemMaker *mediaItemMaker;
@property (copy) CKMergeableDeltaID *mergeableDeltaID;
@property (copy) NSURL *nullableFileURL;
@property (copy) NSString *owner;
@property unsigned long long paddedFileSize;
@property (nonatomic) unsigned long long reReferenceItemID;
@property (copy) NSURL *realPathURL;
@property CKRecord *record;
@property (copy) CKRecordID *recordID;
@property (copy) NSString *recordKey;
@property (copy) NSData *referenceSignature;
@property (copy) NSString *requestor;
@property (nonatomic, readonly) bool shouldReadAssetContentUsingClientProxy;
@property bool shouldReadRawEncryptedData;
@property (copy) NSData *signature;
@property unsigned long long size;
@property long long storageGroupingPolicy;
@property (readonly) Class superclass;
@property long long uploadRank;
@property (copy) NSString *uploadReceipt;
@property double uploadReceiptExpiration;
@property bool uploaded;
@property (retain) NSData *verificationKey;
@property bool wasCached;
@property (copy) NSData *wrappedAssetKey;

// Image: /System/Library/Frameworks/CloudKit.framework/CloudKit

+ (id)_canonicalizeTemplateURL:(id)arg1;
+ (id)_downloadURLWithTemplate:(id)arg1 fileName:(id)arg2;
+ (id)_expandTemplateURL:(id)arg1 fieldValues:(id)arg2;
+ (id)_openUnencryptedWithOpenInfo:(id)arg1 genCountCheck:(bool)arg2 error:(id*)arg3;
+ (bool)_templateURLRequiresAssetKey:(id)arg1;
+ (id)assetWithFileURL:(id)arg1;
+ (int)errorCodeFromPOSIXCode:(int)arg1;
+ (id)getFileMetadataAtPath:(id)arg1 error:(id*)arg2;
+ (id)getFileMetadataWithFileHandle:(id)arg1 openInfo:(id)arg2 error:(id*)arg3;
+ (id)getFileSizeWithOpenInfo:(id)arg1 error:(id*)arg2;
+ (void)initialize;
+ (id)openToReadRawEncryptedDataWithOpenInfo:(id)arg1 error:(id*)arg2;
+ (id)openWithOpenInfo:(id)arg1 error:(id*)arg2;
+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)CKDescriptionPropertiesWithPublic:(bool)arg1 private:(bool)arg2 shouldExpand:(bool)arg3;
- (id)UUID;
- (long long)arrayIndex;
- (id)assetChunkerOptions;
- (id)assetContent;
- (id)assetHandleUUID;
- (id)assetKey;
- (id)assetReference;
- (id)assetRereferenceInfo;
- (id)assetTransferOptions;
- (id)authToken;
- (id)boundaryKey;
- (id)clearAssetKey;
- (id)constructedAssetDownloadParameters;
- (id)constructedAssetDownloadURL;
- (id)constructedAssetDownloadURLTemplate;
- (unsigned long long)constructedAssetEstimatedSize;
- (id)contentBaseURL;
- (id)description;
- (id)deviceID;
- (id)downloadBaseURL;
- (id)downloadPreauthorization;
- (unsigned long long)downloadTokenExpiration;
- (id)downloadURL;
- (id)downloadURLExpiration;
- (id)downloadURLTemplate;
- (id)downloadURLWithFileName:(id)arg1;
- (bool)downloaded;
- (void)encodeWithCoder:(id)arg1;
- (bool)exposeAssetKey;
- (id)fileHandle;
- (id)fileID;
- (id)fileURL;
- (id)generationCountToSave;
- (bool)hasSize;
- (id)init;
- (id)initInternal;
- (id)initWithAssetContent:(id)arg1 itemTypeHint:(id)arg2;
- (id)initWithAssetReference:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (id)initWithDeviceID:(id)arg1 fileID:(id)arg2 generationID:(id)arg3;
- (id)initWithFileDescriptor:(int)arg1;
- (id)initWithFileURL:(id)arg1;
- (id)initWithFileURL:(id)arg1 signature:(id)arg2;
- (id)initWithFileURL:(id)arg1 signature:(id)arg2 assetHandleUUID:(id)arg3;
- (bool)isConstructedAsset;
- (bool)isReference;
- (bool)isRereferencedAssetUpload;
- (id)itemTypeHint;
- (id)mediaItemMaker;
- (id)mergeableDeltaID;
- (id)nullableFileURL;
- (id)openWithError:(id*)arg1;
- (id)owner;
- (unsigned long long)paddedFileSize;
- (unsigned long long)reReferenceItemID;
- (id)realPathURL;
- (id)record;
- (id)recordID;
- (id)recordKey;
- (id)referenceSignature;
- (id)requestor;
- (void)setArrayIndex:(long long)arg1;
- (void)setAssetChunkerOptions:(id)arg1;
- (void)setAssetContent:(id)arg1;
- (void)setAssetContent_modelMutation:(id)arg1;
- (void)setAssetKey:(id)arg1;
- (void)setAssetReference:(id)arg1;
- (void)setAssetRereferenceInfo:(id)arg1;
- (void)setAssetTransferOptions:(id)arg1;
- (void)setAuthToken:(id)arg1;
- (void)setBoundaryKey:(id)arg1;
- (void)setClearAssetKey:(id)arg1;
- (void)setConstructedAssetDownloadParameters:(id)arg1;
- (void)setConstructedAssetDownloadURL:(id)arg1;
- (void)setConstructedAssetDownloadURLTemplate:(id)arg1;
- (void)setConstructedAssetEstimatedSize:(unsigned long long)arg1;
- (void)setContentBaseURL:(id)arg1;
- (void)setDeviceID_modelMutation:(id)arg1;
- (void)setDownloadBaseURL:(id)arg1;
- (void)setDownloadPreauthorization:(id)arg1;
- (void)setDownloadTokenExpiration:(unsigned long long)arg1;
- (void)setDownloadURLExpiration:(id)arg1;
- (void)setDownloadURLTemplate:(id)arg1;
- (void)setDownloaded:(bool)arg1;
- (void)setExposeAssetKey:(bool)arg1;
- (void)setFileHandle:(id)arg1;
- (void)setFileID_modelMutation:(id)arg1;
- (void)setFileURL:(id)arg1;
- (void)setGenerationCountToSave:(id)arg1;
- (void)setHasSize:(bool)arg1;
- (void)setItemTypeHint:(id)arg1;
- (void)setMediaItemMaker:(id)arg1;
- (void)setMergeableDeltaID:(id)arg1;
- (void)setNullableFileURL:(id)arg1;
- (void)setOwner:(id)arg1;
- (void)setPaddedFileSize:(unsigned long long)arg1;
- (void)setReReferenceItemID:(unsigned long long)arg1;
- (void)setRealPathURL:(id)arg1;
- (void)setRecord:(id)arg1;
- (void)setRecordID:(id)arg1;
- (void)setRecordKey:(id)arg1;
- (void)setReferenceSignature:(id)arg1;
- (void)setRequestor:(id)arg1;
- (void)setShouldReadRawEncryptedData:(bool)arg1;
- (void)setSignature:(id)arg1;
- (void)setSize:(unsigned long long)arg1;
- (void)setStorageGroupingPolicy:(long long)arg1;
- (void)setUUID:(id)arg1;
- (void)setUploadRank:(long long)arg1;
- (void)setUploadReceipt:(id)arg1;
- (void)setUploadReceiptExpiration:(double)arg1;
- (void)setUploaded:(bool)arg1;
- (void)setVerificationKey:(id)arg1;
- (void)setWasCached:(bool)arg1;
- (void)setWrappedAssetKey:(id)arg1;
- (bool)shouldReadAssetContentUsingClientProxy;
- (bool)shouldReadRawEncryptedData;
- (id)signature;
- (unsigned long long)size;
- (long long)storageGroupingPolicy;
- (long long)uploadRank;
- (id)uploadReceipt;
- (double)uploadReceiptExpiration;
- (bool)uploaded;
- (bool)useMMCSEncryptionV2;
- (id)verificationKey;
- (bool)wasCached;
- (id)wrappedAssetKey;

// Image: /System/Library/PrivateFrameworks/CloudDocsDaemon.framework/CloudDocsDaemon

+ (id)br_assetWithAssetReferece:(id)arg1 boundaryKey:(id)arg2;
+ (id)br_assetWithDeviceID:(id)arg1 fileID:(id)arg2 generationID:(id)arg3 boundaryKey:(id)arg4;
+ (id)br_assetWithDeviceID:(id)arg1 fileID:(id)arg2 generationID:(id)arg3 useMMCSEncryptionV2:(bool)arg4;
+ (id)br_assetWithFileURL:(id)arg1 boundaryKey:(id)arg2;
+ (id)br_assetWithFileURL:(id)arg1 useMMCSEncryptionV2:(bool)arg2;
+ (id)transferOptionsWithMMCSV2:(bool)arg1;

// Image: /System/Library/PrivateFrameworks/CloudKitDaemon.framework/CloudKitDaemon

- (void)_prepareForDownloadWithOperation:(id)arg1;
- (id)_prepareForUploadWithOperation:(id)arg1;
- (bool)fillInDownloadURLsForAssetWithFieldName:(id)arg1 fileName:(id)arg2 recordPCS:(struct _OpaquePCSShareProtection { }*)arg3 pcsManager:(id)arg4 useEncryption:(bool)arg5 useClearAssetEncryption:(bool)arg6 alwaysAllowKeyExport:(bool)arg7 outError:(id*)arg8;

// Image: /System/Library/PrivateFrameworks/HomeKitBackingStore.framework/HomeKitBackingStore

+ (id)hmbDecodeData:(id)arg1 fromStorageLocation:(unsigned long long)arg2 error:(id*)arg3;

- (id)hmbEncodeForStorageLocation:(unsigned long long)arg1 error:(id*)arg2;

// Image: /System/Library/PrivateFrameworks/iCloudDriveCore.framework/iCloudDriveCore

+ (id)br_assetWithAssetReferece:(id)arg1 boundaryKey:(id)arg2;
+ (id)br_assetWithDeviceID:(id)arg1 fileID:(id)arg2 generationID:(id)arg3 boundaryKey:(id)arg4;
+ (id)br_assetWithDeviceID:(id)arg1 fileID:(id)arg2 generationID:(id)arg3 useMMCSEncryptionV2:(bool)arg4;
+ (id)br_assetWithFileURL:(id)arg1 boundaryKey:(id)arg2;
+ (id)br_assetWithFileURL:(id)arg1 useMMCSEncryptionV2:(bool)arg2;
+ (id)transferOptionsWithMMCSV2:(bool)arg1;

@end
