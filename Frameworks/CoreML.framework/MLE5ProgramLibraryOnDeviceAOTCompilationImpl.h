/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/CoreML.framework/CoreML
 */

@interface MLE5ProgramLibraryOnDeviceAOTCompilationImpl : NSObject <MLE5ProgramLibraryImpl> {
    MLModelConfiguration * _configuration;
    MLProgramE5Container * _container;
    id /* block */  _deallocator;
    struct shared_ptr<MIL::IRProgram> { 
        struct IRProgram {} *__ptr_; 
        struct __shared_weak_count {} *__cntrl_; 
    }  _irProgram;
    NSURL * _milTextURL;
    NSString * _serializedMILText;
}

@property (nonatomic, readonly) MLModelConfiguration *configuration;
@property (nonatomic, readonly) MLProgramE5Container *container;
@property (nonatomic, readonly, copy) NSString *modelDisplayName;
@property (nonatomic, copy) NSString *serializedMILText;

- (id).cxx_construct;
- (void).cxx_destruct;
- (id)configuration;
- (id)container;
- (struct e5rt_program_library { }*)createProgramLibraryHandleWithRespecialization:(bool)arg1 error:(id*)arg2;
- (void)dealloc;
- (id)initWithIRProgram:(void*)arg1 container:(id)arg2 configuration:(id)arg3 deallocator:(id /* block */)arg4;
- (id)initWithMILTextAtURL:(id)arg1 container:(id)arg2 configuration:(id)arg3;
- (id)initWithMILTextAtURL:(id)arg1 irProgram:(void*)arg2 deallocator:(id /* block */)arg3 container:(id)arg4 configuration:(id)arg5;
- (id)modelDisplayName;
- (id)serializedMILText;
- (void)setSerializedMILText:(id)arg1;

@end
