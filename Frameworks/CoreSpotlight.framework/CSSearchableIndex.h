/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/CoreSpotlight.framework/CoreSpotlight
 */

@interface CSSearchableIndex : NSObject {
    CSIndexingQueue * _activityQueue;
    bool  _batchOpen;
    NSMutableArray * _batchedItemIdentifiersToDelete;
    NSMutableArray * _batchedItemsToIndex;
    NSString * _bundleIdentifier;
    NSObject<OS_dispatch_queue> * _delegateQueue;
    <CSSearchableIndexDelegate> * _indexDelegate;
    NSNumber * _indexID;
    NSString * _name;
    bool  _noBatching;
    long long  _options;
    NSString * _protectionClass;
    unsigned int  _user;
}

@property (nonatomic, readonly) CSIndexingQueue *activityQueue;
@property (nonatomic) bool batchOpen;
@property (nonatomic, retain) NSMutableArray *batchedItemIdentifiersToDelete;
@property (nonatomic, retain) NSMutableArray *batchedItemsToIndex;
@property (nonatomic, copy) NSString *bundleIdentifier;
@property (readonly) CSIndexConnection *connection;
@property (nonatomic, retain) NSObject<OS_dispatch_queue> *delegateQueue;
@property <CSSearchableIndexDelegate> *indexDelegate;
@property (nonatomic, readonly) NSNumber *indexID;
@property (nonatomic, copy) NSString *name;
@property (nonatomic) bool noBatching;
@property (nonatomic) long long options;
@property (nonatomic, copy) NSString *protectionClass;

// Image: /System/Library/Frameworks/CoreSpotlight.framework/CoreSpotlight

+ (void)_setLastUpdateTime;
+ (bool)activityShouldBeIndexed:(id)arg1 bundleID:(id)arg2;
+ (id)allSupportedProtectionClasses;
+ (id)codedIdentifiers:(id)arg1;
+ (id)codedNSUAPersistentIdentifiers:(id)arg1;
+ (id)codedUniqueIdentifiers:(id)arg1;
+ (id)computePartialPathWithOID:(id)arg1 pathKeyIdx:(unsigned long long)arg2 pathDictionary:(id)arg3 attributeValues:(id)arg4 depth:(long long)arg5;
+ (id)defaultSearchableIndex;
+ (void)initialize;
+ (bool)isIndexingAvailable;
+ (double)lastUpdateTime;
+ (id)mainBundleID;
+ (id)mainBundleLocalizedString;
+ (void)notifyIndexDelegates;
+ (id)partialPathAttributes;
+ (void)setConnectionName:(id)arg1;
+ (void)volumeMountedAtURL:(id)arg1 withOptions:(id)arg2;

- (void).cxx_destruct;
- (void)_bulkFetchPartialPathsForObjects:(id)arg1 protectionClass:(id)arg2 bundleID:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)_changeStateOfSearchableItemsWithUIDs:(id)arg1 toState:(long long)arg2 forUser:(unsigned int)arg3 forBundleID:(id)arg4 forUTIType:(id)arg5;
- (void)_commonInit;
- (void)_delegateCheckIn:(id)arg1;
- (void)_deleleActionsWithIdentifiers:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)_deleteActionsBeforeTime:(double)arg1 completionHandler:(id /* block */)arg2;
- (void)_fetchAttributes:(id)arg1 protectionClass:(id)arg2 bundleID:(id)arg3 items:(id)arg4 includeParents:(bool)arg5 options:(long long)arg6 completionHandler:(id /* block */)arg7;
- (void)_fetchCacheFileDescriptorsForProtectionClass:(id)arg1 bundleID:(id)arg2 items:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)_indexActivities:(id)arg1 flush:(bool)arg2;
- (id)_initWithName:(id)arg1 protectionClass:(id)arg2 bundleIdentifier:(id)arg3 options:(long long)arg4;
- (id)_initWithName:(id)arg1 protectionClass:(id)arg2 bundleIdentifier:(id)arg3 options:(long long)arg4 disableBatching:(bool)arg5;
- (void)_issueCommand:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)_issueDiagnose:(int)arg1 logQuery:(bool)arg2 completionHandler:(id /* block */)arg3;
- (void)_issueNonLaunchingCommand:(id)arg1;
- (id)_itemsBySanitizingItemsForSpotlight:(id)arg1;
- (void)_makeActivityQueueIfNecessary;
- (void)_openJournalFile:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)_photosLibraryDeleted:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)_setFPAttributes:(id)arg1;
- (void)_setMailMessageAttributes:(id)arg1;
- (void)_setUser:(unsigned int)arg1;
- (long long)_standardizeItems:(id)arg1;
- (id)_validateClientState:(id)arg1;
- (id)_validateOperation;
- (id)_validateOperationWithIdentifiers:(id)arg1;
- (id)_validateOperationWithItems:(id)arg1;
- (id)_validateOperationWithItems:(id)arg1 identifiers:(id)arg2;
- (id)activityQueue;
- (void)addInteraction:(id)arg1 bundleID:(id)arg2 protectionClass:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)addInteraction:(id)arg1 completionHandler:(id /* block */)arg2;
- (bool)batchOpen;
- (id)batchedItemIdentifiersToDelete;
- (id)batchedItemsToIndex;
- (void)beginIndexBatch;
- (void)bulkFetchAttributes:(id)arg1 protectionClass:(id)arg2 bundleID:(id)arg3 objects:(id)arg4 attributeKeyIndex:(long long)arg5 includeParents:(bool)arg6 completionHandler:(id /* block */)arg7;
- (void)bulkFetchCacheFileDescriptorForProtectionClass:(id)arg1 bundleID:(id)arg2 identifiers:(id)arg3 reason:(long long)arg4 completionHandler:(id /* block */)arg5;
- (id)bundleIdentifier;
- (void)changeStateOfSearchableItemsWithUIDs:(id)arg1 toState:(long long)arg2;
- (id)connection;
- (id)criticalQueue;
- (void)dealloc;
- (id)delegateQueue;
- (void)deleteAllInteractionsWithBundleID:(id)arg1 protectionClass:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)deleteAllInteractionsWithCompletionHandler:(id /* block */)arg1;
- (void)deleteAllSearchableItemsForBundleID:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteAllSearchableItemsWithCompletionHandler:(id /* block */)arg1;
- (void)deleteAllSearchableItemsWithProtectionClass:(id)arg1 forBundleID:(id)arg2 options:(long long)arg3 completionHandler:(id /* block */)arg4;
- (void)deleteAllSearchableItemsWithProtectionClass:(id)arg1 forBundleID:(id)arg2 options:(long long)arg3 reason:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)deleteAllSearchableItemsWithReason:(long long)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteAllUserActivities:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteInteractionsWithGroupIdentifiers:(id)arg1 bundleID:(id)arg2 protectionClass:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)deleteInteractionsWithGroupIdentifiers:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteInteractionsWithIdentifiers:(id)arg1 bundleID:(id)arg2 protectionClass:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)deleteInteractionsWithIdentifiers:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteSearchableItemsSinceDate:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteSearchableItemsSinceDate:(id)arg1 protectionClass:(id)arg2 forBundleID:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)deleteSearchableItemsWithDomainIdentifiers:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteSearchableItemsWithDomainIdentifiers:(id)arg1 protectionClass:(id)arg2 forBundleID:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)deleteSearchableItemsWithDomainIdentifiers:(id)arg1 protectionClass:(id)arg2 forBundleID:(id)arg3 options:(long long)arg4 reason:(long long)arg5 completionHandler:(id /* block */)arg6;
- (void)deleteSearchableItemsWithDomainIdentifiers:(id)arg1 reason:(long long)arg2 completionHandler:(id /* block */)arg3;
- (void)deleteSearchableItemsWithIdentifiers:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)deleteSearchableItemsWithIdentifiers:(id)arg1 reason:(long long)arg2 completionHandler:(id /* block */)arg3;
- (void)deleteUserActivitiesWithPersistentIdentifiers:(id)arg1 bundleID:(id)arg2 completionHandler:(id /* block */)arg3;
- (id)description;
- (void)donateRelevantActions:(id)arg1 bundleID:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)donateRelevantShortcuts:(id)arg1 bundleID:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)endIndexBatchWithClientState:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)endIndexBatchWithClientState:(id)arg1 critical:(bool)arg2 completionHandler:(id /* block */)arg3;
- (void)endIndexBatchWithClientState:(id)arg1 critical:(bool)arg2 reason:(long long)arg3 completionHandler:(id /* block */)arg4;
- (void)endIndexBatchWithExpectedClientState:(id)arg1 newClientState:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)endIndexBatchWithExpectedClientState:(id)arg1 newClientState:(id)arg2 critical:(bool)arg3 completionHandler:(id /* block */)arg4;
- (void)endIndexBatchWithExpectedClientState:(id)arg1 newClientState:(id)arg2 critical:(bool)arg3 reason:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)endIndexBatchWithExpectedClientState:(id)arg1 newClientState:(id)arg2 reason:(long long)arg3 completionHandler:(id /* block */)arg4;
- (void)fetchDataForBundleIdentifier:(id)arg1 itemIdentifier:(id)arg2 contentType:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)fetchLastClientStateWithCompletionHandler:(id /* block */)arg1;
- (void)fetchLastClientStateWithProtectionClass:(id)arg1 forBundleID:(id)arg2 clientStateName:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)finishIndexingWhileLocked:(id)arg1 protectionClass:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)flushUserActivities;
- (id)indexDelegate;
- (id)indexID;
- (void)indexSearchableItems:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 clientStateName:(id)arg4 protectionClass:(id)arg5 forBundleID:(id)arg6 options:(long long)arg7 completionHandler:(id /* block */)arg8;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 clientStateName:(id)arg4 protectionClass:(id)arg5 forBundleID:(id)arg6 options:(long long)arg7 reason:(long long)arg8 completionHandler:(id /* block */)arg9;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 expectedClientState:(id)arg4 clientStateName:(id)arg5 protectionClass:(id)arg6 forBundleID:(id)arg7 options:(long long)arg8 reason:(long long)arg9 completionHandler:(id /* block */)arg10;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 expectedClientState:(id)arg4 protectionClass:(id)arg5 forBundleID:(id)arg6 options:(long long)arg7 completionHandler:(id /* block */)arg8;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 expectedClientState:(id)arg4 protectionClass:(id)arg5 forBundleID:(id)arg6 options:(long long)arg7 reason:(long long)arg8 completionHandler:(id /* block */)arg9;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 protectionClass:(id)arg4 forBundleID:(id)arg5 options:(long long)arg6 completionHandler:(id /* block */)arg7;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 protectionClass:(id)arg4 forBundleID:(id)arg5 options:(long long)arg6 reason:(long long)arg7 completionHandler:(id /* block */)arg8;
- (void)indexSearchableItems:(id)arg1 deleteSearchableItemsWithIdentifiers:(id)arg2 clientState:(id)arg3 reason:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)indexSearchableItems:(id)arg1 returningItemsSanitizedForSpotlightTo:(id /* block */)arg2;
- (void)indexUserActivity:(id)arg1;
- (id)init;
- (id)initWithName:(id)arg1;
- (id)initWithName:(id)arg1 bundleIdentifier:(id)arg2;
- (id)initWithName:(id)arg1 options:(long long)arg2;
- (id)initWithName:(id)arg1 protectionClass:(id)arg2;
- (id)initWithName:(id)arg1 protectionClass:(id)arg2 bundleIdentifier:(id)arg3;
- (id)initWithName:(id)arg1 protectionClass:(id)arg2 bundleIdentifier:(id)arg3 options:(long long)arg4;
- (bool)isEmbeddingVersionCurrent:(id)arg1 error:(id*)arg2;
- (void)issueCacheCommand:(id)arg1 completionHandler:(id /* block */)arg2;
- (id)name;
- (bool)noBatching;
- (long long)options;
- (void)performDataMigrationWithTimeout:(double)arg1 completionHandler:(id /* block */)arg2;
- (void)performIndexJob:(id)arg1 protectionClass:(id)arg2 acknowledgementHandler:(id /* block */)arg3;
- (void)prepareIndexingWhileLocked:(id)arg1 protectionClass:(id)arg2 holdAssertionFor:(double)arg3 completionHandler:(id /* block */)arg4;
- (id)protectionClass;
- (void)provideDataForBundle:(id)arg1 identifier:(id)arg2 type:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)provideDataForBundle:(id)arg1 itemIdentifier:(id)arg2 typeIdentifier:(id)arg3 allowDownload:(bool)arg4 completionHandler:(id /* block */)arg5;
- (void)provideDataForBundle:(id)arg1 itemIdentifier:(id)arg2 typeIdentifier:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)provideFileURLForBundle:(id)arg1 identifier:(id)arg2 type:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)provideFileURLForBundle:(id)arg1 itemIdentifier:(id)arg2 typeIdentifier:(id)arg3 inPlace:(bool)arg4 allowDownload:(bool)arg5 completionHandler:(id /* block */)arg6;
- (void)provideFileURLForBundle:(id)arg1 itemIdentifier:(id)arg2 typeIdentifier:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (void)provideFileURLsForBundle:(id)arg1 itemIdentifiers:(id)arg2 typeIdentifier:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (id)requestQueue;
- (void)setBatchOpen:(bool)arg1;
- (void)setBatchedItemIdentifiersToDelete:(id)arg1;
- (void)setBatchedItemsToIndex:(id)arg1;
- (void)setBundleIdentifier:(id)arg1;
- (void)setDelegateQueue:(id)arg1;
- (void)setIndexDelegate:(id)arg1;
- (void)setName:(id)arg1;
- (void)setNoBatching:(bool)arg1;
- (void)setOptions:(long long)arg1;
- (void)setProtectionClass:(id)arg1;
- (void)slowFetchAttributes:(id)arg1 protectionClass:(id)arg2 bundleID:(id)arg3 identifiers:(id)arg4 completionHandler:(id /* block */)arg5;
- (void)slowFetchAttributes:(id)arg1 protectionClass:(id)arg2 bundleID:(id)arg3 identifiers:(id)arg4 options:(long long)arg5 completionHandler:(id /* block */)arg6;
- (void)throttle;
- (id)throttleQueue;
- (void)transferDeletionJournalsForProtectionClass:(id)arg1 toDirectory:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)unthrottle;
- (void)updateCorrectionsWithFilePath:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)updateCorrectionsWithFilePath:(id)arg1 options:(long long)arg2 completionHandler:(id /* block */)arg3;
- (void)willModifySearchableItemsWithIdentifiers:(id)arg1 completionHandler:(id /* block */)arg2;
- (void)willModifySearchableItemsWithIdentifiers:(id)arg1 protectionClass:(id)arg2 forBundleID:(id)arg3 options:(long long)arg4 completionHandler:(id /* block */)arg5;
- (id)xpc_dictionary_for_command:(const char *)arg1 requiresInitialization:(bool)arg2;

// Image: /System/Library/PrivateFrameworks/ProactiveSummarization.framework/ProactiveSummarization

+ (id)psu_criticalIndexWithName:(id)arg1 protectionClass:(id)arg2 bundleIdentifier:(id)arg3;

// Image: /System/Library/PrivateFrameworks/VoiceShortcuts.framework/VoiceShortcuts

- (void)indexSearchableItems:(void *)arg1 withBatchSize:(void *)arg2 updatedIdentifiers:(void *)arg3 completionBlock:(void *)arg4; // needs 4 arg types, found 25: id, /* Warning: Unrecognized filer type: '"' using 'void*' */ void*, inout unsigned short, _Atomic /* Warning: Unrecognized filer type: '' using 'void*' */ void*, const const /* Warning: Unrecognized filer type: 'a' using 'void*' */ void*, /* Warning: Unrecognized filer type: 'y' using 'void*' */ void*, /* Warning: Unrecognized filer type: '"' using 'void*' */ void*, unsigned long long, id, /* Warning: Unrecognized filer type: '"' using 'void*' */ void*, inout unsigned short, unsigned short, /* Warning: Unrecognized filer type: 'e' using 'void*' */ void*, /* Warning: Unrecognized filer type: 't' using 'void*' */ void*, /* Warning: Unrecognized filer type: '"' using 'void*' */ void*, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, id, /* Warning: Unrecognized filer type: '"' using 'void*' */ void*, inout unsigned short, /* Warning: Unrecognized filer type: 'E' using 'void*' */ void*, const const out const /* Warning: Unrecognized filer type: '"' using 'void*' */ void*, /* Warning: Unrecognized filer type: '>' using 'void*' */ void*

@end
