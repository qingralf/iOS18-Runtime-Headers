/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/CoreSpotlight.framework/CoreSpotlight
 */

@interface CSXPCConnection : NSObject {
    NSObject<OS_xpc_object> * _connection;
    bool  _listener;
    bool  _machService;
    bool  _nonLaunching;
    bool  _previouslyInitialized;
    bool  _privateDaemon;
    NSObject<OS_dispatch_queue> * _queue;
    NSString * _serviceName;
    unsigned int  _user;
    NSString * _uuid;
}

@property (nonatomic, retain) NSObject<OS_xpc_object> *connection;
@property (nonatomic, readonly) bool listener;
@property (nonatomic, readonly) bool machService;
@property (nonatomic) bool nonLaunching;
@property (nonatomic) bool previouslyInitialized;
@property (nonatomic) bool privateDaemon;
@property (nonatomic, retain) NSObject<OS_dispatch_queue> *queue;
@property (nonatomic, readonly) NSString *serviceName;
@property (nonatomic) unsigned int user;
@property (nonatomic, readonly) NSString *uuid;

+ (id)appIdentifierFromTeamAppTuple:(id)arg1;
+ (void)array:(id)arg1 setValue:(id)arg2 atIndex:(long long)arg3;
+ (id)constructArray:(id)arg1;
+ (id)constructDictionary:(id)arg1;
+ (bool)copyBoolForKey:(const char *)arg1 fromXPCDictionary:(id)arg2;
+ (double)copyDoubleForKey:(const char *)arg1 fromXPCDictionary:(id)arg2;
+ (id)copyNSArrayFromXPCArray:(id)arg1;
+ (id)copyNSArrayFromXPCObject:(id)arg1;
+ (id)copyNSDataForKey:(const char *)arg1 fromXPCDictionary:(id)arg2;
+ (id)copyNSDictionaryFromXPCObject:(id)arg1;
+ (id)copyNSNumberArrayFromXPCArray:(id)arg1;
+ (id)copyNSObject:(id)arg1;
+ (id)copyNSString:(id)arg1;
+ (id)copyNSStringArrayFromXPCArray:(id)arg1;
+ (id)copyNSStringForKey:(const char *)arg1 fromXPCDictionary:(id)arg2;
+ (id)copyNSStringOrDictArrayFromXPCArray:(id)arg1;
+ (id)copyNSStringSetFromXPCArray:(id)arg1;
+ (id)copyPlistFromXPCObject:(id)arg1;
+ (unsigned long long)copyUInt64ForKey:(const char *)arg1 fromXPCDictionary:(id)arg2;
+ (id)dataWrapperForKey:(const char *)arg1 sizeKey:(const char *)arg2 fromXPCDictionary:(id)arg3;
+ (id)dataWrapperForKey:(const char *)arg1 sizeKey:(const char *)arg2 fromXPCDictionary:(id)arg3 allowWritableSharedMemory:(bool)arg4;
+ (void)dictionary:(id)arg1 setArray:(id)arg2 forKey:(const char *)arg3;
+ (void)dictionary:(id)arg1 setNumberArray:(id)arg2 forKey:(const char *)arg3;
+ (void)dictionary:(id)arg1 setPlistBytes:(struct __MDPlistBytes { }*)arg2 forKey:(const char *)arg3 sizeKey:(const char *)arg4;
+ (void)dictionary:(id)arg1 setPlistContainer:(struct _MDPlistContainer { }*)arg2 forKey:(const char *)arg3 sizeKey:(const char *)arg4;
+ (bool)dictionary:(id)arg1 setSharedMemory:(void*)arg2 forKey:(const char *)arg3 size:(unsigned long long)arg4 forSizeKey:(const char *)arg5;
+ (void)dictionary:(id)arg1 setStringArray:(id)arg2 forKey:(const char *)arg3;
+ (void)dictionary:(id)arg1 setStringOrDictionaryArray:(id)arg2 forKey:(const char *)arg3;
+ (void)dictionary:(id)arg1 setValue:(id)arg2 forKey:(const char *)arg3;
+ (void)dictionary:(id)arg1 setXPCFdArray:(id)arg2 forKey:(const char *)arg3;
+ (void)journalDictionary:(id)arg1 toFolderPath:(const char *)arg2 forPID:(int)arg3 withLabel:(const char *)arg4 andID:(unsigned long long)arg5;
+ (bool)journalEnabled;
+ (id)processNameForPID:(int)arg1;
+ (void)setJournalEnabled:(bool)arg1;

- (void).cxx_destruct;
- (void)_lostClientConnection:(id)arg1 error:(id)arg2;
- (void)_setUser:(unsigned int)arg1;
- (bool)addClientConnectionIfAllowedForConfiguration:(id)arg1;
- (bool)addClientConnectionIfAllowedForConnection:(id)arg1;
- (id)connection;
- (bool)handleCommand:(const char *)arg1 info:(id)arg2 connection:(id)arg3;
- (void)handleError:(id)arg1;
- (bool)handleMessage:(id)arg1 type:(struct _xpc_type_s { }*)arg2 connection:(id)arg3;
- (void)handleReply:(id)arg1;
- (id)initListenerWithName:(id)arg1;
- (id)initMachServiceListenerWithName:(id)arg1;
- (id)initMachServiceListenerWithName:(id)arg1 enableConnectionLogging:(bool)arg2;
- (id)initWithMachServiceName:(id)arg1;
- (id)initWithServiceName:(id)arg1;
- (id)initWithServiceName:(id)arg1 machService:(bool)arg2;
- (id)initWithServiceName:(id)arg1 machService:(bool)arg2 uuid:(id)arg3;
- (bool)listener;
- (bool)lostClientConnection:(id)arg1 error:(id)arg2;
- (bool)machService;
- (bool)nonLaunching;
- (bool)previouslyInitialized;
- (bool)privateDaemon;
- (id)queue;
- (void)resume;
- (void)sendMessageAsync:(id)arg1;
- (void)sendMessageAsync:(id)arg1 completion:(id /* block */)arg2;
- (id)serviceName;
- (void)setConnection:(id)arg1;
- (void)setNonLaunching:(bool)arg1;
- (void)setPreviouslyInitialized:(bool)arg1;
- (void)setPrivateDaemon:(bool)arg1;
- (void)setQueue:(id)arg1;
- (void)setUser:(unsigned int)arg1;
- (void)startListener;
- (void)suspend;
- (unsigned int)user;
- (id)uuid;

@end
