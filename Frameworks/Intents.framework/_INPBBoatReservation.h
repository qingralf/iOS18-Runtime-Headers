/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/Intents.framework/Intents
 */

@interface _INPBBoatReservation : PBCodable <NSCopying, NSSecureCoding, _INPBBoatReservation> {
    _INPBBoatTrip * _boatTrip;
    struct { }  _has;
    _INPBReservation * _reservation;
    _INPBSeat * _reservedSeat;
}

@property (nonatomic, retain) _INPBBoatTrip *boatTrip;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (nonatomic, readonly) bool hasBoatTrip;
@property (nonatomic, readonly) bool hasReservation;
@property (nonatomic, readonly) bool hasReservedSeat;
@property (readonly) unsigned long long hash;
@property (nonatomic, retain) _INPBReservation *reservation;
@property (nonatomic, retain) _INPBSeat *reservedSeat;
@property (readonly) Class superclass;

+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)boatTrip;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)dictionaryRepresentation;
- (void)encodeWithCoder:(id)arg1;
- (bool)hasBoatTrip;
- (bool)hasReservation;
- (bool)hasReservedSeat;
- (unsigned long long)hash;
- (id)initWithCoder:(id)arg1;
- (bool)isEqual:(id)arg1;
- (bool)readFrom:(id)arg1;
- (id)reservation;
- (id)reservedSeat;
- (void)setBoatTrip:(id)arg1;
- (void)setReservation:(id)arg1;
- (void)setReservedSeat:(id)arg1;
- (void)writeTo:(id)arg1;

@end
