/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/LinkPresentation.framework/LinkPresentation
 */

@interface LPLinkView : UIView <CAAnimationDelegate, LPComponentViewHost, LPTapToLoadViewDelegate, LPThemeClient, UIContextMenuInteractionDelegate, UIGestureRecognizerDelegate> {
    NSURL * _URL;
    bool  __disableLinkFollowing;
    bool  __useLowMemoryImageFilters;
    bool  _allowsAsynchronousImageDecoding;
    bool  _allowsLoadingMediaWithAutoPlayDisabled;
    bool  _allowsOpeningSensitiveURLs;
    bool  _allowsSkinnyWidth;
    bool  _allowsTapToLoad;
    _TtC16LinkPresentation19LPAnimationMaskView * _animationMaskView;
    long long  _animationOrigin;
    UIView * _animationView;
    bool  _applyCornerRadius;
    bool  _asynchronouslyLoadingMetadataFields;
    UIColor * _backgroundColor;
    bool  _backgroundColorSupportsVibrancy;
    UIVisualEffectView * _blurView;
    unsigned long long  _bytesLoaded;
    LPCaptionButtonPresentationProperties * _captionButton;
    UIView * _captionHighlightView;
    LPCaptionButtonPresentationProperties * _captionTextButton;
    LPCollaborationFooterPresentationProperties * _collaborationFooter;
    bool  _collaborative;
    LPLinkViewComponents * _components;
    LPLinkViewComponents * _componentsForSizing;
    bool  _componentsNeedLayout;
    NSArray * _contactsForAttribution;
    struct UIEdgeInsets { 
        double top; 
        double left; 
        double bottom; 
        double right; 
    }  _contentInset;
    UIView * _contentView;
    UIContextMenuInteraction * _contextMenuInteraction;
    <LPLinkViewDelegate> * _delegate;
    bool  _disableAnimations;
    bool  _disableAutoPlay;
    bool  _disableHighlightGesture;
    bool  _disablePlayback;
    bool  _disablePlaybackControls;
    bool  _disablePreviewGesture;
    bool  _disableTapGesture;
    unsigned long long  _effectiveSizeClass;
    UITraitCollection * _effectiveTraitCollection;
    NSRegularExpression * _emphasizedTextExpression;
    bool  _forceFlexibleWidth;
    bool  _hasEverBuilt;
    bool  _hasOverrideCornerRadius;
    bool  _hasSetDisableHighlightGesture;
    bool  _hasSetDisablePreviewGesture;
    NSMutableArray * _highlightGestureRecognizers;
    bool  _highlightedForAttribution;
    bool  _inComposeContext;
    bool  _inSenderContext;
    LPImage * _lastResortIcon;
    unsigned int  _loggingID;
    bool  _maskEnabled;
    bool  _maskEnabledForAnimation;
    bool  _mayReceiveAdditionalMetadata;
    UIView * _mediaHighlightView;
    LPImage * _mediaOverlayIcon;
    LPLinkMetadata * _metadata;
    bool  _metadataIsComplete;
    NSArray * _multipleMetadata;
    NSArray * _multipleURLs;
    bool  _needsMessagesTranscriptPushCounterAnimation;
    bool  _needsRebuild;
    UIColor * _overrideActionButtonColor;
    NSNumber * _overrideAllowsOpeningSensitiveURLs;
    UIColor * _overrideBackgroundColor;
    double  _overrideCornerRadius;
    UIColor * _overrideMediaBackgroundColor;
    NSAttributedString * _overrideSubtitle;
    UIColor * _overrideSubtitleButtonColor;
    NSURL * _overrideURL;
    NSHashTable * _pendingMetadataProviders;
    unsigned long long  _preferredSizeClass;
    unsigned long long  _presentationPropertyState;
    LPConcretePresentationProperties * _properties;
    NSNumber * _selectedVariantIndex;
    long long  _sharedObjectDownloadState;
    bool  _shouldAnimateDuringNextBuild;
    bool  _showingDisclosureView;
    LPLinkRendererSizeClassParameters * _sizeClassParameters;
    NSString * _sourceBundleIdentifier;
    unsigned long long  _suppressNeedsResizeCount;
    NSMutableArray * _tapGestureRecognizers;
    struct UIEdgeInsets { 
        double top; 
        double left; 
        double bottom; 
        double right; 
    }  _textSafeAreaInset;
    LPTheme * _theme;
    UIColor * _underlyingBackgroundColor;
    bool  _useCPURenderingForMaterials;
    bool  _usesComputedPresentationProperties;
    bool  _usesDeferredLayout;
}

@property (nonatomic, retain) NSURL *URL;
@property (setter=_setAllowsAsynchronousImageDecoding:, nonatomic) bool _allowsAsynchronousImageDecoding;
@property (setter=_setAllowsLoadingMediaWithAutoPlayDisabled:, nonatomic) bool _allowsLoadingMediaWithAutoPlayDisabled;
@property (setter=_setAllowsOpeningSensitiveURLs:, nonatomic) bool _allowsOpeningSensitiveURLs;
@property (setter=_setAllowsSkinnyWidth:, nonatomic) bool _allowsSkinnyWidth;
@property (setter=_setAllowsTapToLoad:, nonatomic) bool _allowsTapToLoad;
@property (setter=_setAnimationOrigin:, nonatomic) long long _animationOrigin;
@property (setter=_setApplyCornerRadius:, nonatomic) bool _applyCornerRadius;
@property (setter=_setBytesLoaded:, nonatomic) unsigned long long _bytesLoaded;
@property (getter=_isCollaborative, setter=_setCollaborative:, nonatomic) bool _collaborative;
@property (setter=_setContactsForAttribution:, nonatomic, retain) NSArray *_contactsForAttribution;
@property (nonatomic, readonly) <UIContextMenuInteractionDelegate> *_contextMenuInteractionDelegate;
@property (setter=_setCornerRadius:, nonatomic) double _cornerRadius;
@property (setter=_setDisableAnimations:, nonatomic) bool _disableAnimations;
@property (setter=_setDisableAutoPlay:, nonatomic) bool _disableAutoPlay;
@property (setter=_setDisableHighlightGesture:, nonatomic) bool _disableHighlightGesture;
@property (setter=_setDisableLinkFollowing:, nonatomic) bool _disableLinkFollowing;
@property (setter=_setDisablePlayback:, nonatomic) bool _disablePlayback;
@property (setter=_setDisablePlaybackControls:, nonatomic) bool _disablePlaybackControls;
@property (setter=_setDisablePreviewGesture:, nonatomic) bool _disablePreviewGesture;
@property (setter=_setDisableTapGesture:, nonatomic) bool _disableTapGesture;
@property (nonatomic, readonly) UIColor *_dominantBackgroundColor;
@property (setter=_setEffectiveCornerRadius:, nonatomic) double _effectiveCornerRadius;
@property (nonatomic, readonly) UITraitCollection *_effectiveTraitCollection;
@property (setter=_setEmphasizedTextExpression:, nonatomic, copy) NSRegularExpression *_emphasizedTextExpression;
@property (nonatomic, readonly) LPLinkMetadata *_flattenedMetadata;
@property (setter=_setForceFlexibleWidth:, nonatomic) bool _forceFlexibleWidth;
@property (getter=_isHighlightedForAttribution, setter=_setHighlightedForAttribution:, nonatomic) bool _highlightedForAttribution;
@property (setter=_setInComposeContext:, nonatomic) bool _inComposeContext;
@property (setter=_setInSenderContext:, nonatomic) bool _inSenderContext;
@property (setter=_setLastResortIcon:, nonatomic, retain) LPImage *_lastResortIcon;
@property (setter=_setMaskEnabled:, nonatomic) bool _maskEnabled;
@property (setter=_setMediaOverlayIcon:, nonatomic, retain) LPImage *_mediaOverlayIcon;
@property (nonatomic, readonly) NSArray *_metadataVariants;
@property (setter=_setNeedsMessagesTranscriptPushCounterAnimation:, nonatomic) bool _needsMessagesTranscriptPushCounterAnimation;
@property (setter=_setOverrideActionButtonColor:, nonatomic, retain) UIColor *_overrideActionButtonColor;
@property (setter=_setOverrideBackgroundColor:, nonatomic, retain) UIColor *_overrideBackgroundColor;
@property (setter=_setOverrideMediaBackgroundColor:, nonatomic, retain) UIColor *_overrideMediaBackgroundColor;
@property (setter=_setOverrideSubtitleButtonColor:, nonatomic, retain) UIColor *_overrideSubtitleButtonColor;
@property (setter=_setOverrideURL:, nonatomic, retain) NSURL *_overrideURL;
@property (setter=_setPreferredSizeClass:, nonatomic) unsigned long long _preferredSizeClass;
@property (nonatomic, readonly) struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; } _primaryCaptionBarFrame;
@property (nonatomic, readonly) UIColor *_resolvedBackgroundColor;
@property (getter=_selectedVariantIndex, setter=_setSelectedVariantIndex:, nonatomic, retain) NSNumber *_selectedVariantIndex;
@property (getter=_isShowingDisclosureView, setter=_setShowingDisclosureView:, nonatomic) bool _showingDisclosureView;
@property (setter=_setSizeClassParameters:, nonatomic, copy) LPLinkRendererSizeClassParameters *_sizeClassParameters;
@property (setter=_setSourceBundleIdentifier:, nonatomic, retain) NSString *_sourceBundleIdentifier;
@property (setter=_setStrictlyRespectsSizeClassHeightConstraints:, nonatomic) bool _strictlyRespectsSizeClassHeightConstraints;
@property (nonatomic, readonly) long long _style;
@property (setter=_setTextSafeAreaInset:, nonatomic) struct UIEdgeInsets { double x1; double x2; double x3; double x4; } _textSafeAreaInset;
@property (setter=_setUnderlyingBackgroundColor:, nonatomic, retain) UIColor *_underlyingBackgroundColor;
@property (setter=_setUseCPURenderingForMaterials:, nonatomic) bool _useCPURenderingForMaterials;
@property (setter=_setUseLowMemoryImageFilters:, nonatomic) bool _useLowMemoryImageFilters;
@property (setter=_setUsesDeferredLayout:, nonatomic) bool _usesDeferredLayout;
@property (nonatomic) struct UIEdgeInsets { double x1; double x2; double x3; double x4; } contentInset;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (nonatomic) <LPLinkViewDelegate> *delegate;
@property (readonly, copy) NSString *description;
@property (readonly) unsigned long long hash;
@property (nonatomic, copy) LPLinkMetadata *metadata;
@property (readonly) Class superclass;

- (void).cxx_destruct;
- (id)URL;
- (void)_addCaptionButton:(id)arg1;
- (void)_addHighlightRecognizerToView:(id)arg1 forMedia:(bool)arg2;
- (void)_addTapRecognizerToView:(id)arg1 forMedia:(bool)arg2;
- (bool)_allowsAsynchronousImageDecoding;
- (bool)_allowsLoadingMediaWithAutoPlayDisabled;
- (bool)_allowsOpeningSensitiveURLs;
- (bool)_allowsSkinnyWidth;
- (bool)_allowsTapToLoad;
- (void)_animateWithDuration:(double)arg1 animations:(id /* block */)arg2;
- (long long)_animationOrigin;
- (bool)_applyCornerRadius;
- (unsigned long long)_axesForDerivingIntrinsicContentSizeFromLayoutSize;
- (bool)_backgroundColorSupportsVibrancy:(id)arg1;
- (long long)_blurEffectStyle;
- (unsigned long long)_bytesLoaded;
- (void)_cancelActiveHighlight;
- (void)_captionBarButtonPressed:(id)arg1;
- (void)_captionHighlightRecognized:(id)arg1;
- (void)_captionTapRecognized:(id)arg1;
- (void)_commonInitWithURL:(id)arg1;
- (void)_computePresentationPropertiesFromMetadataIfNeeded;
- (void)_configureWithoutLoadingMetadataFromURLs:(id)arg1;
- (id)_contactsForAttribution;
- (id)_contextMenuInteractionDelegate;
- (double)_cornerRadius;
- (id)_createBackgroundImageView;
- (id)_createCaptionBar;
- (id)_createComponents;
- (id)_createContactsBadgeView;
- (id)_createDomainNameIndicator;
- (id)_createMediaBottomCaptionBarView;
- (id)_createMediaTopCaptionBarView;
- (id)_createMediaView;
- (id)_createMediaViewForProperties:(id)arg1;
- (id)_createProgressSpinner;
- (id)_createQuotedTextView;
- (id)_createTapToLoadView;
- (void)_didScroll;
- (bool)_disableAnimations;
- (bool)_disableAutoPlay;
- (void)_disableBlur;
- (bool)_disableHighlightGesture;
- (bool)_disableLinkFollowing;
- (bool)_disablePlayback;
- (bool)_disablePlaybackControls;
- (bool)_disablePreviewGesture;
- (bool)_disableTapGesture;
- (id)_dominantBackgroundColor;
- (unsigned long long)_edgesPropagatingSafeAreaInsetsToSubviews;
- (id)_effectiveAppearanceTraits;
- (double)_effectiveCornerRadius;
- (id)_effectiveTraitCollection;
- (id)_emphasizedTextExpression;
- (void)_enableBlur;
- (void)_fetchMetadataForRequest:(id)arg1;
- (id)_fetchMetadataForRequest:(id)arg1 withSubresources:(bool)arg2 completionHandler:(id /* block */)arg3;
- (id)_flattenedMetadata;
- (bool)_forceFlexibleWidth;
- (bool)_hasMediaToPlay;
- (bool)_inComposeContext;
- (bool)_inSenderContext;
- (id)_initWithMetadataLoadedFromRequests:(id)arg1;
- (id)_initWithMetadataLoadedFromURLs:(id)arg1;
- (id)_initWithMultipleMetadata:(id)arg1;
- (id)_initWithNullableURL:(id)arg1;
- (id)_initWithSynapseContentItem:(id)arg1;
- (void)_installHighlightGestureRecognizers;
- (void)_installPreviewGestureRecognizer;
- (void)_installTapGestureRecognizers;
- (void)_invalidateLayout;
- (void)_invalidatePresentationProperties;
- (void)_invalidatePresentationPropertiesAndLayout;
- (bool)_isCollaborative;
- (bool)_isHighlightedForAttribution;
- (bool)_isPlayingMedia;
- (bool)_isShowingDisclosureView;
- (bool)_isUsingAppClipPresentation;
- (id)_lastResortIcon;
- (bool)_layoutHeightDependsOnWidth;
- (void)_layoutLinkView;
- (struct CGSize { double x1; double x2; })_layoutLinkViewForSize:(struct CGSize { double x1; double x2; })arg1 applyingLayout:(bool)arg2;
- (bool)_maskEnabled;
- (id)_mediaBackgroundColor;
- (void)_mediaHighlightRecognized:(id)arg1;
- (id)_mediaOverlayIcon;
- (void)_mediaTapRecognized:(id)arg1;
- (id)_metadataVariants;
- (bool)_needsMessagesTranscriptPushCounterAnimation;
- (void)_openURL;
- (void)_openURLAllowingSensitiveSchemes:(bool)arg1 allowingAssociatedApplications:(bool)arg2;
- (id)_overrideActionButtonColor;
- (id)_overrideBackgroundColor;
- (id)_overrideMediaBackgroundColor;
- (id)_overrideSubtitleButtonColor;
- (id)_overrideURL;
- (void)_pauseMedia;
- (void)_playMedia;
- (id)_playable;
- (unsigned long long)_preferredSizeClass;
- (id)_presentationOverrideBackgroundColor;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_primaryCaptionBarFrame;
- (id)_primaryMediaView;
- (void)_rebuildEntireView;
- (void)_rebuildGestureRecognizersIfNeeded;
- (void)_rebuildSubviewsWithAnimation:(bool)arg1;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_rectForCaptionButtonType:(long long)arg1;
- (void)_removeCaptionButtonWithType:(long long)arg1;
- (void)_resetMediaPlayback;
- (id)_resolvedBackgroundColor;
- (id)_selectedVariantIndex;
- (void)_setAction:(id /* block */)arg1 withAttributedText:(id)arg2 buttonType:(long long)arg3;
- (void)_setAction:(id /* block */)arg1 withText:(id)arg2;
- (void)_setAction:(id /* block */)arg1 withText:(id)arg2 buttonType:(long long)arg3;
- (void)_setAllowsAsynchronousImageDecoding:(bool)arg1;
- (void)_setAllowsLoadingMediaWithAutoPlayDisabled:(bool)arg1;
- (void)_setAllowsOpeningSensitiveURLs:(bool)arg1;
- (void)_setAllowsSkinnyWidth:(bool)arg1;
- (void)_setAllowsTapToLoad:(bool)arg1;
- (void)_setAnimationOrigin:(long long)arg1;
- (void)_setApplyCornerRadius:(bool)arg1;
- (void)_setButtonActions:(id)arg1;
- (void)_setBytesLoaded:(unsigned long long)arg1;
- (void)_setCollaborationFooterViewModel:(id)arg1 action:(id /* block */)arg2;
- (void)_setCollaborative:(bool)arg1;
- (void)_setContactsForAttribution:(id)arg1;
- (void)_setCornerRadius:(double)arg1;
- (void)_setDisableAnimations:(bool)arg1;
- (void)_setDisableAutoPlay:(bool)arg1;
- (void)_setDisableHighlightGesture:(bool)arg1;
- (void)_setDisableLinkFollowing:(bool)arg1;
- (void)_setDisablePlayback:(bool)arg1;
- (void)_setDisablePlaybackControls:(bool)arg1;
- (void)_setDisablePreviewGesture:(bool)arg1;
- (void)_setDisableTapGesture:(bool)arg1;
- (void)_setEffectiveAppearanceTraits:(id)arg1;
- (void)_setEffectiveCornerRadius:(double)arg1;
- (void)_setEmphasizedTextExpression:(id)arg1;
- (void)_setForceFlexibleWidth:(bool)arg1;
- (void)_setHighlightCaption:(bool)arg1;
- (void)_setHighlightMedia:(bool)arg1;
- (void)_setHighlightedForAttribution:(bool)arg1;
- (void)_setInComposeContext:(bool)arg1;
- (void)_setInSenderContext:(bool)arg1;
- (void)_setLastResortIcon:(id)arg1;
- (void)_setMaskEnabled:(bool)arg1;
- (void)_setMaskEnabledForAnimation:(bool)arg1;
- (void)_setMaskImage:(id)arg1;
- (void)_setMediaOverlayIcon:(id)arg1;
- (void)_setMetadata:(id)arg1 isFinal:(bool)arg2;
- (void)_setMetadataInternal:(id)arg1;
- (void)_setMultipleMetadata:(id)arg1;
- (void)_setNeedsMessagesTranscriptPushCounterAnimation:(bool)arg1;
- (void)_setOverrideActionButtonColor:(id)arg1;
- (void)_setOverrideBackgroundColor:(id)arg1;
- (void)_setOverrideMediaBackgroundColor:(id)arg1;
- (void)_setOverrideSubtitle:(id)arg1;
- (void)_setOverrideSubtitleButtonColor:(id)arg1;
- (void)_setOverrideURL:(id)arg1;
- (void)_setPreferredSizeClass:(unsigned long long)arg1;
- (void)_setPresentationProperties:(id)arg1;
- (void)_setSelectedVariantIndex:(id)arg1;
- (void)_setShowingDisclosureView:(bool)arg1;
- (void)_setSizeClassParameters:(id)arg1;
- (void)_setSourceBundleIdentifier:(id)arg1;
- (void)_setStrictlyRespectsSizeClassHeightConstraints:(bool)arg1;
- (void)_setSuppressChatKitMask:(bool)arg1;
- (void)_setTextSafeAreaInset:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)_setUnderlyingBackgroundColor:(id)arg1;
- (void)_setUseCPURenderingForMaterials:(bool)arg1;
- (void)_setUseLowMemoryImageFilters:(bool)arg1;
- (void)_setUsesDeferredLayout:(bool)arg1;
- (void)_setupView;
- (bool)_shouldApplyCornerRadius;
- (bool)_shouldClipAnimationView;
- (bool)_shouldUseAnimations;
- (bool)_shouldUseBlur;
- (id)_sizeClassParameters;
- (id)_sourceBundleIdentifier;
- (bool)_strictlyRespectsSizeClassHeightConstraints;
- (long long)_style;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })_textSafeAreaInset;
- (long long)_themePlatform;
- (id)_underlyingBackgroundColor;
- (void)_uninstallHighlightGestureRecognizers;
- (void)_uninstallPreviewGestureRecognizer;
- (void)_uninstallTapGestureRecognizers;
- (void)_updateBackgroundColor;
- (void)_updateBlur;
- (void)_updateCornerRadius;
- (void)_updateEffectiveAppearanceTraitsIfNeeded;
- (void)_updateMasking;
- (void)_updateMetadataIsComplete;
- (bool)_useCPURenderingForMaterials;
- (bool)_useLowMemoryImageFilters;
- (bool)_useProgressSpinner;
- (bool)_usesDeferredLayout;
- (id)_videoViewConfiguration;
- (bool)allowsAsynchronousImageDecodingForComponentView:(id)arg1;
- (bool)allowsBadgingIconEdgeForComponentView:(id)arg1;
- (bool)allowsVibrancyForComponentView:(id)arg1;
- (void)animateBackgroundColor;
- (void)animateFromOldFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg1 oldMediaBackgroundFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg2 oldCaptionBarView:(id)arg3;
- (void)animateInViews;
- (void)animateOutAndRemoveViews;
- (void)animationDidStop:(id)arg1 finished:(bool)arg2;
- (id)appearanceForComponentView:(id)arg1;
- (long long)blurEffectStyleBehindComponentView:(id)arg1;
- (long long)componentView:(id)arg1 allowedImageFilterForFilter:(long long)arg2;
- (id)componentView:(id)arg1 playerForAudio:(id)arg2;
- (void)componentViewDidChangeIntrinsicContentSize:(id)arg1;
- (void)componentViewDidChangeMediaState:(id)arg1;
- (void)componentViewDidTapCaptionButton:(id)arg1 buttonType:(long long)arg2;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })contentInset;
- (id)contextMenuInteraction:(id)arg1 configurationForMenuAtLocation:(struct CGPoint { double x1; double x2; })arg2;
- (void)contextMenuInteraction:(id)arg1 willDisplayMenuForConfiguration:(id)arg2 animator:(id)arg3;
- (void)dealloc;
- (id)delegate;
- (void)didMoveToWindow;
- (id)downloadProgressForComponentView:(id)arg1;
- (void)encodeWithCoder:(id)arg1;
- (bool)gestureRecognizer:(id)arg1 canPreventGestureRecognizer:(id)arg2;
- (bool)gestureRecognizer:(id)arg1 shouldReceiveTouch:(id)arg2;
- (bool)gestureRecognizer:(id)arg1 shouldRecognizeSimultaneouslyWithGestureRecognizer:(id)arg2;
- (bool)gestureRecognizer:(id)arg1 shouldRequireFailureOfGestureRecognizer:(id)arg2;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg1;
- (id)initWithMetadata:(id)arg1;
- (id)initWithMetadataLoadedFromRequest:(id)arg1;
- (id)initWithMetadataLoadedFromURL:(id)arg1;
- (id)initWithPresentationProperties:(id)arg1;
- (id)initWithPresentationProperties:(id)arg1 URL:(id)arg2;
- (id)initWithURL:(id)arg1;
- (struct CGSize { double x1; double x2; })intrinsicContentSize;
- (id)layoutExclusionsForView:(id)arg1;
- (void)layoutSubviews;
- (bool)linkHasMediaForComponentView:(id)arg1;
- (id)metadata;
- (double)minimumCornerRadiusForComponentView:(id)arg1;
- (long long)rendererStyleForComponentView:(id)arg1;
- (void)resetDownloadState;
- (void)setContentInset:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)setDelegate:(id)arg1;
- (void)setMetadata:(id)arg1;
- (void)setURL:(id)arg1;
- (void)set_preferredSizeClass:(unsigned long long)arg1;
- (long long)sharedObjectDownloadStateForComponentView:(id)arg1;
- (bool)showingDisclosureViewForComponentView:(id)arg1;
- (struct CGSize { double x1; double x2; })sizeThatFits:(struct CGSize { double x1; double x2; })arg1;
- (void)tapToLoadViewWasTapped:(id)arg1;
- (void)themeParametersDidChange;
- (long long)themePlatformForComponentView:(id)arg1;
- (bool)usesBackgroundForSymbolImagesForComponentView:(id)arg1;

@end
