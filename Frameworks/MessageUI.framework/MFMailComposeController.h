/* Generated by RuntimeBrowser
   Image: /System/Library/Frameworks/MessageUI.framework/MessageUI
 */

@interface MFMailComposeController : UIViewController <CNAutocompleteGroupDetailViewControllerDelegate, CNComposeHeaderViewDelegate, CNContactContentViewControllerDelegate, CNContactPickerDelegate, ComposeViewNavigationBarDelegate, MFComposeActivityHandoffOperationDelegate, MFComposeImageSizeViewDelegate, MFComposePhotoPickerControllerDelegate, MFComposeStyleSelectorViewControllerDelegate, MFComposeSubjectViewDelegate, MFComposeTypeFactoryDelegate, MFDatePickerViewControllerDelegate, MFMailComposeRecipientTextViewDelegate, MFMailComposeToFieldDelegate, MFMailComposeViewDelegate, MFMailMenuCommandProvider, MFPhotoPickerControllerDelegate, MFPreferredTitleProvider, MFSMIMEControlDelegate, MFSecureMIMECompositionManagerDelegate, NSUserActivityDelegate, QLPreviewControllerDelegate, UIColorPickerViewControllerDelegate, UIFontPickerViewControllerDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate, UIPopoverPresentationControllerDelegate, VNDocumentCameraViewControllerDelegate, _MFMailCompositionContextDelegate> {
    MFLANHandoffAgent * _LANHandoffAgent;
    <MFMailAccountProxyGenerator> * _accountProxyGenerator;
    NSString * _addressForMissingIdentity;
    MFAddressPickerReformatter * _addressPickerReformatter;
    bool  _allowRestrictedAccounts;
    UIView * _alternateTitleView;
    EMCoreAnalyticsCollector * _analyticsCollector;
    CNComposeRecipientAtom * _atomPresentingCard;
    bool  _attachmentToMarkupIsLoaded;
    _Atomic unsigned int  _autosaveCount;
    NSString * _autosaveIdentifier;
    bool  _autosaveIsValid;
    MFLock * _autosaveLock;
    <EFScheduler> * _autosaveQueue;
    MSAutosaveSession * _autosaveSession;
    NSTimer * _autosaveTimer;
    NSDate * _autosavedDate;
    NSArray * _bccAddresses;
    unsigned int  _bodyTextChanged;
    UIImagePickerController * _cameraPickerController;
    NSArray * _ccAddresses;
    MFMailComposeRecipientTextView * _ccField;
    NSDictionary * _certificatesByRecipient;
    UIBarButtonItem * _closeButtonItem;
    <MFMailComposeCoordinator> * _composeCoordinator;
    MFComposeWebView * _composeWebView;
    struct { 
        bool respondsToCompositionWillFinish; 
        bool respondsToCompositionDidFailToFinish; 
    }  _composeWebViewFlags;
    _MFMailCompositionContext * _compositionContext;
    CNContactViewController * _contactViewController;
    UIWindow * _containingWindow;
    EFFuture * _content;
    bool  _contentVisible;
    long long  _currentlyLoadingCounter;
    bool  _delayToShowMarkupHasPassed;
    id  _delegate;
    MFOutgoingMessageDelivery * _delivery;
    id /* block */  _didSaveQuickReplyDraftHandler;
    bool  _didUseOmittedAttachmentsOrRecipients;
    MFComposeDisplayMetrics * _displayMetrics;
    VNDocumentCameraViewController * _documentCameraViewController;
    NSString * _documentID;
    EMMailboxObjectID * _draftMailboxObjectID;
    NSMutableSet * _drawingFileAttachments;
    unsigned long long  _drawingSequenceNumber;
    long long  _encryptionIdentityStatus;
    bool  _encryptionOverrideSetting;
    bool  _encryptionStatusIsKnown;
    NSDictionary * _errorsByRecipient;
    NSDictionary * _fontAttributes;
    MFComposeActivityHandoffOperation * _handoffOperation;
    UIProgressView * _handoffProgressView;
    bool  _hasActiveWritingToolsSession;
    bool  _hasAddedEditableHyperlink;
    NSNumber * _hasCustomSignature;
    bool  _hasMessageBodyContent;
    bool  _hasPresentedSenderCollaborationAlert;
    unsigned int  _hasViewAppeared;
    NSString * _hideMyEmailAddressIfExists;
    bool  _hideMyEmailMultipleRecipient;
    NSString * _hideMyEmailSenderAddress;
    unsigned int  _hosted;
    NSObject<OS_dispatch_group> * _imageScalingGroup;
    MFComposeImageSizeView * _imageSizeField;
    unsigned long long  _initialAttachmentCount;
    NSArray * _initialMissingAttachmentWarnings;
    NSString * _initialTitle;
    bool  _initializedRecipients;
    EFDebouncer * _inputContextHistoryInvalidationDebouncer;
    bool  _insertingDrawing;
    unsigned int  _isDirty;
    bool  _isModal;
    unsigned int  _isPopoverVisible;
    unsigned int  _isSuspended;
    NSDate * _lastActiveDate;
    EMMessage * _lastDraftMessage;
    MFMailComposeRecipientTextView * _lastFocusedRecipientView;
    unsigned short  _lastTypedCharacter;
    unsigned long long  _markupReplyAttachmentLoadingProgress;
    <EFCancelable> * _messageLoadingTailspinCancellable;
    UIAlertController * _notifyConfirmation;
    unsigned long long  _options;
    NSArray * _originalBccAddresses;
    EFFuture * _originalContentMessagesFuture;
    <EFScheduler> * _originalContentMessagesScheduler;
    bool  _originalMessageWasEncrypted;
    NSString * _originalSendingEmailAddress;
    NSString * _originatingBundleID;
    CNContactPickerViewController * _peoplePicker;
    MFPhotoPickerProgressManager * _photoPickerProgressManager;
    MFPhotoPickerController * _photosPickerController;
    UIAlertController * _popoverAlert;
    MFMailPopoverManager * _popoverManager;
    <EFScheduler> * _prepareImagesForSendScheduler;
    QLPreviewController * _previewController;
    MFAttachment * _previewingAttachment;
    double  _previousPaletteWidth;
    NSString * _primaryAddressForForcedSendingAccount;
    MFMessageContentProgressLayer * _progressIndicatorView;
    <MFMailComposeControllerQuickReplyActionButtonProvider> * _quickReplyActionButtonProvider;
    <MFMailComposeControllerQuickReplyDelegate> * _quickReplyDelegate;
    CNRecentComposeRecipient * _recentRecipientPresentingCard;
    long long  _recipientFieldWhileViewUnloaded;
    CNComposeRecipient * _recipientPresentingCard;
    id /* block */  _releaseActiveFocusedState;
    id  _remoteViewControllerProxy;
    long long  _resolution;
    unsigned int  _rotationSnapshotTaken;
    UIResponder * _savedFirstResponder;
    MFMutableMessageHeaders * _savedHeaders;
    MFSecureMIMECompositionManager * _secureCompositionManager;
    NSDictionary * _securityScopes;
    unsigned long long  _selectedContentVariationIndex;
    UIButton * _sendAccessoryButton;
    UIBarButtonItem * _sendButtonItem;
    UIBarButtonItem * _sendLaterButtonItem;
    NSString * _sendingEmailAddress;
    long long  _sendingEmailAddressIndex;
    unsigned int  _shouldAutosaveWithSuspendInfo;
    MFMailSignatureController * _signatureController;
    long long  _signingIdentityStatus;
    MFSMIMEControl * _smimeStatusControl;
    int  _sourceAccountManagement;
    unsigned int  _stillLoading;
    MFComposeStyleSelectorViewController * _styleSelector;
    NSString * _subject;
    MFComposeSubjectView * _subjectField;
    UIImagePickerController * _systemImagePickerController;
    MFComposePhotoPickerController * _systemPhotosPickerController;
    _TtC9MessageUI20MessageUITipsManager * _tipsManager;
    NSArray * _toAddresses;
    MFMailComposeToField * _toField;
    EFFuture * _undoMoveAction;
    bool  _useMailDrop;
    unsigned int  _useSuspended;
    unsigned int  _viewWasUnloaded;
}

@property (nonatomic, retain) <MFMailAccountProxyGenerator> *accountProxyGenerator;
@property (nonatomic, copy) NSString *addressForMissingIdentity;
@property (nonatomic, readonly) EMCoreAnalyticsCollector *analyticsCollector;
@property (nonatomic, retain) CNComposeRecipientAtom *atomPresentingCard;
@property (nonatomic) bool attachmentToMarkupIsLoaded;
@property (nonatomic, retain) NSString *autosaveIdentifier;
@property (nonatomic) bool autosaveIsValid;
@property (nonatomic, retain) <EFScheduler> *autosaveQueue;
@property (nonatomic, retain) MSAutosaveSession *autosaveSession;
@property (nonatomic, readonly) NSDate *autosavedDate;
@property (nonatomic, retain) NSDictionary *certificatesByRecipient;
@property (nonatomic) <MFMailComposeCoordinator> *composeCoordinator;
@property (nonatomic, retain) CNContactViewController *contactViewController;
@property long long currentlyLoadingCounter;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (nonatomic) bool delayToShowMarkupHasPassed;
@property (nonatomic) id delegate;
@property (nonatomic, readonly) MFOutgoingMessageDelivery *delivery;
@property (readonly, copy) NSString *description;
@property (nonatomic, copy) id /* block */ didSaveQuickReplyDraftHandler;
@property (nonatomic) bool didUseOmittedAttachmentsOrRecipients;
@property (nonatomic, retain) MFComposeDisplayMetrics *displayMetrics;
@property (nonatomic) VNDocumentCameraViewController *documentCameraViewController;
@property (nonatomic, readonly) NSString *documentID;
@property (nonatomic, readonly) EMMailboxObjectID *draftMailboxObjectID;
@property (nonatomic, retain) NSMutableSet *drawingFileAttachments;
@property (nonatomic, retain) NSDictionary *errorsByRecipient;
@property (nonatomic, retain) NSDictionary *fontAttributes;
@property (nonatomic) bool hasActiveWritingToolsSession;
@property (nonatomic) bool hasAddedEditableHyperlink;
@property (nonatomic, retain) NSNumber *hasCustomSignature;
@property (nonatomic) bool hasMessageBodyContent;
@property (nonatomic) bool hasPresentedSenderCollaborationAlert;
@property (readonly) unsigned long long hash;
@property (nonatomic, copy) NSString *hideMyEmailAddressIfExists;
@property (nonatomic) bool hideMyEmailMultipleRecipient;
@property (nonatomic, copy) NSString *hideMyEmailSenderAddress;
@property (nonatomic, retain) NSArray *initialMissingAttachmentWarnings;
@property (nonatomic, retain) EFDebouncer *inputContextHistoryInvalidationDebouncer;
@property (nonatomic) bool insertingDrawing;
@property (nonatomic) bool isDirty;
@property (nonatomic) bool isModal;
@property (nonatomic) bool isQuickReply;
@property (nonatomic, readonly) EMMessage *lastDraftMessage;
@property (nonatomic) unsigned long long markupReplyAttachmentLoadingProgress;
@property (nonatomic, retain) <EFCancelable> *messageLoadingTailspinCancellable;
@property (nonatomic, retain) EFFuture *originalContentMessagesFuture;
@property (nonatomic, retain) <EFScheduler> *originalContentMessagesScheduler;
@property (nonatomic, readonly) EMObjectID *originalMessageObjectID;
@property (nonatomic, copy) NSString *originatingBundleID;
@property (nonatomic, retain) CNContactPickerViewController *peoplePicker;
@property (nonatomic, retain) MFPhotoPickerProgressManager *photoPickerProgressManager;
@property (nonatomic, retain) UIAlertController *popoverAlert;
@property (nonatomic, readonly) <EFScheduler> *prepareImagesForSendScheduler;
@property (nonatomic) QLPreviewController *previewController;
@property (nonatomic, retain) MFAttachment *previewingAttachment;
@property (nonatomic) double previousPaletteWidth;
@property (nonatomic) <MFMailComposeControllerQuickReplyActionButtonProvider> *quickReplyActionButtonProvider;
@property (nonatomic) <MFMailComposeControllerQuickReplyDelegate> *quickReplyDelegate;
@property (nonatomic, retain) CNComposeRecipient *recipientPresentingCard;
@property (nonatomic, retain) id remoteViewControllerProxy;
@property (nonatomic, retain) MFSMIMEControl *smimeStatusControl;
@property (nonatomic) int sourceAccountManagement;
@property (nonatomic, retain) MFComposeStyleSelectorViewController *styleSelector;
@property (nonatomic, readonly) NSString *subject;
@property (readonly) Class superclass;
@property (nonatomic, retain) _TtC9MessageUI20MessageUITipsManager *tipsManager;
@property (nonatomic) bool useMailDrop;
@property (getter=isVerticallyCompact, nonatomic, readonly) bool verticallyCompact;

+ (void)initialize;
+ (bool)isSetupForDeliveryAllowingRestrictedAccounts:(bool)arg1;
+ (bool)isSetupForDeliveryAllowingRestrictedAccounts:(bool)arg1 originatingBundleID:(id)arg2 sourceAccountManagement:(int)arg3;
+ (id)menuCommands;
+ (id)preferenceForKey:(id)arg1;
+ (id)signpostLog;

- (void).cxx_destruct;
- (void)_accountsChanged:(id)arg1;
- (id)_addressFieldForField:(long long)arg1;
- (id)_allRecipientViews;
- (id)_allRecipients;
- (id)_alternateTitleView;
- (bool)_anyRecipientViewContainsAddress:(id)arg1;
- (void)_attachmentLoaderFinishedWithOriginalContent:(id)arg1;
- (id)_attachmentToMarkup;
- (void)_autosaveTimerFired:(id)arg1;
- (id)_availableAccountProxies;
- (id)_backgroundColorForMarkupController;
- (void)_beginBlockingBodyScroll;
- (void)_bodyTextChanged;
- (bool)_canUseOrnamentViewController;
- (void)_checkForCanSendMailWithContinuation:(id /* block */)arg1;
- (void)_checkForEmptySubjectWithContinuation:(id /* block */)arg1;
- (void)_checkForInFlightAssetDownloadsWithContinuation:(id /* block */)arg1;
- (void)_checkForInvalidAddressesWithContinuation:(id /* block */)arg1;
- (void)_checkForOmittedRecipientsOrAttachmentsIfNeededWithContinuation:(id /* block */)arg1;
- (void)_checkForReplyAndForwardRestriction;
- (void)_checkForSendLaterDisclaimerWithContinuation:(id /* block */)arg1;
- (void)_checkForShareParticipantsWithContinuation:(id /* block */)arg1;
- (void)_checkForUnencryptedWithContinuation:(id /* block */)arg1;
- (bool)_checkWhetherForwardingAddressIsPresent;
- (void)_clearMessageValues;
- (void)_composeViewDidDraw:(id)arg1;
- (unsigned long long)_composeWebViewTextContentLength;
- (void)_configureSender:(id)arg1 asSourceForPopoverPresentationController:(id)arg2;
- (id)_contactViewControllerForRecipient:(id)arg1;
- (id)_copyMessageDataForActivityHandoff;
- (id)_copyMessagePlainTextForDonation;
- (id)_copyMessagePlainTextForDonationRemoveQuotes:(bool)arg1;
- (void)_createAndAddHandoffProgressViewIfNecessary;
- (id)_createSendButtonItemWithPossibleTitles:(id)arg1;
- (id)_csPeopleFromAddresses:(id)arg1;
- (id)_defaultAccount;
- (unsigned long long)_defaultAtomPresentationOptions;
- (void)_deleteFileAtURLIfInTmp:(id)arg1;
- (Class)_deliveryClass;
- (void)_didDismissDocumentPicker;
- (void)_dismissAndCleanupCameraPickerController;
- (void)_dismissPeoplePicker:(id)arg1;
- (void)_dismissPersonCard;
- (void)_dismissPresentedHalfSheet;
- (void)_dismissPresentedViewController;
- (void)_displayMetricsDidChange;
- (void)_displayPopoverAlert:(id)arg1;
- (void)_documentCameraViewControllerDidFinish;
- (void)_draftContentDidChange;
- (id)_emailAddressesWithHME;
- (void)_endBlockingBodyScroll;
- (void)_escapeShortcutInvoked:(id)arg1;
- (id)_estimateMessageSize;
- (void)_explainAirplaneMode;
- (long long)_fieldForAddressField:(id)arg1;
- (void)_finishModalAlertClosingComposition:(bool)arg1;
- (void)_finishedComposing;
- (void)_finishedLoadingAllContentAndAttachments;
- (void)_focusBccHeaderCommandInvoked:(id)arg1;
- (void)_focusGained:(id)arg1;
- (void)_getHMEEmailFromOriginalAddress:(id)arg1;
- (void)_getRotationContentSettings:(struct { bool x1; bool x2; bool x3; bool x4; bool x5; double x6; int x7; }*)arg1;
- (bool)_hasCustomSignatureConfigured;
- (bool)_hasEncryptionIdentityError;
- (bool)_hasRecipients;
- (bool)_hasSigningIdentityError;
- (void)_hideHandoffProgressViewAnimated:(bool)arg1;
- (void)_invalidateInputContextHistory;
- (void)_invalidateInputContextHistoryDebounced:(bool)arg1;
- (bool)_isActiveComposeController;
- (bool)_isDummyViewController;
- (bool)_isPopoverOrActionSheetOrAlertVisible;
- (bool)_isReplyOrForward;
- (bool)_isRestoredComposition;
- (bool)_isSetupForDelivery;
- (bool)_isTabKeyCommandInvocationPossible;
- (void)_leaveMessageInOutbox;
- (void)_loadAttachments;
- (void)_loadCompositionContext;
- (void)_loadingContextDidLoadMessage;
- (void)_makeComposeUserActivityCurrent;
- (id)_messageForAutosave;
- (id)_messageForDraft;
- (id)_messageForRemoteDelivery;
- (id)_messageToDonate;
- (id)_messageWithCompositionSpecification:(id)arg1 useSuspendInfo:(bool)arg2 endingEditing:(bool)arg3;
- (id)_missingIdentityErrorWithFormat:(id)arg1 title:(id)arg2;
- (id)_nextDrawingName;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_optimalRectForPresentingPopoverInComposeWebView;
- (id)_originalContentMessagesIfExists;
- (id)_originalContentOfMessagesInReplyToMessage:(id)arg1;
- (id)_originalMessageCategorySubtype;
- (id)_outgoingMessageWithSubstituteDocument:(id)arg1 compositionSpecification:(id)arg2 useSuspendInfo:(bool)arg3 endingEditing:(bool)arg4;
- (void)_performBlockWithStrongComposeCoordinator:(id /* block */)arg1;
- (void)_performDeferredActionIfNecessary;
- (void)_physicallyScaleImagesToScale:(unsigned long long)arg1 attachments:(id)arg2;
- (void)_popoverWillBePresented:(id)arg1;
- (void)_prepareCompositionContextForLoading:(id)arg1;
- (void)_prepareForSuspend;
- (void)_prepareHMEAddressesWithContinuation:(id /* block */)arg1;
- (void)_prepareImagesForSendWithContinuation:(id /* block */)arg1;
- (void)_preparePhotoPicker;
- (void)_presentLinkEditorWithName:(id)arg1 text:(id)arg2 mode:(unsigned long long)arg3 completion:(id /* block */)arg4;
- (void)_presentModalAlert:(id)arg1;
- (void)_presentSenderCollaborationAlertIfNecessaryForSendingAddress:(id)arg1;
- (void)_presentWarnings:(id)arg1 index:(unsigned long long)arg2 suggestionsService:(id)arg3 completionHandler:(id /* block */)arg4;
- (void)_promptForMailDropIfNecessaryWithContinuation:(id /* block */)arg1;
- (void)_recipientTextChanged:(id)arg1;
- (void)_recipientTextChangedForHME;
- (id)_reformattedAddressAtIndex:(unsigned long long)arg1;
- (unsigned long long)_reloadNumberOfReformattedAddressesWithMaximumWidth:(double)arg1 defaultFontSize:(double)arg2;
- (void)_removeAutoBccSendingAddress:(id)arg1;
- (void)_removeRecent;
- (void)_replyAllCommandInvoked:(id)arg1;
- (void)_replyCommandInvoked:(id)arg1;
- (void)_reportUserEnagagement:(bool)arg1 suggestionService:(id)arg2 warning:(id)arg3;
- (void)_resetProxyGenerator;
- (void)_resetSecureCompositionManager;
- (void)_resetSecureCompositionManagerUsingNewAccount:(bool)arg1;
- (void)_restoreMessageValues;
- (void)_retainFocusOfComposeWebViewIfRequired:(id)arg1;
- (void)_saveMessageValues;
- (void)_scaleImages;
- (bool)_secureCompositionManagerHasRecipients;
- (id)_selectedAttachmentURLs;
- (void)_sendAnalyticsSignalForResult:(long long)arg1;
- (void)_sendViewVisibilityNotification:(id)arg1 window:(id)arg2;
- (id)_senderEmailAddress;
- (void)_setAutosaveIsValid:(bool)arg1;
- (void)_setBodyAndSignatureWithCoordinator:(id)arg1 htmlDataObjectWithSignature:(id)arg2 htmlDataObject:(id)arg3;
- (void)_setInitialStateForImageField;
- (void)_setIsLoading:(bool)arg1;
- (void)_setLastFocusedRecipientView:(id)arg1;
- (void)_setRecipients:(id)arg1 forField:(long long)arg2;
- (bool)_setSendingEmailAddress:(id)arg1;
- (bool)_setSendingEmailAddressAsHME:(id)arg1;
- (void)_setTitleBarSubtitleText:(id)arg1 style:(unsigned long long)arg2;
- (void)_setUpDeliveryObject:(unsigned long long)arg1 completionHandler:(id /* block */)arg2;
- (void)_setUpDraftForHME;
- (void)_setupForAutosavedMessage:(id)arg1;
- (void)_setupForDraft:(id)arg1;
- (void)_setupForExistingNewMessage:(id)arg1 content:(id)arg2;
- (void)_setupForOutbox:(id)arg1;
- (void)_setupForQuickReply;
- (void)_setupLargeTitleAccessory;
- (void)_setupNavigationBarItems;
- (void)_setupOrnamentViewController;
- (void)_setupSendLaterOnboardingPaletteIfNecessary;
- (id)_sheetDetentForIdentifier:(id)arg1 viewController:(id)arg2;
- (bool)_shouldAutoSaveQuickReply;
- (bool)_shouldAutosaveAfterTimerFiredWithInterval:(double)arg1;
- (bool)_shouldEnableCloseButton;
- (bool)_shouldEnableSMIMEMenu;
- (bool)_shouldEnableSendButton;
- (bool)_shouldPrependBlankLineForAttachments;
- (bool)_shouldRestoreAttachments:(id)arg1;
- (bool)_shouldShowSendLaterOnboarding;
- (bool)_shouldUseMailDrop;
- (void)_showCamera:(id)arg1;
- (void)_showMissingIdentityAlert;
- (void)_showOriginalAttachmentsIfNecessary:(id)arg1;
- (void)_showPersonCardForRecipient:(id)arg1 showDeleteButton:(bool)arg2;
- (void)_showPhotoPicker:(id)arg1;
- (void)_showSystemPhotoPicker:(id)arg1;
- (unsigned long long)_sizeForScale:(unsigned long long)arg1 imagesOnly:(bool)arg2;
- (void)_startAutosaveTimerIfNeeded;
- (void)_subjectTextChanged:(id)arg1;
- (void)_tabKeyCommandInvoked:(id)arg1;
- (void)_unscaleImages;
- (void)_updateAndStartCompletingCcFieldWithMissingRecipients:(id)arg1;
- (void)_updateAutoBccSendingAddress:(id)arg1 withNewSendingAddress:(id)arg2;
- (void)_updateAutoBccSendingAddress:(id)arg1 withNewSendingAddress:(id)arg2 alwaysAdd:(bool)arg3;
- (void)_updateAutosaveSession;
- (void)_updateIdentityStatus:(long long*)arg1 withPolicy:(long long)arg2 identity:(struct __SecIdentity { }*)arg3 error:(id)arg4;
- (void)_updateImageSizeTitles;
- (void)_updateManagedPasteboardOwner;
- (void)_updateNavigationBarTitleAnimated:(bool)arg1;
- (void)_updateOriginalBccStatusForRestore;
- (void)_updateOriginalBccStatusForRestoreAddingAddress:(bool)arg1;
- (void)_updatePersonCard;
- (void)_updateRecipientAtomStyles;
- (void)_updateSMIMEButtonEnabledForRecipientCount;
- (void)_updateSMIMEButtonForEncryptionStatus:(long long)arg1;
- (void)_updateSMIMEStatusControl;
- (void)_updateSendAndCloseButtonTintColor;
- (void)_updateSendAndCloseEnabled;
- (void)_updateTitleBarForEncryptionStatus:(long long)arg1;
- (bool)_wantsEncryption;
- (bool)_wantsSigning;
- (void)_willPresentDocumentPicker;
- (id)accountProxyGenerator;
- (void)activityHandoffOperation:(id)arg1 didFailWithError:(id)arg2;
- (void)activityHandoffOperation:(id)arg1 didFinishReceivingData:(id)arg2;
- (void)activityHandoffOperation:(id)arg1 didFinishSendingDataWithResult:(long long)arg2;
- (void)activityHandoffOperationReceivedBytes:(id)arg1;
- (long long)adaptivePresentationStyleForPresentationController:(id)arg1 traitCollection:(id)arg2;
- (void)addAddress:(id)arg1 field:(long long)arg2;
- (void)addLink:(id /* block */)arg1;
- (void)addSignature:(bool)arg1;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })additionalContentInsetForComposeWebView:(id)arg1;
- (id)addressForMissingIdentity;
- (id)addressesForField:(long long)arg1;
- (id)allRecipientNamesAndAddresses;
- (id)analyticsCollector;
- (void)applicationDidResume;
- (void)applicationWillSuspend;
- (id)atomPresentingCard;
- (bool)attachmentToMarkupIsLoaded;
- (id)attachments;
- (void)autocompleteGroupDetailViewController:(id)arg1 didAskToRemoveGroup:(id)arg2;
- (void)autocompleteGroupDetailViewController:(id)arg1 didTapComposeRecipient:(id)arg2;
- (void)autocompleteGroupDetailViewControllerDidCancel:(id)arg1;
- (id)autosaveIdentifier;
- (void)autosaveIfNecessaryWithHandler:(id /* block */)arg1;
- (bool)autosaveIsValid;
- (id)autosaveQueue;
- (id)autosaveSession;
- (void)autosaveWithHandler:(id /* block */)arg1;
- (id)autosavedDate;
- (void)backUpDraft;
- (bool)bccAddressesDirtied;
- (id)bccRecipients;
- (bool)canBecomeFirstResponder;
- (bool)canPerformAction:(SEL)arg1 withSender:(id)arg2;
- (bool)canShowAttachmentPicker;
- (bool)canShowContentVariationPicker;
- (bool)canShowFromField;
- (bool)canShowImageSizeField;
- (id)ccRecipients;
- (id)certificatesByRecipient;
- (void)changeQuoteLevel:(long long)arg1;
- (bool)chooseSelectedSearchResultForComposeRecipientView:(id)arg1;
- (void)clearAllFields;
- (void)clearInitialTitle;
- (void)close;
- (void)close:(id)arg1;
- (id)composeCoordinator;
- (void)composeHeaderView:(id)arg1 didChangeSize:(struct CGSize { double x1; double x2; })arg2;
- (void)composeHeaderViewDidChangeValue:(id)arg1;
- (void)composeHeaderViewDidConfirmValue:(id)arg1;
- (id)composeRecipientView:(id)arg1 composeRecipientForAddress:(id)arg2;
- (id)composeRecipientView:(id)arg1 composeRecipientForContact:(id)arg2;
- (void)composeRecipientView:(id)arg1 didAddRecipient:(id)arg2;
- (void)composeRecipientView:(id)arg1 didFinishAddingRecipient:(id)arg2;
- (void)composeRecipientView:(id)arg1 didFinishEnteringAddress:(id)arg2;
- (void)composeRecipientView:(id)arg1 didRemoveRecipient:(id)arg2;
- (void)composeRecipientView:(id)arg1 didReplaceRecipients:(id)arg2 withRecipients:(id)arg3;
- (void)composeRecipientView:(id)arg1 didSelectRecipients:(id)arg2;
- (void)composeRecipientView:(id)arg1 disambiguateRecipientForAtom:(id)arg2;
- (void)composeRecipientView:(id)arg1 showPersonCardForAtom:(id)arg2;
- (void)composeRecipientView:(id)arg1 showPersonCardForRecipient:(id)arg2;
- (void)composeRecipientView:(id)arg1 textDidChange:(id)arg2;
- (void)composeRecipientViewDidBecomeFirstResponder:(id)arg1;
- (void)composeRecipientViewDidFinishPickingRecipient:(id)arg1;
- (void)composeRecipientViewRequestAddRecipient:(id)arg1;
- (void)composeRecipientViewReturnPressed:(id)arg1;
- (bool)composeRecipientViewShowingSearchResults:(id)arg1;
- (void)composeStyleSelector:(id)arg1 didChangeFont:(id)arg2;
- (void)composeStyleSelector:(id)arg1 didChangeFontSize:(double)arg2;
- (void)composeStyleSelector:(id)arg1 didChangeTextColor:(id)arg2;
- (void)composeStyleSelector:(id)arg1 didSelectStyle:(id)arg2;
- (void)composeStyleSelectorDidCancel:(id)arg1;
- (void)composeStyleSelectorDidDismissFontPicker:(id)arg1;
- (void)composeStyleSelectorDidPresentColorPicker:(id)arg1;
- (void)composeSubjectViewDidSelectContentVariationButton:(id)arg1;
- (void)composeSubjectViewTextFieldDidBecomeFirstResponder:(id)arg1;
- (void)composeSubjectViewTextFieldDidResignFirstResponder:(id)arg1;
- (void)composeSubjectViewWillRemoveContent:(id)arg1;
- (long long)composeType;
- (void)composeViewBodyTextChanged:(id)arg1;
- (id)composeWebView;
- (void)composeWebView:(id)arg1 composeBodyIsEmpty:(bool)arg2;
- (void)composeWebView:(id)arg1 didChangeHeight:(double)arg2;
- (void)composeWebView:(id)arg1 hasActiveWritingToolsSession:(bool)arg2;
- (void)composeWebViewDidChangeFontAttributes:(id)arg1;
- (id)compositionContext;
- (long long)compositionType;
- (void)contactPicker:(id)arg1 didSelectContact:(id)arg2;
- (void)contactPicker:(id)arg1 didSelectContactProperty:(id)arg2;
- (void)contactPickerDidCancel:(id)arg1;
- (id)contactViewController;
- (void)contactViewController:(id)arg1 didCompleteWithContact:(id)arg2;
- (void)contactViewController:(id)arg1 didDeleteContact:(id)arg2;
- (bool)contactViewController:(id)arg1 shouldPerformDefaultActionForContact:(id)arg2 propertyKey:(id)arg3 propertyIdentifier:(id)arg4;
- (bool)contactViewController:(id)arg1 shouldPerformDefaultActionForContactProperty:(id)arg2;
- (void)contentDidChange;
- (unsigned long long)contentVariationIndex;
- (id)contentVariationName;
- (long long)countofDrawingAttachmentsLeftInCompose;
- (id)csPeopleForField:(long long)arg1;
- (id)currentScaleImageSize;
- (long long)currentlyLoadingCounter;
- (void)datePickerViewController:(id)arg1 didSelectDate:(id)arg2;
- (void)dealloc;
- (bool)delayToShowMarkupHasPassed;
- (id)delegate;
- (void)deliverMessageRemotely:(id /* block */)arg1;
- (id)delivery;
- (void)didBecomeActiveComposeController;
- (void)didCreateAttachment:(id)arg1;
- (void)didInsertAttachment:(id)arg1;
- (void)didInsertBodyText:(id)arg1;
- (void)didRemoveAttachment:(id)arg1;
- (void)didRotateFromInterfaceOrientation:(long long)arg1;
- (id /* block */)didSaveQuickReplyDraftHandler;
- (bool)didUseOmittedAttachmentsOrRecipients;
- (id)dismissActionsForPreviewController:(id)arg1;
- (void)dismissSearchResultsForComposeRecipientView:(id)arg1;
- (void)dismissSheet;
- (id)displayMetrics;
- (id)documentCameraViewController;
- (void)documentCameraViewController:(id)arg1 didFinishWithScan:(id)arg2;
- (void)documentCameraViewControllerDidCancel:(id)arg1;
- (id)documentID;
- (void)documentPicker:(id)arg1 didPickDocumentsAtURLs:(id)arg2;
- (void)documentPickerWasCancelled:(id)arg1;
- (id)draftMailboxObjectID;
- (id)drawingFileAttachments;
- (void)editLink:(id)arg1 completion:(id /* block */)arg2;
- (void)editTextDescription:(id)arg1 completion:(id /* block */)arg2;
- (id)emailAddresses;
- (id)errorsByRecipient;
- (void)expandQuickReplyToFullCompose;
- (void)expandQuickReplyToFullComposeAndPerformAction:(long long)arg1 additionalUserInfo:(id)arg2;
- (void)finalizeSignature:(id /* block */)arg1;
- (void)finishEnteringRecipients;
- (void)finishPopoverAlertClosingComposition:(bool)arg1;
- (void)finishedBackingUpDraftWithSuccess:(bool)arg1;
- (id)fontAttributes;
- (void)forceSaveAsDraft;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })frameForAttachmentWithIdentifier:(id)arg1;
- (bool)getPrefersLargeTitles;
- (void)goToSettingsForControl:(id)arg1;
- (void)handleLargeMessageComposeHandoffWithInputStream:(id)arg1 outputStream:(id)arg2 error:(id)arg3;
- (bool)hasActiveWritingToolsSession;
- (bool)hasAddedEditableHyperlink;
- (bool)hasAttachments;
- (id)hasCustomSignature;
- (bool)hasMessageBodyContent;
- (bool)hasPresentedSenderCollaborationAlert;
- (id)hideMyEmailAddressIfExists;
- (bool)hideMyEmailMultipleRecipient;
- (id)hideMyEmailSenderAddress;
- (void)hostApplicationDidEnterBackground;
- (void)hostApplicationWillEnterForeground;
- (bool)hosted;
- (void)imagePickerController:(id)arg1 didFinishPickingMediaWithInfo:(id)arg2;
- (void)imagePickerControllerDidCancel:(id)arg1;
- (void)imageSizeView:(id)arg1 changedSelectedScaleTo:(unsigned long long)arg2;
- (void)importDocument;
- (id)initWithCompositionContext:(id)arg1 options:(unsigned long long)arg2;
- (id)initialMissingAttachmentWarnings;
- (id)inputContextHistory;
- (id)inputContextHistoryInvalidationDebouncer;
- (void)insertDrawing;
- (void)insertPhotoOrVideo;
- (void)insertPhotoOrVideoWithAssetIdentifier:(id)arg1 info:(id)arg2;
- (bool)insertingDrawing;
- (bool)isAddressHideMyEmail:(id)arg1;
- (bool)isCloudKitShare;
- (bool)isDirty;
- (bool)isHideMyEmailMessage;
- (bool)isManagedAccount;
- (bool)isModal;
- (bool)isQuickReply;
- (bool)isSavingAsDraft;
- (bool)isShowingRecentPersonCard;
- (bool)isVerticallyCompact;
- (id)keyCommands;
- (id)lastDraftMessage;
- (void)loadView;
- (id)mailComposeView;
- (void)mailCompositionContext:(id)arg1 didFinishLoadingMessage:(id)arg2 legacyMessage:(id)arg3 error:(id)arg4;
- (void)mailCompositionContext:(id)arg1 messageDidChange:(id)arg2;
- (void)markupAttachment:(id)arg1;
- (unsigned long long)markupReplyAttachmentLoadingProgress;
- (id)messageLoadingTailspinCancellable;
- (id)mf_preferredTitle;
- (id)navigationBarTitle;
- (id)navigationControllerForRecentPersonCard;
- (bool)needsDelivery;
- (id)nextResponderForRecipientView:(id)arg1;
- (id)originalContentMessagesFuture;
- (id)originalContentMessagesScheduler;
- (id)originalMessageObjectID;
- (id)originatingBundleID;
- (id)peoplePicker;
- (void)performSend;
- (void)photoPicker:(id)arg1 didDeselectAssetWithIdentifier:(id)arg2;
- (void)photoPicker:(id)arg1 didSelectAssetWithIdentifier:(id)arg2 mediaInfo:(id)arg3;
- (void)photoPickerController:(id)arg1 didAddItem:(id)arg2;
- (void)photoPickerController:(id)arg1 didRemoveAsset:(id)arg2;
- (void)photoPickerControllerDidCancel:(id)arg1;
- (void)photoPickerDidCancel:(id)arg1;
- (void)photoPickerDidCancelSystemImagePicker:(id)arg1;
- (id)photoPickerProgressManager;
- (void)pickInitialFirstResponder;
- (id)popoverAlert;
- (id)popoverManager;
- (id)popoverManagerCreateIfNeeded:(bool)arg1;
- (void)popoverPresentationController:(id)arg1 willRepositionPopoverToRect:(inout struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; }*)arg2 inView:(inout id*)arg3;
- (long long)popoverPresentationStyleForViewController:(id)arg1;
- (void)prepareForPopoverPresentation:(id)arg1;
- (id)prepareImagesForSendScheduler;
- (void)presentSaveDeleteDialogOrClose;
- (bool)presentSearchResultsForComposeRecipientView:(id)arg1;
- (void)presentationControllerDidDismiss:(id)arg1;
- (unsigned long long)presentationOptionsForRecipient:(id)arg1;
- (id)presentationViewController;
- (id)presentingViewControllerForComposeStyleSelector:(id)arg1;
- (id)presentingViewControllerForPhotoPicker:(id)arg1;
- (id)previewController;
- (void)previewController:(id)arg1 didSaveEditedCopyOfPreviewItem:(id)arg2 atURL:(id)arg3;
- (long long)previewController:(id)arg1 editingModeForPreviewItem:(id)arg2;
- (void)previewControllerDidDismiss:(id)arg1;
- (id)previewingAttachment;
- (double)previousPaletteWidth;
- (id)quickReplyActionButtonProvider;
- (id)quickReplyDelegate;
- (id)recipientPresentingCard;
- (void)recipientViewDidBecomeFirstResponder:(id)arg1;
- (void)recipientViewDidResignFirstResponder:(id)arg1;
- (bool)recipientViewShouldIgnoreFirstResponderChanges:(id)arg1;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })rectOfAttachment:(id)arg1;
- (id)remoteViewControllerProxy;
- (void)removeAddress:(id)arg1 field:(long long)arg2;
- (void)removeSignature;
- (long long)resolution;
- (id)rotatingHeaderView;
- (void)saveAndResignFirstResponder;
- (void)saveFirstResponder;
- (id)savedHeaders;
- (void)scanDocument;
- (void)secureMIMECompositionManager:(id)arg1 encryptionStatusDidChange:(long long)arg2 context:(id)arg3;
- (void)secureMIMECompositionManager:(id)arg1 signingStatusDidChange:(long long)arg2 context:(id)arg3;
- (void)securityScopeForURL:(id)arg1 withHandler:(id /* block */)arg2;
- (void)selectNextSearchResultForComposeRecipientView:(id)arg1;
- (void)selectPreviousSearchResultForComposeRecipientView:(id)arg1;
- (void)send:(id)arg1;
- (id)sendButtonItem;
- (id)sendLaterMenu;
- (void)sendMessage:(unsigned long long)arg1;
- (bool)sendingAccountIsExchange;
- (id)sendingAccountProxy;
- (id)sendingAddressForControl:(id)arg1;
- (id)sendingEmailAddress;
- (id)sendingEmailAddressIfExists;
- (bool)sendingEmailDirtied;
- (void)serializedPlaceholderForFileName:(id)arg1 fileSize:(long long)arg2 mimeType:(id)arg3 contentID:(id)arg4 withHandler:(id /* block */)arg5;
- (void)setAccountProxyGenerator:(id)arg1;
- (void)setAddressForMissingIdentity:(id)arg1;
- (void)setAddresses:(id)arg1 field:(long long)arg2;
- (void)setAtomPresentingCard:(id)arg1;
- (void)setAttachmentToMarkupIsLoaded:(bool)arg1;
- (void)setAutosaveIdentifier:(id)arg1;
- (void)setAutosaveIsValid:(bool)arg1;
- (void)setAutosaveQueue:(id)arg1;
- (void)setAutosaveSession:(id)arg1;
- (void)setBccRecipients:(id)arg1;
- (void)setCaretPosition:(unsigned long long)arg1;
- (void)setCcRecipients:(id)arg1;
- (void)setCertificatesByRecipient:(id)arg1;
- (void)setComposeCoordinator:(id)arg1;
- (void)setCompositionContext:(id)arg1;
- (void)setContactViewController:(id)arg1;
- (void)setContentVisible:(bool)arg1;
- (void)setCurrentlyLoadingCounter:(long long)arg1;
- (void)setDelayToShowMarkupHasPassed:(bool)arg1;
- (void)setDelegate:(id)arg1;
- (void)setDidSaveQuickReplyDraftHandler:(id /* block */)arg1;
- (void)setDidUseOmittedAttachmentsOrRecipients:(bool)arg1;
- (void)setDisplayMetrics:(id)arg1;
- (void)setDocumentCameraViewController:(id)arg1;
- (void)setDraftMessage:(id)arg1;
- (void)setDrawingFileAttachments:(id)arg1;
- (void)setErrorsByRecipient:(id)arg1;
- (void)setFontAttributes:(id)arg1;
- (void)setHasActiveWritingToolsSession:(bool)arg1;
- (void)setHasAddedEditableHyperlink:(bool)arg1;
- (void)setHasCustomSignature:(id)arg1;
- (void)setHasMessageBodyContent:(bool)arg1;
- (void)setHasPresentedSenderCollaborationAlert:(bool)arg1;
- (void)setHeadersForDraft;
- (void)setHideMyEmailAddressForMailToURLAddressString:(id)arg1;
- (void)setHideMyEmailAddressIfExists:(id)arg1;
- (void)setHideMyEmailMultipleRecipient:(bool)arg1;
- (void)setHideMyEmailSenderAddress:(id)arg1;
- (void)setHosted:(bool)arg1;
- (void)setInitialMissingAttachmentWarnings:(id)arg1;
- (void)setInitialTitle:(id)arg1;
- (void)setInputContextHistoryInvalidationDebouncer:(id)arg1;
- (void)setInsertingDrawing:(bool)arg1;
- (void)setIsDirty:(bool)arg1;
- (void)setIsModal:(bool)arg1;
- (void)setIsQuickReply:(bool)arg1;
- (void)setMarkupReplyAttachmentLoadingProgress:(unsigned long long)arg1;
- (void)setMessageLoadingTailspinCancellable:(id)arg1;
- (void)setOriginalContentMessagesFuture:(id)arg1;
- (void)setOriginalContentMessagesScheduler:(id)arg1;
- (void)setOriginalMessageWasEncrypted:(bool)arg1;
- (void)setOriginatingBundleID:(id)arg1;
- (void)setPeoplePicker:(id)arg1;
- (void)setPhotoPickerProgressManager:(id)arg1;
- (void)setPopoverAlert:(id)arg1;
- (void)setPrefersLargeTitles:(bool)arg1;
- (void)setPreviewController:(id)arg1;
- (void)setPreviewingAttachment:(id)arg1;
- (void)setPreviousPaletteWidth:(double)arg1;
- (void)setProgressUIVisible:(bool)arg1 animated:(bool)arg2;
- (void)setQuickReplyActionButtonProvider:(id)arg1;
- (void)setQuickReplyDelegate:(id)arg1;
- (void)setRecipientPresentingCard:(id)arg1;
- (void)setRecipientsKeyboardType:(long long)arg1;
- (void)setRemoteViewControllerProxy:(id)arg1;
- (void)setResolution:(long long)arg1;
- (void)setSavedHeaders:(id)arg1;
- (void)setSendingEmailAddress:(id)arg1;
- (void)setSendingEmailAddressAsHME:(id)arg1 isComposeTypeReply:(bool)arg2;
- (void)setSmimeStatusControl:(id)arg1;
- (void)setSourceAccountManagement:(int)arg1;
- (void)setStyleSelector:(id)arg1;
- (void)setSubject:(id)arg1;
- (void)setTipsManager:(id)arg1;
- (void)setToRecipients:(id)arg1;
- (void)setUpForSaveAsDraft;
- (void)setUseMailDrop:(bool)arg1;
- (id)shouldCreateRichTextRepresentation;
- (bool)shouldShowSMIMEButton;
- (void)showMissingAttachmentDataAlert;
- (void)showPhotoPickerWithSourceType:(long long)arg1 fromSource:(id)arg2;
- (void)showStyleSelector:(id)arg1;
- (unsigned long long)signpostID;
- (id)smartReplyThreadIdentifier;
- (id)smimeStatusControl;
- (unsigned long long)smimeStatusOptionsForControl:(id)arg1;
- (int)sourceAccountManagement;
- (void)stripCustomBodyIdentifiers;
- (id)styleSelector;
- (id)subject;
- (void)takeFocusFromComposeWebView:(id)arg1 inDirection:(unsigned long long)arg2;
- (void)takeSnapshotWithCompletionHandler:(id /* block */)arg1;
- (void)tappedSMIMEButton;
- (id)tipsManager;
- (id)toRecipients;
- (void)toggleEncryptionForControl:(id)arg1;
- (void)traitCollectionDidChange:(id)arg1;
- (void)updateSignature;
- (void)updateUserActivityState:(id)arg1;
- (bool)useMailDrop;
- (void)userActivity:(id)arg1 didReceiveInputStream:(id)arg2 outputStream:(id)arg3;
- (void)viewDidAppear:(bool)arg1;
- (void)viewDidDisappear:(bool)arg1;
- (void)viewDidLayoutSubviews;
- (void)viewDidLoad;
- (void)viewDidUnload;
- (void)viewLayoutMarginsDidChange;
- (void)viewWillAppear:(bool)arg1;
- (void)viewWillDisappear:(bool)arg1;
- (void)viewWillUnload;
- (void)willRotateToInterfaceOrientation:(long long)arg1 duration:(double)arg2;

@end
