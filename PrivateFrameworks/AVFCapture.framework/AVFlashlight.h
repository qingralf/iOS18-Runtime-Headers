/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/AVFCapture.framework/AVFCapture
 */

@interface AVFlashlight : NSObject {
    AVFlashlightInternal * _internal;
}

@property (getter=isAvailable, nonatomic, readonly) bool available;
@property (nonatomic) float beamWidth;
@property (nonatomic, readonly) bool beamWidthControlSupported;
@property (nonatomic, readonly) float flashlightLevel;
@property (nonatomic, readonly) float maxBeamWidth;
@property (nonatomic, readonly) float minBeamWidth;
@property (getter=isOverheated, nonatomic, readonly) bool overheated;

+ (bool)hasFlashlight;
+ (void)initialize;

- (void)_handleNotification:(id)arg1 payload:(id)arg2;
- (void)_reconnectToServer;
- (void)_setupFlashlight;
- (void)_teardownFlashlight;
- (float)beamWidth;
- (bool)beamWidthControlSupported;
- (void)dealloc;
- (float)flashlightLevel;
- (id)init;
- (bool)isAvailable;
- (bool)isOverheated;
- (float)maxBeamWidth;
- (float)minBeamWidth;
- (void)setBeamWidth:(float)arg1;
- (bool)setFlashlightLevel:(float)arg1 withError:(id*)arg2;
- (void)turnPowerOff;
- (bool)turnPowerOnWithError:(id*)arg1;

@end
