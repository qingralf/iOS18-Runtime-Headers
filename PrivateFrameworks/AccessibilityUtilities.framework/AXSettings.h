/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/AccessibilityUtilities.framework/AccessibilityUtilities
 */

@interface AXSettings : AXBaseSettings {
    bool  _assistiveTouchInternalOnlyHiddenNubbitModeEnabled;
    bool  _assistiveTouchMotionTrackerShouldOffsetBufferPoints;
    bool  _assistiveTouchOnDeviceEyeTrackingShouldOffsetBufferPoints;
    bool  _guestsShouldInheritPrimaryUserPreferences;
    NSString * _hoverTextContentSize;
    long long  _hoverTextScrollingSpeed;
    bool  _hoverTextShowedBanner;
    bool  _hoverTextTypingShowedBanner;
    double  _lastGuidedAccessTimeLimitResetCount;
    double  _lastGuidedAccessTimeResetCount;
    double  _lastHearingAidControlPanelTimeResetCount;
    double  _lastHearingAidHandoffTimeResetCount;
    double  _lastPlatformSwitchTimeResetCount;
    long long  _liveSpeechMaxRecentsCount;
    double  _magnifierBrightness;
    double  _magnifierContrast;
    bool  _magnifierEnabled;
    bool  _magnifierFilterInverted;
    unsigned long long  _magnifierFilterSetIdentifier;
    bool  _magnifierShouldAdjustFiltersForAmbientLight;
    bool  _magnifierShouldUseVideoStabilization;
    double  _magnifierZoomLevel;
    NSMutableSet * _migratedSwitchControlMenuItemsPreferenceKeys;
    float  _reduceWhitePointLevel;
    AXPreferences * _swiftSettings;
    NSArray * _switchControlCombinedTopLevelMenuItems;
    bool  _touchAccommodationsHoldDurationAllowsSwipeGesturesToBypass;
    NSMutableDictionary * _unarchivedVoiceCache;
    bool  _voiceOverVerbositySpeakCustomActionsHint;
    float  _voiceOverVolume;
    bool  _writeAXLogsToFile;
}

@property (readonly) bool accessibilityEnabled;
@property (getter=isAdaptiveVoiceShortcutsEnabled, nonatomic) bool adaptiveVoiceShortcutsEnabled;
@property (nonatomic, retain) NSURL *alexLocalAssetURL;
@property (nonatomic) bool allowHearingAidControlOnLockScreen;
@property (nonatomic) bool appValidationTestingMode;
@property (readonly) unsigned long long appleTVAppGridItemsPerLine;
@property (nonatomic) bool appleTVRemoteClickpadTapsForDirectionalNavigationEnabled;
@property bool appleTVScaledUIEnabled;
@property bool applicationAccessibilityEnabled;
@property (nonatomic) bool assistiveTouchActionRepeatEnabled;
@property (nonatomic) double assistiveTouchActionRepeatInterval;
@property (nonatomic, retain) NSDictionary *assistiveTouchActionsBySoundAction;
@property (nonatomic) bool assistiveTouchAlwaysShowMenuEnabled;
@property (nonatomic) double assistiveTouchAxisSweepSpeed;
@property (nonatomic) bool assistiveTouchBubbleModeEnabled;
@property (nonatomic, retain) NSSet *assistiveTouchCameraPointPickerSwitches;
@property (nonatomic) bool assistiveTouchCameraSwitchPreviewEnabled;
@property (nonatomic) long long assistiveTouchCursorColor;
@property (nonatomic, retain) NSArray *assistiveTouchCustomGestures;
@property (nonatomic) double assistiveTouchDelayAfterInput;
@property (nonatomic) bool assistiveTouchDelayAfterInputEnabled;
@property (nonatomic, retain) NSString *assistiveTouchDoubleTapAction;
@property (nonatomic) double assistiveTouchDoubleTapActionTimeout;
@property bool assistiveTouchEnabled;
@property (nonatomic) bool assistiveTouchEnabledByOnDeviceEyeTracking;
@property (nonatomic, readonly) NSDictionary *assistiveTouchEyeTrackerCustomizedSettings;
@property (nonatomic) bool assistiveTouchEyeTrackingAutoHideEnabled;
@property (nonatomic) double assistiveTouchEyeTrackingAutoHideOpacity;
@property (nonatomic) double assistiveTouchEyeTrackingAutoHideTimeout;
@property (nonatomic) bool assistiveTouchForceEyeTrackerExperience;
@property (nonatomic) bool assistiveTouchForceOnDeviceEyeTrackingExperience;
@property (nonatomic) bool assistiveTouchGameControllerEnabled;
@property (nonatomic) bool assistiveTouchGroupElementsEnabled;
@property bool assistiveTouchHardwareEnabled;
@property (nonatomic) long long assistiveTouchHeadMovementSensitivity;
@property (nonatomic) double assistiveTouchIdleOpacity;
@property (nonatomic) double assistiveTouchInputCoalescingDuration;
@property (nonatomic) bool assistiveTouchInputCoalescingEnabled;
@property (nonatomic) double assistiveTouchInputHoldDuration;
@property (nonatomic) bool assistiveTouchInputHoldEnabled;
@property (nonatomic) bool assistiveTouchInternalOnlyHiddenNubbitModeEnabled;
@property (nonatomic) bool assistiveTouchInternalOnlyPearlTrackingEnabled;
@property (nonatomic, retain) NSString *assistiveTouchLongPressAction;
@property (nonatomic) double assistiveTouchLongPressActionDuration;
@property (nonatomic) double assistiveTouchLongPressDuration;
@property (nonatomic) bool assistiveTouchLongPressEnabled;
@property (nonatomic) bool assistiveTouchLongPressPauseScanningEnabled;
@property (nonatomic, retain) NSDictionary *assistiveTouchMainScreenCustomization;
@property (nonatomic, readonly) NSDictionary *assistiveTouchMainScreenDefaultCustomization;
@property (nonatomic) bool assistiveTouchMotionTrackerConfigurable;
@property (nonatomic) bool assistiveTouchMotionTrackerShouldOffsetBufferPoints;
@property (nonatomic) unsigned long long assistiveTouchMotionTrackerSmoothingBufferSize;
@property (nonatomic) double assistiveTouchMotionTrackerSmoothingMaxDelta;
@property (nonatomic) double assistiveTouchMotionTrackerXNormalizationOffset;
@property (nonatomic) double assistiveTouchMotionTrackerXNormalizationOrder;
@property (nonatomic) double assistiveTouchMotionTrackerYNormalizationOffset;
@property (nonatomic) double assistiveTouchMotionTrackerYNormalizationOrder;
@property (nonatomic) bool assistiveTouchMouseAllowAppleBluetoothDevicesPairing;
@property (nonatomic) bool assistiveTouchMouseAlwaysShowSoftwareKeyboardEnabled;
@property (nonatomic) bool assistiveTouchMouseBehavesLikeFinger;
@property (nonatomic) bool assistiveTouchMouseClickSoundsEnabled;
@property (nonatomic, readonly) NSArray *assistiveTouchMouseCustomizedClickActions;
@property (nonatomic) double assistiveTouchMouseDwellControlActivationTimeout;
@property (nonatomic, retain) NSString *assistiveTouchMouseDwellControlAutorevertAction;
@property (nonatomic) bool assistiveTouchMouseDwellControlAutorevertEnabled;
@property (nonatomic, retain) NSDictionary *assistiveTouchMouseDwellControlCornerCustomization;
@property (nonatomic) bool assistiveTouchMouseDwellControlEnabled;
@property (nonatomic) double assistiveTouchMouseDwellControlMovementToleranceRadius;
@property (nonatomic) bool assistiveTouchMouseDwellControlMutatedMenu;
@property (nonatomic) bool assistiveTouchMouseDwellControlShowPrompt;
@property (nonatomic) long long assistiveTouchMouseKeysDelay;
@property (nonatomic) bool assistiveTouchMouseKeysEnabled;
@property (nonatomic) long long assistiveTouchMouseKeysMaxSpeed;
@property (nonatomic) bool assistiveTouchMouseKeysOptionToggleEnabled;
@property (nonatomic) bool assistiveTouchMouseKeysUseMainKeyboardKeys;
@property (nonatomic, retain) NSArray *assistiveTouchMouseOnDeviceEyeTrackingCalibratedValues;
@property (nonatomic) long long assistiveTouchMouseOnDeviceEyeTrackingCalibrationStrategy;
@property (nonatomic) bool assistiveTouchMouseOnDeviceEyeTrackingEnabled;
@property (nonatomic) bool assistiveTouchMouseOnDeviceEyeTrackingEnrollmentCompleted;
@property (nonatomic, retain) NSDictionary *assistiveTouchMouseOnDeviceEyeTrackingEnrollmentPointToGazePointMap;
@property (nonatomic) bool assistiveTouchMouseOnDeviceEyeTrackingOnboardingCompleted;
@property (nonatomic) bool assistiveTouchMouseOnDeviceEyeTrackingShouldShowUncalibratedPoints;
@property (nonatomic) long long assistiveTouchMousePointerColor;
@property (nonatomic) double assistiveTouchMousePointerSizeMultiplier;
@property (nonatomic) double assistiveTouchMousePointerTimeout;
@property (nonatomic) bool assistiveTouchMousePointerTimeoutEnabled;
@property (nonatomic) unsigned long long assistiveTouchMouseZoomPanningStyle;
@property (nonatomic) bool assistiveTouchOnDeviceEyeTrackingAutoHideEnabled;
@property (nonatomic) double assistiveTouchOnDeviceEyeTrackingAutoHideOpacity;
@property (nonatomic) double assistiveTouchOnDeviceEyeTrackingAutoHideTimeout;
@property (nonatomic) bool assistiveTouchOnDeviceEyeTrackingShouldOffsetBufferPoints;
@property (nonatomic) bool assistiveTouchOnDeviceEyeTrackingShouldUseGaussianBlur;
@property (nonatomic) bool assistiveTouchOnDeviceEyeTrackingShouldUseMotionFilter;
@property (nonatomic) bool assistiveTouchOnDeviceEyeTrackingShouldUseWeightedAverages;
@property (nonatomic) unsigned long long assistiveTouchOnDeviceEyeTrackingSmoothingBufferSize;
@property (nonatomic) bool assistiveTouchOpenMenuSwaggleEnabled;
@property (nonatomic, retain) NSString *assistiveTouchOrbAction;
@property (nonatomic) long long assistiveTouchPreferredPointPicker;
@property (nonatomic, retain) NSArray *assistiveTouchRecentGestures;
@property (nonatomic, retain) NSArray *assistiveTouchSavedGestures;
@property (nonatomic) long long assistiveTouchScanCycles;
@property (nonatomic) double assistiveTouchScanTimeout;
@property (nonatomic) bool assistiveTouchScanTimeoutEnabled;
@property (nonatomic) bool assistiveTouchScannerAddedTripleClickAutomatically;
@property (nonatomic) bool assistiveTouchScannerCompactMenuEnabled;
@property (nonatomic) bool assistiveTouchScannerCursorHighVisibilityEnabled;
@property (nonatomic) bool assistiveTouchScannerMenuLabelsEnabled;
@property (nonatomic) bool assistiveTouchScannerSoundEnabled;
@property (nonatomic) double assistiveTouchScannerSpeechRate;
@property (nonatomic) long long assistiveTouchScanningMode;
@property (nonatomic) bool assistiveTouchShouldUseExtendedKeyboardPredictions;
@property (nonatomic, retain) NSString *assistiveTouchSingleTapAction;
@property (nonatomic) bool assistiveTouchSleepOnWristDownPreference;
@property (nonatomic) double assistiveTouchSpeed;
@property (nonatomic) double assistiveTouchStepInterval;
@property (nonatomic) bool assistiveTouchSwitchUsageConfirmed;
@property (nonatomic, retain) NSSet *assistiveTouchSwitches;
@property bool assistiveTouchUIEnabled;
@property (nonatomic, copy) NSDictionary *assistiveTouchUsageCount;
@property (nonatomic) long long assistiveTouchVirtualTrackpadBorderColor;
@property (nonatomic) double assistiveTouchVirtualTrackpadBorderOpacity;
@property (nonatomic) double assistiveTouchVirtualTrackpadBorderWidth;
@property (nonatomic) bool assistiveTouchVirtualTrackpadEnabled;
@property (nonatomic) double assistiveTouchVirtualTrackpadFadeDelay;
@property (nonatomic) bool assistiveTouchVirtualTrackpadFullScreenToggleEnabled;
@property (nonatomic) double assistiveTouchVirtualTrackpadHoldDuration;
@property (nonatomic) long long assistiveTouchVirtualTrackpadInnerColor;
@property (nonatomic) double assistiveTouchVirtualTrackpadInnerOpacity;
@property (nonatomic) double assistiveTouchVirtualTrackpadScrollSpeed;
@property (nonatomic) bool assistiveTouchVirtualTrackpadScrollToggleEnabled;
@property (nonatomic) double assistiveTouchVirtualTrackpadSpeed;
@property double audioLeftRightBalance;
@property (nonatomic) double audiogramIngestionLastModelAccess;
@property (nonatomic) bool automaticAccessibilityEnabled;
@property (nonatomic) bool automaticAccessibilityIgnoreAppAccessibilityPreferred;
@property (nonatomic) bool automaticAccessibilityVisualizationsEnabled;
@property bool automationEnabled;
@property bool automationFauxCollectionViewCellsEnabled;
@property bool automationFauxTableViewCellsEnabled;
@property bool automationHitpointWarpingEnabled;
@property bool automationLocalizedStringLookupInfoEnabled;
@property (readonly) NSString *automationPreferredLocalization;
@property (nonatomic) bool automationTrueTouch;
@property (nonatomic, retain) NSString *backTapDoubleTapAction;
@property (nonatomic) bool backTapEnabled;
@property (nonatomic) bool backTapFalsePositiveAlertsEnabled;
@property (nonatomic) bool backTapRNNModelEnabled;
@property (nonatomic, retain) NSString *backTapTripleTapAction;
@property (nonatomic) long long backTapUsageCount;
@property (nonatomic) long long brokenHomeButtonCount;
@property bool buttonShapesEnabled;
@property (nonatomic) double callAudioRoutingAutoAnswerDelay;
@property (nonatomic) bool callAudioRoutingAutoAnswerEnabled;
@property (nonatomic) double cameraButtonSensitivity;
@property (nonatomic) double characterFeedbackDelayDuration;
@property (nonatomic) long long characterVoicesUsageCount;
@property (nonatomic) bool classicInvertColors;
@property (nonatomic, retain) NSArray *customPronunciationSubstitutions;
@property (nonatomic, readonly) NSArray *deviceKeys;
@property (nonatomic) bool didResetD22Preferences;
@property (nonatomic) bool didTriggerSOSToday;
@property (nonatomic) double dockSize;
@property (nonatomic) bool enableHearingAidReporter;
@property (nonatomic) bool enableVoiceOverCaptions;
@property (nonatomic, retain) NSArray *enabledLiveRecognitionModes;
@property bool enhanceBackgroundContrastEnabled;
@property bool enhanceTextLegibilityEnabled;
@property bool enhanceTextTrackingEnabled;
@property (nonatomic, retain) NSData *fullKeyboardAccessCommandMapData;
@property (nonatomic) long long fullKeyboardAccessFocusRingColor;
@property (nonatomic) bool fullKeyboardAccessFocusRingHighContrastEnabled;
@property (nonatomic) double fullKeyboardAccessFocusRingTimeout;
@property (nonatomic) bool fullKeyboardAccessFocusRingTimeoutEnabled;
@property (nonatomic) bool fullKeyboardAccessLargeFocusRingEnabled;
@property (nonatomic) bool fullKeyboardAccessShouldShowDebugKeyCommandsView;
@property (nonatomic) bool fullKeyboardAccessShouldShowTextEditingModeInstructions;
@property (nonatomic, copy) NSString *gaxInternalSettingsActiveAppID;
@property (nonatomic, copy) NSNumber *gaxInternalSettingsActiveAppOrientation;
@property (nonatomic, copy) NSNumber *gaxInternalSettingsECID;
@property (nonatomic) bool gaxInternalSettingsIsActiveAppSelfLocked;
@property (nonatomic, retain) NSDate *gaxInternalSettingsLastActivationDate;
@property (nonatomic, retain) NSDate *gaxInternalSettingsLastPasscodeSetDate;
@property (nonatomic, copy) NSString *gaxInternalSettingsProductBuildVersion;
@property (nonatomic, retain) NSDictionary *gaxInternalSettingsSavedAccessibilityFeatures;
@property (nonatomic, retain) NSArray *gaxInternalSettingsSavedAccessibilityTripleClickOptions;
@property (nonatomic) bool gaxInternalSettingsSystemDidRestartDueToLowBattery;
@property (nonatomic) bool gaxInternalSettingsTimeRestrictionHasExpired;
@property (nonatomic, retain) NSDictionary *gaxInternalSettingsUserAppProfile;
@property (nonatomic, retain) NSArray *gaxInternalSettingsUserConfiguredAppIDs;
@property (nonatomic, retain) NSDictionary *gaxInternalSettingsUserGlobalProfile;
@property (nonatomic, readonly) NSArray *gestureKeys;
@property (nonatomic) bool gizmoApplicationAccessibilityEnabled;
@property (nonatomic) bool guestsShouldInheritPrimaryUserPreferences;
@property (nonatomic) bool guidedAccessAXFeaturesEnabled;
@property (nonatomic, readonly) bool guidedAccessAllowsMultipleWindows;
@property (nonatomic) bool guidedAccessAllowsUnlockWithTouchID;
@property (nonatomic) long long guidedAccessAutoLockTimeInSeconds;
@property (nonatomic, readonly) NSString *guidedAccessDefaultToneIdentifierForTimeRestrictionEvents;
@property (nonatomic) bool guidedAccessDisallowDirectInactiveToActiveTransition;
@property (nonatomic) bool guidedAccessEnableExperimentalUI;
@property (nonatomic, readonly) bool guidedAccessOverrideIdleTime;
@property (nonatomic, retain) NSNumber *guidedAccessOverrideTimeRestrictionDuration;
@property (nonatomic) bool guidedAccessShouldSpeakForTimeRestrictionEvents;
@property (nonatomic) long long guidedAccessTimeLimitsUsageCount;
@property (nonatomic, copy) NSString *guidedAccessToneIdentifierForTimeRestrictionEvents;
@property (nonatomic) long long guidedAccessUsageCount;
@property (nonatomic) bool guidedAccessUserPrefersMirroringForExternalDisplays;
@property (nonatomic) bool hapticMusicActive;
@property (nonatomic) NSString *hapticMusicAlgorithmSelection;
@property (nonatomic) bool hapticMusicEnhancedPercussion;
@property (nonatomic) bool hapticMusicEnhancedVocals;
@property (nonatomic) bool hapticMusicToggledInPreferences;
@property (nonatomic) bool hapticMusicTrackAvailable;
@property (nonatomic) double hapticMusicUsagePerDay;
@property (nonatomic, retain) NSDate *hapticMusicUsageStartDate;
@property (nonatomic) long long hearingAidControlPanelCount;
@property (nonatomic) long long hearingAidHandOffCount;
@property bool highContrastFocusIndicatorsEnabled;
@property bool hoverTextActivationLockEnabled;
@property (copy) NSData *hoverTextBackgroundColorData;
@property double hoverTextBackgroundOpacity;
@property (copy) NSData *hoverTextBorderColorData;
@property (nonatomic, retain) NSString *hoverTextContentSize;
@property long long hoverTextDisplayMode;
@property (nonatomic) bool hoverTextEnabled;
@property (copy) NSString *hoverTextFontName;
@property double hoverTextFontSize;
@property (copy) NSData *hoverTextInsertionPointColorData;
@property long long hoverTextPreferredActivatorKey;
@property (nonatomic) long long hoverTextScrollingSpeed;
@property (nonatomic) bool hoverTextShowedBanner;
@property (copy) NSData *hoverTextTextColorData;
@property (copy) NSData *hoverTextTypingAutocorrectedTextColorData;
@property (copy) NSData *hoverTextTypingBackgroundColorData;
@property (copy) NSData *hoverTextTypingBorderColorData;
@property (nonatomic) long long hoverTextTypingDisplayMode;
@property (nonatomic) bool hoverTextTypingEnabled;
@property (copy) NSString *hoverTextTypingFontName;
@property (copy) NSData *hoverTextTypingInsertionPointColorData;
@property (copy) NSData *hoverTextTypingMisspelledTextColorData;
@property (nonatomic) bool hoverTextTypingShowedBanner;
@property (copy) NSData *hoverTextTypingTextColorData;
@property (copy) NSString *hoverTextTypingTextStyle;
@property (nonatomic) bool ignoreAXAsserts;
@property (nonatomic) bool ignoreAXServerEntitlements;
@property (nonatomic) bool ignoreTrackpad;
@property (nonatomic) unsigned long long imageCaptionGenderStrategy;
@property (nonatomic, copy) NSArray *imageCaptioningDisabledApps;
@property (nonatomic) bool includeBacktraceInLogs;
@property bool increaseButtonLegibilityEnabled;
@property (nonatomic) bool independentHearingAidSettings;
@property bool inspectorEnabled;
@property (nonatomic) long long internalLoggingColorTheme;
@property (nonatomic) bool isAXValidationRunnerCollectingValidations;
@property (nonatomic, readonly) bool laserEnabled;
@property (nonatomic) unsigned long long laserZoomPanningStyle;
@property (nonatomic) double lastAssistiveTouchTimeResetCount;
@property (nonatomic) double lastBrailleScreenInputTimeResetCount;
@property (nonatomic) double lastCharacterVoiceTimeResetCount;
@property (nonatomic) double lastGuidedAccessTimeLimitResetCount;
@property (nonatomic) double lastGuidedAccessTimeResetCount;
@property (nonatomic) double lastHearingAidControlPanelTimeResetCount;
@property (nonatomic) double lastHearingAidHandoffTimeResetCount;
@property (nonatomic) double lastMagnifierResetCount;
@property (nonatomic) double lastPlatformSwitchTimeResetCount;
@property (nonatomic) double lastSmartInvertColorsEnablement;
@property (nonatomic) double lastTapticTimeResetCount;
@property (nonatomic) bool leftRightBalanceEnabled;
@property (nonatomic) double leftRightBalanceValue;
@property (nonatomic) bool letterFeedbackEnabled;
@property (nonatomic) bool liveRecognitionActive;
@property (nonatomic) bool liveRecognitionOverlayOpen;
@property (nonatomic) long long liveSpeechMaxRecentsCount;
@property (nonatomic) bool localizationQACaptionMode;
@property (nonatomic) bool localizationQACaptionShowFilePath;
@property (nonatomic) bool localizationQACaptionShowLocalizedString;
@property (nonatomic) bool localizationQACaptionShowUSString;
@property (nonatomic) bool logAXNotificationPosting;
@property (nonatomic) double magnifierBrightness;
@property (nonatomic) double magnifierContrast;
@property (nonatomic) bool magnifierEnabled;
@property (nonatomic) bool magnifierFilterInverted;
@property (nonatomic) unsigned long long magnifierFilterSetIdentifier;
@property (nonatomic) bool magnifierShouldAdjustFiltersForAmbientLight;
@property (nonatomic) bool magnifierShouldUseVideoStabilization;
@property (nonatomic) long long magnifierUsageCount;
@property (nonatomic) double magnifierZoomLevel;
@property (nonatomic, readonly) NSDictionary *managedAssetsRepresentation;
@property (nonatomic, readonly) NSArray *mediaControlsKeys;
@property (nonatomic, retain) NSString *medicalPreamble;
@property bool monoAudioEnabled;
@property (nonatomic, copy) NSArray *perAppSettingsCustomizedAppIDs;
@property (nonatomic, copy) NSDictionary *perAppSettingsStats;
@property (nonatomic) bool phoneticFeedbackEnabled;
@property (nonatomic) unsigned long long quickSpeakHighlightOption;
@property (nonatomic) struct CGPoint { double x1; double x2; } quickSpeakNubbitNormalizedPosition;
@property (nonatomic) long long quickSpeakSentenceHighlightColor;
@property (nonatomic) unsigned long long quickSpeakSentenceHighlightOption;
@property (nonatomic) float quickSpeakSpeakingRate;
@property (nonatomic) bool quickSpeakUnderlineSentence;
@property (nonatomic) float quickSpeakVolume;
@property (nonatomic) long long quickSpeakWordHighlightColor;
@property (nonatomic) bool quickTypeWordFeedbackEnabled;
@property (nonatomic) bool reachabilityEnabled;
@property bool reduceMotionAutoplayVideoPreviewsEnabled;
@property bool reduceMotionEnabled;
@property bool reduceWhitePointEnabled;
@property (nonatomic) float reduceWhitePointLevel;
@property (nonatomic, retain) NSDictionary *remoteHandGestureCustomizedActions;
@property (nonatomic) bool securePayAssertionActive;
@property (nonatomic, readonly) NSArray *settingsKeys;
@property (nonatomic) bool shouldCaptureVisionEngineDiagnosticsToDisk;
@property (nonatomic) bool shouldFlashForAlertInSilentMode;
@property (nonatomic) bool shouldFlashWhileUnlocked;
@property (nonatomic) bool shouldLimitDisplayRefreshRate;
@property (nonatomic) bool shouldPerformValidationsAtRuntime;
@property (nonatomic) bool shouldSpeakMedicalPreamble;
@property (nonatomic) bool shouldStreamToLeftAid;
@property (nonatomic) bool shouldStreamToRightAid;
@property (nonatomic) bool shouldTTYMedicalPreamble;
@property (nonatomic) bool showSpeechController;
@property (nonatomic) bool siriAutoUpdateListInitialized;
@property bool siriSemanticContextEnabled;
@property (nonatomic) bool skipHearingAidMFiAuth;
@property (nonatomic) double soundActionsLastModelAccess;
@property (nonatomic) double soundDetectionLastModelAccess;
@property (nonatomic) bool speakCorrectionsEnabled;
@property (nonatomic) long long speechControllerDoubleTapAction;
@property (nonatomic) double speechControllerIdleOpacity;
@property (nonatomic) long long speechControllerLongPressAction;
@property (nonatomic, retain) NSString *spokenContentDefaultFallbackLanguage;
@property (nonatomic) bool spokenContentShouldUseLanguageDetection;
@property (nonatomic) bool startupSoundEnabled;
@property (nonatomic) bool stickyKeysBeepEnabled;
@property (nonatomic) bool stickyKeysEnabled;
@property (nonatomic) bool stickyKeysShiftToggleEnabled;
@property (nonatomic) bool supportsAdvancedDisplayFilters;
@property (nonatomic, retain) AXPreferences *swiftSettings;
@property (nonatomic) double switchControlAutoTapTimeout;
@property (nonatomic) double switchControlCameraPointPickerDwellActivationTimeout;
@property (nonatomic) double switchControlCameraPointPickerDwellMovementToleranceRadius;
@property (nonatomic) unsigned long long switchControlCameraPointPickerMode;
@property (nonatomic) double switchControlCameraPointPickerMovementToleranceInJoystickMode;
@property (nonatomic) double switchControlCameraPointPickerSensitivity;
@property (nonatomic, retain) NSArray *switchControlCombinedTopLevelMenuItems;
@property (nonatomic) bool switchControlDebugLoggingEnabled;
@property (nonatomic, retain) NSArray *switchControlDeviceMenuItems;
@property (nonatomic, retain) NSArray *switchControlDeviceTopLevelMenuItems;
@property (nonatomic) double switchControlDwellTime;
@property bool switchControlEnabled;
@property (nonatomic) long long switchControlFirstLaunchScanningMode;
@property (nonatomic, retain) NSArray *switchControlGesturesMenuItems;
@property (nonatomic, retain) NSArray *switchControlGesturesTopLevelMenuItems;
@property (nonatomic, readonly) bool switchControlHasEmptyTopLevelMenu;
@property (nonatomic) bool switchControlIgnoreInvalidSwitchConfiguration;
@property (nonatomic) bool switchControlIsEnabledAsReceiver;
@property (nonatomic, retain) NSUUID *switchControlLaunchRecipeUUID;
@property (nonatomic, retain) NSArray *switchControlMediaControlsMenuItems;
@property (nonatomic, retain) NSArray *switchControlMediaControlsTopLevelMenuItems;
@property (nonatomic) long long switchControlPlatformSwitchedCount;
@property (nonatomic) long long switchControlPointPickerSelectionStyle;
@property (nonatomic, retain) NSArray *switchControlRecipes;
@property (nonatomic) bool switchControlRestartScanningAtCurrentKey;
@property (nonatomic) long long switchControlScanAfterTapLocation;
@property (nonatomic) long long switchControlScanningStyle;
@property (nonatomic, retain) NSArray *switchControlSettingsMenuItems;
@property (nonatomic, retain) NSArray *switchControlSettingsTopLevelMenuItems;
@property (nonatomic) bool switchControlShouldAlwaysActivateKeyboardKeys;
@property (nonatomic) bool switchControlShouldDisallowUSBRestrictedMode;
@property (nonatomic) bool switchControlShouldUseExtendedKeyboardPredictions;
@property (nonatomic) bool switchControlShouldUseShortFirstPage;
@property (nonatomic) long long switchControlTapBehavior;
@property (nonatomic, retain) NSArray *switchControlTopLevelMenuItems;
@property (nonatomic) bool switchControlUseCameraForPointMode;
@property (nonatomic) bool switchControlUserDidReadUSBRestrictedModeAlert;
@property (nonatomic) bool syncPronunciationsWithCloudKit;
@property (nonatomic) long long tapToSpeakTimeAvailability;
@property (nonatomic) bool tapToSpeakTimeEnabled;
@property (nonatomic) bool tapticTimeInternalFlashScreenEnabled;
@property (nonatomic, readonly) bool tapticTimeIsAvailable;
@property (nonatomic) long long tapticTimeUsageCount;
@property (nonatomic) double teachableMomentsDelayBeforeSending;
@property (nonatomic, retain) NSDictionary *teachableMomentsNotificationsSeen;
@property (nonatomic) bool touchAccommodationsAllowsSwipeGesturesToBypass;
@property (nonatomic, readonly) bool touchAccommodationsAreConfigured;
@property (nonatomic) bool touchAccommodationsEnabled;
@property (nonatomic) double touchAccommodationsHoldDuration;
@property (nonatomic) bool touchAccommodationsHoldDurationAllowsSwipeGesturesToBypass;
@property (nonatomic) bool touchAccommodationsHoldDurationEnabled;
@property (nonatomic, readonly) long long touchAccommodationsHoldDurationSwipeGestureSensitivity;
@property (nonatomic) double touchAccommodationsIgnoreRepeatDuration;
@property (nonatomic) bool touchAccommodationsIgnoreRepeatEnabled;
@property (nonatomic) double touchAccommodationsSwipeGestureMinimumDistance;
@property (nonatomic) long long touchAccommodationsTapActivationMethod;
@property (nonatomic) double touchAccommodationsTapActivationTimeout;
@property (nonatomic) bool touchAccommodationsTripleClickConfirmed;
@property (nonatomic) bool touchAccommodationsUsageConfirmed;
@property (retain) NSArray *tripleClickOptions;
@property (nonatomic, copy) NSArray *tripleClickOrderedOptions;
@property (nonatomic) bool useNewAXBundleLoader;
@property (nonatomic) bool validateSecondPartyApps;
@property (nonatomic) bool voiceOverActionFeedbackFirstInListOnly;
@property (nonatomic) long long voiceOverActionsFeedback;
@property (nonatomic) long long voiceOverActivationWorkaround;
@property (nonatomic) bool voiceOverAdjustSoundVolumeIndependently;
@property (nonatomic) bool voiceOverAlwaysTurnOnBluetooth;
@property (nonatomic) bool voiceOverAlwaysUseNemethCodeForMathEnabled;
@property (nonatomic) bool voiceOverAudioDuckingEnabled;
@property (nonatomic) bool voiceOverAudioFollowsHDMIAudio;
@property (nonatomic) bool voiceOverAutomaticButtonLabels;
@property (nonatomic) long long voiceOverBSIUsageCount;
@property (nonatomic) long long voiceOverBannerNotificationOutput;
@property (nonatomic) double voiceOverBrailleAlertDisplayDuration;
@property (nonatomic) bool voiceOverBrailleAlertShowUntilDismissed;
@property (nonatomic) bool voiceOverBrailleAlertsEnabled;
@property (nonatomic) double voiceOverBrailleAutoAdvanceDuration;
@property (copy) NSDictionary *voiceOverBrailleBluetoothDisplay;
@property long long voiceOverBrailleContractionMode;
@property (nonatomic, copy) NSDictionary *voiceOverBrailleDisconnectOnSleep;
@property (nonatomic, copy) NSArray *voiceOverBrailleDisplays;
@property bool voiceOverBrailleEightDotMode;
@property (nonatomic) bool voiceOverBrailleFormattingEnabled;
@property (nonatomic, readonly) bool voiceOverBrailleGesturesEnabled;
@property (nonatomic) long long voiceOverBrailleGesturesTypingFeedback;
@property (nonatomic) bool voiceOverBrailleGradeTwoAutoTranslateEnabled;
@property (nonatomic) double voiceOverBrailleKeyDebounceTimeout;
@property (nonatomic, copy) NSArray *voiceOverBrailleLanguageRotorItems;
@property long long voiceOverBrailleMasterStatusCellIndex;
@property (nonatomic) long long voiceOverBrailleStatusCellPreference;
@property (nonatomic, copy) NSString *voiceOverBrailleTableIdentifier;
@property long long voiceOverBrailleVirtualStatusAlignment;
@property (nonatomic) bool voiceOverBrailleWordWrapEnabled;
@property (nonatomic) long long voiceOverCapitalLetterFeedback;
@property (nonatomic) long long voiceOverContainerOutputFeedback;
@property (nonatomic, retain) NSData *voiceOverCustomCommandProfile;
@property (nonatomic) double voiceOverDelayUntilSpeakUnderTouch;
@property (nonatomic) long long voiceOverDeletionFeedback;
@property (nonatomic) long long voiceOverDescribedMedia;
@property (nonatomic, retain) NSArray *voiceOverDetectionModeItems;
@property (nonatomic, copy) NSDictionary *voiceOverDirectTouchEnabledApps;
@property (nonatomic) long long voiceOverDiscoveredSensitiveContentFeedback;
@property (nonatomic) double voiceOverDoubleTapInterval;
@property (nonatomic) bool voiceOverEditAppsActionEnabled;
@property (nonatomic) float voiceOverEffectiveSpeakingRate;
@property (nonatomic) float voiceOverEffectiveSpeakingVolume;
@property bool voiceOverEnabled;
@property bool voiceOverEnabledThroughAccessory;
@property (nonatomic) bool voiceOverExploreFocusAffectsNativeFocus;
@property (nonatomic) bool voiceOverFlashlightNotificationsEnabled;
@property (nonatomic, retain) NSDictionary *voiceOverHandGesturesActionCustomizations;
@property (nonatomic) bool voiceOverHandGesturesEnabled;
@property (nonatomic, readonly) bool voiceOverHandwritingEnabled;
@property (nonatomic, retain) NSNumber *voiceOverHandwritingWasNativeAutocorrectEnabled;
@property (nonatomic) double voiceOverHapticIntensity;
@property (nonatomic) bool voiceOverHapticsEnabled;
@property (nonatomic) long long voiceOverHardwareTypingFeedback;
@property (nonatomic) long long voiceOverHelpMode;
@property (nonatomic) bool voiceOverHintsEnabled;
@property (nonatomic) bool voiceOverIgnoreTrackpad;
@property (nonatomic) long long voiceOverInlineTextCompletionAppearanceFeedback;
@property (nonatomic) long long voiceOverInlineTextCompletionInsertionFeedback;
@property (nonatomic) long long voiceOverKeyboardModifierChoice;
@property (retain) NSArray *voiceOverLanguageRotorItems;
@property (readonly) bool voiceOverLanguageRotorItemsExist;
@property (nonatomic) long long voiceOverLinkFeedback;
@property (nonatomic) long long voiceOverLockedScreenNotificationOutput;
@property (nonatomic) double voiceOverMediaDuckingAmount;
@property (nonatomic) long long voiceOverMediaDuckingMode;
@property (nonatomic) long long voiceOverMoreContentOutputFeedback;
@property (nonatomic) long long voiceOverNavigateImagesOption;
@property (nonatomic) long long voiceOverNavigationDirectionMode;
@property (nonatomic) long long voiceOverNavigationStyle;
@property (nonatomic) long long voiceOverNeuralElementFeedback;
@property (nonatomic) long long voiceOverNumberFeedback;
@property (nonatomic) long long voiceOverPhoneticsFeedback;
@property (nonatomic) double voiceOverPitch;
@property (nonatomic) long long voiceOverPreferredTVInteractionMode;
@property (nonatomic) bool voiceOverPrefersFollowFocusNavigationStyle;
@property (nonatomic, copy) NSUUID *voiceOverPunctuationGroup;
@property (nonatomic) long long voiceOverPunctuationLevel;
@property (nonatomic) long long voiceOverQuickNavAnnouncementFeedback;
@property (nonatomic, copy) NSArray *voiceOverQuickSettingsItems;
@property (nonatomic, copy) NSArray *voiceOverRotorItems;
@property (nonatomic) long long voiceOverRotorSummaryFeedback;
@property (nonatomic) bool voiceOverRotorUpdatesWithElement;
@property (nonatomic) bool voiceOverSceneDescriptionsEnabled;
@property bool voiceOverScreenCurtainEnabled;
@property (nonatomic) bool voiceOverShouldDisallowUSBRestrictedMode;
@property (nonatomic) bool voiceOverShouldFocusEverywhere;
@property (nonatomic) bool voiceOverShouldOutputToHearingAid;
@property (nonatomic) bool voiceOverShouldSpeakDiscoveredText;
@property (nonatomic, readonly) bool voiceOverShowBrailleWatchSettings;
@property (nonatomic) bool voiceOverShowSoftwareKeyboardWithBraille;
@property (nonatomic) bool voiceOverSilenceAnnouncements;
@property (nonatomic) bool voiceOverSleepOnWristDownPreference;
@property (nonatomic) long long voiceOverSoftwareTypingFeedback;
@property (nonatomic, retain) NSDictionary *voiceOverSoundAndHapticPreferences;
@property (nonatomic) bool voiceOverSoundCurtain;
@property (nonatomic) bool voiceOverSoundEffectsEnabled;
@property (nonatomic) double voiceOverSoundVolume;
@property (nonatomic) bool voiceOverSpeakActionConfirmation;
@property (nonatomic) bool voiceOverSpeakNonfocusableElementsAfterDelay;
@property (nonatomic) bool voiceOverSpeakNotificationsEnabled;
@property (nonatomic) long long voiceOverSpeakSecondsEncoding;
@property (nonatomic) bool voiceOverSpeakTableColumnRowInformation;
@property (nonatomic) bool voiceOverSpeakTableHeaders;
@property float voiceOverSpeakingRate;
@property (nonatomic) bool voiceOverSpeakingRateInRotorEnabled;
@property (nonatomic) bool voiceOverSpeaksOverTelephoneCalls;
@property (nonatomic) long long voiceOverTapticChimesAvailability;
@property (nonatomic) bool voiceOverTapticChimesEnabled;
@property (nonatomic) long long voiceOverTapticChimesFrequencyEncoding;
@property (nonatomic) long long voiceOverTapticChimesSoundType;
@property (nonatomic) long long voiceOverTapticTimeEncoding;
@property (nonatomic) bool voiceOverTapticTimeMode;
@property (nonatomic) float voiceOverTapticTimeSpeed;
@property (nonatomic) bool voiceOverTouchBrailleGestureControl;
@property (nonatomic) bool voiceOverTouchBrailleGesturesDidPlayCalibrationHint;
@property (nonatomic) long long voiceOverTouchBrailleGesturesLockedOrientation;
@property (nonatomic) long long voiceOverTouchBrailleGesturesLockedTypingMode;
@property (nonatomic) bool voiceOverTouchBrailleGesturesShouldUseLockedConfiguration;
@property (nonatomic) bool voiceOverTouchBrailleShouldReverseDots;
@property (nonatomic) bool voiceOverTouchBrailleShowGeneralStatus;
@property (nonatomic) bool voiceOverTouchBrailleShowTextStyleStatus;
@property (copy) NSString *voiceOverTouchBrailleTableIdentifier;
@property (nonatomic) bool voiceOverTouchSingleLetterQuickNavEnabled;
@property (nonatomic) bool voiceOverTouchUpdateBrailleWithoutConnectedDisplay;
@property long long voiceOverTypingMode;
@property bool voiceOverUIEnabled;
@property bool voiceOverUsageConfirmed;
@property (nonatomic) bool voiceOverUseRingerSwitchToControlNotificationOutput;
@property (nonatomic) bool voiceOverUserDidReadUSBRestrictedModeAlert;
@property (nonatomic) long long voiceOverVerbosityEmojiFeedback;
@property (nonatomic) bool voiceOverVerbosityEmojiSuffixEnabled;
@property (nonatomic) bool voiceOverVerbositySpeakCustomActionsHint;
@property (nonatomic) float voiceOverVolume;
@property (nonatomic) bool wordFeedbackEnabled;
@property (nonatomic) bool writeAXLogsToFile;
@property (nonatomic) bool zoomAdjustZoomLevelKbShortcutEnabled;
@property (nonatomic) bool zoomAlwaysUseWindowedZoomForTyping;
@property (nonatomic) bool zoomAutopannerShouldPanWithAcceleration;
@property (nonatomic) long long zoomControllerColor;
@property (nonatomic, retain) NSString *zoomCurrentLensEffect;
@property (nonatomic, retain) NSString *zoomCurrentLensMode;
@property (nonatomic) bool zoomDebugDisableZoomLensScaleTransform;
@property (nonatomic) bool zoomDebugShowExternalFocusRect;
@property bool zoomEnabled;
@property (nonatomic, retain) NSDictionary *zoomHandGesturesActionCustomizations;
@property (nonatomic) bool zoomHandGesturesEnabled;
@property (nonatomic) double zoomIdleSlugOpacity;
@property (nonatomic) bool zoomInStandby;
@property (nonatomic) bool zoomKeyboardShortcutsEnabled;
@property (nonatomic) struct CGPoint { double x1; double x2; } zoomPanOffset;
@property (nonatomic) bool zoomPanZoomKbShortcutEnabled;
@property (nonatomic) bool zoomPeekZoomEnabled;
@property (nonatomic) bool zoomPeekZoomEverEnabled;
@property (nonatomic) bool zoomPreferencesWereInitialized;
@property (nonatomic, retain) NSString *zoomPreferredCurrentDockPosition;
@property (nonatomic, retain) NSString *zoomPreferredCurrentLensMode;
@property (nonatomic, retain) NSOrderedSet *zoomPreferredDockPositions;
@property (nonatomic, retain) NSOrderedSet *zoomPreferredLensModes;
@property (nonatomic) double zoomPreferredMaximumZoomScale;
@property (nonatomic) bool zoomResizeZoomWindowKbShortcutEnabled;
@property (nonatomic) double zoomScale;
@property (nonatomic) bool zoomScrollWheelKbShortcutEnabled;
@property (nonatomic) bool zoomShouldAllowFullscreenAutopanning;
@property (nonatomic) bool zoomShouldFollowFocus;
@property (nonatomic) bool zoomShouldShowSlug;
@property (nonatomic) bool zoomShowWhileMirroring;
@property (nonatomic) bool zoomShowedBanner;
@property (nonatomic) long long zoomSlugDoubleTapAction;
@property (nonatomic) struct CGPoint { double x1; double x2; } zoomSlugNormalizedPosition;
@property (nonatomic) long long zoomSlugSingleTapAction;
@property (nonatomic) bool zoomSlugTapAndSlideToAdjustZoomLevelEnabled;
@property (nonatomic) long long zoomSlugTripleTapAction;
@property bool zoomSmoothScalingEnabled;
@property (nonatomic) bool zoomSwitchZoomModeKbShortcutEnabled;
@property (nonatomic) bool zoomTempToggleZoomKbShortcutEnabled;
@property (nonatomic) bool zoomToggleZoomKbShortcutEnabled;
@property bool zoomToggledByPreferenceSwitch;
@property bool zoomToggledByVoiceOver;
@property (nonatomic) bool zoomTrackpadGestureEnabled;
@property (nonatomic) struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; } zoomWindowFrame;

// Image: /System/Library/PrivateFrameworks/AccessibilityUtilities.framework/AccessibilityUtilities

+ (id)sharedInstance;

- (void).cxx_destruct;
- (id)_allAccessibilityDomains;
- (id)_allSubmenuKeys;
- (void)_assignActionToUnsetHotCorner:(id)arg1;
- (void)_assignDefaultTitleIfNeededToRecipe:(id)arg1;
- (id)_audioHardwareChannelLayout;
- (void)_clearWhitetailMigrationSettings;
- (id)_createAndSaveRecipeForInterfaceOrientedPoint:(struct CGPoint { double x1; double x2; })arg1 forHold:(bool)arg2;
- (id)_customizedAppIDs;
- (bool)_defaultHapticValueForEvent:(id)arg1;
- (unsigned long long)_domainForPreferenceKey:(id)arg1;
- (struct __CFString { }*)_domainNameForDomain:(unsigned long long)arg1;
- (id)_gesturesForPreferenceKeyMigratingIfNecessary:(id)arg1;
- (void)_handleVoiceUnarchivedCache:(id)arg1;
- (bool)_hasSwitchWithUUID:(id)arg1;
- (id)_keysToIgnore;
- (id)_legacyIdentifierForEyeTracker:(id)arg1;
- (void)_listenForAccessibilitySupportPassthroughs;
- (void)_localeChange:(id)arg1;
- (id)_migratePreferenceIfNecessary:(id)arg1;
- (id)_quickSpeakAltVoices;
- (id)_quickSpeakPrefersCompact;
- (void)_setAudioHardwareChannelLayout:(id)arg1;
- (void)_setGestures:(id)arg1 forPreferenceKey:(id)arg2;
- (id)_switchControlAltVoices;
- (void)_switchControlClearMenuItemMigrationTracking;
- (void)_switchControlClearMenuItemPreferences;
- (bool)_switchControlHasEmptyTopLevelMenuItems:(id)arg1;
- (id)_switchControlMenuItemPreferences;
- (id)_switchControlMenuItemsForPreferenceKey:(id)arg1 allTypes:(id)arg2 enabledByDefault:(bool)arg3 topLevel:(bool)arg4;
- (id)_switchControlMigratePreMonarchIncludedMenuItemsForNewTypes:(id)arg1;
- (id)_switchControlPrefersCompact;
- (id)_switchControlScannerDefaultDialect;
- (id)_switchWithAction:(long long)arg1 excludedUUIDs:(id)arg2 needsToSupportLongPress:(bool)arg3;
- (id)_switchWithAnyActionExcludingUUIDs:(id)arg1 needsToSupportLongPress:(bool)arg2;
- (void)_updateTouchAccommodationsTripleClickSetting;
- (bool)_updateWithoutSavingExistingRecipe:(id)arg1 allRecipes:(id)arg2;
- (id)_voiceOverAlternativeVoiceIdentifiers;
- (id)_voiceOverDefaultDialect;
- (id)_voiceOverPrefersCompactByLanguage;
- (bool)_voiceOverRotorItemEnabled:(id)arg1;
- (void)_voiceOverSetSoundOrHapticValue:(id)arg1 forPreference:(id)arg2 outputEvent:(id)arg3;
- (id)_voiceOverSoundOrHapticValueForPreference:(id)arg1 outputEvent:(id)arg2;
- (id)_voiceOverTouchSpeakingRateLanguages;
- (bool)accessibilityEnabled;
- (void)addCustomizedAppID:(id)arg1;
- (void)addRotorOptionsForLoginSession;
- (void)aggregatePerAppSettingsStatistics;
- (id)alexLocalAssetURL;
- (id)allDomainNamesForPreferenceKey:(id)arg1;
- (bool)allowHearingAidControlOnLockScreen;
- (bool)anyUserPreferredLangaugeIsRTL;
- (bool)appValidationTestingMode;
- (unsigned long long)appleTVAppGridItemsPerLine;
- (bool)appleTVRemoteClickpadTapsForDirectionalNavigationEnabled;
- (bool)appleTVScaledUIEnabled;
- (bool)applicationAccessibilityEnabled;
- (bool)assistiveTouchActionRepeatEnabled;
- (double)assistiveTouchActionRepeatInterval;
- (id)assistiveTouchActionsBySoundAction;
- (bool)assistiveTouchAlwaysShowMenuEnabled;
- (bool)assistiveTouchAutoScanningEnabled;
- (double)assistiveTouchAxisSweepSpeed;
- (bool)assistiveTouchBubbleModeEnabled;
- (id)assistiveTouchCameraPointPickerSwitches;
- (bool)assistiveTouchCameraSwitchPreviewEnabled;
- (long long)assistiveTouchCursorColor;
- (id)assistiveTouchCustomGestures;
- (bool)assistiveTouchCustomizationEnabled;
- (double)assistiveTouchDelayAfterInput;
- (bool)assistiveTouchDelayAfterInputEnabled;
- (id)assistiveTouchDoubleTapAction;
- (double)assistiveTouchDoubleTapActionTimeout;
- (bool)assistiveTouchEnabled;
- (bool)assistiveTouchEnabledByOnDeviceEyeTracking;
- (id)assistiveTouchEyeTrackerCustomizedSettings;
- (bool)assistiveTouchEyeTrackingAutoHideEnabled;
- (double)assistiveTouchEyeTrackingAutoHideOpacity;
- (double)assistiveTouchEyeTrackingAutoHideTimeout;
- (bool)assistiveTouchForceEyeTrackerExperience;
- (bool)assistiveTouchForceOnDeviceEyeTrackingExperience;
- (bool)assistiveTouchGameControllerEnabled;
- (bool)assistiveTouchGroupElementsEnabled;
- (bool)assistiveTouchHardwareEnabled;
- (long long)assistiveTouchHeadMovementSensitivity;
- (double)assistiveTouchIdleOpacity;
- (double)assistiveTouchInputCoalescingDuration;
- (bool)assistiveTouchInputCoalescingEnabled;
- (double)assistiveTouchInputHoldDuration;
- (bool)assistiveTouchInputHoldEnabled;
- (bool)assistiveTouchInternalOnlyHiddenNubbitMode;
- (bool)assistiveTouchInternalOnlyHiddenNubbitModeEnabled;
- (bool)assistiveTouchInternalOnlyPearlTrackingEnabled;
- (id)assistiveTouchLongPressAction;
- (double)assistiveTouchLongPressActionDuration;
- (double)assistiveTouchLongPressDuration;
- (bool)assistiveTouchLongPressEnabled;
- (bool)assistiveTouchLongPressPauseScanningEnabled;
- (id)assistiveTouchMainScreenCustomization;
- (id)assistiveTouchMainScreenDefaultCustomization;
- (bool)assistiveTouchMotionTrackerConfigurable;
- (bool)assistiveTouchMotionTrackerShouldOffsetBufferPoints;
- (void)assistiveTouchMotionTrackerShouldOffsetBufferPoints:(bool)arg1;
- (unsigned long long)assistiveTouchMotionTrackerSmoothingBufferSize;
- (double)assistiveTouchMotionTrackerSmoothingMaxDelta;
- (double)assistiveTouchMotionTrackerXNormalizationOffset;
- (double)assistiveTouchMotionTrackerXNormalizationOrder;
- (double)assistiveTouchMotionTrackerYNormalizationOffset;
- (double)assistiveTouchMotionTrackerYNormalizationOrder;
- (bool)assistiveTouchMouseAllowAppleBluetoothDevicesPairing;
- (bool)assistiveTouchMouseAlwaysShowSoftwareKeyboardEnabled;
- (bool)assistiveTouchMouseBehavesLikeFinger;
- (bool)assistiveTouchMouseClickSoundsEnabled;
- (id)assistiveTouchMouseCustomizedClickActions;
- (double)assistiveTouchMouseDwellControlActivationTimeout;
- (id)assistiveTouchMouseDwellControlAutorevertAction;
- (bool)assistiveTouchMouseDwellControlAutorevertEnabled;
- (id)assistiveTouchMouseDwellControlCornerCustomization;
- (bool)assistiveTouchMouseDwellControlEnabled;
- (double)assistiveTouchMouseDwellControlMovementToleranceRadius;
- (bool)assistiveTouchMouseDwellControlMutatedMenu;
- (bool)assistiveTouchMouseDwellControlShowPrompt;
- (long long)assistiveTouchMouseKeysDelay;
- (bool)assistiveTouchMouseKeysEnabled;
- (long long)assistiveTouchMouseKeysMaxSpeed;
- (bool)assistiveTouchMouseKeysOptionToggleEnabled;
- (bool)assistiveTouchMouseKeysUseMainKeyboardKeys;
- (id)assistiveTouchMouseOnDeviceEyeTrackingCalibratedValues;
- (long long)assistiveTouchMouseOnDeviceEyeTrackingCalibrationStrategy;
- (bool)assistiveTouchMouseOnDeviceEyeTrackingEnabled;
- (bool)assistiveTouchMouseOnDeviceEyeTrackingEnrollmentCompleted;
- (id)assistiveTouchMouseOnDeviceEyeTrackingEnrollmentPointToGazePointMap;
- (bool)assistiveTouchMouseOnDeviceEyeTrackingOnboardingCompleted;
- (bool)assistiveTouchMouseOnDeviceEyeTrackingShouldShowUncalibratedPoints;
- (long long)assistiveTouchMousePointerColor;
- (double)assistiveTouchMousePointerSizeMultiplier;
- (double)assistiveTouchMousePointerTimeout;
- (bool)assistiveTouchMousePointerTimeoutEnabled;
- (unsigned long long)assistiveTouchMouseZoomPanningStyle;
- (bool)assistiveTouchOnDeviceEyeTrackingAutoHideEnabled;
- (double)assistiveTouchOnDeviceEyeTrackingAutoHideOpacity;
- (double)assistiveTouchOnDeviceEyeTrackingAutoHideTimeout;
- (bool)assistiveTouchOnDeviceEyeTrackingShouldOffsetBufferPoints;
- (bool)assistiveTouchOnDeviceEyeTrackingShouldUseGaussianBlur;
- (bool)assistiveTouchOnDeviceEyeTrackingShouldUseMotionFilter;
- (bool)assistiveTouchOnDeviceEyeTrackingShouldUseWeightedAverages;
- (unsigned long long)assistiveTouchOnDeviceEyeTrackingSmoothingBufferSize;
- (bool)assistiveTouchOpenMenuSwaggleEnabled;
- (id)assistiveTouchOrbAction;
- (long long)assistiveTouchPreferredPointPicker;
- (id)assistiveTouchRecentGestures;
- (id)assistiveTouchSavedGestures;
- (long long)assistiveTouchScanCycles;
- (double)assistiveTouchScanTimeout;
- (bool)assistiveTouchScanTimeoutEnabled;
- (bool)assistiveTouchScannerAddedTripleClickAutomatically;
- (bool)assistiveTouchScannerCompactMenuEnabled;
- (bool)assistiveTouchScannerCursorHighVisibilityEnabled;
- (bool)assistiveTouchScannerMenuLabelsEnabled;
- (bool)assistiveTouchScannerPrefersCompact:(id)arg1;
- (bool)assistiveTouchScannerSoundEnabled;
- (double)assistiveTouchScannerSpeechRate;
- (long long)assistiveTouchScanningMode;
- (bool)assistiveTouchShouldUseExtendedKeyboardPredictions;
- (id)assistiveTouchSingleTapAction;
- (bool)assistiveTouchSleepOnWristDownPreference;
- (double)assistiveTouchSpeed;
- (double)assistiveTouchStepInterval;
- (bool)assistiveTouchSwitchUsageConfirmed;
- (id)assistiveTouchSwitches;
- (bool)assistiveTouchUIEnabled;
- (id)assistiveTouchUsageCount;
- (long long)assistiveTouchVirtualTrackpadBorderColor;
- (double)assistiveTouchVirtualTrackpadBorderOpacity;
- (double)assistiveTouchVirtualTrackpadBorderWidth;
- (bool)assistiveTouchVirtualTrackpadEnabled;
- (double)assistiveTouchVirtualTrackpadFadeDelay;
- (bool)assistiveTouchVirtualTrackpadFullScreenToggleEnabled;
- (double)assistiveTouchVirtualTrackpadHoldDuration;
- (long long)assistiveTouchVirtualTrackpadInnerColor;
- (double)assistiveTouchVirtualTrackpadInnerOpacity;
- (double)assistiveTouchVirtualTrackpadScrollSpeed;
- (bool)assistiveTouchVirtualTrackpadScrollToggleEnabled;
- (double)assistiveTouchVirtualTrackpadSpeed;
- (double)audioLeftRightBalance;
- (double)audiogramIngestionLastModelAccess;
- (bool)automaticAccessibilityEnabled;
- (bool)automaticAccessibilityIgnoreAppAccessibilityPreferred;
- (long long)automaticAccessibilityModeForBundleIdentifier:(id)arg1;
- (id)automaticAccessibilityModes;
- (bool)automaticAccessibilityVisualizationsEnabled;
- (bool)automationEnabled;
- (bool)automationFauxCollectionViewCellsEnabled;
- (bool)automationFauxTableViewCellsEnabled;
- (bool)automationHitpointWarpingEnabled;
- (bool)automationLocalizedStringLookupInfoEnabled;
- (id)automationPreferredLocalization;
- (bool)automationTrueTouch;
- (id)backTapDoubleTapAction;
- (bool)backTapEnabled;
- (bool)backTapFalsePositiveAlertsEnabled;
- (bool)backTapRNNModelEnabled;
- (id)backTapTripleTapAction;
- (long long)backTapUsageCount;
- (long long)brokenHomeButtonCount;
- (bool)buttonShapesEnabled;
- (double)callAudioRoutingAutoAnswerDelay;
- (bool)callAudioRoutingAutoAnswerEnabled;
- (double)cameraButtonSensitivity;
- (double)characterFeedbackDelayDuration;
- (long long)characterVoicesUsageCount;
- (bool)classicInvertColors;
- (void)clearVoicePrefersCompactSettings;
- (void)configureZoomForLoginSession;
- (id)createAndSaveRecipeForInterfaceOrientedHoldPoint:(struct CGPoint { double x1; double x2; })arg1;
- (id)createAndSaveRecipeForInterfaceOrientedPoint:(struct CGPoint { double x1; double x2; })arg1;
- (id)customPronunciationSubstitutions;
- (id)customSettingsForVoice:(id)arg1 sourceKey:(id)arg2;
- (id)deviceKeys;
- (bool)didResetD22Preferences;
- (bool)didTriggerSOSToday;
- (bool)didTriggerSOSValueSet;
- (double)dockSize;
- (id)domainNameForPreferenceKey:(id)arg1;
- (void)enableAssistiveTechnologies:(bool)arg1 usingDefaults:(id)arg2;
- (bool)enableHearingAidReporter;
- (bool)enableVoiceOverCaptions;
- (id)enabledLiveRecognitionModes;
- (bool)enhanceBackgroundContrastEnabled;
- (bool)enhanceTextLegibilityEnabled;
- (bool)enhanceTextTrackingEnabled;
- (id)fullKeyboardAccessCommandMapData;
- (long long)fullKeyboardAccessFocusRingColor;
- (bool)fullKeyboardAccessFocusRingHighContrastEnabled;
- (double)fullKeyboardAccessFocusRingTimeout;
- (bool)fullKeyboardAccessFocusRingTimeoutEnabled;
- (bool)fullKeyboardAccessLargeFocusRingEnabled;
- (bool)fullKeyboardAccessShouldShowDebugKeyCommandsView;
- (bool)fullKeyboardAccessShouldShowTextEditingModeInstructions;
- (id)gaxInternalSettingsActiveAppID;
- (id)gaxInternalSettingsActiveAppOrientation;
- (id)gaxInternalSettingsECID;
- (bool)gaxInternalSettingsIsActiveAppSelfLocked;
- (id)gaxInternalSettingsLastActivationDate;
- (id)gaxInternalSettingsLastPasscodeSetDate;
- (id)gaxInternalSettingsProductBuildVersion;
- (id)gaxInternalSettingsSavedAccessibilityFeatures;
- (id)gaxInternalSettingsSavedAccessibilityTripleClickOptions;
- (bool)gaxInternalSettingsSystemDidRestartDueToLowBattery;
- (bool)gaxInternalSettingsTimeRestrictionHasExpired;
- (id)gaxInternalSettingsUserAppProfile;
- (id)gaxInternalSettingsUserConfiguredAppIDs;
- (id)gaxInternalSettingsUserGlobalProfile;
- (id)gestureKeys;
- (bool)gizmoApplicationAccessibilityEnabled;
- (bool)gizmoGetAutoSpeakEnabledForComplication:(id)arg1 slot:(id)arg2 face:(id)arg3;
- (id)gizmoGetWatchFaceAutoSpeakPreferences;
- (void)gizmoSetAutoSpeakEnabledForComplication:(id)arg1 slot:(id)arg2 face:(id)arg3 toggle:(bool)arg4;
- (bool)guestsShouldInheritPrimaryUserPreferences;
- (bool)guidedAccessAXFeaturesEnabled;
- (bool)guidedAccessAllowsMultipleWindows;
- (bool)guidedAccessAllowsUnlockWithTouchID;
- (long long)guidedAccessAutoLockTimeInSeconds;
- (id)guidedAccessDefaultToneIdentifierForTimeRestrictionEvents;
- (bool)guidedAccessDisallowDirectInactiveToActiveTransition;
- (bool)guidedAccessEnableExperimentalUI;
- (bool)guidedAccessOverrideIdleTime;
- (id)guidedAccessOverrideTimeRestrictionDuration;
- (bool)guidedAccessShouldSpeakForTimeRestrictionEvents;
- (long long)guidedAccessTimeLimitsUsageCount;
- (id)guidedAccessToneIdentifierForTimeRestrictionEvents;
- (long long)guidedAccessUsageCount;
- (bool)guidedAccessUserPrefersMirroringForExternalDisplays;
- (bool)hapticMusicActive;
- (id)hapticMusicAlgorithmSelection;
- (bool)hapticMusicEnhancedPercussion;
- (bool)hapticMusicEnhancedVocals;
- (bool)hapticMusicToggledInPreferences;
- (bool)hapticMusicTrackAvailable;
- (double)hapticMusicUsagePerDay;
- (id)hapticMusicUsageStartDate;
- (long long)hearingAidControlPanelCount;
- (long long)hearingAidHandOffCount;
- (bool)highContrastFocusIndicatorsEnabled;
- (bool)hoverTextActivationLockEnabled;
- (id)hoverTextBackgroundColorData;
- (double)hoverTextBackgroundOpacity;
- (id)hoverTextBorderColorData;
- (id)hoverTextContentSize;
- (id)hoverTextContentSize;
- (long long)hoverTextDisplayMode;
- (bool)hoverTextEnabled;
- (id)hoverTextFontName;
- (double)hoverTextFontSize;
- (id)hoverTextInsertionPointColorData;
- (long long)hoverTextPreferredActivatorKey;
- (long long)hoverTextScrollingSpeed;
- (long long)hoverTextScrollingSpeed;
- (bool)hoverTextShowedBanner;
- (bool)hoverTextShowedBanner;
- (id)hoverTextTextColorData;
- (id)hoverTextTypingAutocorrectedTextColorData;
- (id)hoverTextTypingBackgroundColorData;
- (id)hoverTextTypingBorderColorData;
- (long long)hoverTextTypingDisplayMode;
- (bool)hoverTextTypingEnabled;
- (id)hoverTextTypingFontName;
- (id)hoverTextTypingInsertionPointColorData;
- (id)hoverTextTypingMisspelledTextColorData;
- (bool)hoverTextTypingShowedBanner;
- (bool)hoverTextTypingShowedBanner;
- (id)hoverTextTypingTextColorData;
- (id)hoverTextTypingTextStyle;
- (bool)ignoreAXAsserts;
- (bool)ignoreAXServerEntitlements;
- (bool)ignoreTrackpad;
- (unsigned long long)imageCaptionGenderStrategy;
- (id)imageCaptioningDisabledApps;
- (bool)includeBacktraceInLogs;
- (bool)increaseButtonLegibilityEnabled;
- (bool)independentHearingAidSettings;
- (id)init;
- (bool)inspectorEnabled;
- (long long)internalLoggingColorTheme;
- (bool)isAXValidationRunnerCollectingValidations;
- (bool)isAdaptiveVoiceShortcutsEnabled;
- (bool)isNewRecipe:(id)arg1;
- (id)keysToIgnoreDuringBuddyTransfer;
- (bool)laserEnabled;
- (unsigned long long)laserZoomPanningStyle;
- (double)lastAssistiveTouchTimeResetCount;
- (double)lastBrailleScreenInputTimeResetCount;
- (double)lastCharacterVoiceTimeResetCount;
- (double)lastGuidedAccessTimeLimitResetCount;
- (double)lastGuidedAccessTimeResetCount;
- (double)lastHearingAidControlPanelTimeResetCount;
- (double)lastHearingAidHandoffTimeResetCount;
- (double)lastMagnifierResetCount;
- (double)lastPlatformSwitchTimeResetCount;
- (double)lastSmartInvertColorsEnablement;
- (double)lastTapticTimeResetCount;
- (bool)leftRightBalanceEnabled;
- (double)leftRightBalanceValue;
- (bool)letterFeedbackEnabled;
- (bool)liveRecognitionActive;
- (bool)liveRecognitionOverlayOpen;
- (bool)liveRegionStatusEverSet;
- (id)liveRegionStatusForWebOrApp:(id)arg1;
- (long long)liveSpeechMaxRecentsCount;
- (id)liveSpeechVoiceIdentifierForKeyboardID:(id)arg1;
- (bool)localizationQACaptionMode;
- (bool)localizationQACaptionShowFilePath;
- (bool)localizationQACaptionShowLocalizedString;
- (bool)localizationQACaptionShowUSString;
- (bool)logAXNotificationPosting;
- (double)magnifierBrightness;
- (double)magnifierContrast;
- (bool)magnifierEnabled;
- (bool)magnifierFilterInverted;
- (unsigned long long)magnifierFilterSetIdentifier;
- (bool)magnifierShouldAdjustFiltersForAmbientLight;
- (bool)magnifierShouldUseVideoStabilization;
- (long long)magnifierUsageCount;
- (double)magnifierZoomLevel;
- (id)managedAssetsRepresentation;
- (id)mediaControlsKeys;
- (id)medicalPreamble;
- (bool)monoAudioEnabled;
- (void)nukeAll;
- (void)nukePrefsForDomains:(unsigned long long*)arg1 numDomains:(unsigned long long)arg2;
- (void)observeValueForKeyPath:(id)arg1 ofObject:(id)arg2 change:(id)arg3 context:(void*)arg4;
- (id)perAppSettingsCustomizedAppIDs;
- (id)perAppSettingsStats;
- (bool)phoneticFeedbackEnabled;
- (id)preferenceKeyForSelector:(SEL)arg1;
- (id)preferredBrailleTableIdentifierForKeyboardLanguage:(id)arg1 keyboardLayout:(id)arg2;
- (unsigned long long)quickSpeakHighlightOption;
- (struct CGPoint { double x1; double x2; })quickSpeakNubbitNormalizedPosition;
- (long long)quickSpeakSentenceHighlightColor;
- (unsigned long long)quickSpeakSentenceHighlightOption;
- (float)quickSpeakSpeakingRate;
- (float)quickSpeakSpeakingRateForLanguage:(id)arg1;
- (bool)quickSpeakUnderlineSentence;
- (float)quickSpeakVolume;
- (long long)quickSpeakWordHighlightColor;
- (bool)quickTypeWordFeedbackEnabled;
- (bool)reachabilityEnabled;
- (bool)reduceMotionAutoplayVideoPreviewsEnabled;
- (bool)reduceMotionEnabled;
- (bool)reduceWhitePointEnabled;
- (float)reduceWhitePointLevel;
- (id)remoteHandGestureCustomizedActions;
- (void)removeCustomizedAppID:(id)arg1;
- (void)resetAllGuestModeDefaults;
- (void)saveRecipe:(id)arg1;
- (bool)securePayAssertionActive;
- (id)selectedSpeechVoiceIdentifiers;
- (id)selectedSpeechVoiceIdentifiersForSourceKey:(id)arg1;
- (id)selectedSpeechVoiceIdentifiersWithLanguageSource;
- (void)setAdaptiveVoiceShortcutsEnabled:(bool)arg1;
- (void)setAlexLocalAssetURL:(id)arg1;
- (void)setAllowHearingAidControlOnLockScreen:(bool)arg1;
- (void)setAppValidationTestingMode:(bool)arg1;
- (void)setAppleTVRemoteClickpadTapsForDirectionalNavigationEnabled:(bool)arg1;
- (void)setAppleTVScaledUIEnabled:(bool)arg1;
- (void)setApplicationAccessibilityEnabled:(bool)arg1;
- (void)setAssistiveTouchActionRepeatEnabled:(bool)arg1;
- (void)setAssistiveTouchActionRepeatInterval:(double)arg1;
- (void)setAssistiveTouchActionsBySoundAction:(id)arg1;
- (void)setAssistiveTouchAlwaysShowMenuEnabled:(bool)arg1;
- (void)setAssistiveTouchAxisSweepSpeed:(double)arg1;
- (void)setAssistiveTouchBubbleModeEnabled:(bool)arg1;
- (void)setAssistiveTouchCameraPointPickerSwitches:(id)arg1;
- (void)setAssistiveTouchCameraSwitchPreviewEnabled:(bool)arg1;
- (void)setAssistiveTouchCursorColor:(long long)arg1;
- (void)setAssistiveTouchCustomGestures:(id)arg1;
- (void)setAssistiveTouchDelayAfterInput:(double)arg1;
- (void)setAssistiveTouchDelayAfterInputEnabled:(bool)arg1;
- (void)setAssistiveTouchDoubleTapAction:(id)arg1;
- (void)setAssistiveTouchDoubleTapActionTimeout:(double)arg1;
- (void)setAssistiveTouchEnabled:(bool)arg1;
- (void)setAssistiveTouchEnabledByOnDeviceEyeTracking:(bool)arg1;
- (void)setAssistiveTouchEyeTrackerCustomizedSettings:(id)arg1;
- (void)setAssistiveTouchEyeTrackingAutoHideEnabled:(bool)arg1;
- (void)setAssistiveTouchEyeTrackingAutoHideOpacity:(double)arg1;
- (void)setAssistiveTouchEyeTrackingAutoHideTimeout:(double)arg1;
- (void)setAssistiveTouchForceEyeTrackerExperience:(bool)arg1;
- (void)setAssistiveTouchForceOnDeviceEyeTrackingExperience:(bool)arg1;
- (void)setAssistiveTouchGameControllerEnabled:(bool)arg1;
- (void)setAssistiveTouchGroupElementsEnabled:(bool)arg1;
- (void)setAssistiveTouchHardwareEnabled:(bool)arg1;
- (void)setAssistiveTouchHeadMovementSensitivity:(long long)arg1;
- (void)setAssistiveTouchIdleOpacity:(double)arg1;
- (void)setAssistiveTouchInputCoalescingDuration:(double)arg1;
- (void)setAssistiveTouchInputCoalescingEnabled:(bool)arg1;
- (void)setAssistiveTouchInputHoldDuration:(double)arg1;
- (void)setAssistiveTouchInputHoldEnabled:(bool)arg1;
- (void)setAssistiveTouchInternalOnlyHiddenNubbitModeEnabled:(bool)arg1;
- (void)setAssistiveTouchInternalOnlyPearlTrackingEnabled:(bool)arg1;
- (void)setAssistiveTouchLongPressAction:(id)arg1;
- (void)setAssistiveTouchLongPressActionDuration:(double)arg1;
- (void)setAssistiveTouchLongPressDuration:(double)arg1;
- (void)setAssistiveTouchLongPressEnabled:(bool)arg1;
- (void)setAssistiveTouchLongPressPauseScanningEnabled:(bool)arg1;
- (void)setAssistiveTouchMainScreenCustomization:(id)arg1;
- (void)setAssistiveTouchMotionTrackerConfigurable:(bool)arg1;
- (void)setAssistiveTouchMotionTrackerShouldOffsetBufferPoints:(bool)arg1;
- (void)setAssistiveTouchMotionTrackerSmoothingBufferSize:(unsigned long long)arg1;
- (void)setAssistiveTouchMotionTrackerSmoothingMaxDelta:(double)arg1;
- (void)setAssistiveTouchMotionTrackerXNormalizationOffset:(double)arg1;
- (void)setAssistiveTouchMotionTrackerXNormalizationOrder:(double)arg1;
- (void)setAssistiveTouchMotionTrackerYNormalizationOffset:(double)arg1;
- (void)setAssistiveTouchMotionTrackerYNormalizationOrder:(double)arg1;
- (void)setAssistiveTouchMouseAllowAppleBluetoothDevicesPairing:(bool)arg1;
- (void)setAssistiveTouchMouseAlwaysShowSoftwareKeyboardEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseBehavesLikeFinger:(bool)arg1;
- (void)setAssistiveTouchMouseClickSoundsEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseDwellControlActivationTimeout:(double)arg1;
- (void)setAssistiveTouchMouseDwellControlAutorevertAction:(id)arg1;
- (void)setAssistiveTouchMouseDwellControlAutorevertEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseDwellControlCornerCustomization:(id)arg1;
- (void)setAssistiveTouchMouseDwellControlEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseDwellControlMovementToleranceRadius:(double)arg1;
- (void)setAssistiveTouchMouseDwellControlMutatedMenu:(bool)arg1;
- (void)setAssistiveTouchMouseDwellControlShowPrompt:(bool)arg1;
- (void)setAssistiveTouchMouseKeysDelay:(long long)arg1;
- (void)setAssistiveTouchMouseKeysEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseKeysMaxSpeed:(long long)arg1;
- (void)setAssistiveTouchMouseKeysOptionToggleEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseKeysUseMainKeyboardKeys:(bool)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingCalibratedValues:(id)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingCalibrationStrategy:(long long)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingEnrollmentCompleted:(bool)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingEnrollmentPointToGazePointMap:(id)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingOnboardingCompleted:(bool)arg1;
- (void)setAssistiveTouchMouseOnDeviceEyeTrackingShouldShowUncalibratedPoints:(bool)arg1;
- (void)setAssistiveTouchMousePointerColor:(long long)arg1;
- (void)setAssistiveTouchMousePointerSizeMultiplier:(double)arg1;
- (void)setAssistiveTouchMousePointerTimeout:(double)arg1;
- (void)setAssistiveTouchMousePointerTimeoutEnabled:(bool)arg1;
- (void)setAssistiveTouchMouseZoomPanningStyle:(unsigned long long)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingAutoHideEnabled:(bool)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingAutoHideOpacity:(double)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingAutoHideTimeout:(double)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingShouldOffsetBufferPoints:(bool)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingShouldUseGaussianBlur:(bool)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingShouldUseMotionFilter:(bool)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingShouldUseWeightedAverages:(bool)arg1;
- (void)setAssistiveTouchOnDeviceEyeTrackingSmoothingBufferSize:(unsigned long long)arg1;
- (void)setAssistiveTouchOpenMenuSwaggleEnabled:(bool)arg1;
- (void)setAssistiveTouchOrbAction:(id)arg1;
- (void)setAssistiveTouchPreferredPointPicker:(long long)arg1;
- (void)setAssistiveTouchRecentGestures:(id)arg1;
- (void)setAssistiveTouchSavedGestures:(id)arg1;
- (void)setAssistiveTouchScanCycles:(long long)arg1;
- (void)setAssistiveTouchScanTimeout:(double)arg1;
- (void)setAssistiveTouchScanTimeoutEnabled:(bool)arg1;
- (void)setAssistiveTouchScannerAddedTripleClickAutomatically:(bool)arg1;
- (void)setAssistiveTouchScannerCompactMenuEnabled:(bool)arg1;
- (void)setAssistiveTouchScannerCursorHighVisibilityEnabled:(bool)arg1;
- (void)setAssistiveTouchScannerMenuLabelsEnabled:(bool)arg1;
- (void)setAssistiveTouchScannerPrefersCompact:(bool)arg1 forLanguage:(id)arg2;
- (void)setAssistiveTouchScannerSoundEnabled:(bool)arg1;
- (void)setAssistiveTouchScannerSpeechRate:(double)arg1;
- (void)setAssistiveTouchScanningMode:(long long)arg1;
- (void)setAssistiveTouchShouldUseExtendedKeyboardPredictions:(bool)arg1;
- (void)setAssistiveTouchSingleTapAction:(id)arg1;
- (void)setAssistiveTouchSleepOnWristDownPreference:(bool)arg1;
- (void)setAssistiveTouchSpeed:(double)arg1;
- (void)setAssistiveTouchStepInterval:(double)arg1;
- (void)setAssistiveTouchSwitchUsageConfirmed:(bool)arg1;
- (void)setAssistiveTouchSwitches:(id)arg1;
- (void)setAssistiveTouchUIEnabled:(bool)arg1;
- (void)setAssistiveTouchUsageCount:(id)arg1;
- (void)setAssistiveTouchVirtualTrackpadBorderColor:(long long)arg1;
- (void)setAssistiveTouchVirtualTrackpadBorderOpacity:(double)arg1;
- (void)setAssistiveTouchVirtualTrackpadBorderWidth:(double)arg1;
- (void)setAssistiveTouchVirtualTrackpadEnabled:(bool)arg1;
- (void)setAssistiveTouchVirtualTrackpadFadeDelay:(double)arg1;
- (void)setAssistiveTouchVirtualTrackpadFullScreenToggleEnabled:(bool)arg1;
- (void)setAssistiveTouchVirtualTrackpadHoldDuration:(double)arg1;
- (void)setAssistiveTouchVirtualTrackpadInnerColor:(long long)arg1;
- (void)setAssistiveTouchVirtualTrackpadInnerOpacity:(double)arg1;
- (void)setAssistiveTouchVirtualTrackpadScrollSpeed:(double)arg1;
- (void)setAssistiveTouchVirtualTrackpadScrollToggleEnabled:(bool)arg1;
- (void)setAssistiveTouchVirtualTrackpadSpeed:(double)arg1;
- (void)setAudioLeftRightBalance:(double)arg1;
- (void)setAudiogramIngestionLastModelAccess:(double)arg1;
- (void)setAutomaticAccessibilityEnabled:(bool)arg1;
- (void)setAutomaticAccessibilityIgnoreAppAccessibilityPreferred:(bool)arg1;
- (void)setAutomaticAccessibilityMode:(long long)arg1 forBundleIdentifier:(id)arg2;
- (void)setAutomaticAccessibilityModes:(id)arg1;
- (void)setAutomaticAccessibilityVisualizationsEnabled:(bool)arg1;
- (void)setAutomationEnabled:(bool)arg1;
- (void)setAutomationFauxCollectionViewCellsEnabled:(bool)arg1;
- (void)setAutomationFauxTableViewCellsEnabled:(bool)arg1;
- (void)setAutomationHitpointWarpingEnabled:(bool)arg1;
- (void)setAutomationLocalizedStringLookupInfoEnabled:(bool)arg1;
- (void)setAutomationTrueTouch:(bool)arg1;
- (void)setBackTapDoubleTapAction:(id)arg1;
- (void)setBackTapEnabled:(bool)arg1;
- (void)setBackTapFalsePositiveAlertsEnabled:(bool)arg1;
- (void)setBackTapRNNModelEnabled:(bool)arg1;
- (void)setBackTapTripleTapAction:(id)arg1;
- (void)setBackTapUsageCount:(long long)arg1;
- (void)setBrokenHomeButtonCount:(long long)arg1;
- (void)setButtonShapesEnabled:(bool)arg1;
- (void)setCallAudioRoutingAutoAnswerDelay:(double)arg1;
- (void)setCallAudioRoutingAutoAnswerEnabled:(bool)arg1;
- (void)setCameraButtonSensitivity:(double)arg1;
- (void)setCharacterFeedbackDelayDuration:(double)arg1;
- (void)setCharacterVoicesUsageCount:(long long)arg1;
- (void)setClassicInvertColors:(bool)arg1;
- (void)setCustomPronunciationSubstitutions:(id)arg1;
- (void)setCustomSettingsForVoice:(id)arg1 sourceKey:(id)arg2 settings:(id)arg3;
- (void)setDefaultVoiceSelection:(id)arg1 languageID:(id)arg2 rate:(id)arg3 pitch:(id)arg4 volume:(id)arg5;
- (void)setDidResetD22Preferences:(bool)arg1;
- (void)setDidTriggerSOSToday:(bool)arg1;
- (void)setDockSize:(double)arg1;
- (void)setEnableHearingAidReporter:(bool)arg1;
- (void)setEnableVoiceOverCaptions:(bool)arg1;
- (void)setEnabledLiveRecognitionModes:(id)arg1;
- (void)setEnhanceBackgroundContrastEnabled:(bool)arg1;
- (void)setEnhanceTextLegibilityEnabled:(bool)arg1;
- (void)setEnhanceTextTrackingEnabled:(bool)arg1;
- (void)setFullKeyboardAccessCommandMapData:(id)arg1;
- (void)setFullKeyboardAccessFocusRingColor:(long long)arg1;
- (void)setFullKeyboardAccessFocusRingHighContrastEnabled:(bool)arg1;
- (void)setFullKeyboardAccessFocusRingTimeout:(double)arg1;
- (void)setFullKeyboardAccessFocusRingTimeoutEnabled:(bool)arg1;
- (void)setFullKeyboardAccessLargeFocusRingEnabled:(bool)arg1;
- (void)setFullKeyboardAccessShouldShowDebugKeyCommandsView:(bool)arg1;
- (void)setFullKeyboardAccessShouldShowTextEditingModeInstructions:(bool)arg1;
- (void)setGaxInternalSettingsActiveAppID:(id)arg1;
- (void)setGaxInternalSettingsActiveAppOrientation:(id)arg1;
- (void)setGaxInternalSettingsECID:(id)arg1;
- (void)setGaxInternalSettingsIsActiveAppSelfLocked:(bool)arg1;
- (void)setGaxInternalSettingsLastActivationDate:(id)arg1;
- (void)setGaxInternalSettingsLastPasscodeSetDate:(id)arg1;
- (void)setGaxInternalSettingsProductBuildVersion:(id)arg1;
- (void)setGaxInternalSettingsSavedAccessibilityFeatures:(id)arg1;
- (void)setGaxInternalSettingsSavedAccessibilityTripleClickOptions:(id)arg1;
- (void)setGaxInternalSettingsSystemDidRestartDueToLowBattery:(bool)arg1;
- (void)setGaxInternalSettingsTimeRestrictionHasExpired:(bool)arg1;
- (void)setGaxInternalSettingsUserAppProfile:(id)arg1;
- (void)setGaxInternalSettingsUserConfiguredAppIDs:(id)arg1;
- (void)setGaxInternalSettingsUserGlobalProfile:(id)arg1;
- (void)setGizmoApplicationAccessibilityEnabled:(bool)arg1;
- (void)setGuestsShouldInheritPrimaryUserPreferences:(bool)arg1;
- (void)setGuidedAccessAXFeaturesEnabled:(bool)arg1;
- (void)setGuidedAccessAllowsUnlockWithTouchID:(bool)arg1;
- (void)setGuidedAccessAutoLockTimeInSeconds:(long long)arg1;
- (void)setGuidedAccessDisallowDirectInactiveToActiveTransition:(bool)arg1;
- (void)setGuidedAccessEnableExperimentalUI:(bool)arg1;
- (void)setGuidedAccessOverrideTimeRestrictionDuration:(id)arg1;
- (void)setGuidedAccessShouldSpeakForTimeRestrictionEvents:(bool)arg1;
- (void)setGuidedAccessTimeLimitsUsageCount:(long long)arg1;
- (void)setGuidedAccessToneIdentifierForTimeRestrictionEvents:(id)arg1;
- (void)setGuidedAccessUsageCount:(long long)arg1;
- (void)setGuidedAccessUserPrefersMirroringForExternalDisplays:(bool)arg1;
- (void)setHapticMusicActive:(bool)arg1;
- (void)setHapticMusicAlgorithmSelection:(id)arg1;
- (void)setHapticMusicEnhancedPercussion:(bool)arg1;
- (void)setHapticMusicEnhancedVocals:(bool)arg1;
- (void)setHapticMusicToggledInPreferences:(bool)arg1;
- (void)setHapticMusicTrackAvailable:(bool)arg1;
- (void)setHapticMusicUsagePerDay:(double)arg1;
- (void)setHapticMusicUsageStartDate:(id)arg1;
- (void)setHearingAidControlPanelCount:(long long)arg1;
- (void)setHearingAidHandOffCount:(long long)arg1;
- (void)setHighContrastFocusIndicatorsEnabled:(bool)arg1;
- (void)setHoverTextActivationLockEnabled:(bool)arg1;
- (void)setHoverTextBackgroundColorData:(id)arg1;
- (void)setHoverTextBackgroundOpacity:(double)arg1;
- (void)setHoverTextBorderColorData:(id)arg1;
- (void)setHoverTextContentSize:(id)arg1;
- (void)setHoverTextContentSize:(id)arg1;
- (void)setHoverTextDisplayMode:(long long)arg1;
- (void)setHoverTextEnabled:(bool)arg1;
- (void)setHoverTextFontName:(id)arg1;
- (void)setHoverTextFontSize:(double)arg1;
- (void)setHoverTextInsertionPointColorData:(id)arg1;
- (void)setHoverTextPreferredActivatorKey:(long long)arg1;
- (void)setHoverTextScrollingSpeed:(long long)arg1;
- (void)setHoverTextScrollingSpeed:(long long)arg1;
- (void)setHoverTextShowedBanner:(bool)arg1;
- (void)setHoverTextShowedBanner:(bool)arg1;
- (void)setHoverTextTextColorData:(id)arg1;
- (void)setHoverTextTypingAutocorrectedTextColorData:(id)arg1;
- (void)setHoverTextTypingBackgroundColorData:(id)arg1;
- (void)setHoverTextTypingBorderColorData:(id)arg1;
- (void)setHoverTextTypingDisplayMode:(long long)arg1;
- (void)setHoverTextTypingEnabled:(bool)arg1;
- (void)setHoverTextTypingFontName:(id)arg1;
- (void)setHoverTextTypingInsertionPointColorData:(id)arg1;
- (void)setHoverTextTypingMisspelledTextColorData:(id)arg1;
- (void)setHoverTextTypingShowedBanner:(bool)arg1;
- (void)setHoverTextTypingShowedBanner:(bool)arg1;
- (void)setHoverTextTypingTextColorData:(id)arg1;
- (void)setHoverTextTypingTextStyle:(id)arg1;
- (void)setIgnoreAXAsserts:(bool)arg1;
- (void)setIgnoreAXServerEntitlements:(bool)arg1;
- (void)setIgnoreTrackpad:(bool)arg1;
- (void)setImageCaptionGenderStrategy:(unsigned long long)arg1;
- (void)setImageCaptioningDisabledApps:(id)arg1;
- (void)setIncludeBacktraceInLogs:(bool)arg1;
- (void)setIncreaseButtonLegibilityEnabled:(bool)arg1;
- (void)setIndependentHearingAidSettings:(bool)arg1;
- (void)setInspectorEnabled:(bool)arg1;
- (void)setInternalLoggingColorTheme:(long long)arg1;
- (void)setIsAXValidationRunnerCollectingValidations:(bool)arg1;
- (void)setLaserZoomPanningStyle:(unsigned long long)arg1;
- (void)setLastAssistiveTouchTimeResetCount:(double)arg1;
- (void)setLastBrailleScreenInputTimeResetCount:(double)arg1;
- (void)setLastCharacterVoiceTimeResetCount:(double)arg1;
- (void)setLastGuidedAccessTimeLimitResetCount:(double)arg1;
- (void)setLastGuidedAccessTimeResetCount:(double)arg1;
- (void)setLastHearingAidControlPanelTimeResetCount:(double)arg1;
- (void)setLastHearingAidHandoffTimeResetCount:(double)arg1;
- (void)setLastMagnifierResetCount:(double)arg1;
- (void)setLastPlatformSwitchTimeResetCount:(double)arg1;
- (void)setLastSmartInvertColorsEnablement:(double)arg1;
- (void)setLastTapticTimeResetCount:(double)arg1;
- (void)setLeftRightBalanceEnabled:(bool)arg1;
- (void)setLeftRightBalanceValue:(double)arg1;
- (void)setLetterFeedbackEnabled:(bool)arg1;
- (void)setLiveRecognitionActive:(bool)arg1;
- (void)setLiveRecognitionOverlayOpen:(bool)arg1;
- (void)setLiveRegionStatusForWebOrApp:(id)arg1 status:(id)arg2;
- (void)setLiveSpeechMaxRecentsCount:(long long)arg1;
- (void)setLocalizationQACaptionMode:(bool)arg1;
- (void)setLocalizationQACaptionShowFilePath:(bool)arg1;
- (void)setLocalizationQACaptionShowLocalizedString:(bool)arg1;
- (void)setLocalizationQACaptionShowUSString:(bool)arg1;
- (void)setLogAXNotificationPosting:(bool)arg1;
- (void)setMagnifierBrightness:(double)arg1;
- (void)setMagnifierContrast:(double)arg1;
- (void)setMagnifierEnabled:(bool)arg1;
- (void)setMagnifierEnabled:(bool)arg1 changeTripleClickMenu:(bool)arg2;
- (void)setMagnifierFilterInverted:(bool)arg1;
- (void)setMagnifierFilterSetIdentifier:(unsigned long long)arg1;
- (void)setMagnifierShouldAdjustFiltersForAmbientLight:(bool)arg1;
- (void)setMagnifierShouldUseVideoStabilization:(bool)arg1;
- (void)setMagnifierUsageCount:(long long)arg1;
- (void)setMagnifierZoomLevel:(double)arg1;
- (void)setMedicalPreamble:(id)arg1;
- (void)setMonoAudioEnabled:(bool)arg1;
- (void)setPerAppSettingsCustomizedAppIDs:(id)arg1;
- (void)setPerAppSettingsStats:(id)arg1;
- (void)setPhoneticFeedbackEnabled:(bool)arg1;
- (void)setPreferredBrailleTableIdentifier:(id)arg1 forKeyboardLanguage:(id)arg2 keyboardLayout:(id)arg3;
- (void)setQuickSpeakHighlightOption:(unsigned long long)arg1;
- (void)setQuickSpeakNubbitNormalizedPosition:(struct CGPoint { double x1; double x2; })arg1;
- (void)setQuickSpeakSentenceHighlightColor:(long long)arg1;
- (void)setQuickSpeakSentenceHighlightOption:(unsigned long long)arg1;
- (void)setQuickSpeakSpeakingRate:(float)arg1;
- (void)setQuickSpeakSpeakingRate:(float)arg1 forLanguage:(id)arg2;
- (void)setQuickSpeakUnderlineSentence:(bool)arg1;
- (void)setQuickSpeakVolume:(float)arg1;
- (void)setQuickSpeakWordHighlightColor:(long long)arg1;
- (void)setQuickTypeWordFeedbackEnabled:(bool)arg1;
- (void)setReachabilityEnabled:(bool)arg1;
- (void)setReduceMotionAutoplayVideoPreviewsEnabled:(bool)arg1;
- (void)setReduceMotionEnabled:(bool)arg1;
- (void)setReduceWhitePointEnabled:(bool)arg1;
- (void)setReduceWhitePointLevel:(float)arg1;
- (void)setRemoteHandGestureCustomizedActions:(id)arg1;
- (void)setSecurePayAssertionActive:(bool)arg1;
- (void)setSelectedSpeechVoiceIdentifiersWithLanguageSource:(id)arg1;
- (void)setShouldCaptureVisionEngineDiagnosticsToDisk:(bool)arg1;
- (void)setShouldFlashForAlertInSilentMode:(bool)arg1;
- (void)setShouldFlashWhileUnlocked:(bool)arg1;
- (void)setShouldLimitDisplayRefreshRate:(bool)arg1;
- (void)setShouldPerformValidationsAtRuntime:(bool)arg1;
- (void)setShouldSpeakMedicalPreamble:(bool)arg1;
- (void)setShouldStreamToLeftAid:(bool)arg1;
- (void)setShouldStreamToRightAid:(bool)arg1;
- (void)setShouldTTYMedicalPreamble:(bool)arg1;
- (void)setShowSpeechController:(bool)arg1;
- (void)setSiriAutoUpdateListInitialized:(bool)arg1;
- (void)setSiriSemanticContextEnabled:(bool)arg1;
- (void)setSkipHearingAidMFiAuth:(bool)arg1;
- (void)setSoundActionsLastModelAccess:(double)arg1;
- (void)setSoundDetectionLastModelAccess:(double)arg1;
- (void)setSpeakCorrectionsEnabled:(bool)arg1;
- (void)setSpeechControllerDoubleTapAction:(long long)arg1;
- (void)setSpeechControllerIdleOpacity:(double)arg1;
- (void)setSpeechControllerLongPressAction:(long long)arg1;
- (void)setSpeechVoiceIdentifier:(id)arg1 forLanguage:(id)arg2 source:(long long)arg3;
- (void)setSpeechVoiceIdentifier:(id)arg1 forLanguage:(id)arg2 sourceKey:(id)arg3;
- (void)setSpokenContentDefaultFallbackLanguage:(id)arg1;
- (void)setSpokenContentShouldUseLanguageDetection:(bool)arg1;
- (void)setSpokenContentVoiceSelectionForLanguage:(id)arg1 selection:(id)arg2;
- (void)setStartupSoundEnabled:(bool)arg1;
- (void)setStickyKeysBeepEnabled:(bool)arg1;
- (void)setStickyKeysEnabled:(bool)arg1;
- (void)setStickyKeysShiftToggleEnabled:(bool)arg1;
- (void)setSupportsAdvancedDisplayFilters:(bool)arg1;
- (void)setSwiftSettings:(id)arg1;
- (void)setSwitchControlAutoTapTimeout:(double)arg1;
- (void)setSwitchControlCameraPointPickerDwellActivationTimeout:(double)arg1;
- (void)setSwitchControlCameraPointPickerDwellMovementToleranceRadius:(double)arg1;
- (void)setSwitchControlCameraPointPickerMode:(unsigned long long)arg1;
- (void)setSwitchControlCameraPointPickerMovementToleranceInJoystickMode:(double)arg1;
- (void)setSwitchControlCameraPointPickerSensitivity:(double)arg1;
- (void)setSwitchControlCombinedTopLevelMenuItems:(id)arg1;
- (void)setSwitchControlDebugLoggingEnabled:(bool)arg1;
- (void)setSwitchControlDeviceMenuItems:(id)arg1;
- (void)setSwitchControlDeviceTopLevelMenuItems:(id)arg1;
- (void)setSwitchControlDwellTime:(double)arg1;
- (void)setSwitchControlEnabled:(bool)arg1;
- (void)setSwitchControlFirstLaunchScanningMode:(long long)arg1;
- (void)setSwitchControlGesturesMenuItems:(id)arg1;
- (void)setSwitchControlGesturesTopLevelMenuItems:(id)arg1;
- (void)setSwitchControlIgnoreInvalidSwitchConfiguration:(bool)arg1;
- (void)setSwitchControlIsEnabledAsReceiver:(bool)arg1;
- (void)setSwitchControlLaunchRecipeUUID:(id)arg1;
- (void)setSwitchControlMediaControlsMenuItems:(id)arg1;
- (void)setSwitchControlMediaControlsTopLevelMenuItems:(id)arg1;
- (void)setSwitchControlPlatformSwitchedCount:(long long)arg1;
- (void)setSwitchControlPointPickerSelectionStyle:(long long)arg1;
- (void)setSwitchControlRecipes:(id)arg1;
- (void)setSwitchControlRestartScanningAtCurrentKey:(bool)arg1;
- (void)setSwitchControlScanAfterTapLocation:(long long)arg1;
- (void)setSwitchControlScanningStyle:(long long)arg1;
- (void)setSwitchControlSettingsMenuItems:(id)arg1;
- (void)setSwitchControlSettingsTopLevelMenuItems:(id)arg1;
- (void)setSwitchControlShouldAlwaysActivateKeyboardKeys:(bool)arg1;
- (void)setSwitchControlShouldDisallowUSBRestrictedMode:(bool)arg1;
- (void)setSwitchControlShouldUseExtendedKeyboardPredictions:(bool)arg1;
- (void)setSwitchControlShouldUseShortFirstPage:(bool)arg1;
- (void)setSwitchControlTapBehavior:(long long)arg1;
- (void)setSwitchControlTopLevelMenuItems:(id)arg1;
- (void)setSwitchControlUseCameraForPointMode:(bool)arg1;
- (void)setSwitchControlUserDidReadUSBRestrictedModeAlert:(bool)arg1;
- (void)setSyncPronunciationsWithCloudKit:(bool)arg1;
- (void)setTapToSpeakTimeAvailability:(long long)arg1;
- (void)setTapToSpeakTimeEnabled:(bool)arg1;
- (void)setTapticTimeInternalFlashScreenEnabled:(bool)arg1;
- (void)setTapticTimeUsageCount:(long long)arg1;
- (void)setTeachableMomentsDelayBeforeSending:(double)arg1;
- (void)setTeachableMomentsNotificationsSeen:(id)arg1;
- (void)setTouchAccommodationsAllowsSwipeGesturesToBypass:(bool)arg1;
- (void)setTouchAccommodationsEnabled:(bool)arg1;
- (void)setTouchAccommodationsHoldDuration:(double)arg1;
- (void)setTouchAccommodationsHoldDurationAllowsSwipeGesturesToBypass:(bool)arg1;
- (void)setTouchAccommodationsHoldDurationEnabled:(bool)arg1;
- (void)setTouchAccommodationsIgnoreRepeatDuration:(double)arg1;
- (void)setTouchAccommodationsIgnoreRepeatEnabled:(bool)arg1;
- (void)setTouchAccommodationsSwipeGestureMinimumDistance:(double)arg1;
- (void)setTouchAccommodationsTapActivationMethod:(long long)arg1;
- (void)setTouchAccommodationsTapActivationTimeout:(double)arg1;
- (void)setTouchAccommodationsTripleClickConfirmed:(bool)arg1;
- (void)setTouchAccommodationsUsageConfirmed:(bool)arg1;
- (void)setTripleClickOptions:(id)arg1;
- (void)setTripleClickOrderedOptions:(id)arg1;
- (void)setUseNewAXBundleLoader:(bool)arg1;
- (void)setUserDidSelectVoiceForLanguage:(id)arg1 sourceKey:(id)arg2;
- (void)setValidateSecondPartyApps:(bool)arg1;
- (void)setVoiceOverActionFeedbackFirstInListOnly:(bool)arg1;
- (void)setVoiceOverActionsFeedback:(long long)arg1;
- (void)setVoiceOverActivationWorkaround:(long long)arg1;
- (void)setVoiceOverAdjustSoundVolumeIndependently:(bool)arg1;
- (void)setVoiceOverAlternativeVoiceIdentifier:(id)arg1 forLanguage:(id)arg2;
- (void)setVoiceOverAlwaysTurnOnBluetooth:(bool)arg1;
- (void)setVoiceOverAlwaysUseNemethCodeForMathEnabled:(bool)arg1;
- (void)setVoiceOverAudioDuckingEnabled:(bool)arg1;
- (void)setVoiceOverAudioFollowsHDMIAudio:(bool)arg1;
- (void)setVoiceOverAutomaticButtonLabels:(bool)arg1;
- (void)setVoiceOverBSIUsageCount:(long long)arg1;
- (void)setVoiceOverBannerNotificationOutput:(long long)arg1;
- (void)setVoiceOverBrailleAlertDisplayDuration:(double)arg1;
- (void)setVoiceOverBrailleAlertShowUntilDismissed:(bool)arg1;
- (void)setVoiceOverBrailleAlertsEnabled:(bool)arg1;
- (void)setVoiceOverBrailleAutoAdvanceDuration:(double)arg1;
- (void)setVoiceOverBrailleBluetoothDisplay:(id)arg1;
- (void)setVoiceOverBrailleContractionMode:(long long)arg1;
- (void)setVoiceOverBrailleDisconnectOnSleep:(id)arg1;
- (void)setVoiceOverBrailleDisplays:(id)arg1;
- (void)setVoiceOverBrailleEightDotMode:(bool)arg1;
- (void)setVoiceOverBrailleFormattingEnabled:(bool)arg1;
- (void)setVoiceOverBrailleGesturesTypingFeedback:(long long)arg1;
- (void)setVoiceOverBrailleGradeTwoAutoTranslateEnabled:(bool)arg1;
- (void)setVoiceOverBrailleKeyDebounceTimeout:(double)arg1;
- (void)setVoiceOverBrailleLanguageRotorItems:(id)arg1;
- (void)setVoiceOverBrailleMasterStatusCellIndex:(long long)arg1;
- (void)setVoiceOverBrailleStatusCellPreference:(long long)arg1;
- (void)setVoiceOverBrailleTableIdentifier:(id)arg1;
- (void)setVoiceOverBrailleVirtualStatusAlignment:(long long)arg1;
- (void)setVoiceOverBrailleWordWrapEnabled:(bool)arg1;
- (void)setVoiceOverCapitalLetterFeedback:(long long)arg1;
- (void)setVoiceOverContainerOutputFeedback:(long long)arg1;
- (void)setVoiceOverCustomCommandProfile:(id)arg1;
- (void)setVoiceOverDelayUntilSpeakUnderTouch:(double)arg1;
- (void)setVoiceOverDeletionFeedback:(long long)arg1;
- (void)setVoiceOverDescribedMedia:(long long)arg1;
- (void)setVoiceOverDetectionModeItems:(id)arg1;
- (void)setVoiceOverDirectTouchEnabledApps:(id)arg1;
- (void)setVoiceOverDiscoveredSensitiveContentFeedback:(long long)arg1;
- (void)setVoiceOverDoubleTapInterval:(double)arg1;
- (void)setVoiceOverEditAppsActionEnabled:(bool)arg1;
- (void)setVoiceOverEffectiveSpeakingRate:(float)arg1;
- (void)setVoiceOverEffectiveSpeakingVolume:(float)arg1;
- (void)setVoiceOverEnabled:(bool)arg1;
- (void)setVoiceOverEnabledThroughAccessory:(bool)arg1;
- (void)setVoiceOverExploreFocusAffectsNativeFocus:(bool)arg1;
- (void)setVoiceOverFlashlightNotificationsEnabled:(bool)arg1;
- (void)setVoiceOverHandGesturesActionCustomizations:(id)arg1;
- (void)setVoiceOverHandGesturesEnabled:(bool)arg1;
- (void)setVoiceOverHandwritingWasNativeAutocorrectEnabled:(id)arg1;
- (void)setVoiceOverHapticIntensity:(double)arg1;
- (void)setVoiceOverHapticsEnabled:(bool)arg1;
- (void)setVoiceOverHardwareTypingFeedback:(long long)arg1;
- (void)setVoiceOverHelpMode:(long long)arg1;
- (void)setVoiceOverHintsEnabled:(bool)arg1;
- (void)setVoiceOverIgnoreTrackpad:(bool)arg1;
- (void)setVoiceOverInlineTextCompletionAppearanceFeedback:(long long)arg1;
- (void)setVoiceOverInlineTextCompletionInsertionFeedback:(long long)arg1;
- (void)setVoiceOverKeyboardModifierChoice:(long long)arg1;
- (void)setVoiceOverLanguageRotorItems:(id)arg1;
- (void)setVoiceOverLinkFeedback:(long long)arg1;
- (void)setVoiceOverLockedScreenNotificationOutput:(long long)arg1;
- (void)setVoiceOverMediaDuckingAmount:(double)arg1;
- (void)setVoiceOverMediaDuckingMode:(long long)arg1;
- (void)setVoiceOverMoreContentOutputFeedback:(long long)arg1;
- (void)setVoiceOverNavigateImagesOption:(long long)arg1;
- (void)setVoiceOverNavigationDirectionMode:(long long)arg1;
- (void)setVoiceOverNavigationStyle:(long long)arg1;
- (void)setVoiceOverNeuralElementFeedback:(long long)arg1;
- (void)setVoiceOverNumberFeedback:(long long)arg1;
- (void)setVoiceOverPhoneticsFeedback:(long long)arg1;
- (void)setVoiceOverPitch:(double)arg1;
- (void)setVoiceOverPreferredTVInteractionMode:(long long)arg1;
- (void)setVoiceOverPrefersFollowFocusNavigationStyle:(bool)arg1;
- (void)setVoiceOverPunctuationGroup:(id)arg1;
- (void)setVoiceOverPunctuationLevel:(long long)arg1;
- (void)setVoiceOverQuickNavAnnouncementFeedback:(long long)arg1;
- (void)setVoiceOverQuickSettingsItems:(id)arg1;
- (void)setVoiceOverRotorItems:(id)arg1;
- (void)setVoiceOverRotorSummaryFeedback:(long long)arg1;
- (void)setVoiceOverRotorUpdatesWithElement:(bool)arg1;
- (void)setVoiceOverSceneDescriptionsEnabled:(bool)arg1;
- (void)setVoiceOverScreenCurtainEnabled:(bool)arg1;
- (void)setVoiceOverSelectedActivity:(id)arg1;
- (void)setVoiceOverShouldDisallowUSBRestrictedMode:(bool)arg1;
- (void)setVoiceOverShouldFocusEverywhere:(bool)arg1;
- (void)setVoiceOverShouldOutputToHearingAid:(bool)arg1;
- (void)setVoiceOverShouldSpeakDiscoveredText:(bool)arg1;
- (void)setVoiceOverShowSoftwareKeyboardWithBraille:(bool)arg1;
- (void)setVoiceOverSilenceAnnouncements:(bool)arg1;
- (void)setVoiceOverSleepOnWristDownPreference:(bool)arg1;
- (void)setVoiceOverSoftwareTypingFeedback:(long long)arg1;
- (void)setVoiceOverSoundAndHapticPreferences:(id)arg1;
- (void)setVoiceOverSoundCurtain:(bool)arg1;
- (void)setVoiceOverSoundEffectsEnabled:(bool)arg1;
- (void)setVoiceOverSoundVolume:(double)arg1;
- (void)setVoiceOverSpeakActionConfirmation:(bool)arg1;
- (void)setVoiceOverSpeakNonfocusableElementsAfterDelay:(bool)arg1;
- (void)setVoiceOverSpeakNotificationsEnabled:(bool)arg1;
- (void)setVoiceOverSpeakSecondsEncoding:(long long)arg1;
- (void)setVoiceOverSpeakTableColumnRowInformation:(bool)arg1;
- (void)setVoiceOverSpeakTableHeaders:(bool)arg1;
- (void)setVoiceOverSpeakingRate:(float)arg1;
- (void)setVoiceOverSpeakingRate:(float)arg1 forLanguage:(id)arg2;
- (void)setVoiceOverSpeakingRateInRotorEnabled:(bool)arg1;
- (void)setVoiceOverSpeaksOverTelephoneCalls:(bool)arg1;
- (void)setVoiceOverTapticChimesAvailability:(long long)arg1;
- (void)setVoiceOverTapticChimesEnabled:(bool)arg1;
- (void)setVoiceOverTapticChimesFrequencyEncoding:(long long)arg1;
- (void)setVoiceOverTapticChimesSoundType:(long long)arg1;
- (void)setVoiceOverTapticTimeEncoding:(long long)arg1;
- (void)setVoiceOverTapticTimeMode:(bool)arg1;
- (void)setVoiceOverTapticTimeSpeed:(float)arg1;
- (void)setVoiceOverTouchBrailleGestureControl:(bool)arg1;
- (void)setVoiceOverTouchBrailleGesturesDidPlayCalibrationHint:(bool)arg1;
- (void)setVoiceOverTouchBrailleGesturesLockedOrientation:(long long)arg1;
- (void)setVoiceOverTouchBrailleGesturesLockedTypingMode:(long long)arg1;
- (void)setVoiceOverTouchBrailleGesturesShouldUseLockedConfiguration:(bool)arg1;
- (void)setVoiceOverTouchBrailleShouldReverseDots:(bool)arg1;
- (void)setVoiceOverTouchBrailleShowGeneralStatus:(bool)arg1;
- (void)setVoiceOverTouchBrailleShowTextStyleStatus:(bool)arg1;
- (void)setVoiceOverTouchBrailleTableIdentifier:(id)arg1;
- (void)setVoiceOverTouchPreferredBrailleTableIdentifiers:(id)arg1;
- (void)setVoiceOverTouchSingleLetterQuickNavEnabled:(bool)arg1;
- (void)setVoiceOverTouchUpdateBrailleWithoutConnectedDisplay:(bool)arg1;
- (void)setVoiceOverTypingMode:(long long)arg1;
- (void)setVoiceOverUIEnabled:(bool)arg1;
- (void)setVoiceOverUsageConfirmed:(bool)arg1;
- (void)setVoiceOverUseRingerSwitchToControlNotificationOutput:(bool)arg1;
- (void)setVoiceOverUserDidReadUSBRestrictedModeAlert:(bool)arg1;
- (void)setVoiceOverVerbosityEmojiFeedback:(long long)arg1;
- (void)setVoiceOverVerbosityEmojiSuffixEnabled:(bool)arg1;
- (void)setVoiceOverVerbositySpeakCustomActionsHint:(bool)arg1;
- (void)setVoiceOverVolume:(float)arg1;
- (void)setVoiceOverVolume:(float)arg1;
- (void)setWordFeedbackEnabled:(bool)arg1;
- (void)setWriteAXLogsToFile:(bool)arg1;
- (void)setZoomAdjustZoomLevelKbShortcutEnabled:(bool)arg1;
- (void)setZoomAlwaysUseWindowedZoomForTyping:(bool)arg1;
- (void)setZoomAutopannerShouldPanWithAcceleration:(bool)arg1;
- (void)setZoomControllerColor:(long long)arg1;
- (void)setZoomCurrentLensEffect:(id)arg1;
- (void)setZoomCurrentLensMode:(id)arg1;
- (void)setZoomDebugDisableZoomLensScaleTransform:(bool)arg1;
- (void)setZoomDebugShowExternalFocusRect:(bool)arg1;
- (void)setZoomEnabled:(bool)arg1;
- (void)setZoomHandGesturesActionCustomizations:(id)arg1;
- (void)setZoomHandGesturesEnabled:(bool)arg1;
- (void)setZoomIdleSlugOpacity:(double)arg1;
- (void)setZoomInStandby:(bool)arg1;
- (void)setZoomKeyboardShortcutsEnabled:(bool)arg1;
- (void)setZoomPanOffset:(struct CGPoint { double x1; double x2; })arg1;
- (void)setZoomPanZoomKbShortcutEnabled:(bool)arg1;
- (void)setZoomPeekZoomEnabled:(bool)arg1;
- (void)setZoomPeekZoomEverEnabled:(bool)arg1;
- (void)setZoomPreferencesWereInitialized:(bool)arg1;
- (void)setZoomPreferredCurrentDockPosition:(id)arg1;
- (void)setZoomPreferredCurrentLensMode:(id)arg1;
- (void)setZoomPreferredDockPositions:(id)arg1;
- (void)setZoomPreferredLensModes:(id)arg1;
- (void)setZoomPreferredMaximumZoomScale:(double)arg1;
- (void)setZoomResizeZoomWindowKbShortcutEnabled:(bool)arg1;
- (void)setZoomScale:(double)arg1;
- (void)setZoomScrollWheelKbShortcutEnabled:(bool)arg1;
- (void)setZoomShouldAllowFullscreenAutopanning:(bool)arg1;
- (void)setZoomShouldFollowFocus:(bool)arg1;
- (void)setZoomShouldShowSlug:(bool)arg1;
- (void)setZoomShowWhileMirroring:(bool)arg1;
- (void)setZoomShowedBanner:(bool)arg1;
- (void)setZoomSlugDoubleTapAction:(long long)arg1;
- (void)setZoomSlugNormalizedPosition:(struct CGPoint { double x1; double x2; })arg1;
- (void)setZoomSlugSingleTapAction:(long long)arg1;
- (void)setZoomSlugTapAndSlideToAdjustZoomLevelEnabled:(bool)arg1;
- (void)setZoomSlugTripleTapAction:(long long)arg1;
- (void)setZoomSmoothScalingEnabled:(bool)arg1;
- (void)setZoomSwitchZoomModeKbShortcutEnabled:(bool)arg1;
- (void)setZoomTempToggleZoomKbShortcutEnabled:(bool)arg1;
- (void)setZoomToggleZoomKbShortcutEnabled:(bool)arg1;
- (void)setZoomToggledByPreferenceSwitch:(bool)arg1;
- (void)setZoomToggledByVoiceOver:(bool)arg1;
- (void)setZoomTrackpadGestureEnabled:(bool)arg1;
- (void)setZoomWindowFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg1;
- (void)setlastGuidedAccessTimeLimitResetCount:(double)arg1;
- (void)setlastGuidedAccessTimeResetCount:(double)arg1;
- (void)setlastHearingAidControlPanelTimeResetCount:(double)arg1;
- (void)setlastHearingAidHandoffTimeResetCount:(double)arg1;
- (void)setlastPlatformSwitchTimeResetCount:(double)arg1;
- (void)setliveSpeechMaxRecentsCount:(long long)arg1;
- (id)settingsKeys;
- (bool)shouldCaptureVisionEngineDiagnosticsToDisk;
- (bool)shouldFlashForAlertInSilentMode;
- (bool)shouldFlashWhileUnlocked;
- (bool)shouldLimitDisplayRefreshRate;
- (bool)shouldPerformValidationsAtRuntime;
- (bool)shouldSpeakMedicalPreamble;
- (bool)shouldStreamToLeftAid;
- (bool)shouldStreamToRightAid;
- (bool)shouldTTYMedicalPreamble;
- (bool)showSpeechController;
- (bool)siriAutoUpdateListInitialized;
- (bool)siriSemanticContextEnabled;
- (bool)skipHearingAidMFiAuth;
- (id)smoothingBufferSizeForEyeTracker:(id)arg1;
- (double)soundActionsLastModelAccess;
- (double)soundDetectionLastModelAccess;
- (bool)speakCorrectionsEnabled;
- (long long)speechControllerDoubleTapAction;
- (double)speechControllerIdleOpacity;
- (long long)speechControllerLongPressAction;
- (id)speechVoiceIdentifierForLanguage:(id)arg1 source:(long long)arg2 exists:(bool*)arg3;
- (id)speechVoiceIdentifierForLanguage:(id)arg1 sourceKey:(id)arg2 exists:(bool*)arg3;
- (id)speechVoiceIdentifierForLanguageWithoutFallback:(id)arg1 sourceKey:(id)arg2;
- (id)spokenContentDefaultFallbackLanguage;
- (bool)spokenContentShouldUseLanguageDetection;
- (id)spokenContentVoiceSelectionForLanguage:(id)arg1;
- (bool)startupSoundEnabled;
- (bool)stickyKeysBeepEnabled;
- (bool)stickyKeysEnabled;
- (bool)stickyKeysShiftToggleEnabled;
- (bool)supportsAdvancedDisplayFilters;
- (id)swiftSettings;
- (double)switchControlAutoTapTimeout;
- (double)switchControlCameraPointPickerDwellActivationTimeout;
- (double)switchControlCameraPointPickerDwellMovementToleranceRadius;
- (unsigned long long)switchControlCameraPointPickerMode;
- (double)switchControlCameraPointPickerMovementToleranceInJoystickMode;
- (double)switchControlCameraPointPickerSensitivity;
- (id)switchControlCombinedTopLevelMenuItems;
- (bool)switchControlDebugLoggingEnabled;
- (id)switchControlDeviceMenuItems;
- (id)switchControlDeviceTopLevelMenuItems;
- (double)switchControlDwellTime;
- (bool)switchControlEnabled;
- (long long)switchControlFirstLaunchScanningMode;
- (id)switchControlGesturesMenuItems;
- (id)switchControlGesturesTopLevelMenuItems;
- (bool)switchControlHasEmptyTopLevelMenu;
- (bool)switchControlIgnoreInvalidSwitchConfiguration;
- (bool)switchControlIsEnabledAsReceiver;
- (id)switchControlLaunchRecipeUUID;
- (id)switchControlLocStringForFirstLaunchScanningMode:(long long)arg1;
- (id)switchControlMediaControlsMenuItems;
- (id)switchControlMediaControlsTopLevelMenuItems;
- (long long)switchControlPlatformSwitchedCount;
- (long long)switchControlPointPickerSelectionStyle;
- (id)switchControlRecipes;
- (bool)switchControlRestartScanningAtCurrentKey;
- (long long)switchControlScanAfterTapLocation;
- (long long)switchControlScanningStyle;
- (id)switchControlSettingsMenuItems;
- (id)switchControlSettingsTopLevelMenuItems;
- (bool)switchControlShouldAlwaysActivateKeyboardKeys;
- (bool)switchControlShouldDisallowUSBRestrictedMode;
- (bool)switchControlShouldUseExtendedKeyboardPredictions;
- (bool)switchControlShouldUseShortFirstPage;
- (long long)switchControlTapBehavior;
- (id)switchControlTopLevelMenuItems;
- (bool)switchControlUseCameraForPointMode;
- (bool)switchControlUserDidReadUSBRestrictedModeAlert;
- (id)switchForUUID:(id)arg1;
- (bool)syncPronunciationsWithCloudKit;
- (long long)tapToSpeakTimeAvailability;
- (bool)tapToSpeakTimeEnabled;
- (bool)tapticTimeInternalFlashScreenEnabled;
- (bool)tapticTimeIsAvailable;
- (long long)tapticTimeUsageCount;
- (double)teachableMomentsDelayBeforeSending;
- (id)teachableMomentsNotificationsSeen;
- (id)topLevelKeys;
- (bool)touchAccommodationsAllowsSwipeGesturesToBypass;
- (bool)touchAccommodationsAreConfigured;
- (bool)touchAccommodationsEnabled;
- (double)touchAccommodationsHoldDuration;
- (bool)touchAccommodationsHoldDurationAllowsSwipeGesturesToBypass;
- (bool)touchAccommodationsHoldDurationEnabled;
- (long long)touchAccommodationsHoldDurationSwipeGestureSensitivity;
- (double)touchAccommodationsIgnoreRepeatDuration;
- (bool)touchAccommodationsIgnoreRepeatEnabled;
- (double)touchAccommodationsSwipeGestureMinimumDistance;
- (long long)touchAccommodationsTapActivationMethod;
- (double)touchAccommodationsTapActivationTimeout;
- (bool)touchAccommodationsTripleClickConfirmed;
- (bool)touchAccommodationsUsageConfirmed;
- (id)tripleClickOptions;
- (id)tripleClickOrderedOptions;
- (void)updateCustomizableMouse:(id)arg1;
- (bool)updateRecipe:(id)arg1;
- (void)updateSmoothingBufferSize:(id)arg1 forEyeTracker:(id)arg2;
- (void)updateWithManagedAssetsRepresentation:(id)arg1;
- (bool)useNewAXBundleLoader;
- (bool)userDidSelectVoiceForLanguage:(id)arg1 sourceKey:(id)arg2;
- (bool)validateAndUpdateRecipeIfNeeded:(id)arg1;
- (bool)validateSecondPartyApps;
- (id)valueForPreferenceKey:(id)arg1;
- (bool)voiceOverActionFeedbackFirstInListOnly;
- (long long)voiceOverActionsFeedback;
- (long long)voiceOverActivationWorkaround;
- (bool)voiceOverAdjustSoundVolumeIndependently;
- (id)voiceOverAlternativeVoiceIdentifier:(id)arg1;
- (bool)voiceOverAlwaysTurnOnBluetooth;
- (bool)voiceOverAlwaysUseNemethCodeForMathEnabled;
- (bool)voiceOverAudioDuckingEnabled;
- (bool)voiceOverAudioFollowsHDMIAudio;
- (bool)voiceOverAutomaticButtonLabels;
- (long long)voiceOverBSIUsageCount;
- (long long)voiceOverBannerNotificationOutput;
- (double)voiceOverBrailleAlertDisplayDuration;
- (bool)voiceOverBrailleAlertShowUntilDismissed;
- (bool)voiceOverBrailleAlertsEnabled;
- (double)voiceOverBrailleAutoAdvanceDuration;
- (id)voiceOverBrailleBluetoothDisplay;
- (long long)voiceOverBrailleContractionMode;
- (id)voiceOverBrailleDisconnectOnSleep;
- (id)voiceOverBrailleDisplays;
- (bool)voiceOverBrailleEightDotMode;
- (bool)voiceOverBrailleFormattingEnabled;
- (bool)voiceOverBrailleGesturesEnabled;
- (long long)voiceOverBrailleGesturesTypingFeedback;
- (bool)voiceOverBrailleGradeTwoAutoTranslateEnabled;
- (double)voiceOverBrailleKeyDebounceTimeout;
- (id)voiceOverBrailleLanguageRotorItems;
- (long long)voiceOverBrailleMasterStatusCellIndex;
- (long long)voiceOverBrailleStatusCellPreference;
- (id)voiceOverBrailleTableIdentifier;
- (long long)voiceOverBrailleVirtualStatusAlignment;
- (bool)voiceOverBrailleWordWrapEnabled;
- (long long)voiceOverCapitalLetterFeedback;
- (long long)voiceOverContainerOutputFeedback;
- (id)voiceOverCustomCommandProfile;
- (double)voiceOverDelayUntilSpeakUnderTouch;
- (long long)voiceOverDeletionFeedback;
- (long long)voiceOverDescribedMedia;
- (id)voiceOverDetectionModeItems;
- (id)voiceOverDirectTouchEnabledApps;
- (long long)voiceOverDiscoveredSensitiveContentFeedback;
- (double)voiceOverDoubleTapInterval;
- (bool)voiceOverEditAppsActionEnabled;
- (float)voiceOverEffectiveSpeakingRate;
- (float)voiceOverEffectiveSpeakingVolume;
- (bool)voiceOverEnabled;
- (bool)voiceOverEnabledThroughAccessory;
- (bool)voiceOverExploreFocusAffectsNativeFocus;
- (bool)voiceOverFlashlightNotificationsEnabled;
- (id)voiceOverHandGesturesActionCustomizations;
- (bool)voiceOverHandGesturesEnabled;
- (bool)voiceOverHandwritingEnabled;
- (id)voiceOverHandwritingWasNativeAutocorrectEnabled;
- (id)voiceOverHapticEnabledForEvent:(id)arg1;
- (double)voiceOverHapticIntensity;
- (bool)voiceOverHapticsEnabled;
- (long long)voiceOverHardwareTypingFeedback;
- (long long)voiceOverHelpMode;
- (bool)voiceOverHintsEnabled;
- (bool)voiceOverIgnoreTrackpad;
- (long long)voiceOverInlineTextCompletionAppearanceFeedback;
- (long long)voiceOverInlineTextCompletionInsertionFeedback;
- (long long)voiceOverKeyboardModifierChoice;
- (id)voiceOverLanguageRotorItems;
- (bool)voiceOverLanguageRotorItemsExist;
- (long long)voiceOverLinkFeedback;
- (long long)voiceOverLockedScreenNotificationOutput;
- (double)voiceOverMediaDuckingAmount;
- (long long)voiceOverMediaDuckingMode;
- (long long)voiceOverMoreContentOutputFeedback;
- (long long)voiceOverNavigateImagesOption;
- (long long)voiceOverNavigationDirectionMode;
- (long long)voiceOverNavigationStyle;
- (long long)voiceOverNeuralElementFeedback;
- (long long)voiceOverNumberFeedback;
- (long long)voiceOverPhoneticsFeedback;
- (double)voiceOverPitch;
- (long long)voiceOverPreferredTVInteractionMode;
- (bool)voiceOverPrefersFollowFocusNavigationStyle;
- (id)voiceOverPunctuationGroup;
- (long long)voiceOverPunctuationLevel;
- (long long)voiceOverQuickNavAnnouncementFeedback;
- (id)voiceOverQuickSettingsItems;
- (void)voiceOverResetSoundAndHapticPreferences;
- (id)voiceOverRotorItemWithVoiceID:(id)arg1 rate:(id)arg2 volume:(id)arg3 pitch:(id)arg4;
- (id)voiceOverRotorItems;
- (long long)voiceOverRotorSummaryFeedback;
- (bool)voiceOverRotorUpdatesWithElement;
- (bool)voiceOverSceneDescriptionsEnabled;
- (bool)voiceOverScreenCurtainEnabled;
- (id)voiceOverSelectedActivity;
- (void)voiceOverSetHapticEnabled:(bool)arg1 forEvent:(id)arg2;
- (void)voiceOverSetSoundEnabled:(bool)arg1 forEvent:(id)arg2;
- (bool)voiceOverShouldDisallowUSBRestrictedMode;
- (bool)voiceOverShouldFocusEverywhere;
- (bool)voiceOverShouldOutputToHearingAid;
- (bool)voiceOverShouldSpeakDiscoveredText;
- (bool)voiceOverShowBrailleWatchSettings;
- (bool)voiceOverShowSoftwareKeyboardWithBraille;
- (bool)voiceOverSilenceAnnouncements;
- (bool)voiceOverSleepOnWristDownPreference;
- (long long)voiceOverSoftwareTypingFeedback;
- (id)voiceOverSoundAndHapticPreferences;
- (bool)voiceOverSoundCurtain;
- (bool)voiceOverSoundEffectsEnabled;
- (id)voiceOverSoundEnabledForEvent:(id)arg1;
- (double)voiceOverSoundVolume;
- (bool)voiceOverSpeakActionConfirmation;
- (bool)voiceOverSpeakNonfocusableElementsAfterDelay;
- (bool)voiceOverSpeakNotificationsEnabled;
- (long long)voiceOverSpeakSecondsEncoding;
- (bool)voiceOverSpeakTableColumnRowInformation;
- (bool)voiceOverSpeakTableHeaders;
- (float)voiceOverSpeakingRate;
- (float)voiceOverSpeakingRateForLanguage:(id)arg1;
- (bool)voiceOverSpeakingRateInRotorEnabled;
- (bool)voiceOverSpeaksOverTelephoneCalls;
- (long long)voiceOverTapticChimesAvailability;
- (bool)voiceOverTapticChimesEnabled;
- (long long)voiceOverTapticChimesFrequencyEncoding;
- (long long)voiceOverTapticChimesSoundType;
- (long long)voiceOverTapticTimeEncoding;
- (bool)voiceOverTapticTimeMode;
- (float)voiceOverTapticTimeSpeed;
- (bool)voiceOverTouchBrailleGestureControl;
- (bool)voiceOverTouchBrailleGesturesDidPlayCalibrationHint;
- (long long)voiceOverTouchBrailleGesturesLockedOrientation;
- (long long)voiceOverTouchBrailleGesturesLockedTypingMode;
- (bool)voiceOverTouchBrailleGesturesShouldUseLockedConfiguration;
- (bool)voiceOverTouchBrailleShouldReverseDots;
- (bool)voiceOverTouchBrailleShowGeneralStatus;
- (bool)voiceOverTouchBrailleShowTextStyleStatus;
- (id)voiceOverTouchBrailleTableIdentifier;
- (id)voiceOverTouchPreferredBrailleTableIdentifiers;
- (bool)voiceOverTouchSingleLetterQuickNavEnabled;
- (bool)voiceOverTouchUpdateBrailleWithoutConnectedDisplay;
- (long long)voiceOverTypingMode;
- (bool)voiceOverUIEnabled;
- (bool)voiceOverUsageConfirmed;
- (bool)voiceOverUseRingerSwitchToControlNotificationOutput;
- (bool)voiceOverUserDidReadUSBRestrictedModeAlert;
- (long long)voiceOverVerbosityEmojiFeedback;
- (bool)voiceOverVerbosityEmojiSuffixEnabled;
- (bool)voiceOverVerbositySpeakCustomActionsHint;
- (float)voiceOverVolume;
- (float)voiceOverVolume;
- (void)willPerformUpdateBlocksForPreferenceKey:(id)arg1;
- (void)willPostNotificationForPreferenceKey:(id)arg1 value:(id)arg2;
- (bool)wordFeedbackEnabled;
- (bool)writeAXLogsToFile;
- (bool)zoomAdjustZoomLevelKbShortcutEnabled;
- (bool)zoomAlwaysUseWindowedZoomForTyping;
- (bool)zoomAutopannerShouldPanWithAcceleration;
- (long long)zoomControllerColor;
- (id)zoomCurrentLensEffect;
- (id)zoomCurrentLensMode;
- (bool)zoomDebugDisableZoomLensScaleTransform;
- (bool)zoomDebugShowExternalFocusRect;
- (bool)zoomEnabled;
- (id)zoomHandGesturesActionCustomizations;
- (bool)zoomHandGesturesEnabled;
- (double)zoomIdleSlugOpacity;
- (bool)zoomInStandby;
- (bool)zoomKeyboardShortcutsEnabled;
- (struct CGPoint { double x1; double x2; })zoomPanOffset;
- (bool)zoomPanZoomKbShortcutEnabled;
- (bool)zoomPeekZoomEnabled;
- (bool)zoomPeekZoomEverEnabled;
- (bool)zoomPreferencesWereInitialized;
- (id)zoomPreferredCurrentDockPosition;
- (id)zoomPreferredCurrentLensMode;
- (id)zoomPreferredDockPositions;
- (id)zoomPreferredLensModes;
- (double)zoomPreferredMaximumZoomScale;
- (bool)zoomResizeZoomWindowKbShortcutEnabled;
- (double)zoomScale;
- (bool)zoomScrollWheelKbShortcutEnabled;
- (bool)zoomShouldAllowFullscreenAutopanning;
- (bool)zoomShouldFollowFocus;
- (bool)zoomShouldShowSlug;
- (bool)zoomShowWhileMirroring;
- (bool)zoomShowedBanner;
- (long long)zoomSlugDoubleTapAction;
- (struct CGPoint { double x1; double x2; })zoomSlugNormalizedPosition;
- (long long)zoomSlugSingleTapAction;
- (bool)zoomSlugTapAndSlideToAdjustZoomLevelEnabled;
- (long long)zoomSlugTripleTapAction;
- (bool)zoomSmoothScalingEnabled;
- (bool)zoomSwitchZoomModeKbShortcutEnabled;
- (bool)zoomTempToggleZoomKbShortcutEnabled;
- (bool)zoomToggleZoomKbShortcutEnabled;
- (bool)zoomToggledByPreferenceSwitch;
- (bool)zoomToggledByVoiceOver;
- (bool)zoomTrackpadGestureEnabled;
- (void)zoomUserHadLegacyZoomEnabled:(bool*)arg1 wasZoomedIn:(bool*)arg2 withScale:(double*)arg3;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })zoomWindowFrame;

// Image: /System/Library/PrivateFrameworks/HomePodSettings.framework/HomePodSettings

+ (id)hps_stringFromTapActivationMethod:(long long)arg1;
+ (long long)hps_tapActivationMethodFromString:(id)arg1;

- (id)hps_touchAccomodationsTapActivationMethodString;

@end
