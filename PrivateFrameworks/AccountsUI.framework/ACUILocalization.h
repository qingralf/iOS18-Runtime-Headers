/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/AccountsUI.framework/AccountsUI
 */

@interface ACUILocalization : NSObject

+ (id)localizedNameForApplicationBundleID:(id)arg1;
+ (id)localizedReferenceToLocalSourceOfDataclass:(id)arg1;
+ (id)localizedStringForDataclass:(id)arg1 withSuffix:(id)arg2 forAccount:(id)arg3;
+ (id)localizedTextForDataclasses:(id)arg1 usedAtBeginningOfSentence:(bool)arg2;
+ (id)localizedTextForDataclasses:(id)arg1 usedAtBeginningOfSentence:(bool)arg2 forAccount:(id)arg3;
+ (id)localizedTitleForDataclass:(id)arg1;
+ (id)localizedTitleForDataclass:(id)arg1 forAccount:(id)arg2;
+ (id)localizedTitleForLocalSourceOfDataclass:(id)arg1 usedAtBeginningOfSentence:(bool)arg2;

@end
