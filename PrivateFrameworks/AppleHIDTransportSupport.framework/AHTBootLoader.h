/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/AppleHIDTransportSupport.framework/AppleHIDTransportSupport
 */

@interface AHTBootLoader : NSObject {
    unsigned int  _connect;
    AHTDevice * _device;
    unsigned int  _imageTag;
    AHTInterface * _interface;
    unsigned int  _machPort;
    bool  _needsRestoreUpdate;
    NSString * _personality;
    unsigned int  _service;
    bool  _supportsMemoryDump;
    NSString * _type;
}

@property (nonatomic) unsigned int connect;
@property (nonatomic, readonly) AHTDevice *device;
@property (nonatomic, readonly) unsigned int imageTag;
@property (nonatomic, readonly) AHTInterface *interface;
@property (nonatomic) unsigned int machPort;
@property (nonatomic, readonly) bool needsRestoreUpdate;
@property (nonatomic, readonly) NSString *personality;
@property (nonatomic) unsigned int service;
@property (nonatomic, readonly) bool supportsMemoryDump;
@property (nonatomic, readonly) NSString *type;

+ (unsigned long long)frameworkToDriverOptions:(unsigned long long)arg1;
+ (id)withDevice:(id)arg1 service:(unsigned int)arg2;
+ (id)withParent:(id)arg1 service:(unsigned int)arg2;
+ (id)withService:(unsigned int)arg1;

- (void).cxx_destruct;
- (bool)clearMemoryDumps:(id*)arg1;
- (void)close;
- (unsigned int)connect;
- (bool)createMachPort:(id*)arg1;
- (void)dealloc;
- (id)description;
- (void)destroyMachPort;
- (id)device;
- (id)dispatchSourceForNotification:(id)arg1 action:(id /* block */)arg2 error:(id*)arg3;
- (bool)getMemoryDumpLevel:(unsigned int*)arg1 error:(id*)arg2;
- (bool)getMemoryDumps:(id*)arg1 error:(id*)arg2;
- (bool)getProperty:(id)arg1 property:(id*)arg2 options:(unsigned long long)arg3 error:(id*)arg4;
- (id)getRegistryProperties;
- (unsigned int)imageTag;
- (id)initWithParent:(id)arg1 service:(unsigned int)arg2;
- (id)initWithService:(unsigned int)arg1;
- (id)interface;
- (bool)isOpen;
- (unsigned int)machPort;
- (bool)needsRestoreUpdate;
- (bool)open:(id*)arg1;
- (bool)overridePersonality:(id)arg1 error:(id*)arg2;
- (bool)overrideProperties:(id)arg1 error:(id*)arg2;
- (id)personality;
- (bool)reloadProperties:(unsigned long long)arg1 error:(id*)arg2;
- (unsigned int)service;
- (void)setConnect:(unsigned int)arg1;
- (void)setMachPort:(unsigned int)arg1;
- (bool)setMemoryDumpLevel:(unsigned int)arg1 error:(id*)arg2;
- (void)setService:(unsigned int)arg1;
- (bool)supportsMemoryDump;
- (id)type;
- (bool)updateProperty:(id)arg1 property:(id)arg2 options:(unsigned long long)arg3 error:(id*)arg4;

@end
