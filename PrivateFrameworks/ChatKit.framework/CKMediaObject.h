/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/ChatKit.framework/ChatKit
 */

@interface CKMediaObject : NSObject <NSSecureCoding, QLPreviewItem> {
    NSString * _UTIType;
    NSURL * _alternateShareURL;
    NSURL * _cachedHighQualityFileURL;
    bool  _cachedValidPreviewExists;
    NSData * _data;
    bool  _didRequestAdaptiveImageGlyphDerivedFromPreviewImage;
    NSURL * _fileURL;
    NSString * _filename;
    bool  _forceInlinePreviewGeneration;
    bool  _generatePreviewOutOfProcess;
    NSData * _generativePlaygroundRecipeData;
    NSObject<OS_dispatch_group> * _highQualityFetchInProgressGroup;
    NSAdaptiveImageGlyph * _internalCachedAdaptiveImageGlyph;
    bool  _isFromMe;
    bool  _isMonoskiAsset;
    bool  _isPhotosCompatible;
    CKMessageContext * _messageContext;
    NSString * _mimeType;
    unsigned long long  _oopPreviewRequestCount;
    IMSenderContext * _senderContext;
    long long  _spatialState;
    bool  _supportsUnknownSenderPreview;
    NSDate * _time;
    UITraitCollection * _transcriptTraitCollection;
    <CKFileTransfer> * _transfer;
    NSString * _transferGUID;
    bool  _validatePreviewFormat;
}

@property (nonatomic, copy) NSString *UTIType;
@property (nonatomic, copy) NSURL *alternateShareURL;
@property (nonatomic, retain) NSURL *cachedHighQualityFileURL;
@property (nonatomic, readonly) bool cachedValidPreviewExists;
@property (nonatomic) bool cachedValidPreviewExists;
@property (nonatomic, readonly) bool canShareItem;
@property (nonatomic, readonly) long long commSafetySensitive;
@property (nonatomic, copy) NSData *data;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (nonatomic) bool didRequestAdaptiveImageGlyphDerivedFromPreviewImage;
@property (nonatomic, copy) NSURL *fileURL;
@property (nonatomic, copy) NSString *filename;
@property (nonatomic) bool forceInlinePreviewGeneration;
@property (nonatomic) bool generatePreviewOutOfProcess;
@property (nonatomic, copy) NSData *generativePlaygroundRecipeData;
@property (readonly) unsigned long long hash;
@property (nonatomic, retain) NSObject<OS_dispatch_group> *highQualityFetchInProgressGroup;
@property (nonatomic, retain) NSAdaptiveImageGlyph *internalCachedAdaptiveImageGlyph;
@property (nonatomic) bool isFromMe;
@property (nonatomic, readonly) bool isMonoskiAsset;
@property (nonatomic) bool isPhotosCompatible;
@property (nonatomic, readonly) int mediaType;
@property (nonatomic, copy) CKMessageContext *messageContext;
@property (nonatomic, readonly) NSString *metricsCollectorMediaType;
@property (nonatomic, copy) NSString *mimeType;
@property (nonatomic, readonly) bool needsAnimation;
@property (nonatomic) unsigned long long oopPreviewRequestCount;
@property (nonatomic, readonly, copy) NSString *previewFilenameExtension;
@property (nonatomic, readonly) NSString *previewItemTitle; /* unknown property attribute: ? */
@property (nonatomic, readonly) NSURL *previewItemURL;
@property (nonatomic, readonly, copy) id previewMetadata;
@property (nonatomic, readonly, copy) IMSenderContext *senderContext;
@property (nonatomic, readonly, copy) IMHandle *senderHandle;
@property (nonatomic, readonly) bool shouldSuppressPreview;
@property (nonatomic) long long spatialState;
@property (readonly) Class superclass;
@property (nonatomic, readonly) bool supportsUnknownSenderPreview;
@property (nonatomic, readonly) NSString *syndicationIdentifier;
@property (nonatomic, retain) NSDate *time;
@property (nonatomic, readonly, copy) NSDictionary *transcoderUserInfo;
@property (nonatomic, retain) UITraitCollection *transcriptTraitCollection;
@property (nonatomic, retain) <CKFileTransfer> *transfer;
@property (nonatomic, copy) NSString *transferGUID;
@property (nonatomic) bool validatePreviewFormat;

+ (id)UTITypes;
+ (Class)__ck_attachmentItemClass;
+ (bool)canGeneratePreviewInMVSHostProcess;
+ (id)fallbackFilenamePrefix;
+ (id)iconCache;
+ (bool)isPreviewable;
+ (id)mediaClasses;
+ (bool)shouldScaleUpPreview;
+ (bool)shouldShadePreview;
+ (bool)shouldUseTranscoderGeneratedPreviewSize;
+ (id)spatialMetadataProcessingQueue;
+ (bool)supportsSecureCoding;
+ (id)transferGUIDSFailingPreviewGeneration;

- (void).cxx_destruct;
- (id)ASTCDataFromImage:(id)arg1;
- (id)JPEGDataFromImage:(id)arg1;
- (id)UTIType;
- (id)_balloonViewForClassWithWidth:(double)arg1 orientation:(BOOL)arg2;
- (id)_composeImageForBalloonView:(id)arg1 colorType:(BOOL)arg2 traitCollection:(id)arg3;
- (id)_generateIconWithSize:(struct CGSize { double x1; double x2; })arg1 scale:(double)arg2;
- (struct IMPreviewConstraints { double x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; double x3; bool x4; bool x5; bool x6; })_previewConstraintsForWidth:(double)arg1;
- (id)_qlThumbnailGeneratorSharedGenerator;
- (void)_refreshSpatialMetadata;
- (void)_sampleImageEdges:(id)arg1 usingRect:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg2 forMostlyWhitePixels:(unsigned long long*)arg3 otherPixels:(unsigned long long*)arg4;
- (bool)_shouldDenyUTITypeFromRichIcon;
- (id)_transcodeControllerSharedInstance;
- (void)_transferUpdated:(id)arg1;
- (id)adaptiveImageGlyph;
- (id)alternateShareURL;
- (id)attachmentSummary:(unsigned long long)arg1;
- (Class)balloonViewClass;
- (Class)balloonViewClassForWidth:(double)arg1 orientation:(BOOL)arg2;
- (id)bbPreviewFillToSize:(struct CGSize { double x1; double x2; })arg1;
- (struct CGSize { double x1; double x2; })bbSize;
- (id)blastdooredAdaptiveImageGlyphFileURL;
- (void)cacheAndPersistPreview:(id)arg1 orientation:(BOOL)arg2;
- (id)cachedBlastdooredAdaptiveImageGlyph;
- (id)cachedHighQualityFileURL;
- (bool)cachedValidPreviewExists;
- (bool)canExport;
- (bool)canPerformQuickAction;
- (bool)canShareItem;
- (long long)commSafetySensitive;
- (id)composeImagesForEntryContentViewWidth:(double)arg1 traitCollection:(id)arg2;
- (id)data;
- (void)dealloc;
- (double)defaultPreviewWidth;
- (id)description;
- (bool)didRequestAdaptiveImageGlyphDerivedFromPreviewImage;
- (void)encodeWithCoder:(id)arg1;
- (void)fetchHighQualityFile:(id /* block */)arg1;
- (id)fileManager;
- (id)fileSizeString;
- (id)fileURL;
- (id)filename;
- (bool)forceInlinePreviewGeneration;
- (void)generateAndCacheBlastdooredAdaptiveImageGlyph;
- (id)generateNewBlastdooredAdaptiveImageGlyph;
- (void)generateOOPPreviewForWidth:(double)arg1 orientation:(BOOL)arg2;
- (id)generatePreviewFromThumbnail:(id)arg1 width:(double)arg2 orientation:(BOOL)arg3;
- (bool)generatePreviewOutOfProcess;
- (id)generateSingleStrikeAdaptiveImageGlyphFromBlastdooredPreviewImage;
- (id)generateThumbnailFillToSize:(struct CGSize { double x1; double x2; })arg1 contentAlignmentInsets:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg2;
- (id)generateThumbnailForWidth:(double)arg1 orientation:(BOOL)arg2;
- (id)generativePlaygroundRecipeData;
- (id)highQualityFetchInProgressGroup;
- (id)icon;
- (id)image:(id)arg1 withBackgroundColor:(id)arg2;
- (id)initWithCoder:(id)arg1;
- (id)initWithTransfer:(id)arg1 context:(id)arg2 forceInlinePreview:(bool)arg3;
- (Class)inlineStickerBalloonViewClass;
- (id)internalCachedAdaptiveImageGlyph;
- (id)invisibleInkEffectImageWithPreview:(id)arg1;
- (bool)isDirectory;
- (bool)isEqual:(id)arg1;
- (bool)isFromMe;
- (bool)isMonoskiAsset;
- (bool)isMonoskiAsset;
- (bool)isPhotosCompatible;
- (bool)isPreviewable;
- (bool)isPromisedItem;
- (id)location;
- (void)mediaIsSpatialBasedOnMetadata:(id /* block */)arg1;
- (void)mediaIsSpatialUsingPhotosFormats:(id /* block */)arg1;
- (int)mediaType;
- (id)messageContext;
- (id)metricsCollectorMediaType;
- (id)mimeType;
- (bool)needsAnimation;
- (unsigned long long)oopPreviewRequestCount;
- (id)pasteboardItemProvider;
- (Class)placeholderBalloonViewClass;
- (void)postPreviewDidChangeNotifications;
- (Class)previewBalloonViewClass;
- (id)previewCacheKeyWithOrientation:(BOOL)arg1;
- (id)previewCachesFileURLWithOrientation:(BOOL)arg1 extension:(id)arg2 generateIntermediaries:(bool)arg3;
- (id)previewCachesFileURLWithOrientation:(BOOL)arg1 extension:(id)arg2 generateIntermediaries:(bool)arg3 transferGUID:(id)arg4;
- (id)previewDispatchCache;
- (id)previewFilenameExtension;
- (id)previewForWidth:(double)arg1 orientation:(BOOL)arg2;
- (id)previewItemURL;
- (id)previewMetadata;
- (void)prewarmPreviewForWidth:(double)arg1 orientation:(BOOL)arg2;
- (Class)replyContextBalloonViewClass;
- (id)richIcon;
- (id)rtfDocumentItemsWithFormatString:(id)arg1 selectedTextRange:(struct _NSRange { unsigned long long x1; unsigned long long x2; })arg2;
- (void)saveAdaptiveImageGlyphToFileCache:(id)arg1;
- (void)savePreview:(id)arg1 toURL:(id)arg2 forOrientation:(BOOL)arg3;
- (id)savedPreviewFromURL:(id)arg1 forOrientation:(BOOL)arg2;
- (id)senderContext;
- (id)senderContext;
- (id)senderHandle;
- (void)setAdaptiveImageGlyph:(id)arg1;
- (void)setAlternateShareURL:(id)arg1;
- (void)setCachedHighQualityFileURL:(id)arg1;
- (void)setCachedValidPreviewExists:(bool)arg1;
- (void)setData:(id)arg1;
- (void)setDidRequestAdaptiveImageGlyphDerivedFromPreviewImage:(bool)arg1;
- (void)setFileURL:(id)arg1;
- (void)setFilename:(id)arg1;
- (void)setForceInlinePreviewGeneration:(bool)arg1;
- (void)setGeneratePreviewOutOfProcess:(bool)arg1;
- (void)setGenerativePlaygroundRecipeData:(id)arg1;
- (void)setHighQualityFetchInProgressGroup:(id)arg1;
- (void)setInternalCachedAdaptiveImageGlyph:(id)arg1;
- (void)setIsFromMe:(bool)arg1;
- (void)setIsPhotosCompatible:(bool)arg1;
- (void)setMessageContext:(id)arg1;
- (void)setMimeType:(id)arg1;
- (void)setOopPreviewRequestCount:(unsigned long long)arg1;
- (void)setSpatialState:(long long)arg1;
- (void)setTime:(id)arg1;
- (void)setTranscriptTraitCollection:(id)arg1;
- (void)setTransfer:(id)arg1;
- (void)setTransferGUID:(id)arg1;
- (void)setUTIType:(id)arg1;
- (void)setValidatePreviewFormat:(bool)arg1;
- (bool)shouldBeQuickLooked;
- (bool)shouldShowDisclosure;
- (bool)shouldShowViewer;
- (bool)shouldSuppressPreview;
- (long long)spatialState;
- (id)subtitle;
- (bool)supportsUnknownSenderPreview;
- (id)syndicationIdentifier;
- (id)time;
- (id)title;
- (struct CGSize { double x1; double x2; })transcoderGeneratedSizeForConstraints:(struct IMPreviewConstraints { double x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; double x3; bool x4; bool x5; bool x6; })arg1;
- (bool)transcoderPreviewGenerationFailed;
- (id)transcoderUserInfo;
- (struct IMPreviewConstraints { double x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; double x3; bool x4; bool x5; bool x6; })transcodingPreviewConstraints;
- (struct CGSize { double x1; double x2; })transcodingPreviewPxSize;
- (id)transcriptTraitCollection;
- (id)transfer;
- (id)transferGUID;
- (void)updateIntentFileManagerIfNecessaryWith:(id)arg1;
- (bool)validPreviewExistsAtURL:(id)arg1;
- (bool)validatePreviewFormat;

@end
