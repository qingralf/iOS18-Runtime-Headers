/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/CloudDocsDaemon.framework/CloudDocsDaemon
 */

@interface _BRCOperation : NSOperation {
    NSObject<OS_os_activity> * _Activity;
    NSMutableArray * _associatedEventMetrics;
    NSObject<OS_dispatch_queue> * _callbackQueue;
    NSObject<OS_os_transaction> * _executionTransaction;
    id /* block */  _finishBlock;
    NSDate * _finishDate;
    bool  _finished;
    NSObject<OS_dispatch_group> * _group;
    NSObject<OS_dispatch_group> * _highPriorityWaitGroup;
    NSObject<OS_dispatch_queue> * _internalQueue;
    NSError * _lastError;
    NSDate * _lastTryDate;
    struct { 
        unsigned long long sectionID; 
        char *function; 
        int line; 
    }  _logSections;
    id /* block */  _mainBlock;
    double  _maxBackoff;
    NSDate * _nextTryDate;
    bool  _nonDiscretionary;
    BRCThrottle * _operationFailureThrottle;
    CKOperationGroup * _operationGroup;
    BRCThrottle * _operationThrottle;
    unsigned char  _operationUUID;
    NSString * _personaID;
    NSObject<OS_dispatch_source> * _retryTimer;
    <BRCSessionContext> * _sessionContext;
    NSDate * _startDate;
    NSHashTable * _subOperations;
    BRCSyncContext * _syncContext;
    long long  _throttleHash;
    bool  _timedOut;
    double  _timeout;
    NSObject<OS_dispatch_source> * _timeoutTimer;
}

@property (nonatomic, readonly) NSObject<OS_dispatch_queue> *callbackQueue;
@property (nonatomic, readonly) NSError *error;
@property (getter=isExecuting, nonatomic) bool executing;
@property (nonatomic, readonly) double executionTimeInSec;
@property (nonatomic, copy) id /* block */ finishBlock;
@property (getter=isFinished, nonatomic) bool finished;
@property (nonatomic, retain) CKOperationGroup *group;
@property (nonatomic, readonly) NSObject<OS_dispatch_group> *highPriorityWaitGroup;
@property (nonatomic, readonly) struct { unsigned long long x1; char *x2; int x3; } logSections;
@property (nonatomic, copy) id /* block */ mainBlock;
@property (nonatomic) double maxBackoff;
@property (nonatomic, readonly) NSDate *nextTryDate;
@property (nonatomic) bool nonDiscretionary;
@property (nonatomic) BRCThrottle *operationFailureThrottle;
@property (nonatomic, readonly) NSUUID *operationID;
@property (nonatomic) BRCThrottle *operationThrottle;
@property (nonatomic, readonly) NSDate *startDate;
@property (nonatomic, readonly) BRCSyncContext *syncContext;
@property (nonatomic) bool timedOut;
@property (nonatomic) double timeout;
@property (nonatomic, readonly) bool usesBackgroundSession;

- (void).cxx_destruct;
- (void)_completedWithResult:(id)arg1 error:(id)arg2;
- (void)_executeWithPreviousError:(id)arg1;
- (bool)_finishIfCancelled;
- (void)_main;
- (void)_scheduleExecutionWithPreviousError:(id)arg1;
- (void)_setDeviceConfigurationHeaderOnCKOp:(id)arg1;
- (void)addDependency:(id)arg1;
- (void)addSubOperation:(id)arg1;
- (void)addSubOperation:(id)arg1 overrideContext:(id)arg2 allowsCellularAccess:(id)arg3;
- (void)associateCKOperationsToEventMetric:(id)arg1;
- (void)blockOnHighPriorityOperation:(id)arg1;
- (id)callbackQueue;
- (void)cancel;
- (void)cancelAfterDelay:(double)arg1;
- (void)completedWithResult:(id)arg1 error:(id)arg2;
- (id)createActivity;
- (void)dealloc;
- (id)description;
- (id)descriptionWithContext:(id)arg1;
- (id)error;
- (double)executionTimeInSec;
- (id /* block */)finishBlock;
- (bool)finishIfCancelled;
- (void)finishWithResult:(id)arg1 error:(id)arg2;
- (id)group;
- (id)highPriorityWaitGroup;
- (id)init;
- (id)initWithName:(id)arg1;
- (id)initWithName:(id)arg1 syncContext:(id)arg2 sessionContext:(id)arg3;
- (id)initWithName:(id)arg1 syncContext:(id)arg2 sessionContext:(id)arg3 group:(id)arg4;
- (bool)isConcurrent;
- (bool)isExecuting;
- (bool)isFinished;
- (id)lastErrorDescription;
- (struct { unsigned long long x1; char *x2; int x3; })logSections;
- (void)main;
- (id /* block */)mainBlock;
- (double)maxBackoff;
- (id)nextTryDate;
- (bool)nonDiscretionary;
- (id)operationFailureThrottle;
- (id)operationID;
- (id)operationThrottle;
- (void)schedule;
- (void)setExecuting:(bool)arg1;
- (void)setFinishBlock:(id /* block */)arg1;
- (void)setFinished:(bool)arg1;
- (void)setGroup:(id)arg1;
- (void)setMainBlock:(id /* block */)arg1;
- (void)setMaxBackoff:(double)arg1;
- (void)setNonDiscretionary:(bool)arg1;
- (void)setOperationFailureThrottle:(id)arg1;
- (void)setOperationThrottle:(id)arg1;
- (void)setTimedOut:(bool)arg1;
- (void)setTimeout:(double)arg1;
- (bool)shouldRetryForError:(id)arg1;
- (void)start;
- (id)startDate;
- (id)stateWithContext:(id)arg1;
- (id)subclassableDescriptionWithContext:(id)arg1;
- (id)syncContext;
- (bool)timedOut;
- (double)timeout;
- (bool)usesBackgroundSession;

@end
