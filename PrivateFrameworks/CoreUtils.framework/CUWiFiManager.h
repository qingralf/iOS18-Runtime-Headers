/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/CoreUtils.framework/CoreUtils
 */

@interface CUWiFiManager : NSObject {
    bool  _activateCalled;
    bool  _activated;
    unsigned int  _controlFlags;
    bool  _degradeOnAnyTrafficUnavailable;
    NSObject<OS_dispatch_queue> * _dispatchQueue;
    bool  _infraDisabled;
    bool  _infraDisabledChanged;
    id /* block */  _interruptionHandler;
    bool  _invalidateCalled;
    bool  _invalidateDone;
    id /* block */  _invalidationHandler;
    NSString * _label;
    int  _lastTrafficRegistrationErrorCode;
    unsigned int  _trafficFlagsUnavailable;
    id /* block */  _trafficFlagsUnavailableUpdatedHandler;
    bool  _trafficPeerChanged;
    bool  _trafficPeerDegraded;
    NSArray * _trafficPeers;
    NSArray * _trafficPeersCurrent;
    struct LogCategory { int x1; int x2; char *x3; unsigned int x4; char *x5; char *x6; int x7; struct LogCategory {} *x8; struct LogOutput {} *x9; struct LogOutput {} *x10; unsigned long long x11; unsigned long long x12; unsigned int x13; unsigned int x14; char *x15; struct LogCategoryPrivate {} *x16; } * _ucat;
    bool  _updating;
    bool  _wakeOnWirelessEnabled;
    bool  _wakeOnWirelessEnabledChanged;
    bool  _wifiAutoJoinDisabled;
    bool  _wifiCriticalSetup;
    bool  _wifiCriticalState;
    struct __WiFiDeviceClient { } * _wifiDevice;
    bool  _wifiDeviceSetup;
    unsigned int  _wifiFlags;
    struct __WiFiManagerClient { } * _wifiManager;
    bool  _wifiManagerSetup;
    struct __WiFiNetwork { } * _wifiNetworkDisabled;
    WiFiP2PAWDLStateMonitor * _wifiP2PStateMonitor;
    NSSet * _wifiP2PStateServices;
    CURetrier * _wifiRetrier;
    int  _wifiState;
    id /* block */  _wifiStateChangedHandler;
    bool  _wifiStateMonitorSetup;
}

@property (nonatomic) unsigned int controlFlags;
@property (nonatomic) bool degradeOnAnyTrafficUnavailable;
@property (nonatomic, retain) NSObject<OS_dispatch_queue> *dispatchQueue;
@property (nonatomic) bool infraDisabled;
@property (nonatomic, copy) id /* block */ interruptionHandler;
@property (nonatomic, copy) id /* block */ invalidationHandler;
@property (nonatomic, copy) NSString *label;
@property (nonatomic, readonly) unsigned int trafficFlagsUnavailable;
@property (nonatomic, copy) id /* block */ trafficFlagsUnavailableUpdatedHandler;
@property (nonatomic, copy) NSArray *trafficPeers;
@property (nonatomic) bool wakeOnWirelessEnabled;
@property (nonatomic, readonly) unsigned int wifiFlags;
@property (nonatomic, readonly) int wifiState;
@property (nonatomic, copy) id /* block */ wifiStateChangedHandler;

- (void).cxx_destruct;
- (void)_activateSafeInvokeBlock:(id /* block */)arg1;
- (void)_interrupted;
- (void)_invalidated;
- (void)_update;
- (void)_updateInfraDisabled;
- (void)_updateTrafficPeerStatusMonitor;
- (void)_updateTrafficPeerStatusUpdated:(id)arg1 isAvailable:(bool)arg2 error:(id)arg3;
- (void)_updateTrafficPeers;
- (void)_updateTrafficPeersWithService:(unsigned int)arg1;
- (void)_updateWakeOnWireless;
- (void)_updateWiFiState;
- (void)_wifiAutoJoinNotification:(id)arg1;
- (void)_wifiEnsureStarted;
- (void)_wifiEnsureStopped;
- (unsigned int)_wifiFlagsUncached;
- (void)_wifiStateChanged;
- (int)_wifiStateUncached;
- (void)activate;
- (void)activateWithCompletion:(id /* block */)arg1;
- (unsigned int)controlFlags;
- (void)dealloc;
- (bool)degradeOnAnyTrafficUnavailable;
- (id)dispatchQueue;
- (bool)infraDisabled;
- (id)init;
- (id /* block */)interruptionHandler;
- (void)invalidate;
- (id /* block */)invalidationHandler;
- (id)label;
- (void)performUpdate:(id /* block */)arg1;
- (void)setControlFlags:(unsigned int)arg1;
- (void)setDegradeOnAnyTrafficUnavailable:(bool)arg1;
- (void)setDispatchQueue:(id)arg1;
- (void)setInfraDisabled:(bool)arg1;
- (void)setInterruptionHandler:(id /* block */)arg1;
- (void)setInvalidationHandler:(id /* block */)arg1;
- (void)setLabel:(id)arg1;
- (void)setTrafficFlagsUnavailableUpdatedHandler:(id /* block */)arg1;
- (void)setTrafficPeers:(id)arg1;
- (void)setWakeOnWirelessEnabled:(bool)arg1;
- (void)setWifiStateChangedHandler:(id /* block */)arg1;
- (unsigned int)trafficFlagsUnavailable;
- (id /* block */)trafficFlagsUnavailableUpdatedHandler;
- (id)trafficPeers;
- (bool)wakeOnWirelessEnabled;
- (unsigned int)wifiFlags;
- (int)wifiState;
- (id /* block */)wifiStateChangedHandler;

@end
