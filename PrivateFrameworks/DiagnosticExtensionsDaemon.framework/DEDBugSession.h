/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/DiagnosticExtensionsDaemon.framework/DiagnosticExtensionsDaemon
 */

@interface DEDBugSession : NSObject <DEDSecureArchiving, NSSecureCoding> {
    <DEDUploadStatusDelegate> * ___uploadDelegate;
    NSObject<OS_dispatch_queue> * __diskAccessQueue;
    <DEDFinisher><DEDSecureArchiving> * __finisher;
    <DEDNotifier><DEDSecureArchiving> * __notifier;
    NSMutableArray * _adoptFilesCompletions;
    NSDictionary * _cachedExtensions;
    NSObject<OS_dispatch_queue> * _callbackQueue;
    <DEDClientProtocol> * _client;
    DEDBugSessionConfiguration * _config;
    NSMutableDictionary * _deCompletions;
    NSMutableArray * _deListCompletions;
    id /* block */  _deTextDataLoadCompletion;
    NSString * _deviceIdentifier;
    NSMutableSet * _filePromises;
    bool  _finisherDidCommit;
    NSString * _identifier;
    DEDExtensionIdentifierManager * _identifierManager;
    NSMutableDictionary * _ongoingCompletionHandlers;
    NSMutableDictionary * _pendingOperations;
    id /* block */  _pingHandler;
    NSString * _priorClientAddress;
    long long  _priorClientTransportType;
    long long  _priorWorkerTransportType;
    id /* block */  _sessionStateCompletionBlock;
    id /* block */  _statusCompletionBlock;
    id /* block */  _syncCompletionBlock;
    NSDate * _uploadStartTime;
    <DEDWorkerProtocol> * _worker;
}

@property (nonatomic) <DEDUploadStatusDelegate> *__uploadDelegate;
@property (nonatomic, retain) NSObject<OS_dispatch_queue> *_diskAccessQueue;
@property (retain) <DEDFinisher><DEDSecureArchiving> *_finisher;
@property (retain) <DEDNotifier><DEDSecureArchiving> *_notifier;
@property (retain) NSMutableArray *adoptFilesCompletions;
@property (readonly) NSArray *allExtensionIdentifiers;
@property (retain) NSDictionary *cachedExtensions;
@property (nonatomic, retain) NSObject<OS_dispatch_queue> *callbackQueue;
@property (retain) <DEDClientProtocol> *client;
@property (retain) DEDBugSessionConfiguration *config;
@property (retain) NSMutableDictionary *deCompletions;
@property (retain) NSMutableArray *deListCompletions;
@property (copy) id /* block */ deTextDataLoadCompletion;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (retain) NSString *deviceIdentifier;
@property (readonly) NSObject<OS_dispatch_queue> *diskAccessQueue;
@property (retain) NSMutableSet *filePromises;
@property bool finisherDidCommit;
@property (readonly) unsigned long long hash;
@property (retain) NSString *identifier;
@property (retain) DEDExtensionIdentifierManager *identifierManager;
@property (readonly) NSObject<OS_os_log> *log;
@property (retain) NSMutableDictionary *ongoingCompletionHandlers;
@property (retain) NSMutableDictionary *pendingOperations;
@property (copy) id /* block */ pingHandler;
@property (nonatomic, retain) NSString *priorClientAddress;
@property long long priorClientTransportType;
@property long long priorWorkerTransportType;
@property (copy) id /* block */ sessionStateCompletionBlock;
@property (copy) id /* block */ statusCompletionBlock;
@property (readonly) Class superclass;
@property (copy) id /* block */ syncCompletionBlock;
@property <DEDUploadStatusDelegate> *uploadDelegate;
@property (retain) NSDate *uploadStartTime;
@property (retain) <DEDWorkerProtocol> *worker;

// Image: /System/Library/PrivateFrameworks/DiagnosticExtensionsDaemon.framework/DiagnosticExtensionsDaemon

+ (id)archivedClasses;
+ (bool)supportsSecureCoding;
+ (id)testingInstance;

- (void).cxx_destruct;
- (id)__uploadDelegate;
- (void)_addSessionData:(id)arg1 filename:(id)arg2;
- (void)_cancel;
- (id)_diskAccessQueue;
- (id)_finisher;
- (void)_logOperationQueue;
- (id)_notifier;
- (void)_prepareForStartDiagnosticExtensionWithDEDIdentifier:(id)arg1 parameters:(id)arg2 newCollectRemovedPendingKill:(bool*)arg3 completion:(id /* block */)arg4;
- (void)_streamOperationQueue;
- (void)_terminateExtensionWithIdentifier:(id)arg1 info:(id)arg2;
- (void)addData:(id)arg1 withFilename:(id)arg2;
- (void)adoptFiles:(id)arg1 withCompletion:(id /* block */)arg2;
- (id)adoptFilesCompletions;
- (id)allExtensionIdentifiers;
- (void)attachCompletionHandlerForDEDExtensionIdentifier:(id)arg1 handler:(id /* block */)arg2;
- (void)attachCompletionHandlerForOngoingOperation:(id)arg1 handler:(id /* block */)arg2;
- (id)cachedExtensions;
- (id)callbackQueue;
- (void)cancel;
- (void)cancelDiagnosticExtension:(id)arg1;
- (void)cancelDiagnosticExtensionWithDEDExtensionIdentifier:(id)arg1;
- (void)cancelDiagnosticExtensionWithIdentifier:(id)arg1;
- (void)cancelDiagnosticExtensionWithIdentifier:(id)arg1 invocationNumber:(long long)arg2;
- (void)cleanup;
- (void)cleanupFinishedUploads:(bool)arg1;
- (void)clearNotification;
- (void)clearNotificationOnFilingDevice;
- (id)client;
- (void)commit;
- (void)compressionProgress:(unsigned long long)arg1 total:(unsigned long long)arg2;
- (id)config;
- (id)configuration;
- (id)deCompletions;
- (id)deListCompletions;
- (id /* block */)deTextDataLoadCompletion;
- (id)description;
- (id)deviceIdentifier;
- (void)didAdoptFilesWithError:(id)arg1;
- (void)didAdoptGroup:(id)arg1;
- (void)didCancel;
- (void)didCommit;
- (void)didFinishAllUploads;
- (void)didFinishOperation:(long long)arg1 withIdentifier:(id)arg2;
- (void)didGetState:(long long)arg1 info:(id)arg2;
- (void)didLoadTextDataForExtensions:(id)arg1 localization:(id)arg2;
- (void)didStart;
- (void)didTerminateDiagnosticExtension:(id)arg1 info:(id)arg2;
- (id)diskAccessQueue;
- (void)encodeWithCoder:(id)arg1;
- (id)filePromises;
- (void)finallyStartDiagnosticWithIdentifier:(id)arg1 parameters:(id)arg2 completion:(id /* block */)arg3;
- (void)finishedDiagnosticWithIdentifier:(id)arg1 result:(id)arg2;
- (id)finisher;
- (bool)finisherDidCommit;
- (id)finisherWithState;
- (void)getSessionStatusWithCompletion:(id /* block */)arg1;
- (void)getStateWithCompletion:(id /* block */)arg1;
- (bool)hasCapability:(id)arg1;
- (void)hasCollected:(id)arg1 isCollecting:(id)arg2;
- (void)hasCollected:(id)arg1 isCollecting:(id)arg2 identifiers:(id)arg3;
- (bool)hasDirectory;
- (bool)hasStaleDirectory;
- (id)hashExtensions:(id)arg1;
- (id)identifier;
- (id)identifierManager;
- (void)idsInbound_clearNotificationOnFilingDevice;
- (void)idsInbound_didClearNotificationOnFilingDevice;
- (void)idsInbound_didPresentNotificationOnFilingDevice;
- (void)idsInbound_presentNotificationOnFilingDevice;
- (id)initWithCoder:(id)arg1;
- (id)initWithConfiguration:(id)arg1;
- (long long)instanceType;
- (bool)isCancelling;
- (bool)isRunningDE:(id)arg1;
- (bool)isTarget;
- (id)knownExtensions;
- (void)listDiagnosticExtensionsWithCompletion:(id /* block */)arg1;
- (void)loadTextDataForExtensions:(id)arg1 localization:(id)arg2 completion:(id /* block */)arg3;
- (id)log;
- (id /* block */)nextCompletionBlockForIdentifier:(id)arg1;
- (id)notifier;
- (void)notifyPromiseCancellationIfNeeded:(id)arg1 info:(id)arg2;
- (id)ongoingCollectOperationsWithOperations:(id)arg1;
- (id)ongoingCompletionHandlers;
- (id)ongoingExtensionCollectionOperations;
- (id)pendingOperations;
- (id /* block */)pingHandler;
- (void)pingWithCallback:(id /* block */)arg1;
- (void)pong;
- (void)presentNotificationOnFilingDevice;
- (id)prettyDescription;
- (id)priorClientAddress;
- (long long)priorClientTransportType;
- (long long)priorWorkerTransportType;
- (bool)readyToCancel;
- (bool)readyToFinish;
- (bool)readyToNotify;
- (void)removeAttachment:(id)arg1;
- (void)removeFromStore;
- (void)resumePendingOperations;
- (void)save;
- (void)scheduleDiagnosticCollectionForClassBAccessWithIdentifier:(id)arg1 parameters:(id)arg2;
- (void)scheduleNotification;
- (id)scheduledDeferredExtensionCollectionOperations;
- (id /* block */)sessionStateCompletionBlock;
- (void)setAdoptFilesCompletions:(id)arg1;
- (void)setCachedExtensions:(id)arg1;
- (void)setCallbackQueue:(id)arg1;
- (void)setClient:(id)arg1;
- (void)setConfig:(id)arg1;
- (void)setDeCompletions:(id)arg1;
- (void)setDeListCompletions:(id)arg1;
- (void)setDeTextDataLoadCompletion:(id /* block */)arg1;
- (void)setDeviceIdentifier:(id)arg1;
- (void)setFilePromises:(id)arg1;
- (void)setFinisherDidCommit:(bool)arg1;
- (void)setIdentifier:(id)arg1;
- (void)setIdentifierManager:(id)arg1;
- (void)setOngoingCompletionHandlers:(id)arg1;
- (void)setPendingOperations:(id)arg1;
- (void)setPingHandler:(id /* block */)arg1;
- (void)setPriorClientAddress:(id)arg1;
- (void)setPriorClientTransportType:(long long)arg1;
- (void)setPriorWorkerTransportType:(long long)arg1;
- (void)setSessionStateCompletionBlock:(id /* block */)arg1;
- (void)setStatusCompletionBlock:(id /* block */)arg1;
- (void)setSyncCompletionBlock:(id /* block */)arg1;
- (void)setUploadDelegate:(id)arg1;
- (void)setUploadStartTime:(id)arg1;
- (void)setWorker:(id)arg1;
- (void)set__uploadDelegate:(id)arg1;
- (void)set_diskAccessQueue:(id)arg1;
- (void)set_finisher:(id)arg1;
- (void)set_notifier:(id)arg1;
- (id)shortDescription;
- (bool)shouldCollectDiagnosticWithId:(id)arg1;
- (void)startDiagnosticExtension:(id)arg1 parameters:(id)arg2 completion:(id /* block */)arg3;
- (void)startDiagnosticExtensionWithDEDIdentifier:(id)arg1 parameters:(id)arg2 completion:(id /* block */)arg3;
- (void)startDiagnosticExtensionWithDEDIdentifier:(id)arg1 parameters:(id)arg2 deferRunUntil:(id)arg3 completion:(id /* block */)arg4;
- (id)startDiagnosticExtensionWithIdentifier:(id)arg1 parameters:(id)arg2 completion:(id /* block */)arg3;
- (id)startDiagnosticExtensionWithIdentifier:(id)arg1 parameters:(id)arg2 deferRunUntil:(id)arg3 completion:(id /* block */)arg4;
- (long long)state;
- (id)stateInfo;
- (id /* block */)statusCompletionBlock;
- (void)storeCollectionCompletion:(id /* block */)arg1 forIdentifier:(id)arg2;
- (void)supportsDiagnostics:(id)arg1;
- (id /* block */)syncCompletionBlock;
- (void)synchronizeSessionStatusWithCompletion:(id /* block */)arg1;
- (void)terminateExtension:(id)arg1 withInfo:(id)arg2;
- (id)uploadDelegate;
- (void)uploadProgress:(unsigned long long)arg1 total:(unsigned long long)arg2;
- (id)uploadStartTime;
- (void)willPerformOperationWithID:(id)arg1 type:(long long)arg2 options:(id)arg3;
- (id)worker;

// Image: /System/Library/PrivateFrameworks/FeedbackCore.framework/FeedbackCore

- (id)deviceFromManager:(id)arg1;

@end
