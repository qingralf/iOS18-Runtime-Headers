/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/GPUToolsTransport.framework/GPUToolsTransport
 */

@interface GTUnsupportedFenumInfo : NSObject <NSSecureCoding> {
    NSString * _category;
    NSString * _customMessage;
    NSString * _customRecoverySuggestion;
    NSString * _fenum;
}

@property (nonatomic, readonly) NSString *category;
@property (nonatomic, readonly) NSString *customMessage;
@property (nonatomic, readonly) NSString *customRecoverySuggestion;
@property (nonatomic, readonly) NSString *fenum;

+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)asError;
- (id)category;
- (id)customMessage;
- (id)customRecoverySuggestion;
- (void)encodeWithCoder:(id)arg1;
- (id)fenum;
- (id)initWithCoder:(id)arg1;
- (id)initWithFenum:(id)arg1 category:(id)arg2 customMessage:(id)arg3 customRecoverySuggestion:(id)arg4;

@end
