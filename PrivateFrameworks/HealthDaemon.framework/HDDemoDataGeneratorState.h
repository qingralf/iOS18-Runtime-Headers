/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/HealthDaemon.framework/HealthDaemon
 */

@interface HDDemoDataGeneratorState : NSObject <NSSecureCoding> {
    bool  _coreTraining;
    bool  _createdFromNSKeyedUnarchiver;
    long long  _currentDay;
    double  _currentTime;
    bool  _dancing;
    double  _endTime;
    bool  _finished;
    bool  _firstRun;
    NSDate * _firstSampleDate;
    bool  _functionalStrengthTraining;
    NSDate * _lastRunDate;
    NSDate * _lastSampleDate;
    double  _startTime;
    HDDemoDataGeneratorWorkoutConfiguration * _workoutConfiguration;
}

@property (nonatomic, readonly) long long calendarDay;
@property (getter=isCoolingDown, nonatomic, readonly) bool coolingDown;
@property (getter=isCoreTraining, nonatomic, readonly) bool coreTraining;
@property (getter=isCrossCountrySkiing, nonatomic, readonly) bool crossCountrySkiing;
@property (nonatomic, readonly) NSDate *currentDate;
@property (nonatomic) long long currentDay;
@property (nonatomic) double currentTime;
@property (getter=isCycling, nonatomic, readonly) bool cycling;
@property (getter=isDancing, nonatomic, readonly) bool dancing;
@property (getter=isDoingElliptical, nonatomic, readonly) bool doingElliptical;
@property (getter=isDoingHIIT, nonatomic, readonly) bool doingHIIT;
@property (getter=isDoingYoga, nonatomic, readonly) bool doingYoga;
@property (getter=isDownhillSnowSports, nonatomic, readonly) bool downhillSnowSports;
@property (nonatomic) double endTime;
@property (getter=isExercising, nonatomic, readonly) bool exercising;
@property (getter=isFinished, nonatomic) bool finished;
@property (getter=isFirstRun, nonatomic) bool firstRun;
@property (nonatomic, retain) NSDate *firstSampleDate;
@property (getter=isFunctionalStrengthTraining, nonatomic, readonly) bool functionalStrengthTraining;
@property (getter=isGolf, nonatomic, readonly) bool golf;
@property (getter=isHiking, nonatomic, readonly) bool hiking;
@property (nonatomic, retain) NSDate *lastRunDate;
@property (nonatomic, retain) NSDate *lastSampleDate;
@property (getter=isPaddleSports, nonatomic, readonly) bool paddleSports;
@property (getter=isRowing, nonatomic, readonly) bool rowing;
@property (getter=isRunning, nonatomic, readonly) bool running;
@property (getter=isSkatingSports, nonatomic, readonly) bool skatingSports;
@property (getter=isSoccer, nonatomic, readonly) bool soccer;
@property (nonatomic) double startTime;
@property (getter=isSwimming, nonatomic, readonly) bool swimming;
@property (getter=isUnderwaterDiving, nonatomic, readonly) bool underwaterDiving;
@property (getter=isWalking, nonatomic, readonly) bool walking;
@property (nonatomic, retain) HDDemoDataGeneratorWorkoutConfiguration *workoutConfiguration;

+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (long long)calendarDay;
- (bool)createdFromNSKeyedUnarchiver;
- (id)currentDate;
- (long long)currentDay;
- (double)currentTime;
- (void)encodeWithCoder:(id)arg1;
- (double)endTime;
- (id)firstSampleDate;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (bool)isCoolingDown;
- (bool)isCoreTraining;
- (bool)isCrossCountrySkiing;
- (bool)isCycling;
- (bool)isDancing;
- (bool)isDoingElliptical;
- (bool)isDoingHIIT;
- (bool)isDoingYoga;
- (bool)isDownhillSnowSports;
- (bool)isExercising;
- (bool)isFinished;
- (bool)isFirstRun;
- (bool)isFunctionalStrengthTraining;
- (bool)isGolf;
- (bool)isHiking;
- (bool)isPaddleSports;
- (bool)isRowing;
- (bool)isRunning;
- (bool)isSkatingSports;
- (bool)isSoccer;
- (bool)isSwimming;
- (bool)isUnderwaterDiving;
- (bool)isWalking;
- (id)lastRunDate;
- (id)lastSampleDate;
- (void)setCurrentDay:(long long)arg1;
- (void)setCurrentTime:(double)arg1;
- (void)setEndTime:(double)arg1;
- (void)setFinished:(bool)arg1;
- (void)setFirstRun:(bool)arg1;
- (void)setFirstSampleDate:(id)arg1;
- (void)setLastRunDate:(id)arg1;
- (void)setLastSampleDate:(id)arg1;
- (void)setStartTime:(double)arg1;
- (void)setWorkoutConfiguration:(id)arg1;
- (double)startTime;
- (id)workoutConfiguration;

@end
