/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/HealthMenstrualCycles.framework/HealthMenstrualCycles
 */

@interface HKMCDaySummary : NSObject <HKMCAdjacentFlowDay, HKRedactedDescription, NSCopying, NSMutableCopying, NSSecureCoding> {
    HKQuantity * _basalBodyTemperature;
    long long  _bleedingAfterPregnancyFlow;
    long long  _bleedingInPregnancyFlow;
    long long  _cervicalMucusQuality;
    long long  _dayIndex;
    NSArray * _endedCycleFactors;
    bool  _intermenstrualBleeding;
    long long  _menstrualFlow;
    long long  _menstrualFlowModificationDayIndex;
    long long  _ovulationTestResult;
    long long  _pregnancyTestResult;
    long long  _progesteroneTestResult;
    NSDictionary * _sampleCountByType;
    long long  _sexualActivity;
    NSNumber * _startOfCycleFromCycleTracking;
    NSArray * _startedCycleFactors;
    unsigned long long  _symptoms;
    HKMCWristTemperature * _wristTemperature;
}

@property (nonatomic, copy) HKQuantity *basalBodyTemperature;
@property (nonatomic) long long bleedingAfterPregnancyFlow;
@property (nonatomic, readonly) long long bleedingAfterPregnancyFlowSampleCount;
@property (nonatomic) long long bleedingInPregnancyFlow;
@property (nonatomic, readonly) long long bleedingInPregnancyFlowSampleCount;
@property (nonatomic) long long cervicalMucusQuality;
@property (nonatomic, readonly) long long cervicalMucusQualitySampleCount;
@property (nonatomic, readonly) long long dayIndex;
@property (nonatomic, readonly, copy) NSArray *endedCycleFactors;
@property (readonly, copy) NSString *hk_redactedDescription;
@property (nonatomic) bool intermenstrualBleeding;
@property (nonatomic, readonly) bool isDataLogged;
@property (nonatomic, readonly) bool isSupplementaryDataLogged;
@property (nonatomic, readonly, copy) NSSet *loggedSupplementaryDisplayTypes;
@property (nonatomic) long long menstrualFlow;
@property (nonatomic, readonly) long long menstrualFlowModificationDayIndex;
@property (nonatomic, readonly) long long menstrualFlowSampleCount;
@property (nonatomic) long long ovulationTestResult;
@property (nonatomic, readonly) long long ovulationTestResultSampleCount;
@property (nonatomic) long long pregnancyTestResult;
@property (nonatomic, readonly) long long pregnancyTestResultSampleCount;
@property (nonatomic) long long progesteroneTestResult;
@property (nonatomic, readonly) long long progesteroneTestResultSampleCount;
@property (nonatomic, readonly, copy) NSDictionary *sampleCountByType;
@property (nonatomic) long long sexualActivity;
@property (nonatomic, readonly) long long sexualActivitySampleCount;
@property (nonatomic, copy) NSNumber *startOfCycleFromCycleTracking;
@property (nonatomic, readonly, copy) NSArray *startedCycleFactors;
@property (nonatomic) unsigned long long symptoms;
@property (nonatomic, readonly, copy) HKMCWristTemperature *wristTemperature;

+ (id)daySummaryWithDayIndex:(long long)arg1 menstrualFlow:(long long)arg2;
+ (id)daySummaryWithDayIndex:(long long)arg1 menstrualFlow:(long long)arg2 bleedingInPregnancyFlow:(long long)arg3 bleedingAfterPregnancyFlow:(long long)arg4 intermenstrualBleeding:(bool)arg5 symptoms:(unsigned long long)arg6 sexualActivity:(long long)arg7 ovulationTestResult:(long long)arg8 pregnancyTestResult:(long long)arg9 progesteroneTestResult:(long long)arg10 cervicalMucusQuality:(long long)arg11 basalBodyTemperature:(id)arg12;
+ (id)daySummaryWithDayIndex:(long long)arg1 menstrualFlow:(long long)arg2 bleedingInPregnancyFlow:(long long)arg3 bleedingAfterPregnancyFlow:(long long)arg4 intermenstrualBleeding:(bool)arg5 symptoms:(unsigned long long)arg6 sexualActivity:(long long)arg7 ovulationTestResult:(long long)arg8 pregnancyTestResult:(long long)arg9 progesteroneTestResult:(long long)arg10 cervicalMucusQuality:(long long)arg11 basalBodyTemperature:(id)arg12 wristTemperature:(id)arg13 menstrualFlowModificationDayIndex:(long long)arg14 startOfCycleFromCycleTracking:(id)arg15 startedCycleFactors:(id)arg16 endedCycleFactors:(id)arg17 sampleCountByType:(id)arg18;
+ (id)daySummaryWithDayIndex:(long long)arg1 ovulationTestResult:(long long)arg2;
+ (id)emptyDaySummaryWithDayIndex:(long long)arg1;
+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)_daySummaryWithOverrideSourceIDBySourceID:(id)arg1 ignoreCycleFactors:(bool)arg2 ignoreOvulationTestResults:(bool)arg3;
- (id)_initFromDaySummary:(id)arg1;
- (id)_initWithDayIndex:(long long)arg1 menstrualFlow:(long long)arg2 bleedingInPregnancyFlow:(long long)arg3 bleedingAfterPregnancyFlow:(long long)arg4 intermenstrualBleeding:(bool)arg5 symptoms:(unsigned long long)arg6 sexualActivity:(long long)arg7 ovulationTestResult:(long long)arg8 pregnancyTestResult:(long long)arg9 progesteroneTestResult:(long long)arg10 cervicalMucusQuality:(long long)arg11 basalBodyTemperature:(id)arg12 wristTemperature:(id)arg13 menstrualFlowModificationDayIndex:(long long)arg14 startOfCycleFromCycleTracking:(id)arg15 startedCycleFactors:(id)arg16 endedCycleFactors:(id)arg17 sampleCountByType:(id)arg18;
- (bool)_isBasalBodyTemperatureLogged;
- (bool)_isBleedingAfterPregnancyFlowLogged;
- (bool)_isBleedingInPregnancyFlowLogged;
- (bool)_isCervicalMucusQualityLogged;
- (bool)_isIntermenstrualBleedingLogged;
- (bool)_isMenstrualFlowLogged;
- (bool)_isOvulationTestResultLogged;
- (bool)_isPregnancyTestResultLogged;
- (bool)_isProgesteroneTestResultLogged;
- (bool)_isSexualActivityLogged;
- (bool)_isSymptomsLogged;
- (id)basalBodyTemperature;
- (long long)bleedingAfterPregnancyFlow;
- (long long)bleedingAfterPregnancyFlowSampleCount;
- (long long)bleedingInPregnancyFlow;
- (long long)bleedingInPregnancyFlowSampleCount;
- (long long)cervicalMucusQuality;
- (long long)cervicalMucusQualitySampleCount;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (long long)dayIndex;
- (id)description;
- (void)encodeWithCoder:(id)arg1;
- (id)endedCycleFactors;
- (bool)hasConfirmedNoFlow;
- (bool)hasFlow;
- (unsigned long long)hash;
- (id)hk_redactedDescription;
- (id)initWithCoder:(id)arg1;
- (bool)intermenstrualBleeding;
- (bool)isDataLogged;
- (bool)isEqual:(id)arg1;
- (bool)isFetched;
- (bool)isSupplementaryDataLogged;
- (bool)isUserEnteredDataEqual:(id)arg1;
- (id)loggedSupplementaryDisplayTypes;
- (long long)menstrualFlow;
- (long long)menstrualFlowModificationDayIndex;
- (long long)menstrualFlowSampleCount;
- (id)mutableCopyWithZone:(struct _NSZone { }*)arg1;
- (long long)ovulationTestResult;
- (long long)ovulationTestResultSampleCount;
- (long long)pregnancyTestResult;
- (long long)pregnancyTestResultSampleCount;
- (long long)progesteroneTestResult;
- (long long)progesteroneTestResultSampleCount;
- (id)sampleCountByType;
- (void)setBasalBodyTemperature:(id)arg1;
- (void)setBleedingAfterPregnancyFlow:(long long)arg1;
- (void)setBleedingInPregnancyFlow:(long long)arg1;
- (void)setCervicalMucusQuality:(long long)arg1;
- (void)setIntermenstrualBleeding:(bool)arg1;
- (void)setMenstrualFlow:(long long)arg1;
- (void)setOvulationTestResult:(long long)arg1;
- (void)setPregnancyTestResult:(long long)arg1;
- (void)setProgesteroneTestResult:(long long)arg1;
- (void)setSexualActivity:(long long)arg1;
- (void)setStartOfCycleFromCycleTracking:(id)arg1;
- (void)setSymptoms:(unsigned long long)arg1;
- (long long)sexualActivity;
- (long long)sexualActivitySampleCount;
- (id)startOfCycleFromCycleTracking;
- (id)startedCycleFactors;
- (unsigned long long)symptoms;
- (id)wristTemperature;

@end
