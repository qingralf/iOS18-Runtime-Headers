/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/HomeKitDaemonLegacy.framework/HomeKitDaemonLegacy
 */

@interface HMDBulletinBoard : HMFObject <HMDCameraBulletinBoard, HMDUserNotificationCenterDelegate, HMFLogging, NSSecureCoding> {
    NSMutableDictionary * _characteristicTuples;
    <HMDDoorbellBulletinUtilities> * _doorbellBulletinUtilities;
    <HMDFileManager> * _fileManager;
    HMDHomeManager * _homeManager;
    NAFuture * _isConfiguredFuture;
    <HMMLogEventSubmitting> * _logEventSubmitter;
    NSSet * _notificationCategories;
    HMDUserNotificationCenter * _notificationCenter;
    NSMutableDictionary * _notificationRequests;
    Class  _persistentStoreClass;
    NSObject<OS_dispatch_queue> * _workQueue;
}

@property (readonly) long long bulletinNotificationsGroupTimeInterval;
@property (readonly) NSMutableDictionary *characteristicTuples;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (retain) <HMDDoorbellBulletinUtilities> *doorbellBulletinUtilities;
@property (readonly) <HMDFileManager> *fileManager;
@property (readonly) unsigned long long hash;
@property (nonatomic) HMDHomeManager *homeManager;
@property (readonly) NAFuture *isConfiguredFuture;
@property (retain) <HMMLogEventSubmitting> *logEventSubmitter;
@property (readonly, copy) NSSet *notificationCategories;
@property (readonly) HMDUserNotificationCenter *notificationCenter;
@property (nonatomic, readonly) <HMDUserNotificationCenterSettingsProviding> *notificationCenterSettingsProvider;
@property (retain) NSMutableDictionary *notificationRequests;
@property (retain) Class persistentStoreClass;
@property (readonly) Class superclass;
@property (readonly) NSObject<OS_dispatch_queue> *workQueue;

+ (id)_supportedNonSecureServices;
+ (id)_supportedSecureServices;
+ (id)attachmentsWithFileURL:(id)arg1;
+ (id)bulletinSupportedCharacteristicsForService:(id)arg1;
+ (id)characteristicTupleKeyFromServiceContextID:(id)arg1 currentType:(id)arg2;
+ (id)createImageAttachmentByHardLinkingFile:(id)arg1 fileManager:(id)arg2;
+ (unsigned long long)interruptionLevelForChangedCharacteristic:(id)arg1;
+ (bool)isBulletinSupportedForCharacteristicType:(id)arg1 serviceType:(id)arg2;
+ (bool)isBulletinSupportedForNonSecureCharacteristicType:(id)arg1 serviceType:(id)arg2;
+ (bool)isCriticalNonSecureServiceType:(id)arg1;
+ (id)logCategory;
+ (id)notificationCategories;
+ (bool)presentationValueOfCharacteristic:(id)arg1 equalTo:(id)arg2;
+ (id)sharedBulletinBoard;
+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)_copyItemAtURL:(id)arg1 toDirectory:(id)arg2;
- (id)_doorbellPressNotificationsNearDate:(id)arg1 forCameraProfile:(id)arg2;
- (void)_handleDidReceiveNotificationResponse:(id)arg1 completionHandler:(id /* block */)arg2;
- (bool)_hasDuplicateBulletinForCharacteristic:(id)arg1;
- (bool)_hasDuplicateBulletinForSnapshotCharacteristic:(id)arg1;
- (void)_insertImageBulletinsForChangedCharacteristics:(id)arg1 snapshotData:(id)arg2 completion:(id /* block */)arg3;
- (id)_insertRequestWithTitle:(id)arg1 snapshotData:(id)arg2 message:(id)arg3 requestIdentifier:(id)arg4 date:(id)arg5 bulletinType:(unsigned long long)arg6 actionURL:(id)arg7 bulletinContext:(id)arg8 actionContext:(id)arg9 interruptionLevel:(unsigned long long)arg10 logEventTopic:(long long)arg11;
- (bool)_shouldPostBulletinOnCurrentValueChangeForCharacteristic:(id)arg1 includeChangeFromNil:(bool)arg2;
- (bool)_shouldSkipBulletinForChangedCharacteristic:(id)arg1;
- (void)_submitCameraClipSignificantEventDetailsFromBulletin:(id)arg1;
- (void)_submitDoorbellEventDetailsFromBulletinWithDate:(id)arg1 significantEvents:(id)arg2;
- (void)_updateCharacteristicTupleFor:(id)arg1 withCurrentType:(id)arg2 changedByThisDevice:(bool)arg3;
- (bool)_updateDoorbellPressNotificationsWithSignificantEventBulletin:(id)arg1;
- (void)_updateDoorbellUserInfo:(id)arg1 withSignificantEventUserInfo:(id)arg2;
- (void)addNotificationRequest:(id)arg1 forIdentifier:(id)arg2;
- (void)archive;
- (long long)bulletinNotificationsGroupTimeInterval;
- (id)characteristicTuples;
- (void)configureWithHomeManager:(id)arg1;
- (void)cullBulletinsToCount:(unsigned long long)arg1;
- (id)doorbellBulletinUtilities;
- (void)encodeWithCoder:(id)arg1;
- (void)fetchAreUserNotificationsEnabledWithCompletion:(id /* block */)arg1;
- (id)fileManager;
- (id)homeManager;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (id)initWithNotificationCenter:(id)arg1 fileManager:(id)arg2 workQueue:(id)arg3 doorbellBulletinUtilities:(id)arg4 logEventSubmitter:(id)arg5;
- (void)insertAccessCodeAddedBulletinForHome:(id)arg1;
- (void)insertAccessCodeChangedBulletinForHome:(id)arg1;
- (void)insertAccessCodeRemovedBulletinForHome:(id)arg1;
- (void)insertBulletinForChangedSecureClassCharacteristic:(id)arg1;
- (id)insertBulletinForIncomingInvitation:(id)arg1;
- (void)insertBulletinForIncompatibleInvitationFromInviterName:(id)arg1 homeName:(id)arg2 ownedHomeIdentifier:(id)arg3;
- (id)insertBulletinForSecureTrigger:(id)arg1;
- (void)insertBulletinsForChangedCharacteristics:(id)arg1 modifiedCharacteristics:(id)arg2 changedByThisDevice:(bool)arg3 changeNotificationFromPrimary:(bool)arg4 completion:(id /* block */)arg5;
- (void)insertBulletinsForMatterFirmwareUpdateInstalledInHome:(id)arg1;
- (void)insertCameraAccessModeChangedBulletin:(id)arg1;
- (void)insertCameraClipSignificantEventBulletin:(id)arg1;
- (void)insertConnectedCHIPEcosystemsChangedBulletin:(id)arg1;
- (void)insertHomeHubReachabilityBulletinForHome:(id)arg1 reachable:(bool)arg2 hasMultipleResidents:(bool)arg3;
- (void)insertImageBulletinsForChangedCharacteristics:(id)arg1 snapshotData:(id)arg2 completion:(id /* block */)arg3;
- (void)insertLockOnboardingBulletinForHome:(id)arg1 serviceType:(id)arg2;
- (void)insertNewRMVLanguageBulletinForHome:(id)arg1 language:(id)arg2;
- (void)insertReachabilityEventBulletinForAccessory:(id)arg1 reachable:(bool)arg2 date:(id)arg3;
- (void)insertWalletKeyExpressModeSetUpBulletinForHome:(id)arg1;
- (void)insertWalletKeySupportAddedBulletinForAccessory:(id)arg1;
- (id)isConfiguredFuture;
- (id)logEventSubmitter;
- (id)messageForChangedCharacteristic:(id)arg1 withSignificantEvents:(id)arg2;
- (id)messageForGarageDoorCharacteristic:(id)arg1 personName:(id)arg2;
- (id)messageForLockCharacteristic:(id)arg1 personName:(id)arg2;
- (id)messageForSecuritySystemCharacteristic:(id)arg1 personName:(id)arg2;
- (id)messageForWindowCharacteristic:(id)arg1 personName:(id)arg2;
- (id)notificationCategories;
- (id)notificationCenter;
- (void)notificationCenter:(id)arg1 didReceiveNotificationResponse:(id)arg2 withCompletionHandler:(id /* block */)arg3;
- (id)notificationCenterSettingsProvider;
- (id)notificationRequests;
- (id)notificationRequestsForCameraClipUUIDs:(id)arg1;
- (id)notificationRequestsSortedByDate;
- (Class)persistentStoreClass;
- (void)refreshHomeBadgeNumber;
- (void)removeAllBulletins;
- (void)removeBulletinWithRecordID:(id)arg1;
- (void)removeBulletinsForAccessory:(id)arg1;
- (void)removeBulletinsForHome:(id)arg1;
- (void)removeBulletinsForService:(id)arg1;
- (void)removeBulletinsForTrigger:(id)arg1;
- (void)removeBulletinsUsingPredicate:(id)arg1;
- (void)removeCameraClipBulletinsForCameraProfile:(id)arg1;
- (void)removeImageFilesForNotificationRequests:(id)arg1;
- (void)removeNotificationRequestsWithIdentifiers:(id)arg1 shouldDeleteAttachments:(bool)arg2;
- (id)removeRedundantSignificantEventNotificationsForSignificantEvents:(id)arg1;
- (void)removeWalletKeyOnboardingBulletinForHome:(id)arg1;
- (void)resolveDisplayNameForChangedCharacteristic:(id)arg1 completion:(id /* block */)arg2;
- (void)setDoorbellBulletinUtilities:(id)arg1;
- (void)setHomeManager:(id)arg1;
- (void)setLogEventSubmitter:(id)arg1;
- (void)setNotificationRequests:(id)arg1;
- (void)setPersistentStoreClass:(Class)arg1;
- (id)threadIdentifierForNotificationGroupWithHomeContextID:(id)arg1;
- (id)titleNameForChangedCharacteristic:(id)arg1;
- (id)updateBulletinForFirmwareUpdateInHome:(id)arg1;
- (void)updateContent:(id)arg1 forNotificationWithRequestIdentifier:(id)arg2;
- (void)updateDoorbellPressNotificationsWithSignificantEventBulletin:(id)arg1 completion:(id /* block */)arg2;
- (void)updateMessageForDoorbellPressNotificationRequestWithIdentifier:(id)arg1 cameraProfile:(id)arg2;
- (id)workQueue;

@end
