/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/IntelligencePlatform.framework/IntelligencePlatform
 */

@interface GDGraphWalkingActivityEventEntityIdentifier : GDEntityIdentifier <GDGraphEntityIdentifier>

@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (readonly) unsigned long long hash;
@property (readonly) Class superclass;

- (id)description;
- (id)graphWalkingActivityEventFromContext:(id)arg1 error:(id*)arg2;
- (id)initByCastingFrom:(id)arg1;
- (id)initFromGDEntityIdentifier:(id)arg1;
- (id)initWithEntityIdentifier:(id)arg1;

@end
