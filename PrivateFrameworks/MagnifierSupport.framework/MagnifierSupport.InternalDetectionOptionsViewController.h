/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/MagnifierSupport.framework/MagnifierSupport
 */

@interface MagnifierSupport.InternalDetectionOptionsViewController : UIViewController <UITableViewDataSource, UITableViewDelegate> {
    void availableInternalDetectionSections;
    void detectionModeAutomationReplayURL;
    void freezeFrameAutomationKey;
    void recordedFileArray;
    void showPhotoSelector;
    void showVideoSelector;
    void tableView;
}

- (void).cxx_destruct;
- (bool)_canShowWhileLocked;
- (void)didTapPhotoSelector:(id)arg1;
- (void)didToggleAutomationSwitch:(id)arg1;
- (void)didToggleFreezeFrameSwitch:(id)arg1;
- (void)didToggleRecordingSwitch:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2;
- (long long)numberOfSectionsInTableView:(id)arg1;
- (id)tableView:(id)arg1 cellForRowAtIndexPath:(id)arg2;
- (void)tableView:(id)arg1 didSelectRowAtIndexPath:(id)arg2;
- (long long)tableView:(id)arg1 numberOfRowsInSection:(long long)arg2;
- (id)tableView:(id)arg1 titleForHeaderInSection:(long long)arg2;
- (void)viewDidLoad;
- (void)viewWillAppear:(bool)arg1;

@end
