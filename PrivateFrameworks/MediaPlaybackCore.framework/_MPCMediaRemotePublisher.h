/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/MediaPlaybackCore.framework/MediaPlaybackCore
 */

@interface _MPCMediaRemotePublisher : NSObject <MPCPlaybackAccountManagerObserver, MPCPlaybackEngineEventObserving, MPCQueueControllerPublisher, MPNowPlayingPlaybackQueueDataSourcePrivate, MPRemoteCommandCenterDelegate> {
    NSArray * _accounts;
    bool  _activeAccountAllowsSubscriptionPlayback;
    bool  _activeAccountRequiresAuthorizationTokensForPlayback;
    NSString * _activeAccountStoreFrontIdentifier;
    MPRemoteCommandCenter * _commandCenter;
    bool  _commandUpdateScheduled;
    long long  _deferredTrackChangeDelta;
    bool  _engineRestoringState;
    bool  _hasBeganFastForward;
    bool  _hasBeganRewind;
    NSString * _hostingSharedSessionID;
    MPNowPlayingInfoCenter * _infoCenter;
    bool  _initializedSupportedCommands;
    NSArray * _lastCommandDescriptions;
    NSDictionary * _lastPerformanceMetrics;
    MPLibraryAddStatusObserver * _libraryAddStatusObserver;
    struct os_unfair_lock_s { 
        unsigned int _os_unfair_lock_opaque; 
    }  _libraryAddStatusObserverLock;
    bool  _mediaServerAvailable;
    NSObject<OS_dispatch_source> * _nextPreviousTrackCooldownTimer;
    MPCPlaybackEngine * _playbackEngine;
    MSVBlockGuard * _setPlaybackQueueExtendedTimeGuard;
    _MPCMediaRemotePublisher_Swift * _swift;
}

@property (nonatomic, readonly) MPRemoteCommandCenter *commandCenter;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (getter=isEngineRestoringState, nonatomic) bool engineRestoringState;
@property (readonly) unsigned long long hash;
@property (nonatomic, readonly, copy) NSString *hostingSharedSessionID;
@property (nonatomic, readonly) MPNowPlayingInfoCenter *infoCenter;
@property (getter=hasInitializedSupportedCommands, nonatomic) bool initializedSupportedCommands;
@property (nonatomic, retain) NSDictionary *lastPerformanceMetrics;
@property (getter=isMediaServerAvailable, nonatomic) bool mediaServerAvailable;
@property (nonatomic, readonly) MPCPlaybackEngine *playbackEngine;
@property (readonly) Class superclass;
@property (nonatomic, readonly) _MPCMediaRemotePublisher_Swift *swift;

- (void).cxx_destruct;
- (void)__updateSupportedCommands;
- (void)_activeSharedSessionDidChange:(id)arg1;
- (void)_beginBackgroundTaskUntilPlayCommandWithCommand:(id)arg1 timeout:(double)arg2;
- (void)_commandEventDidTimeoutNotification:(id)arg1;
- (void)_durationAvailableNotification:(id)arg1;
- (void)_enqueueFallbackIntentIfNeededForCommandEvent:(id)arg1 play:(bool)arg2 completion:(id /* block */)arg3;
- (id)_exportableSessionTypes;
- (void)_leaveSharedSessionWithCommandID:(id)arg1 completion:(id /* block */)arg2;
- (void)_likedStateChangedNotification:(id)arg1;
- (void)_performCommandEvent:(id)arg1 completion:(id /* block */)arg2;
- (void)_performDebugEvent:(id)arg1 completion:(id /* block */)arg2;
- (void)_performSkipChapterCommand:(unsigned int)arg1 withCommandID:(id)arg2 completion:(id /* block */)arg3;
- (void)_podcastsSkipIntervalDidChange:(id)arg1;
- (id)_supportedSessionTypes;
- (void)_updateLaunchCommands;
- (void)_updateSupportedCommands;
- (void)accountManager:(id)arg1 didChangeAccounts:(id)arg2;
- (void)becomeActive;
- (void)becomeActiveIfNeededWithCommandID:(id)arg1 completion:(id /* block */)arg2;
- (id)commandCenter;
- (void)commandCenter:(id)arg1 didTimeoutCommandEvent:(id)arg2 statusHandler:(id /* block */)arg3;
- (void)deactivateAudioSessionIfIdle:(long long)arg1;
- (void)dealloc;
- (void)engine:(id)arg1 didAchieveLikelyToKeepUpWithItem:(id)arg2;
- (void)engine:(id)arg1 didChangeActionAtQueueEnd:(long long)arg2;
- (void)engine:(id)arg1 didChangeCurrentItemVariantID:(id)arg2;
- (void)engine:(id)arg1 didChangeQueueController:(id)arg2;
- (void)engine:(id)arg1 didChangeQueueWithReason:(id)arg2;
- (void)engine:(id)arg1 didChangeRepeatType:(long long)arg2;
- (void)engine:(id)arg1 didChangeShuffleType:(long long)arg2;
- (void)engine:(id)arg1 didChangeToItem:(id)arg2;
- (void)engine:(id)arg1 didChangeToState:(unsigned long long)arg2;
- (void)engine:(id)arg1 didReceivePickedRouteChange:(id)arg2;
- (void)engineDidBlockVocalAttenuation:(id)arg1;
- (void)engineDidChangeVocalAttenuationLevel:(id)arg1;
- (void)engineDidChangeVocalAttenuationState:(id)arg1;
- (void)engineDidEndSleepTimer:(id)arg1;
- (void)engineDidEndStateRestoration:(id)arg1;
- (void)engineDidLoseMediaServices:(id)arg1;
- (void)engineDidResetMediaServices:(id)arg1;
- (void)engineDidUnblockVocalAttenuation:(id)arg1;
- (void)engineWillBeginStateRestoration:(id)arg1;
- (id)getQOSDialogForError:(id)arg1 withItem:(id)arg2;
- (void)getShouldRestoreStateWithCompletion:(id /* block */)arg1;
- (bool)hasInitializedSupportedCommands;
- (id)hostingSharedSessionID;
- (id)infoCenter;
- (id)initWithPlaybackEngine:(id)arg1;
- (void)invalidateQueueTypesWithReason:(id)arg1;
- (void)invalidateSessionTypesWithReason:(id)arg1;
- (bool)isEngineRestoringState;
- (bool)isMediaServerAvailable;
- (id)lastPerformanceMetrics;
- (void)leaveSharedSessionWithCommandID:(id)arg1 remoteControlInterface:(id)arg2 completion:(id /* block */)arg3;
- (id)nowPlayingInfoCenter:(id)arg1 artworkCatalogForContentItem:(id)arg2;
- (id)nowPlayingInfoCenter:(id)arg1 artworkForContentItem:(id)arg2 size:(struct CGSize { double x1; double x2; })arg3 completion:(id /* block */)arg4;
- (id)nowPlayingInfoCenter:(id)arg1 childContentItemIDAtIndex:(long long)arg2 ofItem:(id)arg3;
- (id)nowPlayingInfoCenter:(id)arg1 contentItemForID:(id)arg2;
- (id)nowPlayingInfoCenter:(id)arg1 contentItemIDForOffset:(long long)arg2;
- (id)nowPlayingInfoCenter:(id)arg1 contentItemIDsFromOffset:(long long)arg2 toOffset:(long long)arg3 nowPlayingIndex:(long long*)arg4;
- (void)nowPlayingInfoCenter:(id)arg1 didBeginLyricsEvent:(id)arg2;
- (void)nowPlayingInfoCenter:(id)arg1 didEndLyricsEvent:(id)arg2;
- (void)nowPlayingInfoCenter:(id)arg1 didEndMigrationWithIdentifier:(id)arg2 error:(id)arg3;
- (void)nowPlayingInfoCenter:(id)arg1 getTransportablePlaybackSessionRepresentationForRequest:(id)arg2 completion:(id /* block */)arg3;
- (id)nowPlayingInfoCenter:(id)arg1 infoForContentItem:(id)arg2 completion:(id /* block */)arg3;
- (id)nowPlayingInfoCenter:(id)arg1 lyricsForContentItem:(id)arg2 completion:(id /* block */)arg3;
- (id)nowPlayingInfoCenter:(id)arg1 remoteArtworkForContentItem:(id)arg2 format:(id)arg3 size:(struct CGSize { double x1; double x2; })arg4 completion:(id /* block */)arg5;
- (id)nowPlayingInfoCenter:(id)arg1 transcriptAlignmentsForContentItem:(id)arg2 completion:(id /* block */)arg3;
- (void)nowPlayingInfoCenter:(id)arg1 willBeginSessionMigrationWithIdentifier:(id)arg2;
- (void)performSetQueue:(id)arg1 completion:(id /* block */)arg2;
- (void)performSetQueueWithIntent:(id)arg1 completion:(id /* block */)arg2;
- (id)playbackEngine;
- (id)playbackQueueAuxilaryNowPlayingInfoForNowPlayingInfoCenter:(id)arg1;
- (id)playbackQueueIdentifierForNowPlayingInfoCenter:(id)arg1;
- (id)playbackQueuePropertiesForNowPlayingInfoCenter:(id)arg1;
- (void)publishIfNeeded;
- (void)reportUserBackgroundedApplication;
- (void)setEngineRestoringState:(bool)arg1;
- (void)setInitializedSupportedCommands:(bool)arg1;
- (void)setLastPerformanceMetrics:(id)arg1;
- (void)setMediaServerAvailable:(bool)arg1;
- (id)swift;
- (void)updatePlaybackMetrics:(id)arg1;
- (unsigned long long)vocalAttenuationUnavailabilityReasons;

@end
