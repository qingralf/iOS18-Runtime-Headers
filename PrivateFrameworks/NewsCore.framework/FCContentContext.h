/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/NewsCore.framework/NewsCore
 */

@interface FCContentContext : NSObject <FCContentContext, FCCoreConfigurationObserving, FCNetworkReachabilityRequirementObserving> {
    FCArticleController * _articleController;
    NSURL * _assetCacheDirectoryURL;
    <FCAssetKeyCacheType> * _assetKeyCache;
    <FCAssetKeyManagerType> * _assetKeyManager;
    <FCAssetKeyServiceType> * _assetKeyService;
    FCAssetManager * _assetManager;
    <FCAVAssetCacheType> * _avAssetCache;
    FCAVAssetDownloadManager * _avAssetDownloadManager;
    <FCAVAssetFactoryType> * _avAssetFactory;
    <FCAVAssetKeyCacheType> * _avAssetKeyCache;
    <FCAVAssetKeyManagerType> * _avAssetKeyManager;
    <FCAVAssetKeyServiceType> * _avAssetKeyService;
    <FCAVAssetPrewarming> * _avAssetPrewarmer;
    <FCAVAssetResourceLoaderType> * _avAssetResourceLoader;
    <FCBackgroundTaskable> * _backgroundTaskable;
    <FCCoreConfigurationManager> * _configurationManager;
    NSString * _contentDirectory;
    NSURL * _contentHostDirectoryURL;
    FCContextConfiguration * _contextConfiguration;
    FCFlintResourceManager * _flintResourceManager;
    FCContentContextInternal * _internalContentContext;
    <FCNetworkReachabilityType> * _networkReachability;
    <FCPPTContext> * _pptContext;
    FCPuzzleController * _puzzleController;
    FCPuzzleTypeController * _puzzleTypeController;
    FCSportsEventController * _sportsEventController;
    <FCNetworkReachabilityRequirement> * _supportedCountryNetworkReachabilityRequirement;
    NSString * _tabiModelsContentDirectory;
    NSString * _tabiModelsContentDirectoryLegacy;
    NSString * _tabiRequestsContentDirectory;
    NSString * _tabiResourcesContentDirectory;
    NSString * _tabiResourcesContentDirectoryLegacy;
    FCTagController * _tagController;
}

@property (nonatomic, readonly) <FCNewsAppConfigurationManager> *appConfigurationManager;
@property (nonatomic, readonly) FCArticleController *articleController;
@property (nonatomic, retain) NSURL *assetCacheDirectoryURL;
@property (nonatomic, retain) <FCAssetKeyCacheType> *assetKeyCache;
@property (nonatomic, retain) <FCAssetKeyManagerType> *assetKeyManager;
@property (nonatomic, retain) <FCAssetKeyServiceType> *assetKeyService;
@property (nonatomic, readonly) FCAssetManager *assetManager;
@property (nonatomic, retain) <FCAVAssetCacheType> *avAssetCache;
@property (nonatomic, retain) FCAVAssetDownloadManager *avAssetDownloadManager;
@property (nonatomic, retain) <FCAVAssetFactoryType> *avAssetFactory;
@property (nonatomic, retain) <FCAVAssetKeyCacheType> *avAssetKeyCache;
@property (nonatomic, retain) <FCAVAssetKeyManagerType> *avAssetKeyManager;
@property (nonatomic, retain) <FCAVAssetKeyServiceType> *avAssetKeyService;
@property (nonatomic, readonly) <FCAVAssetPrewarming> *avAssetPrewarmer;
@property (nonatomic, retain) <FCAVAssetResourceLoaderType> *avAssetResourceLoader;
@property (nonatomic, readonly) <FCBackgroundTaskable> *backgroundTaskable;
@property (nonatomic, readonly) <FCCoreConfigurationManager> *configurationManager;
@property (nonatomic, copy) NSString *contentDirectory;
@property (nonatomic, readonly, copy) NSString *contentEnvironment;
@property (nonatomic, readonly, copy) NSString *contentEnvironmentToken;
@property (nonatomic, retain) NSURL *contentHostDirectoryURL;
@property (nonatomic, readonly, copy) NSString *contentStoreFrontID;
@property (nonatomic, copy) FCContextConfiguration *contextConfiguration;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (nonatomic, readonly) <FCFeedDatabaseProtocol> *feedDatabase;
@property (nonatomic, readonly) FCFlintResourceManager *flintResourceManager;
@property (readonly) unsigned long long hash;
@property (nonatomic, retain) FCContentContextInternal *internalContentContext;
@property (nonatomic, readonly) <FCNetworkReachabilityType> *networkReachability;
@property (nonatomic, readonly) <FCPPTContext> *pptContext;
@property (nonatomic, readonly) FCPuzzleController *puzzleController;
@property (nonatomic, readonly) FCPuzzleTypeController *puzzleTypeController;
@property (nonatomic, readonly) FCSportsEventController *sportsEventController;
@property (nonatomic, readonly) long long storageSize;
@property (readonly) Class superclass;
@property (nonatomic, readonly, copy) NSString *supportedContentStoreFrontID;
@property (nonatomic, retain) <FCNetworkReachabilityRequirement> *supportedCountryNetworkReachabilityRequirement;
@property (nonatomic, copy) NSString *tabiModelsContentDirectory;
@property (nonatomic, copy) NSString *tabiModelsContentDirectoryLegacy;
@property (nonatomic, copy) NSString *tabiRequestsContentDirectory;
@property (nonatomic, copy) NSString *tabiResourcesContentDirectory;
@property (nonatomic, copy) NSString *tabiResourcesContentDirectoryLegacy;
@property (nonatomic, readonly) FCTagController *tagController;

// Image: /System/Library/PrivateFrameworks/NewsCore.framework/NewsCore

+ (void)initialize;

- (void).cxx_destruct;
- (void)_updateReachabilityGivenRequirements;
- (id)appConfigurationManager;
- (id)articleController;
- (id)assetCacheDirectoryURL;
- (id)assetKeyCache;
- (id)assetKeyManager;
- (id)assetKeyService;
- (id)assetManager;
- (id)avAssetCache;
- (id)avAssetDownloadManager;
- (id)avAssetFactory;
- (id)avAssetKeyCache;
- (id)avAssetKeyManager;
- (id)avAssetKeyService;
- (id)avAssetPrewarmer;
- (id)avAssetResourceLoader;
- (id)backgroundTaskable;
- (id)configurationManager;
- (void)configurationManager:(id)arg1 configurationDidChange:(id)arg2;
- (void)configurationManagerScienceExperimentFieldsDidChange:(id)arg1;
- (id)contentDirectory;
- (id)contentEnvironment;
- (id)contentEnvironmentToken;
- (id)contentHostDirectoryURL;
- (id)contentStoreFrontID;
- (id)contextConfiguration;
- (id)convertRecords:(id)arg1;
- (void)dealloc;
- (void)enableFlushingWithFlushingThreshold:(unsigned long long)arg1;
- (void)enableFlushingWithFlushingThreshold:(unsigned long long)arg1 exceptForFlusher:(id)arg2;
- (id)feedDatabase;
- (id)flintResourceManager;
- (id)initWithConfiguration:(id)arg1 configurationManager:(id)arg2 contentDatabase:(id)arg3 contentHostDirectory:(id)arg4 networkBehaviorMonitor:(id)arg5 networkReachability:(id)arg6 setupCustomURLProtocols:(bool)arg7 desiredHeadlineFieldOptions:(unsigned long long)arg8 feedUsage:(long long)arg9 assetKeyManagerDelegate:(id)arg10 appActivityMonitor:(id)arg11 backgroundTaskable:(id)arg12 pptContext:(id)arg13;
- (id)initWithConfiguration:(id)arg1 configurationManager:(id)arg2 contentHostDirectory:(id)arg3 networkBehaviorMonitor:(id)arg4 desiredHeadlineFieldOptions:(unsigned long long)arg5 feedUsage:(long long)arg6 appActivityMonitor:(id)arg7 backgroundTaskable:(id)arg8 pptContext:(id)arg9;
- (id)initWithConfiguration:(id)arg1 configurationManager:(id)arg2 contentHostDirectory:(id)arg3 networkBehaviorMonitor:(id)arg4 networkReachability:(id)arg5 desiredHeadlineFieldOptions:(unsigned long long)arg6 feedUsage:(long long)arg7 assetKeyManagerDelegate:(id)arg8 appActivityMonitor:(id)arg9 backgroundTaskable:(id)arg10 pptContext:(id)arg11;
- (id)interestTokenForContentManifest:(id)arg1;
- (id)internalContentContext;
- (id)magazinesConfigurationManager;
- (id)networkReachability;
- (void)networkReachabilityRequirementDidBecomeDirty:(id)arg1;
- (id)news_core_ConfigurationManager;
- (id)pptContext;
- (void)ppt_overrideFeedEndpoint:(long long)arg1;
- (void)ppt_prewarmFeedDatabase;
- (void)prewarmStores;
- (id)puzzleController;
- (id)puzzleTypeController;
- (id)recordSourceWithSchema:(id)arg1;
- (id)recordTreeSourceWithRecordSources:(id)arg1;
- (void)save;
- (void)setAssetCacheDirectoryURL:(id)arg1;
- (void)setAssetKeyCache:(id)arg1;
- (void)setAssetKeyManager:(id)arg1;
- (void)setAssetKeyService:(id)arg1;
- (void)setAvAssetCache:(id)arg1;
- (void)setAvAssetDownloadManager:(id)arg1;
- (void)setAvAssetFactory:(id)arg1;
- (void)setAvAssetKeyCache:(id)arg1;
- (void)setAvAssetKeyManager:(id)arg1;
- (void)setAvAssetKeyService:(id)arg1;
- (void)setAvAssetResourceLoader:(id)arg1;
- (void)setContentDirectory:(id)arg1;
- (void)setContentHostDirectoryURL:(id)arg1;
- (void)setContextConfiguration:(id)arg1;
- (void)setInternalContentContext:(id)arg1;
- (void)setSupportedCountryNetworkReachabilityRequirement:(id)arg1;
- (void)setTabiModelsContentDirectory:(id)arg1;
- (void)setTabiModelsContentDirectoryLegacy:(id)arg1;
- (void)setTabiRequestsContentDirectory:(id)arg1;
- (void)setTabiResourcesContentDirectory:(id)arg1;
- (void)setTabiResourcesContentDirectoryLegacy:(id)arg1;
- (id)sportsEventController;
- (long long)storageSize;
- (id)supportedContentStoreFrontID;
- (id)supportedCountryNetworkReachabilityRequirement;
- (id)tabiModelsContentDirectory;
- (id)tabiModelsContentDirectoryLegacy;
- (id)tabiRequestsContentDirectory;
- (id)tabiResourcesContentDirectory;
- (id)tabiResourcesContentDirectoryLegacy;
- (id)tagController;

// Image: /System/Library/PrivateFrameworks/NewsToday.framework/NewsToday

+ (id)nt_contentContextWithConfigurationManager:(id)arg1 contentHostDirectory:(id)arg2 networkBehaviorMonitor:(id)arg3;

@end
