/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/PassKitCore.framework/PassKitCore
 */

@interface PKEntitlementWhitelist : NSObject {
    bool  _AMPCardEnrollment;
    bool  _appleMediaServicesPrivate;
    bool  _applePayTrustAllAccess;
    NSString * _applicationID;
    bool  _badgeCountItemsAccess;
    bool  _carKeyCredentialProvisioning;
    bool  _carKeyPassThroughAccess;
    bool  _cardOnFilePayments;
    bool  _credentialStateAccess;
    NSString * _developerTeamID;
    bool  _externalizedContextPayments;
    bool  _featuresAllAccess;
    bool  _generateSecurePassData;
    bool  _inAppPayments;
    bool  _inAppPaymentsPrivate;
    NSArray * _merchantIdentifiers;
    bool  _passPresentationSuppression;
    bool  _passPresentationSuppressionFromBackground;
    NSArray * _passTypeIDPrefixes;
    NSArray * _passTypeIDs;
    bool  _passesAddSilently;
    bool  _passesAddUnsigned;
    bool  _passesAllAccess;
    bool  _passesEntangledAccess;
    bool  _passesOverviewAccess;
    bool  _payLaterPayInFourMerchandising;
    bool  _paymentAllAccess;
    bool  _paymentConfiguration;
    bool  _paymentPassProvisioning;
    bool  _paymentPresentation;
    bool  _peerPaymentAllAccess;
    int  _processIdentifier;
    bool  _remoteNetworkPaymentInitiate;
    bool  _searchAllAccess;
    bool  _secureElementPassProvisioning;
    bool  _secureElementPassProvisioningForMAIDs;
    bool  _shareableCredentialProvisioning;
    bool  _sharingAllAccess;
    NSArray * _teamIDs;
    bool  _trustedDeviceEnrollmentInfo;
    bool  _webPayments;
    bool  _winterpegPayments;
}

@property (readonly) bool AMPCardEnrollment;
@property (readonly) bool allAccess;
@property (readonly) bool appleMediaServicesPrivate;
@property (readonly) bool applePayTrustAllAccess;
@property (readonly, copy) NSString *applicationID;
@property (readonly) bool badgeCountItemsAccess;
@property (readonly) bool carKeyCredentialProvisioning;
@property (readonly) bool carKeyPassThroughAccess;
@property (readonly) bool cardOnFilePayments;
@property (readonly) bool credentialStateAccess;
@property (readonly, copy) NSString *developerTeamID;
@property (readonly) bool externalizedContextPayments;
@property (readonly) bool featuresAllAccess;
@property (readonly) bool generateSecurePassData;
@property (readonly) bool inAppPayments;
@property (readonly) bool inAppPaymentsPrivate;
@property (readonly, copy) NSArray *merchantIdentifiers;
@property (readonly) bool passPresentationSuppression;
@property (readonly) bool passPresentationSuppressionFromBackground;
@property (readonly, copy) NSArray *passTypeIDPrefixes;
@property (readonly, copy) NSArray *passTypeIDs;
@property (readonly) bool passesAddSilently;
@property (readonly) bool passesAddUnsigned;
@property (readonly) bool passesAllAccess;
@property (readonly) bool passesEntangledAccess;
@property (readonly) bool passesOverviewAccess;
@property (readonly) bool payLaterPayInFourMerchandising;
@property (readonly) bool paymentAllAccess;
@property (readonly) bool paymentConfiguration;
@property (readonly) bool paymentPassProvisioning;
@property (readonly) bool paymentPresentation;
@property (readonly) bool peerPaymentAllAccess;
@property (readonly) bool remoteNetworkPaymentInitiate;
@property (readonly) bool searchAllAccess;
@property (readonly) bool secureElementPassProvisioning;
@property (readonly) bool secureElementPassProvisioningForMAIDs;
@property (readonly) bool shareableCredentialProvisioning;
@property (readonly) bool sharingAllAccess;
@property (readonly) bool supportsWebPayments;
@property (readonly, copy) NSArray *teamIDs;
@property (readonly) bool trustedDeviceEnrollmentInfo;
@property (readonly) bool webPayments;
@property (readonly) bool winterpegPayments;

+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (bool)AMPCardEnrollment;
- (id)_arrayValueOfEntitlement:(id)arg1 fromSecTask:(struct __SecTask { }*)arg2;
- (bool)_boolValueOfEntitlement:(id)arg1 fromSecTask:(struct __SecTask { }*)arg2;
- (void*)_copyValueOfEntitlement:(id)arg1 fromSecTask:(struct __SecTask { }*)arg2;
- (bool)_probeEntitlementsWithAuditToken:(struct { unsigned int x1[8]; })arg1;
- (id)_stringValueOfEntitlement:(id)arg1 fromSecTask:(struct __SecTask { }*)arg2;
- (bool)allAccess;
- (bool)appleMediaServicesPrivate;
- (bool)applePayTrustAllAccess;
- (id)applicationID;
- (bool)badgeCountItemsAccess;
- (bool)carKeyCredentialProvisioning;
- (bool)carKeyPassThroughAccess;
- (bool)cardOnFilePayments;
- (bool)credentialStateAccess;
- (id)developerTeamID;
- (bool)entitledToPerformPassAction:(unsigned long long)arg1 pass:(id)arg2;
- (bool)entitledToPerformPassAction:(unsigned long long)arg1 passTypeID:(id)arg2 teamID:(id)arg3 associatedPassTypeIdentifiers:(id)arg4 associatedApplicationIdentifiers:(id)arg5;
- (bool)externalizedContextPayments;
- (bool)featuresAllAccess;
- (bool)generateSecurePassData;
- (bool)inAppPayments;
- (bool)inAppPaymentsPrivate;
- (id)init;
- (id)initWithConnection:(id)arg1;
- (id)initWithProcessIdentifier:(int)arg1 auditToken:(struct { unsigned int x1[8]; })arg2;
- (bool)isEntitledForAnyFromUniquePassTypeIDs:(id)arg1;
- (bool)isEntitledForMerchantSession:(id)arg1;
- (bool)isEntitledForPassTypeID:(id)arg1;
- (bool)isEntitledForPaymentRequest:(id)arg1;
- (id)merchantIdentifiers;
- (bool)passPresentationSuppression;
- (bool)passPresentationSuppressionFromBackground;
- (id)passTypeIDPrefixes;
- (id)passTypeIDs;
- (bool)passesAddSilently;
- (bool)passesAddUnsigned;
- (bool)passesAllAccess;
- (bool)passesEntangledAccess;
- (bool)passesOverviewAccess;
- (bool)payLaterPayInFourMerchandising;
- (bool)paymentAllAccess;
- (bool)paymentConfiguration;
- (bool)paymentPassProvisioning;
- (bool)paymentPresentation;
- (bool)peerPaymentAllAccess;
- (bool)remoteNetworkPaymentInitiate;
- (bool)searchAllAccess;
- (bool)secureElementPassProvisioning;
- (bool)secureElementPassProvisioningForMAIDs;
- (bool)shareableCredentialProvisioning;
- (bool)sharingAllAccess;
- (bool)supportsWebPayments;
- (id)teamIDs;
- (bool)trustedDeviceEnrollmentInfo;
- (bool)webPayments;
- (bool)winterpegPayments;

@end
