/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/PassKitCore.framework/PassKitCore
 */

@interface PKPaymentAuthorizationStateMachine : NSObject <PKBluetoothMonitorerDelegate, PKContinuityPaymentCoordinatorDelegate, PKPayLaterFinancingControllerObserver, PKPaymentOffersControllerObserver> {
    PKAccountService * _accountService;
    <PKAggregateDictionaryProtocol> * _aggregateDictionary;
    PKAccount * _appleCardAccount;
    bool  _awaitingClientCallbackReply;
    bool  _awaitingWebServiceResponse;
    PKBluetoothMonitorer * _bluetoothMonitor;
    NSMutableArray * _callbackQueue;
    long long  _cancelReason;
    NSObject<OS_dispatch_source> * _clientCallbackTimer;
    PKContinuityPaymentCoordinator * _continuityPaymentCoordinator;
    PKContinuityPaymentService * _continuityPaymentService;
    NSObject<OS_dispatch_group> * _delayAuthorizedStateGroup;
    <PKPaymentAuthorizationStateMachineDelegate> * _delegate;
    bool  _detectedBluetoothOn;
    PKAccountEnhancedMerchantsFetcher * _enhancedMerchantsFetcher;
    NSUUID * _enhancedMerchantsFetcherUpdateToken;
    unsigned long long  _failureStatusCount;
    PKPayLaterFinancingController * _financingController;
    bool  _hasReceivedRemoteDeviceUpdate;
    unsigned long long  _hostApplicationState;
    PKInAppPaymentSession * _inAppPaymentSession;
    NSString * _instanceIdentifier;
    PKPaymentAuthorizationDataModel * _model;
    PKPaymentAuthorizationClientCallbackStateParam * _mostRecentClientCallback;
    bool  _odiAssessmentInFlight;
    PKODIServiceProviderAssessment * _odiServiceProviderAssessment;
    PKODITransactionAssessment * _odiTransactionAssessment;
    PKPaymentOffersController * _paymentOffersController;
    PKPaymentService * _paymentService;
    PKPaymentSheetExperiment * _paymentSheetExperiment;
    PKPaymentWebService * _paymentWebService;
    PKPeerPaymentService * _peerPaymentService;
    PKPeerPaymentSession * _peerPaymentSession;
    unsigned long long  _prepareTransactionDetailsCounter;
    NSArray * _remoteDevicesToUpdate;
    PKSecureElement * _secureElement;
    bool  _showingExternalPasswordEntry;
    unsigned long long  _state;
    double  _updatePaymentDeviceTimeout;
}

@property (nonatomic, retain) PKAccountService *accountService;
@property (nonatomic, retain) <PKAggregateDictionaryProtocol> *aggregateDictionary;
@property (nonatomic, retain) PKAccount *appleCardAccount;
@property (nonatomic) bool awaitingClientCallbackReply;
@property (nonatomic) bool awaitingWebServiceResponse;
@property (nonatomic, retain) PKBluetoothMonitorer *bluetoothMonitor;
@property (nonatomic, retain) NSMutableArray *callbackQueue;
@property (nonatomic) long long cancelReason;
@property (nonatomic, retain) NSObject<OS_dispatch_source> *clientCallbackTimer;
@property (nonatomic, retain) PKContinuityPaymentCoordinator *continuityPaymentCoordinator;
@property (nonatomic, retain) PKContinuityPaymentService *continuityPaymentService;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (nonatomic, retain) NSObject<OS_dispatch_group> *delayAuthorizedStateGroup;
@property (nonatomic) <PKPaymentAuthorizationStateMachineDelegate> *delegate;
@property (readonly, copy) NSString *description;
@property (nonatomic) bool detectedBluetoothOn;
@property (nonatomic, retain) PKAccountEnhancedMerchantsFetcher *enhancedMerchantsFetcher;
@property (nonatomic, retain) NSUUID *enhancedMerchantsFetcherUpdateToken;
@property (nonatomic) unsigned long long failureStatusCount;
@property (nonatomic, retain) PKPayLaterFinancingController *financingController;
@property (nonatomic) bool hasReceivedRemoteDeviceUpdate;
@property (readonly) unsigned long long hash;
@property (nonatomic) unsigned long long hostApplicationState;
@property (nonatomic, retain) PKInAppPaymentSession *inAppPaymentSession;
@property (nonatomic, retain) NSString *instanceIdentifier;
@property (nonatomic, retain) PKPaymentAuthorizationDataModel *model;
@property (nonatomic, retain) PKPaymentAuthorizationClientCallbackStateParam *mostRecentClientCallback;
@property (nonatomic) bool odiAssessmentInFlight;
@property (nonatomic, retain) PKODIServiceProviderAssessment *odiServiceProviderAssessment;
@property (nonatomic, retain) PKODITransactionAssessment *odiTransactionAssessment;
@property (nonatomic, retain) PKPaymentOffersController *paymentOffersController;
@property (nonatomic, retain) PKPaymentService *paymentService;
@property (nonatomic, retain) PKPaymentSheetExperiment *paymentSheetExperiment;
@property (nonatomic, retain) PKPaymentWebService *paymentWebService;
@property (nonatomic, retain) PKPeerPaymentService *peerPaymentService;
@property (nonatomic, retain) PKPeerPaymentSession *peerPaymentSession;
@property (nonatomic) unsigned long long prepareTransactionDetailsCounter;
@property (nonatomic, retain) NSArray *remoteDevicesToUpdate;
@property (nonatomic, retain) PKSecureElement *secureElement;
@property (nonatomic) bool showingExternalPasswordEntry;
@property (setter=_setState:, nonatomic) unsigned long long state;
@property (readonly) Class superclass;
@property (nonatomic) double updatePaymentDeviceTimeout;
@property (nonatomic, readonly) bool useSecureElement;

- (void).cxx_destruct;
- (void)__setState:(unsigned long long)arg1 param:(id)arg2;
- (id)_activeODIAssessment;
- (unsigned long long)_adamIdentifier;
- (id)_additionalPayLaterServiceProviderData;
- (id)_additionalRelevantAnalyticsDictionary;
- (void)_advanceToNextState;
- (void)_advanceToNextStateFromResolveError:(bool)arg1;
- (void)_applyBillingInformationToAuthorizedQuote:(id)arg1;
- (void)_applyBillingInformationToPayment:(id)arg1;
- (void)_applyShippingInformationToPayment:(id)arg1;
- (void)_applyShippingMethodToPayment:(id)arg1;
- (void)_applyWebServiceConfigurationIfNeeded;
- (void)_augmentBaseRequestWithCompletion:(id /* block */)arg1;
- (void)_augmentSessionResponseWithCompletion:(id /* block */)arg1;
- (id)_billingInformationFromPaymentRequest:(id)arg1;
- (bool)_canUpdateWithAutomaticReloadPaymentRequest:(id)arg1 error:(id*)arg2;
- (bool)_canUpdateWithBillingAgreement:(id)arg1 oldAgreement:(id)arg2 error:(id*)arg3;
- (bool)_canUpdateWithDeferredPaymentRequest:(id)arg1 error:(id*)arg2;
- (bool)_canUpdateWithMultiTokenContexts:(id)arg1 error:(id*)arg2;
- (bool)_canUpdateWithRecurringPaymentRequest:(id)arg1 error:(id*)arg2;
- (void)_cancelClientCallbackTimer;
- (void)_clientCallbackTimedOut;
- (void)_computeAssessmentIfNecessary;
- (void)_configureAdditionalPaymentMethodsIfNeeeded;
- (void)_configurePayLaterOptionsIfNeeded;
- (void)_configurePaymentOffersIfNeeded;
- (id)_createNewRemotePaymentRequest;
- (id)_dequeuePendingCallbackParam;
- (void)_deviceUpdateDidFailWithNoEiligbleRemoteDevices:(id)arg1;
- (void)_dispatchNextCallbackParam;
- (void)_emitSignpostEvent:(unsigned long long)arg1;
- (void)_enqeueDidAuthorizePurchaseWithParam:(id)arg1;
- (void)_enqueueCallbackOfKind:(long long)arg1 withObject:(id)arg2;
- (void)_enqueueDidAuthorizeAccountServicePaymentRequestWithApplePayTrustSignature:(id)arg1;
- (void)_enqueueDidAuthorizeContext;
- (void)_enqueueDidAuthorizePaymentWithByPassPayment:(id)arg1 rewrapResponse:(id)arg2;
- (void)_enqueueDidAuthorizePaymentWithInstallmentAuthorizationToken:(id)arg1 rewrapResponse:(id)arg2;
- (void)_enqueueDidAuthorizePaymentWithPayment:(id)arg1 rewrapResponse:(id)arg2;
- (void)_enqueueDidAuthorizePaymentWithRemotePayment:(id)arg1 rewrapResponse:(id)arg2;
- (void)_enqueueDidAuthorizePaymentWithToken:(id)arg1 rewrapResponse:(id)arg2;
- (void)_enqueueDidAuthorizePeerPaymentQuoteWithAuthorizedQuote:(id)arg1;
- (void)_enqueueDidRequestMerchantSession;
- (void)_enqueueDidSelectBankAccount:(id)arg1;
- (void)_enqueueDidSelectPayLaterFinancingWithAccount:(id)arg1;
- (void)_enqueueDidSelectPaymentMethodWithBindToken:(id)arg1;
- (void)_enqueueDidSelectPaymentMethodWithQuote:(id)arg1;
- (void)_enqueueDidSelectPaymentPass:(id)arg1;
- (void)_enqueueDidSelectPaymentPass:(id)arg1 paymentApplication:(id)arg2;
- (void)_enqueueDidSelectRemotePaymentInstrument:(id)arg1;
- (void)_enqueueDidSelectRemotePaymentInstrument:(id)arg1 paymentApplication:(id)arg2;
- (void)_enqueueDidSelectShippingContact:(id)arg1;
- (void)_enqueueDidUpdateAccountServicePaymentMethod:(id)arg1;
- (void)_enqueueInitialCallbacks;
- (void)_enrichPaymentRewardsRedemptionWithPaymentTransaction:(id)arg1 usingPass:(id)arg2;
- (void)_fetchAdditionalPaymentMethodDataIfNeeded;
- (void)_fetchFinancingOptionsIfNeeded;
- (void)_fetchPaymentOffersIfNeeded;
- (void)_handleStateMachineWillStartNotification:(id)arg1;
- (id)_inAppSessionAuthorizationRequest;
- (void)_insertPendingOrderDetails:(bool*)arg1;
- (void)_insertPendingPaymentTransactionsWithParam:(id)arg1;
- (void)_insertPendingTransactionRegistration;
- (bool)_isDeviceTransactionAssessmentEnabled;
- (bool)_isMerchantTokensSupported;
- (bool)_isMultiTokenContextsSupported;
- (bool)_isRequestUsedForTransactionAssessment;
- (bool)_isServiceProviderAssessmentEnabled;
- (void)_nonceResponseWithCompletion:(id /* block */)arg1;
- (id)_odiServiceProviderAssessmentIdentifier;
- (id)_paymentRequestPurpleTrustData;
- (id)_pendingTransactionOnAlternateFundingSourceForAutorizedPeerPaymentQuote:(id)arg1;
- (id)_pendingTransactionOnPeerPaymentPassForAuthorizedPeerPaymentQuote:(id)arg1;
- (void)_performAuthorizationWithParam:(id)arg1;
- (void)_performCancelRemotePaymentRequest;
- (void)_performDataProcessingWithParam:(id)arg1;
- (void)_performDidAuthorizeCallbackWithParam:(id)arg1;
- (void)_performInstallmentBind;
- (void)_performNonceRequestWithParam:(id)arg1;
- (void)_performPassRewardsUpdateForCooldownLevel:(unsigned long long)arg1 ignoreErrorBackoff:(bool)arg2 merchantIdentifier:(id)arg3;
- (void)_performPrepareTransactionDetailsRequestWithParam:(id)arg1;
- (void)_performRewrapRequestImplWithParam:(id)arg1;
- (void)_performRewrapRequestWithParam:(id)arg1;
- (void)_performSendClientUpdateWithShippingMethods:(id)arg1 paymentSummaryItems:(id)arg2 multiTokenContexts:(id)arg3 recurringPaymentRequest:(id)arg4 automaticReloadPaymentRequest:(id)arg5 deferredPaymentRequest:(id)arg6 paymentApplication:(id)arg7 status:(long long)arg8;
- (void)_performSendPaymentResult:(id)arg1;
- (void)_performSendRemotePaymentRequest;
- (void)_performUpdatePaymentDevices;
- (void)_postStateMachineWillStartNotification;
- (void)_processBluetoothState:(long long)arg1;
- (void)_processErrorsForDataType:(long long)arg1;
- (void)_processPaymentRewardsRedemption:(id)arg1;
- (void)_promptTapToRadar;
- (void)_purchaseMerchantIdentifier:(id*)arg1 originURL:(id*)arg2 webMerchantIdentifier:(id*)arg3 webMerchantName:(id*)arg4 adamIdentifier:(id*)arg5 isMerchantTokenRequest:(bool*)arg6 isMultiTokenRequest:(bool*)arg7;
- (bool)_purchaseSupportsFinancingOptions;
- (bool)_purchaseSupportsPaymentOffers;
- (void)_registerForNotifications;
- (id)_relevantAnalyticsSubject;
- (void)_removeWebServiceConfigurationIfNeeded;
- (void)_reportAnalyticsTransactionAuthorizationWithAdditionalDictionary:(id)arg1 eventType:(id)arg2;
- (void)_reportToAutoBugCapture:(id)arg1;
- (void)_retrieveVPANPaymentSession;
- (id)_rewrapCompleteSessionRequestWithParam:(id)arg1 serviceURL:(id*)arg2;
- (void)_sendDidTransitionFromState:(unsigned long long)arg1 toState:(unsigned long long)arg2 withParam:(id)arg3;
- (void)_setState:(unsigned long long)arg1;
- (void)_setState:(unsigned long long)arg1 param:(id)arg2;
- (bool)_shouldSignPurpleTrustData;
- (void)_simulatePayment;
- (void)_start;
- (void)_startAssessmentIfNecessary;
- (void)_startClientCallbackTimer;
- (void)_startHandoff;
- (void)_startPayment;
- (void)_startRemoteDeviceUpdate;
- (void)_trackCouponCodeOutcomeWithError:(id)arg1;
- (id)_transactionWithPaymentToken:(id)arg1;
- (id)_transactionWithPurchase:(id)arg1 paymentHash:(id)arg2;
- (void)_unregisterForNotifications;
- (void)_updateAssessmentAttributes;
- (void)_updateFinancingAssessmentsWithCollection:(id)arg1;
- (void)_updateModelBillingAddressIfNeededForPass:(id)arg1;
- (void)_updateModelCanAddPasses;
- (void)_updateModelWithPaymentSummaryItems:(id)arg1;
- (void)_updateModelWithRemoteDevices:(id)arg1;
- (void)_updateModelWithShippingMethods:(id)arg1 paymentSummaryItems:(id)arg2 multiTokenContexts:(id)arg3 recurringPaymentRequest:(id)arg4 automaticReloadPaymentRequest:(id)arg5 deferredPaymentRequest:(id)arg6 contentItems:(id)arg7;
- (void)_updatePassRewardsInfo;
- (void)_updatePayLaterFundingSourceActionRequest:(id)arg1 param:(id)arg2;
- (void)_updatePaymentRequestBillingAddressWithPaymentMethod:(id)arg1;
- (void)_updatePostPurchasePayLaterFinancingPlan:(id)arg1 account:(id)arg2 completion:(id /* block */)arg3;
- (id)_updateRewrapBaseRequest:(id)arg1 param:(id)arg2;
- (void)_updateRewrapRequest:(id)arg1 param:(id)arg2 serviceURL:(id*)arg3;
- (id)accountService;
- (id)aggregateDictionary;
- (id)appleCardAccount;
- (bool)awaitingClientCallbackReply;
- (bool)awaitingWebServiceResponse;
- (void)beginDelayingAuthorizedState;
- (id)bluetoothMonitor;
- (void)bluetoothMonitorerRecievedUpdatedBluetoothState:(long long)arg1;
- (id)callbackQueue;
- (bool)canSelectPaymentOptions;
- (long long)cancelReason;
- (id)clientCallbackTimer;
- (id)continuityPaymentCoordinator;
- (void)continuityPaymentCoordinator:(id)arg1 didReceivePayment:(id)arg2;
- (void)continuityPaymentCoordinator:(id)arg1 didReceiveUpdatedPaymentDevices:(id)arg2;
- (void)continuityPaymentCoordinator:(id)arg1 didTimeoutTotalWithPaymentDevices:(id)arg2;
- (void)continuityPaymentCoordinatorDidReceiveCancellation:(id)arg1;
- (void)continuityPaymentCoordinatorDidTimeoutUpdatePaymentDevices:(id)arg1;
- (id)continuityPaymentService;
- (void)dealloc;
- (void)delayAuthorizedStateByDuration:(double)arg1;
- (id)delayAuthorizedStateGroup;
- (id)delegate;
- (id)description;
- (bool)detectedBluetoothOn;
- (void)didAuthenticateWithAuthenticatorEvaluationResponse:(id)arg1;
- (void)didAuthenticateWithCredential:(id)arg1;
- (void)didBecomeActive:(bool)arg1;
- (void)didBecomeArmable:(bool)arg1;
- (void)didBecomeOccluded:(bool)arg1;
- (void)didCancel;
- (void)didChangeCouponCode:(id)arg1;
- (void)didChangeFundingMode:(long long)arg1;
- (void)didChangePeerPaymentUsage:(bool)arg1;
- (void)didEncounterError:(id)arg1;
- (void)didEncounterFatalError:(id)arg1;
- (void)didEncounterShippingEmailError:(id)arg1;
- (void)didReceiveCouponCodeCompleteWithUpdate:(id)arg1;
- (void)didReceiveMerchantSessionCompleteWithSession:(id)arg1 error:(id)arg2;
- (void)didReceiveMerchantSessionCompleteWithUpdate:(id)arg1;
- (void)didReceivePaymentAuthorizationResult:(id)arg1;
- (void)didReceivePaymentAuthorizationStatus:(long long)arg1;
- (void)didReceivePaymentMethodCompleteWithSummaryItems:(id)arg1;
- (void)didReceivePaymentMethodCompleteWithUpdate:(id)arg1;
- (void)didReceiveShippingContactCompleteWithStatus:(long long)arg1 shippingMethods:(id)arg2 paymentSummaryItems:(id)arg3;
- (void)didReceiveShippingContactCompleteWithUpdate:(id)arg1;
- (void)didReceiveShippingMethodCompleteWithStatus:(long long)arg1 paymentSummaryItems:(id)arg2;
- (void)didReceiveShippingMethodCompleteWithUpdate:(id)arg1;
- (void)didReceiveUpdateAccountServicePaymentMethodCompleteWithUpdate:(id)arg1 signatureRequest:(id)arg2;
- (void)didRequestMerchantSession;
- (void)didRequestRefreshPaymentMethods;
- (void)didResignActive:(bool)arg1;
- (void)didResolveError;
- (void)didResolveLoadingState;
- (void)didSelectBankAccount:(id)arg1;
- (void)didSelectBillingAddress:(id)arg1;
- (void)didSelectFinancingOption:(id)arg1;
- (void)didSelectPaymentOffer:(id)arg1;
- (void)didSelectPaymentOffer:(id)arg1 updateReason:(unsigned long long)arg2;
- (void)didSelectPaymentPass:(id)arg1;
- (void)didSelectPaymentPass:(id)arg1 paymentApplication:(id)arg2;
- (void)didSelectRemotePaymentInstrument:(id)arg1;
- (void)didSelectRemotePaymentInstrument:(id)arg1 paymentApplication:(id)arg2;
- (void)didSelectShippingContact:(id)arg1;
- (void)didSelectShippingEmail:(id)arg1;
- (void)didSelectShippingMethod:(id)arg1;
- (void)didSelectShippingName:(id)arg1;
- (void)didSelectShippingPhoneNumber:(id)arg1;
- (void)endDelayingAuthorizedState;
- (id)enhancedMerchantsFetcher;
- (id)enhancedMerchantsFetcherUpdateToken;
- (unsigned long long)failureStatusCount;
- (id)financingController;
- (bool)hasPendingCallbacks;
- (bool)hasReceivedRemoteDeviceUpdate;
- (unsigned long long)hostApplicationState;
- (id)inAppPaymentSession;
- (id)init;
- (id)instanceIdentifier;
- (void)invalidate;
- (bool)isErrorAboutPostalAddressField:(id)arg1;
- (id)model;
- (id)mostRecentClientCallback;
- (bool)odiAssessmentInFlight;
- (id)odiServiceProviderAssessment;
- (id)odiTransactionAssessment;
- (void)payLaterAvailableFundingSourcesDidChange:(id)arg1;
- (void)payLaterFinancingAssessmentCollectionDidUpdate:(id)arg1;
- (void)payLaterFinancingController:(id)arg1 financingOptionCancelled:(id)arg2;
- (void)paymentOfferActionsChanged:(id)arg1;
- (void)paymentOfferCatalogChanged:(id)arg1;
- (void)paymentOfferDynamicContentChanged:(id)arg1;
- (void)paymentOffersChanged:(id)arg1 forPassUniqueIdentifier:(id)arg2;
- (id)paymentOffersController;
- (id)paymentService;
- (id)paymentSheetExperiment;
- (id)paymentWebService;
- (id)peerPaymentService;
- (id)peerPaymentSession;
- (unsigned long long)prepareTransactionDetailsCounter;
- (id)remoteDevicesToUpdate;
- (id)sanitizeClientErrors:(id)arg1;
- (id)secureElement;
- (void)selectedPaymentOfferChanged:(id)arg1 forPassUniqueIdentifier:(id)arg2;
- (void)setAccountService:(id)arg1;
- (void)setAggregateDictionary:(id)arg1;
- (void)setAppleCardAccount:(id)arg1;
- (void)setAwaitingClientCallbackReply:(bool)arg1;
- (void)setAwaitingWebServiceResponse:(bool)arg1;
- (void)setBluetoothMonitor:(id)arg1;
- (void)setCallbackQueue:(id)arg1;
- (void)setCancelReason:(long long)arg1;
- (void)setClientCallbackTimer:(id)arg1;
- (void)setContinuityPaymentCoordinator:(id)arg1;
- (void)setContinuityPaymentService:(id)arg1;
- (void)setDelayAuthorizedStateGroup:(id)arg1;
- (void)setDelegate:(id)arg1;
- (void)setDetectedBluetoothOn:(bool)arg1;
- (void)setEnhancedMerchantsFetcher:(id)arg1;
- (void)setEnhancedMerchantsFetcherUpdateToken:(id)arg1;
- (void)setFailureStatusCount:(unsigned long long)arg1;
- (void)setFinancingController:(id)arg1;
- (void)setHasReceivedRemoteDeviceUpdate:(bool)arg1;
- (void)setHostApplicationState:(unsigned long long)arg1;
- (void)setInAppPaymentSession:(id)arg1;
- (void)setInstanceIdentifier:(id)arg1;
- (void)setModel:(id)arg1;
- (void)setMostRecentClientCallback:(id)arg1;
- (void)setOdiAssessmentInFlight:(bool)arg1;
- (void)setOdiServiceProviderAssessment:(id)arg1;
- (void)setOdiTransactionAssessment:(id)arg1;
- (void)setPaymentOffersController:(id)arg1;
- (void)setPaymentService:(id)arg1;
- (void)setPaymentSheetExperiment:(id)arg1;
- (void)setPaymentWebService:(id)arg1;
- (void)setPeerPaymentService:(id)arg1;
- (void)setPeerPaymentSession:(id)arg1;
- (void)setPrepareTransactionDetailsCounter:(unsigned long long)arg1;
- (void)setRemoteDevicesToUpdate:(id)arg1;
- (void)setSecureElement:(id)arg1;
- (void)setShowingExternalPasswordEntry:(bool)arg1;
- (void)setUpdatePaymentDeviceTimeout:(double)arg1;
- (bool)showingExternalPasswordEntry;
- (void)start;
- (unsigned long long)state;
- (long long)statusFromPaymentError:(id)arg1;
- (double)updatePaymentDeviceTimeout;
- (bool)useSecureElement;

@end
