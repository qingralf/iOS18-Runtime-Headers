/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/PassKitCore.framework/PassKitCore
 */

@interface PKProtobufPeerPaymentMessage : PBCodable <NSCopying> {
    long long  _amount;
    NSString * _currency;
    PKProtobufNSDecimalNumber * _decimalAmount;
    struct { 
        unsigned int amount : 1; 
        unsigned int recurringPaymentStartDate : 1; 
        unsigned int type : 1; 
    }  _has;
    NSString * _identifier;
    NSData * _localData;
    NSString * _memo;
    NSString * _paymentIdentifier;
    NSString * _recipientAddress;
    NSString * _recurringPaymentColor;
    NSString * _recurringPaymentEmoji;
    NSString * _recurringPaymentFrequency;
    NSString * _recurringPaymentIdentifier;
    double  _recurringPaymentStartDate;
    NSString * _requestDeviceScoreIdentifier;
    NSString * _requestToken;
    NSString * _senderAddress;
    NSString * _transactionIdentifier;
    int  _type;
    unsigned int  _version;
}

@property (nonatomic) long long amount;
@property (nonatomic, retain) NSString *currency;
@property (nonatomic, retain) PKProtobufNSDecimalNumber *decimalAmount;
@property (nonatomic) bool hasAmount;
@property (nonatomic, readonly) bool hasCurrency;
@property (nonatomic, readonly) bool hasDecimalAmount;
@property (nonatomic, readonly) bool hasIdentifier;
@property (nonatomic, readonly) bool hasLocalData;
@property (nonatomic, readonly) bool hasMemo;
@property (nonatomic, readonly) bool hasPaymentIdentifier;
@property (nonatomic, readonly) bool hasRecipientAddress;
@property (nonatomic, readonly) bool hasRecurringPaymentColor;
@property (nonatomic, readonly) bool hasRecurringPaymentEmoji;
@property (nonatomic, readonly) bool hasRecurringPaymentFrequency;
@property (nonatomic, readonly) bool hasRecurringPaymentIdentifier;
@property (nonatomic) bool hasRecurringPaymentStartDate;
@property (nonatomic, readonly) bool hasRequestDeviceScoreIdentifier;
@property (nonatomic, readonly) bool hasRequestToken;
@property (nonatomic, readonly) bool hasSenderAddress;
@property (nonatomic, readonly) bool hasTransactionIdentifier;
@property (nonatomic) bool hasType;
@property (nonatomic, retain) NSString *identifier;
@property (nonatomic, retain) NSData *localData;
@property (nonatomic, retain) NSString *memo;
@property (nonatomic, retain) NSString *paymentIdentifier;
@property (nonatomic, retain) NSString *recipientAddress;
@property (nonatomic, retain) NSString *recurringPaymentColor;
@property (nonatomic, retain) NSString *recurringPaymentEmoji;
@property (nonatomic, retain) NSString *recurringPaymentFrequency;
@property (nonatomic, retain) NSString *recurringPaymentIdentifier;
@property (nonatomic) double recurringPaymentStartDate;
@property (nonatomic, retain) NSString *requestDeviceScoreIdentifier;
@property (nonatomic, retain) NSString *requestToken;
@property (nonatomic, retain) NSString *senderAddress;
@property (nonatomic, retain) NSString *transactionIdentifier;
@property (nonatomic) int type;
@property (nonatomic) unsigned int version;

- (void).cxx_destruct;
- (int)StringAsType:(id)arg1;
- (long long)amount;
- (void)copyTo:(id)arg1;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)currency;
- (id)decimalAmount;
- (id)description;
- (id)dictionaryRepresentation;
- (bool)hasAmount;
- (bool)hasCurrency;
- (bool)hasDecimalAmount;
- (bool)hasIdentifier;
- (bool)hasLocalData;
- (bool)hasMemo;
- (bool)hasPaymentIdentifier;
- (bool)hasRecipientAddress;
- (bool)hasRecurringPaymentColor;
- (bool)hasRecurringPaymentEmoji;
- (bool)hasRecurringPaymentFrequency;
- (bool)hasRecurringPaymentIdentifier;
- (bool)hasRecurringPaymentStartDate;
- (bool)hasRequestDeviceScoreIdentifier;
- (bool)hasRequestToken;
- (bool)hasSenderAddress;
- (bool)hasTransactionIdentifier;
- (bool)hasType;
- (unsigned long long)hash;
- (id)identifier;
- (bool)isEqual:(id)arg1;
- (id)localData;
- (id)memo;
- (void)mergeFrom:(id)arg1;
- (id)paymentIdentifier;
- (bool)readFrom:(id)arg1;
- (id)recipientAddress;
- (id)recurringPaymentColor;
- (id)recurringPaymentEmoji;
- (id)recurringPaymentFrequency;
- (id)recurringPaymentIdentifier;
- (double)recurringPaymentStartDate;
- (id)requestDeviceScoreIdentifier;
- (id)requestToken;
- (id)senderAddress;
- (void)setAmount:(long long)arg1;
- (void)setCurrency:(id)arg1;
- (void)setDecimalAmount:(id)arg1;
- (void)setHasAmount:(bool)arg1;
- (void)setHasRecurringPaymentStartDate:(bool)arg1;
- (void)setHasType:(bool)arg1;
- (void)setIdentifier:(id)arg1;
- (void)setLocalData:(id)arg1;
- (void)setMemo:(id)arg1;
- (void)setPaymentIdentifier:(id)arg1;
- (void)setRecipientAddress:(id)arg1;
- (void)setRecurringPaymentColor:(id)arg1;
- (void)setRecurringPaymentEmoji:(id)arg1;
- (void)setRecurringPaymentFrequency:(id)arg1;
- (void)setRecurringPaymentIdentifier:(id)arg1;
- (void)setRecurringPaymentStartDate:(double)arg1;
- (void)setRequestDeviceScoreIdentifier:(id)arg1;
- (void)setRequestToken:(id)arg1;
- (void)setSenderAddress:(id)arg1;
- (void)setTransactionIdentifier:(id)arg1;
- (void)setType:(int)arg1;
- (void)setVersion:(unsigned int)arg1;
- (id)transactionIdentifier;
- (int)type;
- (id)typeAsString:(int)arg1;
- (unsigned int)version;
- (void)writeTo:(id)arg1;

@end
