/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/PerfPowerServicesReader.framework/PerfPowerServicesReader
 */

@interface AWDMETRICSCellularNrRadioStats : PBCodable <NSCopying> {
    NSMutableArray * _bwpInfos;
    unsigned int  _durationMs;
    struct { 
        unsigned int timestamp : 1; 
        unsigned int durationMs : 1; 
        unsigned int subsId : 1; 
    }  _has;
    unsigned int  _subsId;
    unsigned long long  _timestamp;
}

@property (nonatomic, retain) NSMutableArray *bwpInfos;
@property (nonatomic) unsigned int durationMs;
@property (nonatomic) bool hasDurationMs;
@property (nonatomic) bool hasSubsId;
@property (nonatomic) bool hasTimestamp;
@property (nonatomic) unsigned int subsId;
@property (nonatomic) unsigned long long timestamp;

+ (Class)bwpInfoType;

- (void).cxx_destruct;
- (void)addBwpInfo:(id)arg1;
- (id)bwpInfoAtIndex:(unsigned long long)arg1;
- (id)bwpInfos;
- (unsigned long long)bwpInfosCount;
- (void)clearBwpInfos;
- (void)copyTo:(id)arg1;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)description;
- (id)dictionaryRepresentation;
- (unsigned int)durationMs;
- (bool)hasDurationMs;
- (bool)hasSubsId;
- (bool)hasTimestamp;
- (unsigned long long)hash;
- (bool)isEqual:(id)arg1;
- (void)mergeFrom:(id)arg1;
- (bool)readFrom:(id)arg1;
- (void)setBwpInfos:(id)arg1;
- (void)setDurationMs:(unsigned int)arg1;
- (void)setHasDurationMs:(bool)arg1;
- (void)setHasSubsId:(bool)arg1;
- (void)setHasTimestamp:(bool)arg1;
- (void)setSubsId:(unsigned int)arg1;
- (void)setTimestamp:(unsigned long long)arg1;
- (unsigned int)subsId;
- (unsigned long long)timestamp;
- (void)writeTo:(id)arg1;

@end
