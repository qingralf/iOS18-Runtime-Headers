/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/PhotosUIPrivate.framework/PhotosUIPrivate
 */

@interface PUOneUpViewController : UIViewController <PHAirPlayControllerContentProvider, PUAccessoryTileViewControllerDelegate, PUAccessoryVisibilityInteractionControllerDelegate, PUAssetDisplayDescriptorNavigator, PUBadgeTileViewControllerDelegate, PUBarsControllerDelegate, PUCropButtonTileViewControllerDelegate, PUDoubleTapZoomControllerDelegate, PUImageAnalysisInteractionDelegate, PUImageTileViewControllerPresentingDelegate, PUInteractiveDismissalControllerDelegate, PUIrisImageTileViewControllerDelegate, PULivePhotoVideoOverlayTileViewControllerDelegate, PUMenuActionControllerDelegate, PUOneUpAccessoryViewControllersManagerDelegate, PUOneUpActionsControllerDelegate, PUOneUpAssetTransitionViewController, PUOneUpBarsControllerDelegate, PUOneUpChromeViewControllerDelegate, PUOneUpDefaultActionsControllerActionManagerSource, PUOneUpGestureControllerDelegate, PUOneUpGestureRecognizerCoordinatorDelegate, PUOneUpTilingLayoutDelegate, PUOverOneUpPresentationSessionDelegate, PUOverOneUpPresentationSessionViewController, PUPeopleTileDelegate, PUPlayButtonTileViewControllerDelegate, PUSyndicationAttributionTileViewControllerDelegate, PUTilingViewControllerTransitionEndPoint, PUTilingViewScrollDelegate, PUTilingViewTileSource, PUTilingViewTileTransitionDelegate, PUTilingViewTileUseDelegate, PUUserTransformTileViewControllerDelegate, PUViewControllerSpecChangeObserver, PUViewModelChangeObserver, PXActionPerformerDelegate, PXAppIntentsViewAnnotationDelegate, PXContentPrivacyAuthenticationContext, PXDiagnosticsEnvironment, PXFloatingCardContainer, PXFloatingCardPresentationControllerDelegate, PXForcedDismissableViewController, PXLoadingStatusManagerObserver, PXPreferencesObserver, PXPurgeableController, PXSettingsKeyObserver, PXTipsPresentationDelegate, UIActivityItemsConfigurationProviding, UIScrollViewDelegate> {
    PUOneUpAccessoryViewControllersManager * __accessoryViewControllersManager;
    PUAccessoryVisibilityInteractionController * __accessoryVisibilityInteractionController;
    PUBrowsingBackgroundTileViewController * __backgroundTileViewController;
    PUOneUpBarsController * __barsController;
    NSTimer * __chromeAutoHideTimer;
    bool  __didPlayCurrentLivePhoto;
    PUDoubleTapZoomController * __doubleTapZoomController;
    PUOneUpGestureRecognizerCoordinator * __gestureRecognizerCoordinator;
    bool  __hasShownRelevantTip;
    NSArray * __hiddenTilesController;
    unsigned long long  __initialActivity;
    PUInteractivePinchDismissalController * __interactivePinchDismissalController;
    PUInteractiveSwipeDismissalController * __interactiveSwipeDismissalController;
    bool  __isIrisPlaying;
    bool  __isPresentedForSecondScreen;
    struct CGSize { 
        double width; 
        double height; 
    }  __layoutReferenceSize;
    struct UIEdgeInsets { 
        double top; 
        double left; 
        double bottom; 
        double right; 
    }  __layoutSafeAreaInsets;
    PUOneUpViewController * __mainOneUpForSecondScreenBrowser;
    bool  __needsUpdateAccessoryView;
    bool  __needsUpdateBarsController;
    bool  __needsUpdateEditMode;
    bool  __needsUpdateImageAnalysisInteractionAdditionalActionInfoEdgeInsets;
    bool  __needsUpdateLayout;
    bool  __needsUpdatePreferredContentSize;
    bool  __needsUpdatePreloadInsets;
    bool  __needsUpdateSpec;
    bool  __needsUpdateTipsPopover;
    bool  __needsUpdateUserActivity;
    bool  __needsUpdateVideoPlayer;
    unsigned long long  __options;
    PUOverOneUpPresentationSession * __overOneUpPresentationSession;
    PUParallaxComputer * __parallaxComputer;
    struct CGSize { 
        double width; 
        double height; 
    }  __pendingViewTransitionSize;
    bool  __prefersCompactLayoutForSplitScreen;
    bool  __prefersHomeIndicatorHidden;
    double  __preloadInsetsBasedOffViewWidth;
    PUReviewScreenControlBarTileViewController * __reviewScreenControlBarTileViewController;
    PUReviewScreenScrubberBarTileViewController * __reviewScreenScrubberBarTileViewController;
    PUReviewScreenTopBarTileViewController * __reviewScreenTopBarTileViewController;
    PUOneUpViewController * __secondScreenBrowser;
    bool  __shouldDisableTransitionsUntilAppeared;
    bool  __shouldShowAccessoryAfterUnlock;
    PUOneUpViewControllerSpec * __spec;
    NSArray * __tileKindsToHide;
    PUTilingView * __tilingView;
    long long  __windowInterfaceOrientation;
    unsigned long long  _actionTypeToPerformInViewDidAppear;
    <PUOneUpActionsController> * _actionsController;
    NSMutableSet * _activeVideoTileControllers;
    bool  _allowsPreviewActions;
    int  _appearState;
    bool  _appearanceTransitionAnimationsDisabled;
    PXAssetActionManager * _assetActionManager;
    NSArray * _assetUUIDsAllowedToHighlightText;
    PXBoopableItemsProvider * _boopableItemsProvider;
    PUBrowsingOneUpVisibilityHelper * _browsingOneUpVisibilityHelper;
    PUBrowsingSession * _browsingSession;
    PUOneUpChromeViewController * _chromeViewController;
    UIView * _contentContainerView;
    UIViewController<PUAccessoryContentViewController> * _currentFloatingInfoPanelAccessoryViewController;
    long long  _detailsBecameVisibleSignPost;
    id /* block */  _editActivityCompletion;
    <PUOneUpEventTracker> * _eventTracker;
    PUOneUpFeedbackController * _feedbackController;
    PUOneUpGestureController * _gestureController;
    <PUDisplayLocationProvider> * _lastLivePhotoGestureLocationProvider;
    PULoadingIndicatorController * _loadingIndicatorController;
    PUMenuActionController * _menuActionController;
    bool  _needsUpdateChromeViewController;
    NSString * _ppt_assetUUIDNeedsDeferredProcessingFinishNotification;
    id /* block */  _ppt_deferredProcessingCompletionBlock;
    NSTimer * _ppt_deferredProcessingWaitTimer;
    struct CGSize { 
        double width; 
        double height; 
    }  _preferredContentSizeOverride;
    NSObject<OS_dispatch_queue> * _preheatAssetsQueue;
    long long  _presentationOrigin;
    NSString * _presentedTipID;
    NSString * _scrubbingIdentifier;
    NSUserActivity * _siriActionActivity;
    NSArray * _textsToHighlight;
    id /* block */  _tipDismissedCompletion;
    unsigned long long  _touchingGestureRecognizerStateBeginID;
    id /* block */  _unlockDeviceHandlerWithActionType;
    id /* block */  _unlockDeviceStatus;
    NSString * _uuidOfAssetPresentedInFloatingInfoPanel;
    PUOneUpVisualImageAnalyzingController * _visualImageAnalyzingController;
    bool  _wantsActionsMenu;
    bool  _wasChromeVisibleBeforeZoomIn;
    NSTimer * _xct_waitForDeferredProcessingTimer;
    NSTimer * _xct_waitForFullQualityImageTimer;
    PXFloatingCardPresentationController * cardPresentationController;
}

@property (nonatomic, readonly) PUOneUpAccessoryViewControllersManager *_accessoryViewControllersManager;
@property (nonatomic, readonly) PUAccessoryVisibilityInteractionController *_accessoryVisibilityInteractionController;
@property (setter=_setBackgroundTileViewController:, nonatomic, retain) PUBrowsingBackgroundTileViewController *_backgroundTileViewController;
@property (nonatomic, readonly) PUOneUpBarsController *_barsController;
@property (setter=_setChromeAutoHideTimer:, nonatomic, retain) NSTimer *_chromeAutoHideTimer;
@property (setter=_setDidPlayCurrentLivePhoto:, nonatomic) bool _didPlayCurrentLivePhoto;
@property (nonatomic, readonly) PUDoubleTapZoomController *_doubleTapZoomController;
@property (nonatomic, readonly) PUOneUpGestureRecognizerCoordinator *_gestureRecognizerCoordinator;
@property (setter=_setHasShownRelevantTip:, nonatomic) bool _hasShownRelevantTip;
@property (setter=_setHiddenTilesController:, nonatomic, retain) NSArray *_hiddenTilesController;
@property (setter=_setInitialActivity:, nonatomic) unsigned long long _initialActivity;
@property (nonatomic, readonly) PUInteractivePinchDismissalController *_interactivePinchDismissalController;
@property (nonatomic, readonly) PUInteractiveSwipeDismissalController *_interactiveSwipeDismissalController;
@property (setter=_setIrisPlaying:, nonatomic) bool _isIrisPlaying;
@property (nonatomic, readonly) bool _isPresentedForSecondScreen;
@property (setter=_setLayoutReferenceSize:, nonatomic) struct CGSize { double x1; double x2; } _layoutReferenceSize;
@property (setter=_setLayoutSafeAreaInsets:, nonatomic) struct UIEdgeInsets { double x1; double x2; double x3; double x4; } _layoutSafeAreaInsets;
@property (setter=_setMainOneUpForSecondScreenBrowser:, nonatomic) PUOneUpViewController *_mainOneUpForSecondScreenBrowser;
@property (setter=_setNeedsUpdateAccessoryView:, nonatomic) bool _needsUpdateAccessoryView;
@property (setter=_setNeedsUpdateBarsController:, nonatomic) bool _needsUpdateBarsController;
@property (setter=_setNeedsUpdateEditMode:, nonatomic) bool _needsUpdateEditMode;
@property (setter=_setNeedsUpdateImageAnalysisInteractionAdditionalActionInfoEdgeInsets:, nonatomic) bool _needsUpdateImageAnalysisInteractionAdditionalActionInfoEdgeInsets;
@property (setter=_setNeedsUpdateLayout:, nonatomic) bool _needsUpdateLayout;
@property (setter=_setNeedsUpdatePreferredContentSize:, nonatomic) bool _needsUpdatePreferredContentSize;
@property (setter=_setNeedsUpdatePreloadInsets:, nonatomic) bool _needsUpdatePreloadInsets;
@property (setter=_setNeedsUpdateSpec:, nonatomic) bool _needsUpdateSpec;
@property (setter=_setNeedsUpdateTipPopovers:, nonatomic) bool _needsUpdateTipsPopover;
@property (setter=_setNeedsUpdateUserActivity:, nonatomic) bool _needsUpdateUserActivity;
@property (setter=_setNeedsUpdateVideoPlayer:, nonatomic) bool _needsUpdateVideoPlayer;
@property (nonatomic, readonly) unsigned long long _options;
@property (setter=_setOverOneUpPresentationSession:, nonatomic, retain) PUOverOneUpPresentationSession *_overOneUpPresentationSession;
@property (nonatomic, readonly) PUParallaxComputer *_parallaxComputer;
@property (setter=_setPendingViewTransitionSize:, nonatomic) struct CGSize { double x1; double x2; } _pendingViewTransitionSize;
@property (nonatomic, readonly) bool _prefersCompactLayoutForSplitScreen;
@property (setter=_setPrefersHomeIndicatorHidden:, nonatomic) bool _prefersHomeIndicatorHidden;
@property (setter=_setPreloadInsetsBasedOffViewWidth:, nonatomic) double _preloadInsetsBasedOffViewWidth;
@property (setter=_setReviewScreenControlBarTileViewController:, nonatomic, retain) PUReviewScreenControlBarTileViewController *_reviewScreenControlBarTileViewController;
@property (setter=_setReviewScreenScrubberBarTileViewController:, nonatomic, retain) PUReviewScreenScrubberBarTileViewController *_reviewScreenScrubberBarTileViewController;
@property (setter=_setReviewScreenTopBarTileViewController:, nonatomic, retain) PUReviewScreenTopBarTileViewController *_reviewScreenTopBarTileViewController;
@property (setter=_setSecondScreenBrowser:, nonatomic, retain) PUOneUpViewController *_secondScreenBrowser;
@property (setter=_setShouldDisableTransitionsUntilAppeared:, nonatomic) bool _shouldDisableTransitionsUntilAppeared;
@property (setter=_setShouldShowAccessoryAfterUnlock:, nonatomic) bool _shouldShowAccessoryAfterUnlock;
@property (nonatomic, readonly) PUOneUpViewControllerSpec *_spec;
@property (setter=_setTileKindsToHide:, nonatomic, retain) NSArray *_tileKindsToHide;
@property (setter=_setTilingView:, nonatomic, retain) PUTilingView *_tilingView;
@property (setter=_setWindowInterfaceOrientation:, nonatomic) long long _windowInterfaceOrientation;
@property (nonatomic, readonly) <PUOneUpActionsController> *actionsController;
@property (nonatomic, readonly) NSMutableSet *activeVideoTileControllers;
@property (nonatomic, readonly) <UIActivityItemsConfigurationReading> *activityItemsConfiguration;
@property (setter=setAllowsPreviewActions:, nonatomic) bool allowsPreviewActions;
@property (nonatomic, readonly) NSString *appIntentsDebugDescription;
@property (nonatomic) int appearState;
@property (nonatomic) bool appearanceTransitionAnimationsDisabled;
@property (nonatomic, readonly) PXAssetActionManager *assetActionManager;
@property (nonatomic, copy) NSArray *assetUUIDsAllowedToHighlightText;
@property (nonatomic, readonly) NSString *authenticationContextIdentifier; /* unknown property attribute: ? */
@property (nonatomic, readonly) NSString *authenticationTitle;
@property (nonatomic, retain) PXBoopableItemsProvider *boopableItemsProvider;
@property (nonatomic, retain) PUBrowsingOneUpVisibilityHelper *browsingOneUpVisibilityHelper;
@property (nonatomic, readonly) PUBrowsingSession *browsingSession;
@property (nonatomic, retain) PXFloatingCardPresentationController *cardPresentationController;
@property (readonly) UIView *chromeView;
@property (nonatomic, readonly) PUOneUpChromeViewController *chromeViewController;
@property (nonatomic, readonly) UIView *contentContainerView;
@property (nonatomic) UIViewController<PUAccessoryContentViewController> *currentFloatingInfoPanelAccessoryViewController;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (nonatomic) long long detailsBecameVisibleSignPost;
@property (nonatomic, copy) id /* block */ editActivityCompletion;
@property (nonatomic, readonly) <PUOneUpEventTracker> *eventTracker;
@property (nonatomic, readonly) PUOneUpFeedbackController *feedbackController;
@property (nonatomic, readonly) PUOneUpGestureController *gestureController;
@property (readonly) unsigned long long hash;
@property (nonatomic, readonly) bool isFloatingInfoPanelPresented;
@property (nonatomic, retain) <PUDisplayLocationProvider> *lastLivePhotoGestureLocationProvider;
@property (nonatomic, readonly) PULoadingIndicatorController *loadingIndicatorController;
@property (nonatomic, readonly) <PXMemoryAssetsActionFactory> *memoryAssetsActionFactory; /* unknown property attribute: ? */
@property (nonatomic, readonly) PUMenuActionController *menuActionController;
@property (nonatomic) bool needsUpdateChromeViewController;
@property (nonatomic, readonly) NSString *passcodeAuthenticationReason;
@property (nonatomic, readonly) <PUOneUpActionsController> *ppt_actionsController;
@property (nonatomic, retain) NSString *ppt_assetUUIDNeedsDeferredProcessingFinishNotification;
@property (nonatomic, readonly) PUOneUpBarsController *ppt_barsController;
@property (nonatomic, readonly) UIViewController *ppt_currentAccessoryViewController;
@property (nonatomic, copy) id /* block */ ppt_deferredProcessingCompletionBlock;
@property (nonatomic, retain) NSTimer *ppt_deferredProcessingWaitTimer;
@property (nonatomic, readonly) UIScrollView *ppt_mainScrollView;
@property (nonatomic, readonly) PUOverOneUpPresentationSession *ppt_overOneUpPresentationSession;
@property (nonatomic, readonly) UIView *ppt_scrubberView;
@property (nonatomic) struct CGSize { double x1; double x2; } preferredContentSizeOverride;
@property (nonatomic, readonly) NSObject<OS_dispatch_queue> *preheatAssetsQueue;
@property (nonatomic, readonly) long long presentationOrigin;
@property (nonatomic, retain) NSString *presentedTipID;
@property (nonatomic, retain) NSUserActivity *siriActionActivity;
@property (readonly) Class superclass;
@property (nonatomic, copy) NSArray *textsToHighlight;
@property (nonatomic, readonly) UIViewController *tilingViewControllerTransitionEndPointHostViewController;
@property (nonatomic, copy) id /* block */ tipDismissedCompletion;
@property (nonatomic) unsigned long long touchingGestureRecognizerStateBeginID;
@property (nonatomic, copy) id /* block */ unlockDeviceHandlerWithActionType;
@property (nonatomic, copy) id /* block */ unlockDeviceStatus;
@property (nonatomic, retain) NSString *uuidOfAssetPresentedInFloatingInfoPanel;
@property (nonatomic, readonly) bool videoTilesCanPlayVideo;
@property (nonatomic, readonly) PUOneUpVisualImageAnalyzingController *visualImageAnalyzingController;
@property (nonatomic, readonly) bool wantsActionsMenu;
@property (nonatomic) bool wasChromeVisibleBeforeZoomIn;
@property (nonatomic, readonly) bool xct_isCurrentImageDeferredProcessingFinished;
@property (nonatomic, readonly) bool xct_isCurrentImageFullQuality;
@property (nonatomic, retain) NSTimer *xct_waitForDeferredProcessingTimer;
@property (nonatomic, retain) NSTimer *xct_waitForFullQualityImageTimer;

+ (id)oneUpActivityTitle;

- (void).cxx_destruct;
- (void)_abandonTileController:(id)arg1;
- (long long)_accessoryContentKindForAsset:(id)arg1;
- (long long)_accessoryContentKindForCurrentAsset;
- (id)_accessoryViewControllersManager;
- (id)_accessoryVisibilityInteractionController;
- (bool)_allowsShowingVisualIntelligenceForAsset:(id)arg1;
- (bool)_allowsVisualLookupInInfoPanelMode;
- (void)_arrowKey:(id)arg1;
- (id)_assetDisplayDescriptorForAsset:(id)arg1;
- (id)_assetReferenceAtIndexPath:(id)arg1 layout:(id)arg2;
- (id)_assetViewModelAtIndexPath:(id)arg1 layout:(id)arg2;
- (id)_assetsToPrefetch;
- (id)_assetsToPrefetchUsingDataSource:(id)arg1 startingAt:(id)arg2 count:(long long)arg3 reverseDirection:(bool)arg4;
- (id)_backgroundTileViewController;
- (id)_barsController;
- (void)_beginVKSubjectAnalyzingWithGestureRecognizer:(id)arg1 beginID:(unsigned long long)arg2;
- (void)_browsingVideoPlayerDidPlayToEndTime:(id)arg1;
- (bool)_canAdvertiseKeyCommands;
- (bool)_canAttemptNavigationToAssetDisplayDescriptor:(id)arg1;
- (bool)_canShowWhileLocked;
- (void)_cancelTimedChromeAutoHide;
- (id)_chromeAutoHideTimer;
- (void)_chromeAutoHideTimerFired:(id)arg1;
- (void)_configureAdoptedTileController:(id)arg1;
- (void)_configureVitalityController;
- (void)_contentSizeCategoryDidChange:(id)arg1;
- (id)_contentTileControllerForAssetReference:(id)arg1;
- (id)_cropButtonTileViewControllerForCurrentAsset;
- (id)_currentAccessoryViewController;
- (id)_currentAssetViewModel;
- (id)_currentContentTileController;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_currentFrameForTileController:(id)arg1;
- (id)_currentPlaceholderSnapshotViewForTileController:(id)arg1;
- (void)_didEndTransition;
- (bool)_didPlayCurrentLivePhoto;
- (void)_dimFloatingInfoPanelAnimated:(bool)arg1;
- (void)_dismissAnyPresentedTip;
- (void)_dismissFloatingInfoPanelAnimated:(bool)arg1;
- (void)_dismissTipPopoverAnimated:(bool)arg1;
- (id)_doubleTapZoomController;
- (void)_editAdjust:(id)arg1;
- (void)_editCleanup:(id)arg1;
- (void)_editCrop:(id)arg1;
- (void)_editFilter:(id)arg1;
- (void)_editStyles:(id)arg1;
- (id)_gestureRecognizerCoordinator;
- (void)_handleDeferredProcessingFinishedForAssetUUID:(id)arg1;
- (void)_handleSuggestedActionCompletedForTip:(id)arg1 completion:(id /* block */)arg2;
- (void)_handleTouchGesture:(id)arg1;
- (void)_handleUnlockDeviceForActionType:(unsigned long long)arg1 success:(bool)arg2;
- (void)_handleZoomOutCurrentAsset;
- (bool)_hasShownRelevantTip;
- (id)_hiddenTilesController;
- (void)_hideChromeOnPlayButtonTapWithItemIsNowPlaying:(bool)arg1 buttonIsOverlayed:(bool)arg2;
- (void)_hideOverlays;
- (void)_informVKCOverlayToKickOffSubjectAnalyzing;
- (unsigned long long)_initialActivity;
- (long long)_insetModeForAsset:(id)arg1;
- (id)_interactivePinchDismissalController;
- (id)_interactiveSwipeDismissalController;
- (void)_invalidateAccessoryView;
- (void)_invalidateAssetActionManager;
- (void)_invalidateBarsControllers;
- (void)_invalidateCachedBoopableItemProvider;
- (void)_invalidateChromeViewController;
- (void)_invalidateEditMode;
- (void)_invalidateImageAnalysisInteractionAdditionalActionInfoEdgeInsets;
- (void)_invalidateInfoPanelLayoutAnimated:(bool)arg1;
- (void)_invalidateLayout;
- (void)_invalidatePreferredContentSize;
- (void)_invalidatePreloadInsets;
- (void)_invalidateSpec;
- (void)_invalidateTipPopovers;
- (void)_invalidateUserActivity;
- (void)_invalidateVideoPlayer;
- (bool)_isAccessoryAvailableForCurrentAsset;
- (bool)_isAccessoryVisible;
- (bool)_isAssetEligibleForTip:(id)arg1;
- (bool)_isCameraDetailsAccessory;
- (bool)_isCurrentAssetAndInEditModeForAssetReference:(id)arg1;
- (bool)_isDetailsAccessoryAvailableForAssetReference:(id)arg1;
- (bool)_isDrivingScrubbing;
- (bool)_isIrisPlaying;
- (bool)_isLayoutAffectedBySpecChange:(id)arg1;
- (bool)_isLocationFromProvider:(id)arg1 overTileOfKind:(id)arg2;
- (bool)_isLocationFromProvider:(id)arg1 overTileOfKind:(id)arg2 insetBy:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg3;
- (bool)_isLocationFromProviderInAccessoryArea:(id)arg1;
- (bool)_isLocationFromProviderOverActionInfoItem:(id)arg1;
- (bool)_isLocationFromProviderOverControl:(id)arg1;
- (bool)_isLocationFromProviderOverImageSubject:(id)arg1;
- (bool)_isLocationFromProviderOverInteractiveItem:(id)arg1;
- (bool)_isLocationFromProviderOverInteractiveItemExcludingImageSubject:(id)arg1;
- (bool)_isLocationFromProviderOverSyndicationAttributionPill:(id)arg1;
- (bool)_isLocationFromProviderOverVisualSearch:(id)arg1;
- (void)_isPresentedForPreviewDidChange;
- (bool)_isPresentedForSecondScreen;
- (bool)_isSecondScreenBrowserVisible;
- (bool)_isTipPopoverVisible;
- (bool)_isVisualImageHasSubjectLiftedFromProvider:(id)arg1;
- (void)_keyboardWillShow:(id)arg1;
- (struct CGSize { double x1; double x2; })_layoutReferenceSize;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })_layoutSafeAreaInsets;
- (id)_leadingContentTileController;
- (id)_livePhotoBadge;
- (id)_mainOneUpForSecondScreenBrowser;
- (bool)_needsUpdate;
- (bool)_needsUpdateAccessoryView;
- (bool)_needsUpdateBarsController;
- (bool)_needsUpdateEditMode;
- (bool)_needsUpdateImageAnalysisInteractionAdditionalActionInfoEdgeInsets;
- (bool)_needsUpdateLayout;
- (bool)_needsUpdatePreferredContentSize;
- (bool)_needsUpdatePreloadInsets;
- (bool)_needsUpdateSpec;
- (bool)_needsUpdateTipsPopover;
- (bool)_needsUpdateUserActivity;
- (bool)_needsUpdateVideoPlayer;
- (id)_newOneUpLayout;
- (void)_notifyTileIsReadyWithTimeOut:(double)arg1 completionHandler:(id /* block */)arg2;
- (unsigned long long)_options;
- (id)_overOneUpPresentationSession;
- (id)_parallaxComputer;
- (id)_passthroughViews;
- (struct CGSize { double x1; double x2; })_pendingViewTransitionSize;
- (void)_performNavigationToAssetDisplayDescriptor:(id)arg1 beforeDate:(id)arg2 completionHandler:(id /* block */)arg3;
- (bool)_prefersCompactLayoutForSplitScreen;
- (bool)_prefersFloatingInfoPanel;
- (bool)_prefersHomeIndicatorHidden;
- (void)_preheatAdjacentAssetsForPhotosDetailsContext;
- (double)_preloadInsetsBasedOffViewWidth;
- (void)_presentAlertForUnplayableAssetReference:(id)arg1;
- (void)_presentFloatingInfoPanelAnimated:(bool)arg1;
- (id)_presentedTileControllerForCurrentIndexPathWithKind:(id)arg1;
- (id)_presentedTileControllerForIndexPath:(id)arg1 kind:(id)arg2;
- (bool)_requireUnlockedDeviceForAccessoryView;
- (id)_reviewScreenControlBarTileViewController;
- (id)_reviewScreenScrubberBarTileViewController;
- (id)_reviewScreenTopBarTileViewController;
- (void)_scheduleTimedChromeAutoHide;
- (id)_secondScreenBrowser;
- (void)_setAccessoryVisible:(bool)arg1;
- (void)_setAccessoryVisible:(bool)arg1 changeReason:(long long)arg2;
- (void)_setBackgroundTileViewController:(id)arg1;
- (void)_setChromeAutoHideTimer:(id)arg1;
- (void)_setContentUnavailableConfiguration:(id)arg1;
- (void)_setDidPlayCurrentLivePhoto:(bool)arg1;
- (void)_setHasShownRelevantTip:(bool)arg1;
- (void)_setHiddenTilesController:(id)arg1;
- (void)_setInitialActivity:(unsigned long long)arg1;
- (void)_setIrisPlaying:(bool)arg1;
- (void)_setLayoutReferenceSize:(struct CGSize { double x1; double x2; })arg1;
- (void)_setLayoutSafeAreaInsets:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)_setMainOneUpForSecondScreenBrowser:(id)arg1;
- (void)_setNeedsUpdateAccessoryView:(bool)arg1;
- (void)_setNeedsUpdateBarsController:(bool)arg1;
- (void)_setNeedsUpdateEditMode:(bool)arg1;
- (void)_setNeedsUpdateImageAnalysisInteractionAdditionalActionInfoEdgeInsets:(bool)arg1;
- (void)_setNeedsUpdateLayout:(bool)arg1;
- (void)_setNeedsUpdatePreferredContentSize:(bool)arg1;
- (void)_setNeedsUpdatePreloadInsets:(bool)arg1;
- (void)_setNeedsUpdateSpec:(bool)arg1;
- (void)_setNeedsUpdateTipPopovers:(bool)arg1;
- (void)_setNeedsUpdateUserActivity:(bool)arg1;
- (void)_setNeedsUpdateVideoPlayer:(bool)arg1;
- (void)_setOverOneUpPresentationSession:(id)arg1;
- (void)_setPendingViewTransitionSize:(struct CGSize { double x1; double x2; })arg1;
- (void)_setPrefersHomeIndicatorHidden:(bool)arg1;
- (void)_setPreloadInsetsBasedOffViewWidth:(double)arg1;
- (void)_setReviewScreenControlBarTileViewController:(id)arg1;
- (void)_setReviewScreenScrubberBarTileViewController:(id)arg1;
- (void)_setReviewScreenTopBarTileViewController:(id)arg1;
- (void)_setSecondScreenBrowser:(id)arg1;
- (void)_setShouldDisableTransitionsUntilAppeared:(bool)arg1;
- (void)_setShouldShowAccessoryAfterUnlock:(bool)arg1;
- (void)_setTileKindsToHide:(id)arg1;
- (void)_setTilingView:(id)arg1;
- (void)_setWindowInterfaceOrientation:(long long)arg1;
- (bool)_shouldDisableTransitionsUntilAppeared;
- (bool)_shouldMakeChromeVisibleWhenVideoPlayerPlaysToEndTime:(id)arg1;
- (bool)_shouldShowAccessoryAfterUnlock;
- (id)_spec;
- (id)_syndicationPillView;
- (id)_tileKindsToHide;
- (id)_tilingView;
- (void)_toggleAccessoryVisibility;
- (void)_toggleCommentsVisibility;
- (void)_toggleDetailsVisibility;
- (id)_trailingContentTileController;
- (void)_unhideOverlays;
- (void)_updateAccessoryViewIfNeeded;
- (void)_updateActiveVideoTilesCanDisplayVideo;
- (void)_updateBackgroundTileViewController;
- (void)_updateBarsControllerIfNeeded;
- (void)_updateChromeViewControllerDismissalProgress;
- (void)_updateChromeViewControllerIfNeeded;
- (void)_updateContentUnavailableConfigurationUsingState:(id)arg1;
- (void)_updateEditModeIfNeeded;
- (void)_updateFloatingInfoPanel;
- (void)_updateFloatingInfoPanelIfNeeded;
- (void)_updateGestureController;
- (void)_updateIfNeeded;
- (void)_updateImageAnalysisInteractionAdditionalActionInfoEdgeInsetsIfNeeded;
- (void)_updateImageAnalysisInteractionDelegate;
- (void)_updateLayout:(id)arg1;
- (void)_updateLayoutIfNeeded;
- (void)_updateLayoutReferenceSize;
- (void)_updateMergedVideoProviderEnabled;
- (void)_updatePreferredContentSizeIfNeeded;
- (void)_updatePrefersHomeIndicatorHidden;
- (void)_updatePreloadInsetsIfNeeded;
- (void)_updateReviewScreenBars;
- (void)_updateSpecIfNeeded;
- (void)_updateTipPopoversIfNeeded;
- (void)_updateUserActivityIfNeeded;
- (void)_updateVideoPlayerIfNeeded;
- (void)_updateViewModelWithCurrentScrollPosition;
- (void)_updateWindowInterfaceOrientation;
- (id)_userTransformTileControllerAtLocationFromProvider:(id)arg1;
- (id)_userTransformTileViewControllerForCurrentAsset;
- (id)_videoPlayerAtIndexPath:(id)arg1 layout:(id)arg2;
- (bool)_visualImageHasActiveTextSelectionFromProvider:(id)arg1;
- (bool)_vkImageOverlayAllowsVKCToHandleLongPressGestureFromProvider:(id)arg1;
- (bool)_wantsShowInLibraryButtonForAllAssets;
- (void)_willBeginTransition;
- (long long)_windowInterfaceOrientation;
- (void)_xct_beginSignpostingForImageTileImageLoadingAndProcessing;
- (void)_xct_endSignpostingForImageDeferredProcessing;
- (void)_xct_endSignpostingForImageFullQuality;
- (void)_xct_endSignpostingForImageTileImageLoadingAndProcessing;
- (id)accessoryTileViewControllerAccessoryViewController:(id)arg1;
- (id)accessoryTileViewControllerHostViewController:(id)arg1;
- (bool)accessoryTileViewControllerShouldUseSquareImageWhenShowingAccessoryView:(id)arg1;
- (id)accessoryTileViewControllerViewHostingGestureRecognizers:(id)arg1;
- (bool)accessoryTileViewControllerWillHideNavigationBarWhenShowingAccessoryView:(id)arg1;
- (bool)accessoryTileViewControllerWillHideToolbarWhenShowingAccessoryView:(id)arg1;
- (bool)accessoryVisibilityInteractionController:(id)arg1 canBeginAtLocationFromProvider:(id)arg2;
- (bool)accessoryVisibilityInteractionController:(id)arg1 isLocationFromProviderInAccessoryContent:(id)arg2;
- (void)accessoryVisibilityInteractionController:(id)arg1 setAccessoryVisible:(bool)arg2 changeReason:(long long)arg3;
- (void)accessoryVisibilityInteractionControllerDidEnd:(id)arg1;
- (id)accessoryVisibilityInteractionControllerViewHostingGestureRecognizers:(id)arg1;
- (id)actionsController;
- (id)activeVideoTileControllers;
- (id)activityItemsConfiguration;
- (void)addAssetsToAlbum:(id)arg1;
- (void)addAssetsToLastUsedAlbum:(id)arg1;
- (id)additionalContextMenuActionsForAttributionTileViewController:(id)arg1;
- (void)airPlayControllerScreenAvailabilityChanged:(id)arg1;
- (bool)allowsPreviewActions;
- (double)alphaForReviewScreenBarsInLayout:(id)arg1;
- (bool)analysisInteractionAllowedForImageAnalysisInteraction:(id)arg1;
- (id)appIntentsDebugDescription;
- (void)appIntentsEnumerateAssetCollections:(id /* block */)arg1 completionHandler:(id /* block */)arg2;
- (void)appIntentsEnumerateAssets:(id /* block */)arg1 completionHandler:(id /* block */)arg2;
- (int)appearState;
- (bool)appearanceTransitionAnimationsDisabled;
- (id)assetActionManager;
- (id)assetUUIDsAllowedToHighlightText;
- (id)authenticationTitle;
- (void)autoEnhanceAssets:(id)arg1;
- (void)badgeTileViewController:(id)arg1 presentViewController:(id)arg2;
- (void)badgeVisibilityDidChange:(id)arg1;
- (id)barsController:(id)arg1 delegateForGestureRecognizer:(id)arg2;
- (void)barsControllerContentGuideInsetsDidChange:(id)arg1;
- (id)barsControllerViewHostingGestureRecognizers:(id)arg1;
- (id)boopableItemsProvider;
- (id)browsingOneUpVisibilityHelper;
- (id)browsingSession;
- (id)browsingTileViewAnimator;
- (bool)canBecomeFirstResponder;
- (bool)canPerformAction:(SEL)arg1 withSender:(id)arg2;
- (bool)canPerformAuthentication;
- (id)cardPresentationController;
- (id)childViewControllerForStatusBarHidden;
- (id)chromeView;
- (id)chromeViewController;
- (id)contentContainerView;
- (id)contentScrollView;
- (id)contentViewControllerForAirPlayController:(id)arg1;
- (id)contentViewForImageAnalysisInteraction:(id)arg1;
- (void)copy:(id)arg1;
- (void)copyAdjustments:(id)arg1;
- (void)copyAdjustmentsSkippingOptions:(id)arg1;
- (id)createAssetTransitionInfo;
- (void)createMenuActionControllerForManagerIfNeeded:(id)arg1 withPresentingViewController:(id)arg2 regionOfInterestProvider:(id)arg3;
- (void)cropButtonTileViewController:(id)arg1 buttonVisibilityDidChange:(bool)arg2;
- (void)cropButtonTileViewController:(id)arg1 didSelectAspectRatio:(struct CGSize { double x1; double x2; })arg2;
- (id)currentFloatingInfoPanelAccessoryViewController;
- (id)currentImageAnalysisInteraction;
- (void)dealloc;
- (void)deleteAssets:(id)arg1;
- (long long)detailsBecameVisibleSignPost;
- (void)dismissPresentedViewController:(id)arg1 presentMenuWithCompletion:(id /* block */)arg2;
- (bool)doubleTapZoomController:(id)arg1 canDoubleTapBeginAtLocationFromProvider:(id)arg2;
- (id)doubleTapZoomController:(id)arg1 delegateForGestureRecognizer:(id)arg2;
- (id)doubleTapZoomControllerTilingView:(id)arg1;
- (id)doubleTapZoomControllerViewHostingGestureRecognizers:(id)arg1;
- (void)duplicateAssets:(id)arg1;
- (id /* block */)editActivityCompletion;
- (id)eventTracker;
- (void)favoriteAssets:(id)arg1;
- (id)feedbackController;
- (id)gestureController;
- (void)hideAssets:(id)arg1;
- (id)hostViewControllerForActionPerformer:(id)arg1;
- (void)imageAnalysisInteractionIsDrivingLivePhotoPlaybackDidChange:(id)arg1;
- (bool)imageAnalysisInteractionShouldHighlightSubjectWhenAccessoryIsVisible:(id)arg1;
- (void)imageAnalysisInteractionSubjectInteractionInProgressDidChange:(id)arg1;
- (void)imageTileViewController:(id)arg1 didDisplayFullQualityImageWithDeferredProcessingFinished:(bool)arg2;
- (id)initWithBrowsingSession:(id)arg1;
- (id)initWithBrowsingSession:(id)arg1 options:(unsigned long long)arg2;
- (id)initWithBrowsingSession:(id)arg1 options:(unsigned long long)arg2 initialActivity:(unsigned long long)arg3;
- (id)initWithBrowsingSession:(id)arg1 options:(unsigned long long)arg2 initialActivity:(unsigned long long)arg3 presentationOrigin:(long long)arg4;
- (id)initWithCoder:(id)arg1;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2;
- (bool)interactiveDismissalController:(id)arg1 canBeginDismissalAtLocationFromProvider:(id)arg2;
- (void)interactiveDismissalControllerChangedDismissalInteractionProgress:(id)arg1;
- (void)interactiveDismissalControllerChangedIsHandlingDismissalInteraction:(id)arg1;
- (id)interactiveDismissalControllerDesignatedTileController:(id)arg1;
- (id)interactiveDismissalControllerTilingView:(id)arg1;
- (id)interactiveDismissalControllerViewController:(id)arg1;
- (id)interactiveDismissalControllerViewHostingGestureRecognizers:(id)arg1;
- (id)irisImageTileViewController:(id)arg1 delegateForGestureRecognizer:(id)arg2;
- (void)irisImageTileViewControllerDidBeginHinting:(id)arg1;
- (void)irisImageTileViewControllerDidBeginPlaying:(id)arg1;
- (void)irisImageTileViewControllerDidEndPlaying:(id)arg1;
- (void)irisImageTileViewControllerDidEndVitality:(id)arg1;
- (id)irisImageTileViewControllerViewHostingGestureRecognizers:(id)arg1;
- (bool)isFloatingInfoPanelPresented;
- (id)keyCommands;
- (id)lastLivePhotoGestureLocationProvider;
- (struct CGPoint { double x1; double x2; })layout:(id)arg1 accessoryOffsetForItemAtIndexPath:(id)arg2;
- (double)layout:(id)arg1 aspectRatioForItemAtIndexPath:(id)arg2;
- (struct CGSize { double x1; double x2; })layout:(id)arg1 assetExplorerReviewScreenBadgeSizeForItemAtIndexPath:(id)arg2;
- (struct CGSize { double x1; double x2; })layout:(id)arg1 badgeSizeForItemAtIndexPath:(id)arg2 contentWidth:(double)arg3;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })layout:(id)arg1 bestSquareRectForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 canApplyInsetStylingToItemAtIndexPath:(id)arg2;
- (struct CGPoint { double x1; double x2; })layout:(id)arg1 contentOffsetForItemAtIndexPath:(id)arg2;
- (long long)layout:(id)arg1 insetModeForItemAtIndexPath:(id)arg2;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })layout:(id)arg1 insetsWhenScaledToFitForItemAtIndexPath:(id)arg2 inViewportSize:(struct CGSize { double x1; double x2; })arg3;
- (bool)layout:(id)arg1 isShowingInfoPanelForItemAtIndexPath:(id)arg2;
- (double)layout:(id)arg1 leadingBadgesWidthForItemAtIndexPath:(id)arg2 contentWidth:(double)arg3;
- (struct CGSize { double x1; double x2; })layout:(id)arg1 loadingIndicatorSizeForItemAtIndexPath:(id)arg2;
- (double)layout:(id)arg1 minimumItemHeightNeededInAccessoryForItemAtIndexPath:(id)arg2;
- (double)layout:(id)arg1 minimumVisibleAccessoryHeightForItemAtIndexPath:(id)arg2;
- (double)layout:(id)arg1 minimumVisibleContentHeightForItemAtIndexPath:(id)arg2;
- (id)layout:(id)arg1 modelTileTransformForItemAtIndexPath:(id)arg2;
- (struct CGSize { double x1; double x2; })layout:(id)arg1 pixelSizeForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldInitiallyZoomToFillForItemAtIndexPath:(id)arg2 contentSize:(struct CGSize { double x1; double x2; })arg3 viewportSize:(struct CGSize { double x1; double x2; })arg4;
- (bool)layout:(id)arg1 shouldMovePeopleRowForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldMoveProgressIndicatorForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowAccessoryForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowAttributionViewForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowBufferingIndicatorForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowCropButtonForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowPeopleRowForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowPlayButtonForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldShowVideoPlaceholderForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 shouldUseSquareImageInAccessoryForItemAtIndexPath:(id)arg2;
- (bool)layout:(id)arg1 wantsContentLegibilityTreatmentForItemAtIndexPath:(id)arg2;
- (void)layout:(id)arg1 willApplyDisplayTileTransform:(id)arg2 forItemAtIndexPath:(id)arg3;
- (bool)layoutShouldShowReviewScreenScrubberBar:(id)arg1;
- (bool)livePhotoVideoPlaybackTileViewControllerCanBeginPlaying:(id)arg1 locationProvider:(id)arg2;
- (struct CGImage { }*)livePhotoVideoPlaybackTileViewControllerCurrentGainMapImage:(id)arg1 outGainMapAvailable:(bool*)arg2 outGainMapValue:(float*)arg3;
- (id)livePhotoVideoPlaybackTileViewControllerCurrentImage:(id)arg1;
- (void)livePhotoVideoPlaybackTileViewControllerDidEndPlaying:(id)arg1;
- (id)livePhotoVideoPlaybackTileViewControllerDisplayTileTransform:(id)arg1;
- (double)livePhotoVideoPlaybackTileViewControllerExtraMinimumTouchDuration:(id)arg1 locationProvider:(id)arg2;
- (void)livePhotoVideoPlaybackTileViewControllerWillBeginPlaying:(id)arg1;
- (void)loadView;
- (id)loadingIndicatorController;
- (void)loadingStatusManager:(id)arg1 didUpdateLoadingStatus:(id)arg2 forItemIdentifier:(id)arg3;
- (id)menuActionController;
- (bool)menuActionControllerPreventRevealInMomentAction:(id)arg1;
- (void)navigateToAssetDisplayDescriptor:(id)arg1 beforeDate:(id)arg2 completionHandler:(id /* block */)arg3;
- (void)navigateToDestination:(id)arg1 options:(unsigned long long)arg2 completionHandler:(id /* block */)arg3;
- (bool)needsUpdateChromeViewController;
- (bool)oneUpAccessoryViewControllersManager:(id)arg1 preventAlbumAttributionWidgetForAssetReference:(id)arg2;
- (bool)oneUpAccessoryViewControllersManager:(id)arg1 preventRevealInMomentActionForAssetReference:(id)arg2;
- (void)oneUpAccessoryViewControllersManagerInvalidateAccessoryLayout:(id)arg1;
- (bool)oneUpAccessoryViewControllersManagerRequestAccessoryDismissal:(id)arg1;
- (void)oneUpActionsController:(id)arg1 zoomAtLocationProvider:(id)arg2;
- (double)oneUpActionsControllerBottomPaddingForPresentingToast:(id)arg1;
- (bool)oneUpActionsControllerDismissPresentedViewController:(id)arg1;
- (id)oneUpActionsControllerHostViewControllerForPresentingToast:(id)arg1;
- (bool)oneUpActionsControllerIsDetailsAccessoryAvailable:(id)arg1;
- (bool)oneUpActionsControllerPrefersInclusionAfterRemoval:(id)arg1 forActionType:(unsigned long long)arg2;
- (bool)oneUpActionsControllerShouldAnimateViewControllerPresentation:(id)arg1;
- (void)oneUpActionsControllerToggleChromeVisibility:(id)arg1;
- (void)oneUpActionsControllerToggleCommentsVisibility:(id)arg1;
- (void)oneUpActionsControllerToggleDetailsVisibility:(id)arg1;
- (void)oneUpAssetTransition:(id)arg1 animateTransitionWithContext:(id)arg2 duration:(double)arg3 completion:(id /* block */)arg4;
- (void)oneUpAssetTransition:(id)arg1 requestTransitionContextWithCompletion:(id /* block */)arg2;
- (void)oneUpAssetTransition:(id)arg1 requestTransitionContextWithCompletion:(id /* block */)arg2 options:(unsigned long long)arg3;
- (void)oneUpAssetTransitionDidEnd:(id)arg1;
- (void)oneUpAssetTransitionWillBegin:(id)arg1;
- (bool)oneUpBarsController:(id)arg1 canShowCommentsForAsset:(id)arg2;
- (void)oneUpBarsController:(id)arg1 didTapPlayPauseButton:(bool)arg2;
- (bool)oneUpBarsController:(id)arg1 isDetailsAccessoryAvailableForAssetReference:(id)arg2;
- (bool)oneUpBarsController:(id)arg1 shouldHideNavigationBarWhenShowingAccessoryViewForAssetReference:(id)arg2;
- (bool)oneUpBarsController:(id)arg1 shouldHideStatusBarWhenShowingAccessoryViewForAssetReference:(id)arg2;
- (bool)oneUpBarsController:(id)arg1 shouldHideToolbarWhenShowingAccessoryViewForAssetReference:(id)arg2;
- (bool)oneUpBarsController:(id)arg1 shouldTapBeginAtLocationFromProvider:(id)arg2;
- (id)oneUpBarsControllerAssetActionManager:(id)arg1;
- (void)oneUpBarsControllerDidChangeShowingPlayPauseButton:(id)arg1;
- (void)oneUpBarsControllerDidTapTitle:(id)arg1;
- (void)oneUpBarsControllerToggleCommentsVisibility;
- (void)oneUpBarsControllerToggleDetailsVisibility;
- (bool)oneUpBarsControllerWantsActionsMenu:(id)arg1;
- (void)oneUpBarsControllerWillPresentActionsMenu:(id)arg1 completionHandler:(id /* block */)arg2;
- (bool)oneUpChromeViewControllerAccessoryIsFloatingWhenPresented:(id)arg1;
- (void)oneUpChromeViewControllerAdditionalActionInfoEdgeInsetsDidChange:(id)arg1;
- (id)oneUpChromeViewControllerAssetActionManager:(id)arg1;
- (void)oneUpChromeViewControllerBackgroundColorOverrideDidChange:(id)arg1;
- (void)oneUpChromeViewControllerContentDecorationAdditionalInsetsDidChange:(id)arg1;
- (void)oneUpChromeViewControllerContentGuideInsetsDidChange:(id)arg1;
- (id)oneUpChromeViewControllerExcludedMenuActionTypes:(id)arg1;
- (id)oneUpChromeViewControllerFullSizeOneUpHostingView:(id)arg1;
- (bool)oneUpChromeViewControllerIsDeviceUnlocked:(id)arg1;
- (void)oneUpChromeViewControllerLegibilityInsetsDidChange:(id)arg1;
- (void)oneUpChromeViewControllerRequestDeviceUnlock:(id)arg1 for:(unsigned long long)arg2;
- (id)oneUpDefaultActionsControllerAssetActionManager:(id)arg1;
- (bool)oneUpGestureController:(id)arg1 canBeginGestureAt:(id)arg2;
- (bool)oneUpGestureController:(id)arg1 canDoubleTapToZoomAtPoint:(id)arg2;
- (bool)oneUpGestureController:(id)arg1 canPerformSelectionAtPoint:(id)arg2;
- (bool)oneUpGestureController:(id)arg1 canTapToToggleChromeAtPoint:(id)arg2;
- (bool)oneUpGestureRecognizerCoordinator:(id)arg1 shouldAllowIrisGestureAtLocationFromProvider:(id)arg2;
- (id)oneUpGestureRecognizerCoordinatorViewHostingTouchingGesture:(id)arg1;
- (void)overOneUpPresentationSession:(id)arg1 didPresent:(id)arg2;
- (id)overOneUpPresentationSessionBrowserViewModel:(id)arg1;
- (id)overOneUpPresentationSessionCreateIfNeeded:(bool)arg1;
- (id)overOneUpPresentationSessionCurrentTileController:(id)arg1;
- (void)overOneUpPresentationSessionDidFinish:(id)arg1;
- (bool)overOneUpPresentationSessionPresentingViewControllerIsReady:(id)arg1;
- (id)overOneUpPresentationSessionTilingView:(id)arg1;
- (id)overOneUpPresentationSessionViewController:(id)arg1;
- (id)passcodeAuthenticationReason;
- (void)pasteAdjustments:(id)arg1;
- (double)playButtonTileViewController:(id)arg1 delayForButtonAnimation:(bool)arg2;
- (void)playButtonTileViewControllerDidTapButton:(id)arg1;
- (bool)playButtonTileViewControllerShouldShowPauseButton:(id)arg1;
- (bool)playButtonTileViewControllerShouldShowPlayButtonWhileActivated:(id)arg1;
- (id)ppt_actionsController;
- (id)ppt_assetUUIDNeedsDeferredProcessingFinishNotification;
- (id)ppt_barsController;
- (id)ppt_currentAccessoryViewController;
- (id /* block */)ppt_deferredProcessingCompletionBlock;
- (id)ppt_deferredProcessingWaitTimer;
- (bool)ppt_imageTileViewControllerShouldSignalDeferredProcessingDoneAndFinalImageDisplayed:(id)arg1;
- (id)ppt_mainScrollView;
- (void)ppt_notifyWhenDeferredProcessingIsDoneAndFinalImageDisplayedWithTimeout:(double)arg1 handler:(id /* block */)arg2;
- (void)ppt_notifyWhenFullQualityIsDisplayedWithTimeout:(double)arg1 handler:(id /* block */)arg2;
- (void)ppt_notifyWhenVKAnalysisIsReadyWithTimeout:(double)arg1 handler:(id /* block */)arg2;
- (id)ppt_overOneUpPresentationSession;
- (void)ppt_playCurrentLivePhotoWithCompletionHandler:(id /* block */)arg1;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })ppt_scrollableBoundsForTesting;
- (id)ppt_scrubberView;
- (void)ppt_toggleAccessoryView;
- (void)preferencesDidChange;
- (void)preferredContentSizeDidChangeForChildContentContainer:(id)arg1;
- (struct CGSize { double x1; double x2; })preferredContentSizeOverride;
- (long long)preferredStatusBarUpdateAnimation;
- (bool)prefersHomeIndicatorAutoHidden;
- (bool)prefersStatusBarHidden;
- (id)preheatAssetsQueue;
- (bool)prepareForDismissingForced:(bool)arg1;
- (void)prepareTipPopover:(id)arg1 tipID:(id)arg2;
- (void)presentErrorFromLoadingIndicator:(id)arg1;
- (void)presentErrorFromVideoPlaybackAttempt:(id)arg1;
- (void)presentPeopleViewController:(id)arg1;
- (void)presentationController:(id)arg1 dismissTappedForViewController:(id)arg2;
- (id)presentationControllerForTipID:(id)arg1;
- (id)presentationEnvironmentForActionPerformer:(id)arg1;
- (long long)presentationOrigin;
- (id)presentedTipID;
- (id)presentingViewControllerForImageAnalysisInteraction:(id)arg1;
- (void)printAssets:(id)arg1;
- (id)pu_debugCurrentAsset;
- (id)pu_debugCurrentPrivacyController;
- (id)pu_debugCurrentViewModel;
- (id)pu_debugRows;
- (long long)pu_preferredBarStyle;
- (bool)pu_shouldOptOutFromChromelessBars;
- (bool)pu_wantsNavigationBarVisible;
- (bool)pu_wantsTabBarVisible;
- (bool)pu_wantsToolbarVisible;
- (void)purgeIfPossible;
- (void)pushPeopleViewController:(id)arg1;
- (double)px_HDRFocus;
- (bool)px_determinesPreferredStatusBarHidden;
- (id)px_diagnosticsItemProvidersForPoint:(struct CGPoint { double x1; double x2; })arg1 inCoordinateSpace:(id)arg2;
- (double)px_imageModulationIntensity;
- (void)restoreAssets:(id)arg1;
- (void)restoreAssetsWithoutConfirmation:(id)arg1;
- (void)revertAdjustments:(id)arg1;
- (void)rotateAssetsClockwise:(id)arg1;
- (void)rotateAssetsCounterclockwise:(id)arg1;
- (unsigned long long)routingOptionsForDestination:(id)arg1;
- (void)scrollViewDidEndDecelerating:(id)arg1;
- (void)scrollViewDidEndDragging:(id)arg1 willDecelerate:(bool)arg2;
- (void)scrollViewDidScroll:(id)arg1;
- (void)scrollViewWillBeginDragging:(id)arg1;
- (void)scrollViewWillEndDragging:(id)arg1 withVelocity:(struct CGPoint { double x1; double x2; })arg2 targetContentOffset:(inout struct CGPoint { double x1; double x2; }*)arg3;
- (void)setAllowsPreviewActions:(bool)arg1;
- (void)setAppearState:(int)arg1;
- (void)setAppearanceTransitionAnimationsDisabled:(bool)arg1;
- (void)setAssetUUIDsAllowedToHighlightText:(id)arg1;
- (void)setBoopableItemsProvider:(id)arg1;
- (void)setBrowsingOneUpVisibilityHelper:(id)arg1;
- (void)setCardPresentationController:(id)arg1;
- (void)setCurrentFloatingInfoPanelAccessoryViewController:(id)arg1;
- (void)setDetailsBecameVisibleSignPost:(long long)arg1;
- (void)setEditActivityCompletion:(id /* block */)arg1;
- (void)setLastLivePhotoGestureLocationProvider:(id)arg1;
- (void)setNeedsUpdateChromeViewController:(bool)arg1;
- (void)setPpt_assetUUIDNeedsDeferredProcessingFinishNotification:(id)arg1;
- (void)setPpt_deferredProcessingCompletionBlock:(id /* block */)arg1;
- (void)setPpt_deferredProcessingWaitTimer:(id)arg1;
- (void)setPreferredContentSizeOverride:(struct CGSize { double x1; double x2; })arg1;
- (void)setPresentedTipID:(id)arg1;
- (void)setSiriActionActivity:(id)arg1;
- (void)setTextsToHighlight:(id)arg1;
- (void)setTipDismissedCompletion:(id /* block */)arg1;
- (void)setTouchingGestureRecognizerStateBeginID:(unsigned long long)arg1;
- (void)setUnlockDeviceHandlerWithActionType:(id /* block */)arg1;
- (void)setUnlockDeviceStatus:(id /* block */)arg1;
- (void)setUuidOfAssetPresentedInFloatingInfoPanel:(id)arg1;
- (void)setWasChromeVisibleBeforeZoomIn:(bool)arg1;
- (void)setXct_waitForDeferredProcessingTimer:(id)arg1;
- (void)setXct_waitForFullQualityImageTimer:(id)arg1;
- (void)settings:(id)arg1 changedValueForKey:(id)arg2;
- (void)shareAssets:(id)arg1;
- (void)showDetailsViewForImageAnalysisInteraction:(id)arg1;
- (id)siriActionActivity;
- (id)sourceItemForTipID:(id)arg1;
- (id)textsToHighlight;
- (id)textsToHighlightForImageAnalysisInteraction:(id)arg1;
- (id)tilingView:(id)arg1 dataSourceConverterForTransitionFromLayout:(id)arg2 toLayout:(id)arg3;
- (struct CGPoint { double x1; double x2; })tilingView:(id)arg1 initialVisibleOriginWithLayout:(id)arg2;
- (id)tilingView:(id)arg1 scrollInfoWithLayout:(id)arg2;
- (struct CGPoint { double x1; double x2; })tilingView:(id)arg1 targetVisibleOriginForProposedVisibleOrigin:(struct CGPoint { double x1; double x2; })arg2 withLayout:(id)arg3;
- (id)tilingView:(id)arg1 tileControllerWithIndexPath:(id)arg2 kind:(id)arg3 dataSource:(id)arg4;
- (id)tilingView:(id)arg1 tileTransitionCoordinatorForChangeFromFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg2 toFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg3 duration:(double)arg4;
- (id)tilingView:(id)arg1 tileTransitionCoordinatorForLayoutInvalidationContext:(id)arg2;
- (id)tilingView:(id)arg1 tileTransitionCoordinatorForReattachedTileControllers:(id)arg2 context:(id)arg3;
- (id)tilingView:(id)arg1 tileTransitionCoordinatorForTransitionFromLayout:(id)arg2 toLayout:(id)arg3 withContext:(id)arg4;
- (bool)tilingViewCanBypass20069585Check:(id)arg1;
- (void)tilingViewControllerTransition:(id)arg1 abandonTilingView:(id)arg2 toEndPoint:(id)arg3;
- (void)tilingViewControllerTransition:(id)arg1 adoptTilingView:(id)arg2 fromEndPoint:(id)arg3 isCancelingTransition:(bool)arg4 animationSetupCompletionHandler:(id /* block */)arg5;
- (id)tilingViewControllerTransition:(id)arg1 tilingViewToTransferToEndPoint:(id)arg2;
- (id)tilingViewControllerTransitionEndPointHostViewController;
- (double)tilingViewControllerTransitionProgressAtWhichChromeIsFullyFadedOut:(id)arg1;
- (id)tilingViewControllerTransitionTilingViewHostView:(id)arg1;
- (id /* block */)tipDismissedCompletion;
- (void)tipPopoverDidDismissWithTipID:(id)arg1;
- (void)tipPopoverDidPresentWithTipID:(id)arg1;
- (void)toggleEditor:(id)arg1;
- (void)toggleInspector:(id)arg1;
- (void)togglePlayback:(id)arg1;
- (void)toggleViewer:(id)arg1;
- (unsigned long long)touchingGestureRecognizerStateBeginID;
- (void)traitCollectionDidChange:(id)arg1;
- (void)trashAssets:(id)arg1;
- (void)trashAssetsWithoutConfirmation:(id)arg1;
- (id)undoManagerForActionPerformer:(id)arg1;
- (id /* block */)unlockDeviceHandlerWithActionType;
- (id /* block */)unlockDeviceStatus;
- (void)userTransformTileViewController:(id)arg1 didChangeIsUserInteracting:(bool)arg2;
- (long long)userTransformTileViewController:(id)arg1 insetModeForItemAtIndexPath:(id)arg2;
- (bool)userTransformTileViewController:(id)arg1 shouldReceiveTouchAtLocationFromProvider:(id)arg2;
- (id)uuidOfAssetPresentedInFloatingInfoPanel;
- (void)validateCommand:(id)arg1;
- (bool)videoTilesCanPlayVideo;
- (void)viewControllerSpec:(id)arg1 didChange:(id)arg2;
- (void)viewDidAppear:(bool)arg1;
- (void)viewDidDisappear:(bool)arg1;
- (void)viewDidLoad;
- (void)viewDidMoveToWindow:(id)arg1 shouldAppearOrDisappear:(bool)arg2;
- (void)viewModel:(id)arg1 didChange:(id)arg2;
- (void)viewSafeAreaInsetsDidChange;
- (void)viewWillAppear:(bool)arg1;
- (void)viewWillDisappear:(bool)arg1;
- (void)viewWillLayoutSubviews;
- (void)viewWillTransitionToSize:(struct CGSize { double x1; double x2; })arg1 withTransitionCoordinator:(id)arg2;
- (id)visualImageAnalyzingController;
- (bool)wantsActionsMenu;
- (bool)wantsContentVisibleAfterUnregisteringWithAirPlayController:(id)arg1;
- (bool)wantsSpotlightStyling;
- (bool)wasChromeVisibleBeforeZoomIn;
- (bool)xct_isCurrentImageDeferredProcessingFinished;
- (bool)xct_isCurrentImageFullQuality;
- (id)xct_waitForDeferredProcessingTimer;
- (id)xct_waitForFullQualityImageTimer;

@end
