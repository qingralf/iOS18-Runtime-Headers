/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/Portrait.framework/Portrait
 */

@interface PTRenderRequest : NSObject <NSCopying> {
    int  _AGC;
    float  _alphaLowLight;
    <MTLTexture> * _colorCube;
    int  _conversionGain;
    PTTexture * _destinationColor;
    float  _disparityMax;
    float  _disparityMin;
    float  _fNumber;
    float  _focalLenIn35mmFilm;
    float  _focusDisparity;
    unsigned int  _frameId;
    float  _highlightBoostFactor;
    float  _highlightChromaFactor;
    <MTLTexture> * _integratedStyleCoefficientsTextureArray;
    float  _networkBias;
    NSDictionary * _options;
    int  _readNoise_1x;
    int  _readNoise_8x;
    <PTRenderState> * _renderState;
    struct { 
        unsigned long long x; 
        unsigned long long y; 
        unsigned long long width; 
        unsigned long long height; 
    }  _scissorRect;
    int  _sensorID;
    PTTexture * _sourceColor;
    <MTLTexture> * _sourceDisparity;
    struct CGSize { 
        double width; 
        double height; 
    }  _totalSensorCropRectSize;
    void _visCropFactor;
    void _visCropFactorPreview;
}

@property (nonatomic) int AGC;
@property (nonatomic) float alphaLowLight;
@property (nonatomic) float aperture;
@property (nonatomic, retain) <MTLTexture> *colorCube;
@property (nonatomic) int conversionGain;
@property (nonatomic, retain) PTTexture *destinationColor;
@property (nonatomic) float disparityMax;
@property (nonatomic) float disparityMin;
@property (nonatomic) float fNumber;
@property (nonatomic) float focalLenIn35mmFilm;
@property (nonatomic) float focusDisparity;
@property (nonatomic) float focusDistance;
@property (nonatomic) unsigned int frameId;
@property (nonatomic) float highlightBoostFactor;
@property (nonatomic) float highlightChromaFactor;
@property (nonatomic, retain) <MTLTexture> *integratedStyleCoefficientsTextureArray;
@property (nonatomic) float networkBias;
@property (nonatomic, retain) NSDictionary *options;
@property (nonatomic) int readNoise_1x;
@property (nonatomic) int readNoise_8x;
@property (nonatomic, retain) <PTRenderState> *renderState;
@property (nonatomic) struct { unsigned long long x1; unsigned long long x2; unsigned long long x3; unsigned long long x4; } scissorRect;
@property (nonatomic) int sensorID;
@property (nonatomic, retain) PTTexture *sourceColor;
@property (nonatomic, retain) <MTLTexture> *sourceDisparity;
@property (nonatomic) struct CGSize { double x1; double x2; } totalSensorCropRectSize;
@property (nonatomic) void visCropFactor;
@property (nonatomic) void visCropFactorPreview;

- (void).cxx_destruct;
- (int)AGC;
- (float)alphaLowLight;
- (float)aperture;
- (id)colorCube;
- (int)conversionGain;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)destinationColor;
- (float)disparityMax;
- (float)disparityMin;
- (float)fNumber;
- (float)focalLenIn35mmFilm;
- (float)focusDisparity;
- (float)focusDistance;
- (unsigned int)frameId;
- (float)highlightBoostFactor;
- (float)highlightChromaFactor;
- (id)init;
- (id)integratedStyleCoefficientsTextureArray;
- (float)networkBias;
- (id)options;
- (int)readNoise_1x;
- (int)readNoise_8x;
- (id)renderState;
- (struct { unsigned long long x1; unsigned long long x2; unsigned long long x3; unsigned long long x4; })scissorRect;
- (int)sensorID;
- (void)setAGC:(int)arg1;
- (void)setAlphaLowLight:(float)arg1;
- (void)setAperture:(float)arg1;
- (void)setColorCube:(id)arg1;
- (void)setConversionGain:(int)arg1;
- (void)setDestinationColor:(id)arg1;
- (void)setDisparityMax:(float)arg1;
- (void)setDisparityMin:(float)arg1;
- (void)setFNumber:(float)arg1;
- (void)setFocalLenIn35mmFilm:(float)arg1;
- (void)setFocusDisparity:(float)arg1;
- (void)setFocusDistance:(float)arg1;
- (void)setFrameId:(unsigned int)arg1;
- (void)setHighlightBoostFactor:(float)arg1;
- (void)setHighlightChromaFactor:(float)arg1;
- (void)setIntegratedStyleCoefficientsTextureArray:(id)arg1;
- (void)setNetworkBias:(float)arg1;
- (void)setOptions:(id)arg1;
- (void)setReadNoise_1x:(int)arg1;
- (void)setReadNoise_8x:(int)arg1;
- (void)setRenderState:(id)arg1;
- (void)setScissorRect:(struct { unsigned long long x1; unsigned long long x2; unsigned long long x3; unsigned long long x4; })arg1;
- (void)setSensorID:(int)arg1;
- (void)setSourceColor:(id)arg1;
- (void)setSourceDisparity:(id)arg1;
- (void)setTotalSensorCropRectSize:(struct CGSize { double x1; double x2; })arg1;
- (void)setVisCropFactor;
- (void)setVisCropFactorPreview;
- (id)sourceColor;
- (id)sourceDisparity;
- (struct CGSize { double x1; double x2; })totalSensorCropRectSize;
- (void)visCropFactor;
- (void)visCropFactorPreview;

@end
