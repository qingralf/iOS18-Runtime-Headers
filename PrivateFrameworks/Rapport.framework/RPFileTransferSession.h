/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/Rapport.framework/Rapport
 */

@interface RPFileTransferSession : NSObject <NSSecureCoding, RPMessageable> {
    bool  _activateCalled;
    NSMutableSet * _addedItems;
    CUBonjourAdvertiser * _bonjourAdvertiser;
    CUBonjourBrowser * _bonjourBrowser;
    CUCoalescer * _bonjourCoalescer;
    CUBonjourDevice * _bonjourDevice;
    unsigned int  _cnxIDLast;
    id /* block */  _completionHandler;
    bool  _compressionEnabled;
    NSMutableSet * _connections;
    RPConnection * _controlCnx;
    NSObject<OS_dispatch_source> * _controlRetryTimer;
    int  _controlState;
    unsigned long long  _debugFlags;
    int  _debugNotifyToken;
    NSObject<OS_dispatch_queue> * _dispatchQueue;
    unsigned long long  _fileIDLastReceive;
    unsigned long long  _fileIDLastSend;
    unsigned int  _fileWritesOutstanding;
    bool  _finishPending;
    unsigned int  _flags;
    id /* block */  _flowControlChangedHandler;
    int  _flowControlState;
    bool  _invalidateCalled;
    bool  _invalidateDone;
    NSMutableArray * _ioQueues;
    NSString * _label;
    unsigned long long  _largeFileBufferBytes;
    NSMutableDictionary * _largeFileReceiveTasks;
    NSMutableSet * _largeFileSendTasks;
    bool  _lockedInterface;
    unsigned long long  _maxLargeFileTasks;
    unsigned long long  _maxSmallFileTasks;
    unsigned long long  _metricCompressedBytes;
    unsigned long long  _metricCompressionErrors;
    unsigned int  _metricDisconnects;
    unsigned long long  _metricFileSizeBuckets;
    unsigned long long  _metricLastFileCompletionTicks;
    unsigned int  _metricLinkTypeCountAWDL;
    unsigned int  _metricLinkTypeCountOther;
    unsigned int  _metricLinkTypeCountUSB;
    unsigned long long  _metricNotEnoughFiles;
    unsigned long long  _metricPrematureSmallSets;
    unsigned long long  _metricRetries;
    unsigned long long  _metricTCPRanDryCount;
    unsigned long long  _metricTicksActivate;
    unsigned long long  _metricTicksConnectStart;
    unsigned long long  _metricTicksConnected;
    unsigned long long  _metricTotalBytes;
    unsigned long long  _metricTotalFiles;
    unsigned int  _metricTotalIOQueues;
    unsigned long long  _metricUncompressedBytes;
    unsigned long long  _metricUncompressibleChunks;
    RPIdentity * _peerIdentity;
    bool  _peerInfraWiFiDisabled;
    NSData * _peerPublicKey;
    bool  _prefCompress;
    int  _prefLargeFileMaxOutstanding;
    unsigned int  _prefSmallFilesMaxBytes;
    double  _prefsStateFileFlushInterval;
    RPFileTransferProgress * _progressCurrent;
    unsigned long long  _progressCurrentBytes;
    unsigned long long  _progressCurrentFiles;
    bool  _progressDirty;
    id /* block */  _progressHandler;
    double  _progressHandlerTimeInterval;
    NSObject<OS_dispatch_source> * _progressTimer;
    unsigned long long  _progressTotalBytes;
    unsigned long long  _progressTotalFiles;
    NSString * _receiveFileParentPath;
    id /* block */  _receivedItemHandler;
    id /* block */  _receivedItemsHandler;
    NSMutableDictionary * _registeredEvents;
    NSMutableDictionary * _registeredRequests;
    bool  _resumable;
    RPFileTransferResumeState * _resumeState;
    double  _resumeStateFlushInterval;
    RPIdentity * _selfIdentity;
    NSData * _selfPublicKey;
    NSData * _selfSecretKey;
    NSString * _serviceType;
    NSMutableDictionary * _smallFilesReceiveTasks;
    NSMutableSet * _smallFilesSendTasks;
    NSString * _targetID;
    unsigned long long  _taskIDLast;
    CUTCPServer * _tcpServer;
    NSURL * _temporaryDirectoryURL;
    struct LogCategory { int x1; int x2; char *x3; unsigned int x4; char *x5; char *x6; int x7; struct LogCategory {} *x8; struct LogOutput {} *x9; struct LogOutput {} *x10; unsigned long long x11; unsigned long long x12; unsigned int x13; unsigned int x14; char *x15; struct LogCategoryPrivate {} *x16; } * _ucat;
    CUWiFiManager * _wifiManager;
}

@property (nonatomic, copy) id /* block */ completionHandler;
@property (nonatomic) unsigned long long debugFlags;
@property (nonatomic, retain) NSObject<OS_dispatch_queue> *dispatchQueue;
@property (nonatomic) unsigned int flags;
@property (nonatomic, copy) id /* block */ flowControlChangedHandler;
@property (nonatomic, readonly) int flowControlState;
@property (nonatomic, copy) NSString *label;
@property (nonatomic) unsigned long long largeFileBufferBytes;
@property (nonatomic) unsigned long long maxLargeFileTasks;
@property (nonatomic) unsigned long long maxSmallFileTasks;
@property (nonatomic, copy) NSData *peerPublicKey;
@property (nonatomic, readonly) RPFileTransferProgress *progressCurrent;
@property (nonatomic, copy) id /* block */ progressHandler;
@property (nonatomic) double progressHandlerTimeInterval;
@property (nonatomic, copy) id /* block */ receivedItemHandler;
@property (nonatomic, copy) id /* block */ receivedItemsHandler;
@property (nonatomic, copy) NSData *selfPublicKey;
@property (nonatomic, copy) NSData *selfSecretKey;
@property (nonatomic, retain) NSString *serviceType;
@property (nonatomic, copy) NSString *targetID;
@property (nonatomic, copy) NSURL *temporaryDirectoryURL;

+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (bool)_activateAndReturnError:(id*)arg1;
- (bool)_activateSourceAndReturnError:(id*)arg1;
- (bool)_activateTargetAndReturnError:(id*)arg1;
- (void)_completeItem:(id)arg1 error:(id)arg2;
- (void)_completeItemDirect:(id)arg1 error:(id)arg2;
- (id)_compressData:(id)arg1 error:(id*)arg2;
- (void)_controlCnxRetryIfNeeded;
- (void)_controlCnxStartIfNeeded;
- (void)_debugSetup;
- (void)_debugUpdate;
- (id)_decompressAndDecodeData:(id)arg1 originalSize:(unsigned long long)arg2 error:(id*)arg3;
- (id)_decompressData:(id)arg1 originalSize:(unsigned long long)arg2 error:(id*)arg3;
- (id)_encodeAndCompressObject:(id)arg1 originalSize:(unsigned long long*)arg2 error:(id*)arg3;
- (void)_handleDeviceFound:(id)arg1;
- (void)_handleDeviceLost:(id)arg1;
- (void)_handleDevicesCoalesced;
- (void)_handleIncomingConnectionEnded:(id)arg1;
- (void)_handleIncomingConnectionStarted:(id)arg1;
- (void)_invalidate;
- (void)_invalidated;
- (id)_ioQueueDequeue;
- (void)_ioQueueEnqueue:(id)arg1;
- (void)_largeFileReceiveRequest:(id)arg1 responseHandler:(id /* block */)arg2;
- (void)_largeFileReceiveTaskInvalidate:(id)arg1;
- (void)_largeFileReceiveTaskRespond:(id)arg1 error:(id)arg2 complete:(bool)arg3 responseHandler:(id /* block */)arg4;
- (void)_largeFileReceiveTaskRun:(id)arg1 data:(id)arg2 sendFlags:(unsigned int)arg3 responseHandler:(id /* block */)arg4;
- (bool)_largeFileReceiveTaskUpdateSHA256CtxFromFileItem:(id)arg1 length:(unsigned long long)arg2 error:(id*)arg3;
- (id)_largeFileSendTaskCreate;
- (void)_largeFileSendTaskEnd:(id)arg1 error:(id)arg2;
- (void)_largeFileSendTaskFailed:(id)arg1 error:(id)arg2;
- (void)_largeFileSendTaskNext:(id)arg1 xid:(unsigned int)arg2;
- (void)_largeFileSendTaskResponse:(id)arg1 error:(id)arg2 end:(bool)arg3 xid:(unsigned int)arg4;
- (void)_largeFileSendTaskSend:(id)arg1 data:(id)arg2 end:(bool)arg3 xid:(unsigned int)arg4;
- (void)_largeFileSendTaskStart:(id)arg1;
- (void)_metricAddFileSize:(long long)arg1;
- (id)_modTimeForFileWithPath:(const char *)arg1 error:(id*)arg2;
- (int)_openReadFileItem:(id)arg1 error:(id*)arg2;
- (int)_openReadPath:(const char *)arg1 error:(id*)arg2;
- (int)_openWriteFileItem:(id)arg1 size:(long long)arg2 error:(id*)arg3;
- (int)_openWriteFileItem:(id)arg1 size:(long long)arg2 truncate:(bool)arg3 error:(id*)arg4;
- (int)_openWritePath:(id)arg1 size:(long long)arg2 error:(id*)arg3;
- (int)_openWritePath:(id)arg1 size:(long long)arg2 truncate:(bool)arg3 error:(id*)arg4;
- (void)_prefsChanged;
- (bool)_prepareItem:(id)arg1 error:(id*)arg2;
- (void)_processFinish;
- (void)_processReceivedItem:(id)arg1 responseHandler:(id /* block */)arg2;
- (void)_processReceivedItems:(id)arg1 responseHandler:(id /* block */)arg2;
- (bool)_readFD:(int)arg1 buffer:(char *)arg2 size:(unsigned long long)arg3 error:(id*)arg4;
- (id)_readFD:(int)arg1 size:(unsigned long long)arg2 error:(id*)arg3;
- (id)_readPath:(const char *)arg1 size:(unsigned long long)arg2 error:(id*)arg3;
- (void)_receivedEventID:(id)arg1 event:(id)arg2 options:(id)arg3;
- (void)_receivedPeerUpdate:(id)arg1;
- (void)_receivedRequestID:(id)arg1 request:(id)arg2 options:(id)arg3 responseHandler:(id /* block */)arg4;
- (void)_reportCompletion:(id)arg1;
- (void)_reportDataTransferred;
- (void)_reportProgressControlState;
- (void)_reportProgressType:(int)arg1;
- (void)_requestResumeStateFromReceiver;
- (void)_resumeStateReceiveRequest:(id)arg1 responseHandler:(id /* block */)arg2;
- (void)_scheduleItems;
- (void)_smallFilesReceiveRequest:(id)arg1 responseHandler:(id /* block */)arg2;
- (void)_smallFilesReceiveTaskComplete:(id)arg1 error:(id)arg2 responseHandler:(id /* block */)arg3;
- (bool)_smallFilesReceiveTaskFileItem:(id)arg1 error:(id*)arg2;
- (void)_smallFilesReceiveTaskRun:(id)arg1 responseHandler:(id /* block */)arg2;
- (id)_smallFilesSendTaskCreate;
- (void)_smallFilesSendTaskEnd:(id)arg1 error:(id)arg2;
- (id)_smallFilesSendTaskReadItem:(id)arg1 error:(id*)arg2;
- (void)_smallFilesSendTaskRun:(id)arg1;
- (void)_startProgressReportTimer;
- (void)_stopProgressReportTimer;
- (bool)_truncateFD:(int)arg1 size:(long long)arg2 error:(id*)arg3;
- (void)_updateIfNeededWithBlock:(id /* block */)arg1;
- (void)_updateLargeFilesTaskResumeState:(id)arg1 data:(id)arg2 sendFlags:(unsigned int)arg3 error:(id)arg4 complete:(bool)arg5;
- (void)_updateSmallFilesTaskResumeState:(id)arg1 error:(id)arg2;
- (void)_updateWiFi;
- (bool)_writeFD:(int)arg1 buffer:(const char *)arg2 size:(unsigned long long)arg3 error:(id*)arg4;
- (void)activate;
- (void)addItem:(id)arg1;
- (void)addItems:(id)arg1;
- (id /* block */)completionHandler;
- (void)dealloc;
- (unsigned long long)debugFlags;
- (void)deregisterEventID:(id)arg1;
- (void)deregisterRequestID:(id)arg1;
- (id)description;
- (id)descriptionWithLevel:(int)arg1;
- (id)dispatchQueue;
- (void)encodeWithCoder:(id)arg1;
- (void)finish;
- (unsigned int)flags;
- (id /* block */)flowControlChangedHandler;
- (int)flowControlState;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (void)invalidate;
- (bool)isResumable;
- (id)label;
- (unsigned long long)largeFileBufferBytes;
- (unsigned long long)maxLargeFileTasks;
- (unsigned long long)maxSmallFileTasks;
- (id)peerPublicKey;
- (bool)prepareTemplateAndReturnError:(id*)arg1;
- (id)progressCurrent;
- (id /* block */)progressHandler;
- (double)progressHandlerTimeInterval;
- (id /* block */)receivedItemHandler;
- (id /* block */)receivedItemsHandler;
- (void)registerEventID:(id)arg1 options:(id)arg2 handler:(id /* block */)arg3;
- (void)registerRequestID:(id)arg1 options:(id)arg2 handler:(id /* block */)arg3;
- (id)resumeState;
- (id)resumeStateDict;
- (id)selfPublicKey;
- (id)selfSecretKey;
- (void)sendEventID:(id)arg1 event:(id)arg2 destinationID:(id)arg3 options:(id)arg4 completion:(id /* block */)arg5;
- (void)sendEventID:(id)arg1 event:(id)arg2 options:(id)arg3 completion:(id /* block */)arg4;
- (void)sendRequestID:(id)arg1 request:(id)arg2 destinationID:(id)arg3 options:(id)arg4 responseHandler:(id /* block */)arg5;
- (void)sendRequestID:(id)arg1 request:(id)arg2 options:(id)arg3 responseHandler:(id /* block */)arg4;
- (id)serviceType;
- (void)setCompletionHandler:(id /* block */)arg1;
- (void)setDebugFlags:(unsigned long long)arg1;
- (void)setDispatchQueue:(id)arg1;
- (void)setFlags:(unsigned int)arg1;
- (void)setFlowControlChangedHandler:(id /* block */)arg1;
- (void)setLabel:(id)arg1;
- (void)setLargeFileBufferBytes:(unsigned long long)arg1;
- (void)setMaxLargeFileTasks:(unsigned long long)arg1;
- (void)setMaxSmallFileTasks:(unsigned long long)arg1;
- (void)setPeerPublicKey:(id)arg1;
- (void)setProgressHandler:(id /* block */)arg1;
- (void)setProgressHandlerTimeInterval:(double)arg1;
- (void)setReceivedItemHandler:(id /* block */)arg1;
- (void)setReceivedItemsHandler:(id /* block */)arg1;
- (void)setSelfPublicKey:(id)arg1;
- (void)setSelfSecretKey:(id)arg1;
- (void)setServiceType:(id)arg1;
- (void)setTargetID:(id)arg1;
- (void)setTemporaryDirectoryURL:(id)arg1;
- (id)targetID;
- (id)temporaryDirectoryURL;

@end
