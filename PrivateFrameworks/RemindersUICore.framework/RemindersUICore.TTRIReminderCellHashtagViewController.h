/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/RemindersUICore.framework/RemindersUICore
 */

@interface RemindersUICore.TTRIReminderCellHashtagViewController : NSObject <UITextViewDelegate> {
    void $__lazy_storage_$_committedHashtagsBoldFont;
    void committedHashtagsFont;
    void descriptionAndTagsView;
    void hashtagsAttributedText;
    void lastTextChangeDetail;
    void presenter;
    void tokenInteraction;
    void uncommittedHashtagsBoldFont;
    void uncommittedHashtagsFont;
    void uncommittedTagsColor;
    void userPressedReturn;
}

- (void).cxx_destruct;
- (id)init;
- (id)textView:(id)arg1 editMenuForTextInRange:(struct _NSRange { unsigned long long x1; unsigned long long x2; })arg2 suggestedActions:(id)arg3;
- (bool)textView:(id)arg1 shouldChangeTextInRange:(struct _NSRange { unsigned long long x1; unsigned long long x2; })arg2 replacementText:(id)arg3;
- (void)textViewDidBeginEditing:(id)arg1;
- (void)textViewDidChange:(id)arg1;
- (void)textViewDidChangeSelection:(id)arg1;
- (void)textViewDidEndEditing:(id)arg1;
- (bool)textViewShouldBeginEditing:(id)arg1;

@end
