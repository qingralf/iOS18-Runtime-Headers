/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/SeymourUI.framework/SeymourUI
 */

@interface SeymourUI.RootViewController : UIViewController {
    union { in /* Warning: Unrecognized filer type: 'u' using 'void*' */ void*x1; long x2; long x3; }  _lastWorkoutPlanTapToRadarAlertTimestamp;
    void accountButton;
    void accountFitnessSettingsProvider;
    void activeWorkoutPlanProvider;
    void avatarSettingsProvider;
    void bag;
    void catalogClient;
    void contentInsetInitialBehavior;
    void controllerDisplayCoordinator;
    void currentContentInsetBehavior;
    void dataProvider;
    void dependencies;
    void dynamicOfferCoordinator;
    void engagementPresentability;
    void engagementPresentationCoordinator;
    void eventHub;
    void hasContentInsetChanged;
    void isBeyondScrollBoundary;
    void isTransitioningContentInset;
    void largeTitleUpNextQueueButton;
    void marketingHostedContentCoordinator;
    void marketingMessageViewControllerHandler;
    void marketingMetricFieldsProvider;
    void pageNavigator;
    void platform;
    void purchaseCoordinator;
    void purchaseHandler;
    void resignActiveObserver;
    void serviceSubscriptionCache;
    void storefrontLocalizer;
    void subscriptionToken;
    void upNextQueueButton;
    void upNextQueueButtonPresenter;
    void upNextQueueClient;
    void visibility;
    void webUserInterfaceCoordinator;
}

- (void).cxx_destruct;
- (void)accountButtonTapped:(id)arg1;
- (id)initWithCoder:(id)arg1;
- (id)initWithNibName:(id)arg1 bundle:(id)arg2;
- (void)viewDidAppear:(bool)arg1;
- (void)viewDidDisappear:(bool)arg1;
- (void)viewDidLayoutSubviews;
- (void)viewDidLoad;
- (void)viewIsAppearing:(bool)arg1;
- (void)viewWillAppear:(bool)arg1;
- (void)viewWillDisappear:(bool)arg1;
- (void)viewWillTransitionToSize:(struct CGSize { double x1; double x2; })arg1 withTransitionCoordinator:(id)arg2;

@end
