/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/SpotlightServices.framework/SpotlightServices
 */

@interface SFSearchResult_SpotlightExtras : SFSearchResult <NSSecureCoding> {
    NSArray * _answerAttributes;
    NSData * _attributeData;
    NSMutableDictionary * _backendData;
    SFPunchout * _cachedPunchout;
    NSString * _contentURL;
    NSNumber * _displayOrder;
    NSNumber * _documentIdentifier;
    NSMutableArray * _duplicatedItems;
    unsigned int  _feedbackBlockId;
    NSNumber * _fileIdentifier;
    NSString * _filename;
    bool  _forceNoTopHit;
    bool  _hasCommunicationContent;
    bool  _hasTextContent;
    bool  _hasTextContentMatch;
    bool  _isAppClip;
    bool  _isSafariTopHit;
    bool  _isWebClip;
    float  _l2score;
    NSDate * _lastUsedDate;
    NSString * _launchString;
    IQFMapsPersonalizationResult * _mapsPersonalizationResult;
    NSNumber * _parentFileIdentifier;
    NSString * _personIdentifier;
    NSString * _personQueryIdentifier;
    NSString * _personType;
    NSString * _personalAnswerString;
    unsigned long long  _predictionsFeedbackActionType;
    NSString * _protectionClass;
    PRSRankingItem * _rankingItem;
    NSString * _relatedAppIdentifier;
    /* Warning: Unrecognized filer type: 'T' using 'void*' */ void* _score;
    NSData * _suggestionsFeedbackData;
    NSString * _userActivityType;
}

@property (nonatomic, retain) NSArray *answerAttributes;
@property (nonatomic, retain) NSData *attributeData;
@property (nonatomic, retain) NSMutableDictionary *backendData;
@property (nonatomic, retain) NSString *contentURL;
@property (nonatomic, retain) NSNumber *displayOrder;
@property (nonatomic, retain) NSNumber *documentIdentifier;
@property (nonatomic, retain) NSMutableArray *duplicatedItems;
@property (nonatomic) unsigned int feedbackBlockId;
@property (nonatomic, retain) NSNumber *fileIdentifier;
@property (nonatomic, retain) NSString *filename;
@property (nonatomic) bool forceNoTopHit;
@property (nonatomic) bool hasCommunicationContent;
@property (nonatomic) bool hasTextContent;
@property (nonatomic) bool hasTextContentMatch;
@property (nonatomic) bool isAppClip;
@property (nonatomic) bool isSafariTopHit;
@property (nonatomic) bool isWebClip;
@property (nonatomic) float l2score;
@property (nonatomic, retain) NSDate *lastUsedDate;
@property (nonatomic, retain) NSString *launchString;
@property (retain) IQFMapsPersonalizationResult *mapsPersonalizationResult;
@property (nonatomic, retain) NSNumber *parentFileIdentifier;
@property (nonatomic, retain) NSString *personIdentifier;
@property (nonatomic, retain) NSString *personQueryIdentifier;
@property (nonatomic, retain) NSString *personType;
@property (nonatomic, retain) NSString *personalAnswerString;
@property (nonatomic) unsigned long long predictionsFeedbackActionType;
@property (nonatomic, retain) NSString *protectionClass;
@property (retain) PRSRankingItem *rankingItem;
@property (nonatomic, retain) NSString *relatedAppIdentifier;
@property (nonatomic) /* Warning: Unrecognized filer type: 'T' using 'void*' */ void*score;
@property (nonatomic, retain) NSData *suggestionsFeedbackData;
@property (nonatomic, retain) NSString *userActivityType;

+ (void)initialize;
+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)answerAttributes;
- (id)attributeData;
- (id)backendData;
- (void)clearBackendData;
- (id)contentURL;
- (id)debugDescription;
- (id)displayOrder;
- (id)documentIdentifier;
- (id)duplicatedItems;
- (void)encodeWithCoder:(id)arg1;
- (unsigned int)feedbackBlockId;
- (id)fileIdentifier;
- (id)filename;
- (bool)forceNoTopHit;
- (id)getStableServerResultIdentifier;
- (bool)hasCommunicationContent;
- (bool)hasTextContent;
- (bool)hasTextContentMatch;
- (id)init;
- (id)initWithCoder:(id)arg1;
- (id)initWithIdentifier:(id)arg1 bundleIdentifier:(id)arg2 protectionClass:(id)arg3 attributeKeys:(id)arg4 attributeValues:(id)arg5;
- (id)initWithIdentifier:(id)arg1 bundleIdentifier:(id)arg2 protectionClass:(id)arg3 attributeKeys:(id)arg4 attributeValues:(id)arg5 type:(int)arg6 completion:(id)arg7;
- (id)initWithIdentifier:(id)arg1 bundleIdentifier:(id)arg2 protectionClass:(id)arg3 attributes:(id)arg4 type:(int)arg5 completion:(id)arg6;
- (id)initWithResult:(id)arg1;
- (id)initWithResult:(id)arg1 groupId:(id)arg2;
- (bool)isAppClip;
- (bool)isSafariTopHit;
- (bool)isSafariTopHitForQuery:(id)arg1;
- (bool)isWebClip;
- (float)l2score;
- (id)lastUsedDate;
- (id)launchString;
- (id)mapsPersonalizationResult;
- (id)parentFileIdentifier;
- (id)personIdentifier;
- (id)personQueryIdentifier;
- (id)personType;
- (id)personalAnswerString;
- (unsigned long long)predictionsFeedbackActionType;
- (id)protectionClass;
- (id)punchout;
- (id)rankingItem;
- (id)relatedAppIdentifier;
- (/* Warning: Unrecognized filer type: 'T' using 'void*' */ void*)score;
- (void)setAnswerAttributes:(id)arg1;
- (void)setAttributeData:(id)arg1;
- (void)setBackendData:(id)arg1;
- (void)setContentURL:(id)arg1;
- (void)setDisplayOrder:(id)arg1;
- (void)setDocumentIdentifier:(id)arg1;
- (void)setDuplicatedItems:(id)arg1;
- (void)setFeedbackBlockId:(unsigned int)arg1;
- (void)setFeedbackPropertiesOnResultCopy:(id)arg1;
- (void)setFileIdentifier:(id)arg1;
- (void)setFilename:(id)arg1;
- (void)setForceNoTopHit:(bool)arg1;
- (void)setHasCommunicationContent:(bool)arg1;
- (void)setHasTextContent:(bool)arg1;
- (void)setHasTextContentMatch:(bool)arg1;
- (void)setIsAppClip:(bool)arg1;
- (void)setIsSafariTopHit:(bool)arg1;
- (void)setIsWebClip:(bool)arg1;
- (void)setL2score:(float)arg1;
- (void)setLastUsedDate:(id)arg1;
- (void)setLaunchString:(id)arg1;
- (void)setMapsPersonalizationResult:(id)arg1;
- (void)setParentFileIdentifier:(id)arg1;
- (void)setPersonIdentifier:(id)arg1;
- (void)setPersonQueryIdentifier:(id)arg1;
- (void)setPersonType:(id)arg1;
- (void)setPersonalAnswerString:(id)arg1;
- (void)setPredictionsFeedbackActionType:(unsigned long long)arg1;
- (void)setProtectionClass:(id)arg1;
- (void)setRankingItem:(id)arg1;
- (void)setRelatedAppIdentifier:(id)arg1;
- (void)setScore:(/* Warning: Unrecognized filer type: 'T' using 'void*' */ void*)arg1;
- (void)setSuggestionsFeedbackData:(id)arg1;
- (void)setUrl:(id)arg1;
- (void)setUserActivityType:(id)arg1;
- (id)suggestionsFeedbackData;
- (id)ttrDescription;
- (id)urlValueForAttribute:(id)arg1;
- (id)userActivityType;
- (id)valueForAttribute:(id)arg1 withType:(Class)arg2;

@end
