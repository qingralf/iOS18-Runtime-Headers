/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/SystemStatusUI.framework/SystemStatusUI
 */

@interface STUIStatusBarRegion : NSObject <BSDebugDescriptionProviding, STUIStatusBarActionable> {
    STUIStatusBarAction * _action;
    struct UIEdgeInsets { 
        double top; 
        double left; 
        double bottom; 
        double right; 
    }  _actionInsets;
    double  _alpha;
    long long  _automaticStyle;
    UIView * _backgroundView;
    NSLayoutConstraint * _centerXConstraint;
    NSLayoutConstraint * _centerYConstraint;
    UIView * _containerView;
    UIView * _contentView;
    NSSet * _dependentRegionIdentifiers;
    bool  _disableItemFrameBasedOverflow;
    NSMutableIndexSet * _disablingTokens;
    NSOrderedSet * _displayItems;
    STUIStatusBarRegion * _enabilityRegion;
    struct NSDirectionalEdgeInsets { 
        double top; 
        double leading; 
        double bottom; 
        double trailing; 
    }  _extendedHoverInsets;
    UIView * _frozenView;
    UIView * _highlightView;
    STUIStatusBarAction * _hoverAction;
    NSString * _identifier;
    <STUIStatusBarRegionLayout> * _layout;
    UILayoutGuide * _layoutGuide;
    bool  _needsReLayout;
    struct UIOffset { 
        double horizontal; 
        double vertical; 
    }  _offset;
    bool  _offsetable;
    long long  _overriddenStyle;
    STUIStatusBarStyleAttributes * _overriddenStyleAttributes;
    STUIStatusBar * _statusBar;
    long long  _style;
}

@property (nonatomic, readonly) struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; } absoluteHoverFrame;
@property (nonatomic, retain) STUIStatusBarAction *action;
@property (nonatomic) struct UIEdgeInsets { double x1; double x2; double x3; double x4; } actionInsets;
@property (nonatomic) double alpha;
@property (nonatomic) long long automaticStyle;
@property (nonatomic, retain) UIView *backgroundView;
@property (nonatomic, retain) NSLayoutConstraint *centerXConstraint;
@property (nonatomic, retain) NSLayoutConstraint *centerYConstraint;
@property (nonatomic, readonly) <UILayoutItem> *containerItem;
@property (nonatomic, readonly) UIView *containerView;
@property (nonatomic, retain) UIView *contentView;
@property (nonatomic, readonly) NSArray *currentEnabledDisplayItems;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (nonatomic, retain) NSSet *dependentRegionIdentifiers;
@property (readonly, copy) NSString *description;
@property (nonatomic) bool disableItemFrameBasedOverflow;
@property (nonatomic, retain) NSMutableIndexSet *disablingTokens;
@property (nonatomic, readonly, copy) NSDictionary *displayItemAbsoluteFrames;
@property (nonatomic, readonly, copy) NSDictionary *displayItemAbsolutePresentationFrames;
@property (nonatomic, retain) NSOrderedSet *displayItems;
@property (nonatomic, readonly) long long effectiveStyle;
@property (nonatomic) STUIStatusBarRegion *enabilityRegion;
@property (getter=isEnabled, nonatomic, readonly) bool enabled;
@property (nonatomic, readonly) NSArray *enabledDisplayItems;
@property (nonatomic) struct NSDirectionalEdgeInsets { double x1; double x2; double x3; double x4; } extendedHoverInsets;
@property (getter=isFrozen, nonatomic) bool frozen;
@property (nonatomic, retain) UIView *frozenView;
@property (readonly) unsigned long long hash;
@property (nonatomic, retain) UIView *highlightView;
@property (nonatomic, retain) STUIStatusBarAction *hoverAction;
@property (nonatomic, readonly) bool hoverHighlightsAsRegion;
@property (nonatomic, readonly) UIView *hoverView;
@property (nonatomic, readonly, copy) NSString *identifier;
@property (nonatomic, retain) <STUIStatusBarRegionLayout> *layout;
@property (nonatomic, retain) UILayoutGuide *layoutGuide;
@property (nonatomic, readonly) <UILayoutItem> *layoutItem;
@property (nonatomic) bool needsReLayout;
@property (nonatomic) struct UIOffset { double x1; double x2; } offset;
@property (nonatomic) bool offsetable;
@property (nonatomic, readonly) STUIStatusBarDisplayItem *overflowedDisplayItem;
@property (nonatomic) long long overriddenStyle;
@property (nonatomic, retain) STUIStatusBarStyleAttributes *overriddenStyleAttributes;
@property (nonatomic) STUIStatusBar *statusBar;
@property (nonatomic) long long style;
@property (readonly) Class superclass;

- (void).cxx_destruct;
- (void)_addSubview:(id)arg1 atBack:(bool)arg2;
- (id)_descriptionBuilderWithMultilinePrefix:(id)arg1 forDebug:(bool)arg2;
- (void)_overriddenStyleAttributesChanged;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })absoluteHoverFrame;
- (id)action;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })actionInsets;
- (double)alpha;
- (long long)automaticStyle;
- (id)backgroundView;
- (id)centerXConstraint;
- (id)centerYConstraint;
- (id)containerItem;
- (id)containerView;
- (id)contentView;
- (id)currentEnabledDisplayItems;
- (id)debugDescription;
- (id)debugDescriptionWithMultilinePrefix:(id)arg1;
- (id)dependentRegionIdentifiers;
- (id)description;
- (id)descriptionBuilderWithMultilinePrefix:(id)arg1;
- (id)descriptionWithMultilinePrefix:(id)arg1;
- (bool)disableItemFrameBasedOverflow;
- (bool)disableWithToken:(unsigned long long)arg1;
- (id)disablingTokens;
- (id)displayItemAbsoluteFrames;
- (id)displayItemAbsolutePresentationFrames;
- (id)displayItemForHUDAtLocation:(struct CGPoint { double x1; double x2; })arg1;
- (id)displayItems;
- (long long)effectiveStyle;
- (id)enabilityRegion;
- (bool)enableWithToken:(unsigned long long)arg1;
- (id)enabledDisplayItems;
- (struct NSDirectionalEdgeInsets { double x1; double x2; double x3; double x4; })extendedHoverInsets;
- (id)frozenView;
- (id)highlightView;
- (id)hoverAction;
- (bool)hoverHighlightsAsRegion;
- (id)hoverView;
- (id)identifier;
- (id)initWithIdentifier:(id)arg1;
- (bool)isEnabled;
- (bool)isFrozen;
- (id)layout;
- (id)layoutGuide;
- (id)layoutItem;
- (bool)needsReLayout;
- (struct UIOffset { double x1; double x2; })offset;
- (bool)offsetable;
- (id)overflowedDisplayItem;
- (long long)overriddenStyle;
- (id)overriddenStyleAttributes;
- (void)setAction:(id)arg1;
- (void)setActionInsets:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)setAlpha:(double)arg1;
- (void)setAutomaticStyle:(long long)arg1;
- (void)setBackgroundView:(id)arg1;
- (void)setCenterXConstraint:(id)arg1;
- (void)setCenterYConstraint:(id)arg1;
- (void)setContentView:(id)arg1;
- (void)setDependentRegionIdentifiers:(id)arg1;
- (void)setDisableItemFrameBasedOverflow:(bool)arg1;
- (void)setDisablingTokens:(id)arg1;
- (void)setDisplayItems:(id)arg1;
- (void)setEnabilityRegion:(id)arg1;
- (void)setExtendedHoverInsets:(struct NSDirectionalEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)setFrozen:(bool)arg1;
- (void)setFrozenView:(id)arg1;
- (void)setHighlightView:(id)arg1;
- (void)setHoverAction:(id)arg1;
- (void)setIdentifier:(id)arg1;
- (void)setLayout:(id)arg1;
- (void)setLayoutGuide:(id)arg1;
- (void)setNeedsReLayout:(bool)arg1;
- (void)setOffset:(struct UIOffset { double x1; double x2; })arg1;
- (void)setOffsetable:(bool)arg1;
- (void)setOverriddenStyle:(long long)arg1;
- (void)setOverriddenStyleAttributes:(id)arg1;
- (void)setStatusBar:(id)arg1;
- (void)setStyle:(long long)arg1;
- (id)statusBar;
- (long long)style;
- (id)succinctDescription;
- (id)succinctDescriptionBuilder;

@end
