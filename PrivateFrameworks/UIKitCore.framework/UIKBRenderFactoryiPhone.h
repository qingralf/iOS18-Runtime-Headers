/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/UIKitCore.framework/UIKitCore
 */

@interface UIKBRenderFactoryiPhone : UIKBRenderFactory

+ (id)_textOffsetHints;

- (struct CGPoint { double x1; double x2; })ZWNJKeyOffset;
- (void)_configureTraitsForPopupStyle:(id)arg1 withKey:(id)arg2 onKeyplane:(id)arg3;
- (void)_customizeGeometry:(id)arg1 forKey:(id)arg2 contents:(id)arg3;
- (void)_customizePopupTraits:(id)arg1 forKey:(id)arg2 onKeyplane:(id)arg3;
- (void)_customizeSymbolStyle:(id)arg1 forKey:(id)arg2 contents:(id)arg3;
- (void)_customizeTraits:(id)arg1 forMultiscriptKey:(id)arg2 style:(long long)arg3;
- (void)_customizeTraits:(id)arg1 forPredictionCellKey:(id)arg2;
- (void)_customizeTraits:(id)arg1 forZhuyinGridDualStringKey:(id)arg2;
- (bool)_popupMenuStyleForKey:(id)arg1;
- (bool)_popupStyleForKey:(id)arg1;
- (double)_row4ControlSegmentWidthLeft;
- (double)_row4ControlSegmentWidthRight;
- (bool)_shouldConfigureSecondarySymbolStyleForSpaceKey:(id)arg1;
- (id)_textStyleForSpaceKey;
- (id)_traitsForKey:(id)arg1 onKeyplane:(id)arg2;
- (id)activeControlKeyTraits;
- (long long)assetIdiom;
- (double)assistKeyFontSize;
- (struct CGPoint { double x1; double x2; })boldKeyOffset;
- (struct CGPoint { double x1; double x2; })conjunctStringKeyOffsetTelugu;
- (id)controlKeyTraits;
- (struct CGPoint { double x1; double x2; })copyKeyOffset;
- (struct { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; double x2; })customizedOffsetForString:(id)arg1 isPrimary:(bool)arg2 isLandscape:(bool)arg3 withFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg4;
- (struct CGPoint { double x1; double x2; })cutKeyOffset;
- (double)deleteKeyFontSize;
- (struct CGPoint { double x1; double x2; })deleteKeyOffset;
- (struct CGPoint { double x1; double x2; })dictationKeyOffset;
- (struct CGPoint { double x1; double x2; })dismissKeyOffset;
- (double)dualStringBottomAdditionalOffsetForDisplayContents:(id)arg1;
- (double)dynamicBottomRowMultiplier;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })dynamicInsets;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })dynamicInsetsForNumberOfRows:(unsigned long long)arg1;
- (double)gurmukhiEkOnkarFontSize;
- (struct CGPoint { double x1; double x2; })gurmukhiEkOnkarOffset;
- (double)hintNoneKeyFontSize;
- (bool)iPadFudgeLayout;
- (bool)iPadSansHomeButtonLayout;
- (struct CGPoint { double x1; double x2; })internationalKeyOffset;
- (bool)isCustomizedKeyStyle;
- (bool)isTallPopup;
- (struct CGPoint { double x1; double x2; })leftArrowKeyOffset;
- (long long)lightHighQualityEnabledBlendForm;
- (struct CGPoint { double x1; double x2; })more123KeyOffset;
- (double)moreABCKeyFontSize;
- (struct CGPoint { double x1; double x2; })moreABCKeyOffset;
- (double)moreABCKeyWideCellFontSize;
- (struct CGPoint { double x1; double x2; })moreABCKeyWideCellOffset;
- (double)moreKeyFontSize;
- (id)multitapCompleteKeyImageName;
- (id)muttitapReverseKeyImageName;
- (bool)needsSmallerFontSizeForKey:(id)arg1;
- (struct CGPoint { double x1; double x2; })pasteKeyOffset;
- (double)popupFontSize;
- (struct CGPoint { double x1; double x2; })popupSymbolTextOffset;
- (struct CGPoint { double x1; double x2; })realEmojiKeyOffset;
- (double)returnKeyFontSize;
- (struct CGPoint { double x1; double x2; })returnKeyOffset;
- (struct CGPoint { double x1; double x2; })rightArrowKeyOffset;
- (long long)rowLimitForKey:(id)arg1;
- (struct CGPoint { double x1; double x2; })secondaryShiftKeyOffset;
- (void)setupLayoutSegments;
- (id)shiftDeleteGlyphTraits;
- (double)shiftKeyFontSize;
- (struct CGPoint { double x1; double x2; })shiftKeyOffset;
- (id)shiftLockControlKeyTraits;
- (id)shiftedControlKeyTraits;
- (double)skinnyKeyThreshold;
- (double)smallSymbolImageFontSize;
- (double)stringKeyFontSize;
- (double)stringKeyFontSizeBengali;
- (double)stringKeyFontSizeDevanagari;
- (double)stringKeyFontSizeGujarati;
- (double)stringKeyFontSizeGurmukhi;
- (double)stringKeyFontSizeKannada;
- (double)stringKeyFontSizeMalayalam;
- (double)stringKeyFontSizeOriya;
- (double)stringKeyFontSizeTamil;
- (double)stringKeyFontSizeTelugu;
- (struct CGPoint { double x1; double x2; })stringKeyOffset;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetBengali;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetDevanagari;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetGujarati;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetGurmukhi;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetKannada;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetMalayalam;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetOriya;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetTamil;
- (struct CGPoint { double x1; double x2; })stringKeyOffsetTelugu;
- (double)symbolImageControlKeyFontSize;
- (struct CGPoint { double x1; double x2; })symbolImageControlKeyOffset;
- (struct CGPoint { double x1; double x2; })undoKeyOffset;
- (double)variantAnnotationTextFontSize;
- (struct CGPoint { double x1; double x2; })variantAnnotationTextOffset;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })variantDisplayFrameInsets;
- (id)variantGeometriesForGeometry:(id)arg1 variantCount:(unsigned long long)arg2 rowLimit:(long long)arg3 annotationIndex:(unsigned long long)arg4;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })variantPaddedFrameInsets;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })variantSymbolFrameInsets;
- (struct CGPoint { double x1; double x2; })variantSymbolTextOffset;
- (double)variantWideShadowWeight;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })wideShadowPaddleInsets;
- (struct UIEdgeInsets { double x1; double x2; double x3; double x4; })wideShadowPopupMenuInsets;
- (double)zhuyinFirstToneKeyFontSize;
- (struct CGPoint { double x1; double x2; })zhuyinFirstToneKeyOffset;

@end
