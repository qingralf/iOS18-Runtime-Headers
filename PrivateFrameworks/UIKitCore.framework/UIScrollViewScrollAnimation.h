/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/UIKitCore.framework/UIKitCore
 */

@interface UIScrollViewScrollAnimation : UIAnimation {
    double  _accuracy;
    bool  _adjustsForContentOffsetDelta;
    bool  _beingReplacedForDiscreteScroll;
    CABasicAnimation * _customAnimation;
    bool  _forDiscreteScroll;
    struct CGPoint { 
        double x; 
        double y; 
    }  _originalOffset;
    bool  _revealsHorizontalScrollIndicator;
    bool  _revealsVerticalScrollIndicator;
    struct CGPoint { 
        double x; 
        double y; 
    }  _targetOffset;
    bool  _targetOffsetValidAtStart;
}

@property (nonatomic) bool revealsHorizontalScrollIndicator;
@property (nonatomic) bool revealsVerticalScrollIndicator;

- (void).cxx_destruct;
- (void)adjustForContentOffsetDelta:(struct CGPoint { double x1; double x2; })arg1;
- (void)dealloc;
- (float)progressForFraction:(float)arg1;
- (bool)revealsHorizontalScrollIndicator;
- (bool)revealsVerticalScrollIndicator;
- (void)setProgress:(float)arg1;
- (void)setRevealsHorizontalScrollIndicator:(bool)arg1;
- (void)setRevealsVerticalScrollIndicator:(bool)arg1;

@end
