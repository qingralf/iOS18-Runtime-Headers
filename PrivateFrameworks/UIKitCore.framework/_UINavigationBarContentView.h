/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/UIKitCore.framework/UIKitCore
 */

@interface _UINavigationBarContentView : _UIBarContentView <_UIBarButtonItemViewOwner, _UIButtonBarButtonChangeObserver, _UINavigationBarContentView, _UINavigationBarTitleViewDataSource, _UINavigationBarTransitionContextParticipant> {
    UIDeferredMenuElement * _additionalItems;
    bool  _allowLeadingAlignedLargeTitle;
    _UIPointerInteractionAssistant * _assistant;
    UIAction * _backAction;
    UIBarButtonItem * _backActionItem;
    bool  _backButtonHidden;
    UIBarButtonItem * _backButtonItem;
    double  _backButtonMaximumWidth;
    UIImage * _backIndicatorImage;
    long long  _barMetrics;
    NSArray * _centerBarGroups;
    NSMutableArray * _currentAnimations;
    NSMutableArray * _currentCompletions;
    NSString * _customizationIdentifier;
    _UINavigationItemRenameHandler * _dci_renameHandler;
    <_UINavigationBarContentViewDelegate> * _delegate;
    UIDocumentProperties * _documentProperties;
    NSDictionary * _effectiveTitleAttributes;
    bool  _enableAlternatePopItem;
    UIBarButtonItemGroup * _fixedTrailingGroup;
    bool  _hasLeadingInsetFromTideBar;
    NSArray * _hostedTabBarConstraints;
    UILayoutGuide * _hostedTabBarGuide;
    bool  _hostedTabBarVisibilityAffectsTitle;
    _UIBarButtonItemSearchBarGroup * _inlineSearchBarGroup;
    double  _inlineTitleViewAlpha;
    bool  _isHostedTabBarVisible;
    _UINavigationBarContentViewLayout * _layout;
    NSArray * _leadingBarButtonItems;
    NSArray * _leadingBarGroups;
    bool  _leadingItemsSupplementBackItem;
    UILayoutGuide * _navItemContentLayoutGuide;
    NSArray * _navItemContentLayoutGuideHorizEdgeConstraints;
    NSLayoutConstraint * _navItemContentLayoutGuideStaticButtonVisibilityAnimationConstraint;
    struct { 
        unsigned int deferResolvedSizeChange : 1; 
        unsigned int outstandingDeferredResolvedSizeChange : 1; 
        unsigned int needsStaticNavBarButtonUpdate : 1; 
        unsigned int isHidingLeadingBar : 1; 
        unsigned int isAnimatingNavItemContentLayoutGuideForStaticNavBarButtonVisibility : 1; 
        unsigned int navItemContentLayoutGuideStaticButtonVisibilityAnimationConstraintNeedsInitialization : 1; 
        unsigned int shouldFadeStaticNavBarButton : 1; 
        unsigned int staticNavBarButtonLingers : 1; 
        unsigned int staticNavBarButtonIsActingAsFakeBackButton : 1; 
        unsigned int hostedTabBarEnabled : 1; 
    }  _navigationBarContentViewFlags;
    double  _overrideSize;
    bool  _placeInlineSearchBarInCenter;
    long long  _requestedContentSize;
    bool  _showTitleWithTabBar;
    _UIButtonBarButton * _staticNavBarButton;
    UIBarButtonItem * _staticNavBarButtonItem;
    bool  _staticNavBarButtonTrailing;
    NSArray * _staticNavBarButtonVerticalAlignmentConstraints;
    NSLayoutConstraint * _staticNavBarButtonXPositionConstraint;
    NSString * _title;
    NSDictionary * _titleAttributes;
    id /* block */  _titleMenuProvider;
    struct UIOffset { 
        double horizontal; 
        double vertical; 
    }  _titlePositionAdjustment;
    UIView * _titleView;
    long long  _toolbarStyle;
    NSArray * _trailingBarButtonItems;
    NSArray * _trailingBarGroups;
    _UINavigationBarTransitionContext * _transitionContext;
    bool  _useInlineLargeTitleMetrics;
    <_UINavigationBarContentViewVisualProvider> * _visualProvider;
}

@property (nonatomic, readonly) UIView *_effectiveTitleView;
@property (nonatomic, retain) UIDeferredMenuElement *additionalItems;
@property (nonatomic) bool allowLeadingAlignedLargeTitle;
@property (getter=isAnimatingNavItemContentLayoutGuideForStaticButtonVisibilityChange, nonatomic, readonly) bool animatingNavItemContentLayoutGuideForStaticButtonVisibilityChange;
@property (nonatomic) _UIPointerInteractionAssistant *assistant;
@property (nonatomic, retain) UIAction *backAction;
@property (nonatomic, copy) _UIBarButtonItemData *backButtonAppearance;
@property (nonatomic) bool backButtonHidden;
@property (nonatomic, retain) UIBarButtonItem *backButtonItem;
@property (setter=_setBackButtonMaximumWidth:, nonatomic) double backButtonMaximumWidth;
@property (nonatomic) long long barMetrics;
@property (nonatomic, readonly) double buttonBarMinimumInterGroupSpace;
@property (nonatomic, readonly) double buttonBarMinimumInterItemSpace;
@property (nonatomic, copy) NSArray *centerBarGroups;
@property (nonatomic, readonly) long long currentContentSize;
@property (nonatomic, copy) NSString *customizationIdentifier;
@property (setter=dci_setRenameHandler:, nonatomic, retain) _UINavigationItemRenameHandler *dci_renameHandler;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (nonatomic) <_UINavigationBarContentViewDelegate> *delegate;
@property (readonly, copy) NSString *description;
@property (nonatomic, retain) UIDocumentProperties *documentProperties;
@property (nonatomic, copy) _UIBarButtonItemData *doneItemAppearance;
@property (nonatomic, readonly) NSDictionary *effectiveTitleAttributes;
@property (nonatomic) bool enableAlternatePopItem;
@property (nonatomic, retain) UIBarButtonItemGroup *fixedTrailingGroup;
@property (nonatomic) bool hasLeadingInsetFromTideBar;
@property (readonly) unsigned long long hash;
@property (nonatomic) bool hostedTabBarEnabled;
@property (nonatomic) struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; } hostedTabBarFrame;
@property (nonatomic) bool hostedTabBarVisibilityAffectsTitle;
@property (nonatomic, retain) _UIBarButtonItemSearchBarGroup *inlineSearchBarGroup;
@property (nonatomic) double inlineTitleViewAlpha;
@property (nonatomic) bool isHostedTabBarVisible;
@property (nonatomic, readonly) bool isNavItemContentLayoutGuideFlushLeading;
@property (nonatomic, readonly) bool isNavItemContentLayoutGuideFlushTrailing;
@property (nonatomic, readonly) _UINavigationBarContentViewLayout *layout;
@property (nonatomic, copy) NSArray *leadingBarButtonItems;
@property (nonatomic, copy) NSArray *leadingBarGroups;
@property (nonatomic) bool leadingItemsSupplementBackItem;
@property (nonatomic, readonly) UILayoutGuide *navItemContentLayoutGuide;
@property (nonatomic, readonly) double navItemContentLayoutGuideAnimationDistance;
@property (nonatomic) double overrideSize;
@property (nonatomic) bool placeInlineSearchBarInCenter;
@property (nonatomic, copy) _UIBarButtonItemData *plainItemAppearance;
@property (nonatomic) long long requestedContentSize;
@property (nonatomic) bool shouldFadeStaticNavBarButton;
@property (nonatomic) bool showTitleWithTabBar;
@property (nonatomic, retain) UIBarButtonItem *staticNavBarButtonItem;
@property (nonatomic) bool staticNavBarButtonLingers;
@property (getter=isStaticNavBarButtonTrailing, nonatomic) bool staticNavBarButtonTrailing;
@property (readonly) Class superclass;
@property (nonatomic, copy) NSString *title;
@property (nonatomic, copy) NSDictionary *titleAttributes;
@property (nonatomic, copy) id /* block */ titleMenuProvider;
@property (nonatomic) struct UIOffset { double x1; double x2; } titlePositionAdjustment;
@property (nonatomic, retain) UIView *titleView;
@property (nonatomic) long long toolbarStyle;
@property (nonatomic, copy) NSArray *trailingBarButtonItems;
@property (nonatomic, copy) NSArray *trailingBarGroups;
@property (nonatomic) bool useInlineLargeTitleMetrics;
@property (nonatomic, readonly) <_UINavigationBarContentViewVisualProvider> *visualProvider;
@property (nonatomic) bool wantsHostedTabBarMetrics;

- (void).cxx_destruct;
- (void)__backButtonAction:(id)arg1;
- (id)_accessibility_HUDItemForPoint:(struct CGPoint { double x1; double x2; })arg1;
- (id)_accessibility_barButtonItemAtPoint:(struct CGPoint { double x1; double x2; })arg1;
- (id)_accessibility_controlToActivateForHUDGestureLiftAtPoint:(struct CGPoint { double x1; double x2; })arg1;
- (void)_addCoordinatedAnimation:(id /* block */)arg1 completion:(id /* block */)arg2;
- (void)_appearanceChanged;
- (bool)_backButtonLayoutGuideIncludesFullyHiddenConstant;
- (void)_clearAllAnimations;
- (void)_clearAssistants;
- (void)_configureBackButton:(id)arg1;
- (double)_contentLayoutGuideAnimationConstraintConstantForFullyHiddenButton;
- (double)_contentLayoutGuideAnimationConstraintConstantForFullyVisibleButton;
- (long long)_currentContentSize;
- (bool)_effectiveStaticNavBarButtonLingers;
- (id)_effectiveTitleView;
- (long long)_effectiveToolbarStyle;
- (void)_ensureHostedTabBarConstraintsWithUpdateBlock:(id /* block */)arg1;
- (void)_intrinsicContentSizeInvalidatedForChildView:(id)arg1;
- (double)_intrinsicHeight;
- (bool)_isStaticNavBarButtonOnRight;
- (bool)_item:(id)arg1 addSymbolEffect:(id)arg2 options:(id)arg3 animated:(bool)arg4;
- (void)_item:(id)arg1 applyContentTransition:(id)arg2 options:(id)arg3;
- (bool)_item:(id)arg1 removeAllSymbolEffectsWithOptions:(id)arg2 animated:(bool)arg3;
- (bool)_item:(id)arg1 removeSymbolEffectOfType:(id)arg2 options:(id)arg3 animated:(bool)arg4;
- (void)_itemCustomViewDidChange:(id)arg1 fromView:(id)arg2;
- (void)_itemDidChangeEnabledState:(id)arg1;
- (void)_itemDidChangeHiddenState:(id)arg1;
- (void)_itemDidChangeSelectionState:(id)arg1;
- (void)_itemDidChangeWidth:(id)arg1;
- (void)_itemStandardViewNeedsUpdate:(id)arg1;
- (id)_layoutForAugmentedTitleView:(id)arg1;
- (id)_newLayout;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_overlayRectForFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg1 inView:(id)arg2 inTargetView:(id)arg3;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_overlayRectForGuide:(id)arg1 inTargetView:(id)arg2;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })_overlayRectForView:(id)arg1 inTargetView:(id)arg2;
- (double)_resolvedInlineTitleViewAlpha;
- (void)_runAllScheduledAnimations:(bool)arg1;
- (void)_safeAreaInsetsDidChangeFromOldInsets:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)_setAssistants;
- (void)_setBackButtonMaximumWidth:(double)arg1;
- (void)_setDirectionalLayoutMargins:(struct NSDirectionalEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)_setupBackButton;
- (void)_setupCenterButtonBar;
- (void)_setupLeadingButtonBarAnimated:(bool)arg1;
- (void)_setupStaticNavBarButtonAnimated:(bool)arg1;
- (void)_setupTitleView;
- (void)_setupTrailingButtonBarAnimated:(bool)arg1;
- (bool)_showTitleWithTabBar;
- (id)_traitCollectionForChildEnvironment:(id)arg1;
- (void)_updateLayoutMarginsForLayout:(id)arg1;
- (void)_updateNavItemContentLayoutGuideEdgeConstraints;
- (void)_updateStaticNavBarButtonVerticalAlignmentConstraints;
- (void)_updateTitleAlignment;
- (bool)_useLeadingAlignedTitle;
- (id)additionalItems;
- (void)adoptLayout:(id)arg1;
- (void)adoptNewLayout;
- (bool)allowLeadingAlignedLargeTitle;
- (id)assistant;
- (id)backAction;
- (id)backButtonAppearance;
- (bool)backButtonHidden;
- (id)backButtonItem;
- (double)backButtonMargin;
- (double)backButtonMaximumWidth;
- (void)backButtonTitleDidChange;
- (id)backIndicatorImage;
- (long long)barMetrics;
- (long long)barType;
- (void)beginAnimatingNavItemContentLayoutGuideForStaticButtonVisibilityChange;
- (double)buttonBarMinimumInterGroupSpace;
- (double)buttonBarMinimumInterItemSpace;
- (bool)canPerformAction:(SEL)arg1 withSender:(id)arg2;
- (id)centerBarGroups;
- (void)clearTransitionContext;
- (bool)compactMetrics;
- (long long)currentContentSize;
- (id)customizationIdentifier;
- (id)dci_renameHandler;
- (void)dci_setRenameHandler:(id)arg1;
- (double)defaultEdgeSpacing;
- (id)delegate;
- (id)description;
- (id)documentProperties;
- (id)doneItemAppearance;
- (unsigned long long)edgesPaddingBarButtonItem:(id)arg1;
- (long long)effectiveStyleForStyle:(long long)arg1;
- (id)effectiveTitleAttributes;
- (bool)enableAlternatePopItem;
- (void)endAnimatingNavItemContentLayoutGuideForStaticButtonVisibilityChange;
- (void)ensureBackButtonTruncationOccursWithContext:(id)arg1;
- (void)finalizeStateFromTransition:(id)arg1;
- (id)fixedTrailingGroup;
- (bool)hasLeadingInsetFromTideBar;
- (bool)hostedTabBarEnabled;
- (struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })hostedTabBarFrame;
- (bool)hostedTabBarVisibilityAffectsTitle;
- (id)initWithFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg1 visualProvider:(id)arg2;
- (id)inlineSearchBarGroup;
- (double)inlineTitleViewAlpha;
- (struct CGSize { double x1; double x2; })intrinsicContentSize;
- (bool)isAnimatingNavItemContentLayoutGuideForStaticButtonVisibilityChange;
- (bool)isHostedTabBarVisible;
- (bool)isNavItemContentLayoutGuideFlushLeading;
- (bool)isNavItemContentLayoutGuideFlushTrailing;
- (bool)isRTL;
- (bool)isStaticNavBarButtonTrailing;
- (id)layout;
- (void)layoutMarginsDidChange;
- (void)layoutSubviews;
- (id)leadingBarButtonItems;
- (id)leadingBarGroups;
- (bool)leadingItemsSupplementBackItem;
- (id)navItemContentLayoutGuide;
- (double)navItemContentLayoutGuideAnimationDistance;
- (double)overrideSize;
- (bool)placeInlineSearchBarInCenter;
- (id)plainItemAppearance;
- (void)prepareToRecordToState:(id)arg1;
- (void)recordFromStateForTransition:(id)arg1;
- (void)recordToStateForTransition:(id)arg1;
- (void)relinquishLayout;
- (long long)requestedContentSize;
- (void)resolvedHeightDidChange;
- (void)setAdditionalItems:(id)arg1;
- (void)setAllowLeadingAlignedLargeTitle:(bool)arg1;
- (void)setAssistant:(id)arg1;
- (void)setBackAction:(id)arg1;
- (void)setBackButtonAppearance:(id)arg1;
- (void)setBackButtonHidden:(bool)arg1;
- (void)setBackButtonItem:(id)arg1;
- (void)setBackIndicatorImage:(id)arg1;
- (void)setBarMetrics:(long long)arg1;
- (void)setCenterBarGroups:(id)arg1;
- (void)setCustomizationIdentifier:(id)arg1;
- (void)setDelegate:(id)arg1;
- (void)setDirectionalLayoutMargins:(struct NSDirectionalEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)setDocumentProperties:(id)arg1;
- (void)setDoneItemAppearance:(id)arg1;
- (void)setEnableAlternatePopItem:(bool)arg1;
- (void)setFixedTrailingGroup:(id)arg1;
- (void)setHasLeadingInsetFromTideBar:(bool)arg1;
- (void)setHostedTabBarEnabled:(bool)arg1;
- (void)setHostedTabBarFrame:(struct CGRect { struct CGPoint { double x_1_1_1; double x_1_1_2; } x1; struct CGSize { double x_2_1_1; double x_2_1_2; } x2; })arg1;
- (void)setHostedTabBarVisibilityAffectsTitle:(bool)arg1;
- (void)setInlineSearchBarGroup:(id)arg1;
- (void)setInlineTitleViewAlpha:(double)arg1;
- (void)setIsHostedTabBarVisible:(bool)arg1;
- (void)setLayoutMargins:(struct UIEdgeInsets { double x1; double x2; double x3; double x4; })arg1;
- (void)setLeadingBarButtonItems:(id)arg1;
- (void)setLeadingBarGroups:(id)arg1;
- (void)setLeadingItemsSupplementBackItem:(bool)arg1;
- (void)setNeedsStaticNavBarButtonUpdate;
- (void)setOverrideSize:(double)arg1;
- (void)setPlaceInlineSearchBarInCenter:(bool)arg1;
- (void)setPlainItemAppearance:(id)arg1;
- (void)setRequestedContentSize:(long long)arg1;
- (void)setShouldFadeStaticNavBarButton:(bool)arg1;
- (void)setShowTitleWithTabBar:(bool)arg1;
- (void)setStaticNavBarButtonItem:(id)arg1;
- (void)setStaticNavBarButtonLingers:(bool)arg1;
- (void)setStaticNavBarButtonTrailing:(bool)arg1;
- (void)setTitle:(id)arg1;
- (void)setTitleAttributes:(id)arg1;
- (void)setTitleMenuProvider:(id /* block */)arg1;
- (void)setTitlePositionAdjustment:(struct UIOffset { double x1; double x2; })arg1;
- (void)setTitleView:(id)arg1;
- (void)setToolbarStyle:(long long)arg1;
- (void)setTrailingBarButtonItems:(id)arg1;
- (void)setTrailingBarGroups:(id)arg1;
- (void)setUseInlineLargeTitleMetrics:(bool)arg1;
- (void)setWantsHostedTabBarMetrics:(bool)arg1;
- (bool)shouldFadeStaticNavBarButton;
- (bool)showTitleWithTabBar;
- (struct CGSize { double x1; double x2; })sizeThatFits:(struct CGSize { double x1; double x2; })arg1;
- (id)staticNavBarButtonItem;
- (bool)staticNavBarButtonLingers;
- (void)tintColorDidChange;
- (id)title;
- (id)titleAttributes;
- (id /* block */)titleMenuProvider;
- (struct UIOffset { double x1; double x2; })titlePositionAdjustment;
- (id)titleView;
- (void)titleView:(id)arg1 needsUpdatedContentOverlayRects:(id)arg2;
- (id)titleViewBackButtonMenu:(id)arg1;
- (void)titleViewChangedDisplayItemAlpha:(id)arg1;
- (void)titleViewChangedHeight:(id)arg1;
- (void)titleViewChangedLayout:(id)arg1;
- (void)titleViewChangedMaximumBackButtonWidth:(id)arg1;
- (void)titleViewChangedPreferredDisplaySize:(id)arg1;
- (void)titleViewChangedStandardDisplayItems:(id)arg1;
- (void)titleViewChangedTabBarSizingDisposition:(id)arg1;
- (double)titleViewContentBaseHeight:(id)arg1;
- (double)titleViewContentBaselineOffsetFromTop:(id)arg1;
- (double)titleViewFloatingTabBarHeight:(id)arg1;
- (double)titleViewLargeTitleHeight:(id)arg1;
- (struct { double x1; double x2; })titleViewLargeTitleHeightRange:(id)arg1;
- (long long)toolbarStyle;
- (id)trailingBarButtonItems;
- (id)trailingBarGroups;
- (void)traitCollectionDidChange:(id)arg1;
- (void)updateAccessibilityContents:(id)arg1;
- (void)updateAugmentedTitleViewNavigationBarTraitsTo:(id)arg1 from:(id)arg2;
- (void)updateContentAnimated:(bool)arg1;
- (bool)updateNavItemContentLayoutGuideAnimationConstraintConstant:(double)arg1;
- (bool)useInlineLargeTitleMetrics;
- (id)visualProvider;
- (bool)wantsHostedTabBarMetrics;

@end
