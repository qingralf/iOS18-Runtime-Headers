/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/VisualVoicemail.framework/VisualVoicemail
 */

@interface VMVoicemail : NSObject <NSCopying, NSMutableCopying, NSSecureCoding> {
    NSString * _callbackDestinationID;
    NSString * _callbackISOCountryCode;
    NSURL * _dataURL;
    NSDate * _date;
    double  _duration;
    unsigned long long  _flags;
    unsigned long long  _identifier;
    NSString * _receiverDestinationID;
    NSString * _receiverISOCountryCode;
    NSString * _receiverLabelID;
    unsigned long long  _remoteUID;
    NSString * _senderDestinationID;
    NSString * _senderISOCountryCode;
    NSDictionary * _summarizationMetaData;
    NSURL * _summarizationMetaDataURL;
    NSURL * _transcriptionURL;
    NSUUID * _uuid;
}

@property (getter=isBlocked, nonatomic, readonly) bool blocked;
@property (getter=isBlocked, nonatomic) bool blocked;
@property (nonatomic, retain) NSString *callbackDestinationID;
@property (nonatomic, retain) NSString *callbackISOCountryCode;
@property (nonatomic, readonly) NSString *callbackNumber;
@property (nonatomic, readonly) NSData *data;
@property (getter=isDataAvailable, nonatomic, readonly) bool dataAvailable;
@property (getter=isDataAvailable, nonatomic) bool dataAvailable;
@property (nonatomic, readonly) NSString *dataPath;
@property (nonatomic, retain) NSURL *dataURL;
@property (nonatomic, retain) NSDate *date;
@property (getter=isDeleted, nonatomic, readonly) bool deleted;
@property (getter=isDeleted, nonatomic) bool deleted;
@property (getter=isDetached, nonatomic, readonly) bool detached;
@property (getter=isDetached, nonatomic) bool detached;
@property (getter=isDownloading, nonatomic, readonly) bool downloading;
@property (getter=isDownloading, nonatomic) bool downloading;
@property (nonatomic) double duration;
@property (nonatomic) unsigned long long flags;
@property (nonatomic, readonly) bool hasCallbackNumber;
@property (nonatomic) unsigned long long identifier;
@property (nonatomic) long long mailboxType;
@property (getter=isRead, nonatomic) bool read;
@property (nonatomic, retain) NSString *receiverDestinationID;
@property (nonatomic, retain) NSString *receiverISOCountryCode;
@property (nonatomic, retain) NSString *receiverLabelID;
@property (nonatomic) unsigned long long remoteUID;
@property (nonatomic, readonly) NSString *sender;
@property (nonatomic, retain) NSString *senderDestinationID;
@property (nonatomic, retain) NSString *senderISOCountryCode;
@property (nonatomic, retain) NSDictionary *summarizationMetaData;
@property (nonatomic, retain) NSURL *summarizationMetaDataURL;
@property (getter=isTemporary, nonatomic, readonly) bool temporary;
@property (getter=isTemporary, nonatomic) bool temporary;
@property (nonatomic, readonly, retain) VMVoicemailTranscript *transcript;
@property (getter=isTranscriptionAvailable, nonatomic) bool transcriptionAvailable;
@property (getter=isTranscriptionRated, nonatomic, readonly) bool transcriptionRated;
@property (nonatomic, readonly) unsigned long long transcriptionState;
@property (nonatomic, retain) NSURL *transcriptionURL;
@property (getter=isTrashed, nonatomic, readonly) bool trashed;
@property (getter=isTrashed, nonatomic) bool trashed;
@property (getter=isUnread, nonatomic, readonly) bool unread;
@property (nonatomic, retain) NSUUID *uuid;

+ (unsigned long long)flagsByApplyingMailboxType:(long long)arg1 toFlags:(unsigned long long)arg2;
+ (long long)mailboxTypeForFlags:(unsigned long long)arg1;
+ (bool)supportsSecureCoding;

- (void).cxx_destruct;
- (id)callbackDestinationID;
- (id)callbackISOCountryCode;
- (id)callbackNumber;
- (id)contactUsingContactStore:(id)arg1;
- (id)contactUsingContactStore:(id)arg1 withKeysToFetch:(id)arg2;
- (id)copyWithZone:(struct _NSZone { }*)arg1;
- (id)data;
- (id)dataPath;
- (id)dataURL;
- (id)date;
- (id)description;
- (id)displayLabelUsingContactStore:(id)arg1;
- (id)displayNameUsingContactStore:(id)arg1;
- (bool)doesNotHaveFlags:(unsigned long long)arg1;
- (double)duration;
- (void)encodeWithCoder:(id)arg1;
- (id)flagDescription;
- (unsigned long long)flags;
- (bool)hasCallbackNumber;
- (bool)hasFlags:(unsigned long long)arg1;
- (bool)hasSameContent:(id)arg1;
- (bool)hasSameFlags:(id)arg1;
- (unsigned long long)hash;
- (unsigned long long)identifier;
- (id)initWithCoder:(id)arg1;
- (id)initWithData:(id)arg1;
- (id)initWithMessage:(id)arg1;
- (id)initWithVoicemailMessage:(id)arg1;
- (bool)isBlocked;
- (bool)isContactSuggested:(id)arg1;
- (bool)isDataAvailable;
- (bool)isDeleted;
- (bool)isDetached;
- (bool)isDownloading;
- (bool)isEqual:(id)arg1;
- (bool)isRead;
- (bool)isSameMessage:(id)arg1;
- (bool)isTemporary;
- (bool)isTranscribing;
- (bool)isTranscriptionAvailable;
- (bool)isTranscriptionDeferred;
- (bool)isTranscriptionNotSupported;
- (bool)isTranscriptionRated;
- (bool)isTrashed;
- (bool)isUnread;
- (long long)mailboxType;
- (id)mailboxTypeDescription;
- (id)mutableCopyWithZone:(struct _NSZone { }*)arg1;
- (id)receiverDestinationID;
- (id)receiverISOCountryCode;
- (id)receiverLabelID;
- (unsigned long long)remoteUID;
- (id)sender;
- (id)senderDestinationID;
- (id)senderISOCountryCode;
- (void)setBlocked:(bool)arg1;
- (void)setCallbackDestinationID:(id)arg1;
- (void)setCallbackISOCountryCode:(id)arg1;
- (void)setDataAvailable:(bool)arg1;
- (void)setDataURL:(id)arg1;
- (void)setDate:(id)arg1;
- (void)setDeleted:(bool)arg1;
- (void)setDetached:(bool)arg1;
- (void)setDownloading:(bool)arg1;
- (void)setDuration:(double)arg1;
- (void)setFlag:(unsigned long long)arg1 enabled:(bool)arg2;
- (void)setFlags:(unsigned long long)arg1;
- (void)setIdentifier:(unsigned long long)arg1;
- (void)setMailboxType:(long long)arg1;
- (void)setRead:(bool)arg1;
- (void)setReceiverDestinationID:(id)arg1;
- (void)setReceiverISOCountryCode:(id)arg1;
- (void)setReceiverLabelID:(id)arg1;
- (void)setRemoteUID:(unsigned long long)arg1;
- (void)setSenderDestinationID:(id)arg1;
- (void)setSenderISOCountryCode:(id)arg1;
- (void)setSummarizationMetaData:(id)arg1;
- (void)setSummarizationMetaDataURL:(id)arg1;
- (void)setTemporary:(bool)arg1;
- (void)setTranscriptionAvailable:(bool)arg1;
- (void)setTranscriptionURL:(id)arg1;
- (void)setTrashed:(bool)arg1;
- (void)setUuid:(id)arg1;
- (id)summarizationMetaData;
- (id)summarizationMetaDataURL;
- (id)transcript;
- (unsigned long long)transcriptionState;
- (id)transcriptionURL;
- (id)uuid;
- (bool)wasTranscriptionAttempted;

@end
