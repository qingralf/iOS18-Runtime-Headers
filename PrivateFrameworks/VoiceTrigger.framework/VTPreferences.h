/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/VoiceTrigger.framework/VoiceTrigger
 */

@interface VTPreferences : NSObject

+ (id)deviceCategoryStringRepresentationForCategoryType:(unsigned long long)arg1;
+ (id)sharedPreferences;

- (id)_VTSATBasePath;
- (id)_VTSATCachePath;
- (unsigned long long)_getCSDeviceTypeWithPhraseDeviceType:(unsigned long long)arg1;
- (id)_getSATEnrollmentAudioPathForLanguageCodeForLegacyVoiceProfile:(id)arg1;
- (id)_getVTEnableLoggingMetadata;
- (id)_getVoiceTriggerEnabledDidChangeNotificationStringWithDeviceType:(unsigned long long)arg1;
- (id)_getVoiceTriggerEnabledKeyWithDeviceType:(unsigned long long)arg1;
- (bool)_gibraltarHasBuiltInMic;
- (bool)_isLocalVoiceTriggerAvailable;
- (bool)_isNonEndpointDeviceType:(unsigned long long)arg1;
- (bool)_isSATMarkedForMarker:(id)arg1 languageCode:(id)arg2;
- (id)_languageCode;
- (id)_localeIdentifier;
- (void)_logMetadataForVTEnabled:(bool)arg1;
- (id)_preferencesPhraseDeviceStringWithPhraseDeviceType:(unsigned long long)arg1;
- (void)_setCanUseVoiceTriggerDuringPhoneCall:(bool)arg1 sender:(id)arg2;
- (void)_setVoiceTriggerEnabled:(bool)arg1 sender:(id)arg2 deviceType:(unsigned long long)arg3;
- (void)_setVoiceTriggerEnabled:(bool)arg1 sender:(id)arg2 deviceType:(unsigned long long)arg3 endpointId:(id)arg4;
- (bool)_storeModeEnabled;
- (bool)_voiceTriggerEnabledWithDeviceType:(unsigned long long)arg1;
- (bool)_voiceTriggerEnabledWithDeviceType:(unsigned long long)arg1 endpointId:(id)arg2;
- (id)audioInjectionFilePath;
- (bool)canUseVoiceTriggerDuringPhoneCall;
- (unsigned long long)canUseVoiceTriggerDuringPhoneCallWithState;
- (bool)corespeechDaemonEnabled;
- (id)devicesWithVoiceProfileIniCloudForLanguage:(id)arg1;
- (void)discardAllSATEnrollment;
- (void)discardSATEnrollmentForLanguageCode:(id)arg1;
- (void)enableVoiceTriggerUponVoiceProfileSyncForLanguage:(id)arg1;
- (bool)everNotifyUserCompactTrigger;
- (bool)fileLoggingIsEnabled;
- (id)fileLoggingLevel;
- (bool)gestureSubscriptionEnabled;
- (id)getMD5HashForSATEnrollmentAudioForLanguageCode:(id)arg1;
- (id)getPreferencesForEndpoint:(id)arg1 fromPreferenceDomain:(id)arg2 error:(id*)arg3;
- (id)getSATEnrollmentPath;
- (unsigned long long)getUserPreferredVoiceTriggerPhraseTypeForDeviceType:(unsigned long long)arg1 endpointId:(id)arg2 error:(id*)arg3;
- (bool)hasExplicitlySetVoiceTriggerEnabled;
- (bool)hasVoiceProfileIniCloudForLanguageCode:(id)arg1;
- (id)init;
- (bool)isCompactVoiceTriggerAvailableForLanguageCode:(id)arg1;
- (bool)isCompactVoiceTriggerAvailableForLanguageCode:(id)arg1 forPhraseDeviceType:(unsigned long long)arg2;
- (bool)isMphVTEnabled;
- (bool)isRemoteVoiceTriggerAvailable;
- (bool)isSATAvailable;
- (bool)isSATEnrolledForLanguageCode:(id)arg1;
- (bool)isSATEnrollmentMigratedForLanguageCode:(id)arg1;
- (bool)isVoiceTriggerAvailable;
- (id)localizedCompactTriggerPhraseForLanguageCode:(id)arg1;
- (id)localizedTriggerPhrase;
- (id)localizedTriggerPhraseForLanguageCode:(id)arg1;
- (void)markSATEnrollmentSuccessForLanguageCode:(id)arg1;
- (bool)onetimeRemoteAssetQueryRanSuccessfully;
- (bool)overwritingVoiceTriggerMLock;
- (bool)phraseSpotterEnabled;
- (bool)remoteDarwinWasEverConnected;
- (bool)secondPassAudioLoggingEnabled;
- (void)setCanUseVoiceTriggerDuringPhoneCall:(bool)arg1;
- (void)setEverNotifyUserCompactTrigger;
- (void)setFileLoggingIsEnabled:(bool)arg1;
- (void)setFileLoggingLevel:(id)arg1;
- (void)setOnetimeRemoteAssetQueryRanSuccessfully:(bool)arg1;
- (void)setPhraseSpotterEnabled:(bool)arg1;
- (void)setPhraseSpotterEnabled:(bool)arg1 sender:(id)arg2;
- (id)setPreferences:(id)arg1 forEndpoint:(id)arg2 inPreferenceDomain:(id)arg3;
- (void)setSecondPassAudioLoggingEnabled:(bool)arg1;
- (id)setUserPreferredVoiceTriggerPhraseType:(unsigned long long)arg1 sender:(id)arg2 deviceType:(unsigned long long)arg3 endpointId:(id)arg4;
- (void)setVoiceTriggerEnabled:(bool)arg1;
- (void)setVoiceTriggerEnabled:(bool)arg1 sender:(id)arg2;
- (void)setVoiceTriggerEnabled:(bool)arg1 sender:(id)arg2 deviceType:(unsigned long long)arg3 endpointId:(id)arg4;
- (void)setVoiceTriggerEnabled:(bool)arg1 sender:(id)arg2 endpointId:(id)arg3;
- (void)setVoiceTriggerEnabledWhenChargerConnected:(bool)arg1;
- (void)setVoiceTriggerEnabledWhenChargerDisconnected:(bool)arg1;
- (bool)shouldOverwriteVoiceTriggerMLock;
- (void)synchronize;
- (bool)useSiriActivationSPIForiOS;
- (bool)useSiriActivationSPIForwatchOS;
- (bool)voiceTriggerEnabled;
- (bool)voiceTriggerEnabledForEndpointId:(id)arg1;
- (bool)voiceTriggerEnabledWhenChargerConnected;
- (bool)voiceTriggerEnabledWhenChargerDisconnected;
- (bool)voiceTriggerEnabledWithDeviceType:(unsigned long long)arg1 endpointId:(id)arg2;
- (bool)voiceTriggerInCoreSpeech;
- (bool)voiceTriggerWasEverUsed;

@end
