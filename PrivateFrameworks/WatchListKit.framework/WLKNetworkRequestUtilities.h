/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/WatchListKit.framework/WatchListKit
 */

@interface WLKNetworkRequestUtilities : NSObject

+ (id)_defaultAppSessionConfiguration;
+ (id)_prepareEncoder:(id)arg1 account:(id)arg2 sessionConfiguration:(id)arg3 options:(long long)arg4 error:(id*)arg5;
+ (id)_sharedCacheSessionConfiguration;
+ (id)configurationBaseURLString;
+ (id)defaultSessionConfiguration;
+ (bool)isGDPRAccepted;
+ (void)startNetworkRequest:(id)arg1 account:(id)arg2 sessionConfiguration:(id)arg3 options:(long long)arg4 activity:(id)arg5 completion:(id /* block */)arg6;
+ (void)startNetworkRequest:(id)arg1 account:(id)arg2 sessionConfiguration:(id)arg3 options:(long long)arg4 completion:(id /* block */)arg5;

@end
