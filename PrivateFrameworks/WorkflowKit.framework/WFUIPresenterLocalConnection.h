/* Generated by RuntimeBrowser
   Image: /System/Library/PrivateFrameworks/WorkflowKit.framework/WorkflowKit
 */

@interface WFUIPresenterLocalConnection : NSObject <WFUIPresenterConnection> {
    <WFUIPresenterInterface> * _presenter;
}

@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (readonly) unsigned long long hash;
@property (nonatomic) <WFUIPresenterHostInterface> *host; /* unknown property attribute: ? */
@property (nonatomic, copy) id /* block */ interruptionHandler; /* unknown property attribute: ? */
@property (nonatomic, readonly) bool isConnected; /* unknown property attribute: ? */
@property (nonatomic, readonly) <WFUIPresenterInterface> *presenter;
@property (readonly) Class superclass;

- (void).cxx_destruct;
- (id)initWithPresenter:(id)arg1;
- (bool)isConnected;
- (id)presenter;
- (id)presenterWithErrorHandler:(id /* block */)arg1;
- (id)synchronousPresenterWithErrorHandler:(id /* block */)arg1;

@end
