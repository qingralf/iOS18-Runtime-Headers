/* Generated by RuntimeBrowser
   Image: /usr/lib/libacmobileshim.dylib
 */

@interface ACCAuthContext : NSObject <ACCAuthContextProtocol> {
    NSNumber * _appID;
    NSString * _appIDKey;
    NSNumber * _authenticationType;
    NSNumber * _clientSecretCreateDate;
    NSString * _clientSecretTokenHmac;
    NSDate * _creationDate;
    NSString * _currentUserKey;
    NSString * _dsKeyVersion;
    NSString * _encryptionHash;
    NSData * _encryptionKey;
    NSDate * _expirationDate;
    NSData * _initializationVector;
    NSString * _nonce;
    NSString * _oldUserKey;
    NSNumber * _personID;
    ACFPrincipal * _principal;
    NSString * _recoveryKey;
    ACFMessage * _request;
    NSString * _selectedDeviceId;
    NSString * _selectedDeviceType;
    NSString * _serviceName;
    NSString * _sessionToken;
    NSString * _tokenVersion;
    NSNumber * _touchIDSupport;
    NSString * _twoStepVerificationCode;
}

@property (retain) NSNumber *appID;
@property (retain) NSString *appIDKey;
@property NSNumber *authenticationType;
@property (nonatomic, retain) NSNumber *clientSecretCreateDate; /* unknown property attribute: ? */
@property (nonatomic, retain) NSString *clientSecretTokenHmac; /* unknown property attribute: ? */
@property (retain) NSDate *creationDate;
@property (retain) NSString *currentUserKey;
@property (readonly, copy) NSString *debugDescription; /* unknown property attribute: ? */
@property (readonly, copy) NSString *description;
@property (retain) NSString *dsKeyVersion;
@property (retain) NSString *encryptionHash;
@property (nonatomic, retain) NSData *encryptionKey;
@property (retain) NSDate *expirationDate;
@property (readonly) unsigned long long hash;
@property (nonatomic, retain) NSData *initializationVector;
@property (retain) NSString *nonce;
@property (retain) NSString *oldUserKey;
@property (nonatomic, readonly) NSMutableDictionary *parametersDictionary;
@property (nonatomic, retain) NSNumber *personID;
@property (retain) ACFPrincipal *principal;
@property (nonatomic, retain) NSString *recoveryKey; /* unknown property attribute: ? */
@property (retain) ACFMessage *request;
@property (nonatomic, retain) NSString *selectedDeviceId;
@property (nonatomic, retain) NSString *selectedDeviceType;
@property (nonatomic, retain) NSString *serviceName; /* unknown property attribute: ? */
@property (retain) NSString *sessionToken;
@property (readonly) Class superclass;
@property (nonatomic, readonly) NSString *tokenSourceString;
@property (retain) NSString *tokenVersion;
@property (nonatomic, retain) NSNumber *touchIDSupport; /* unknown property attribute: ? */
@property (nonatomic, retain) NSString *twoStepVerificationCode;
@property (nonatomic, readonly) NSString *xmlTokenSourceString; /* unknown property attribute: ? */

+ (id)authContextWithRequest:(id)arg1;

- (id)appID;
- (id)appIDKey;
- (id)authenticationType;
- (id)clientSecretCreateDate;
- (id)clientSecretTokenHmac;
- (id)creationDate;
- (id)currentUserKey;
- (void)dealloc;
- (id)dsKeyVersion;
- (id)encryptionHash;
- (id)encryptionKey;
- (id)expirationDate;
- (id)initWithRequest:(id)arg1;
- (id)initializationVector;
- (id)nonce;
- (id)oldUserKey;
- (id)parametersDictionary;
- (id)personID;
- (id)principal;
- (id)recoveryKey;
- (id)request;
- (id)selectedDeviceId;
- (id)selectedDeviceType;
- (id)serviceName;
- (id)sessionToken;
- (void)setAppID:(id)arg1;
- (void)setAppIDKey:(id)arg1;
- (void)setAuthenticationType:(id)arg1;
- (void)setClientSecretCreateDate:(id)arg1;
- (void)setClientSecretTokenHmac:(id)arg1;
- (void)setCreationDate:(id)arg1;
- (void)setCurrentUserKey:(id)arg1;
- (void)setDsKeyVersion:(id)arg1;
- (void)setEncryptionHash:(id)arg1;
- (void)setEncryptionKey:(id)arg1;
- (void)setExpirationDate:(id)arg1;
- (void)setInitializationVector:(id)arg1;
- (void)setNonce:(id)arg1;
- (void)setOldUserKey:(id)arg1;
- (void)setPersonID:(id)arg1;
- (void)setPrincipal:(id)arg1;
- (void)setRecoveryKey:(id)arg1;
- (void)setRequest:(id)arg1;
- (void)setSelectedDeviceId:(id)arg1;
- (void)setSelectedDeviceType:(id)arg1;
- (void)setServiceName:(id)arg1;
- (void)setSessionToken:(id)arg1;
- (void)setTokenVersion:(id)arg1;
- (void)setTouchIDSupport:(id)arg1;
- (void)setTwoStepVerificationCode:(id)arg1;
- (id)strippedPropertyListString:(id)arg1;
- (id)tokenSourceString;
- (id)tokenVersion;
- (id)touchIDSupport;
- (id)twoStepVerificationCode;
- (id)xmlTokenSourceString;

@end
