/* Generated by RuntimeBrowser.
 */

@protocol CDPRemoteDeviceSecretValidatorProtocolInternal <NSObject>

@required

- (void)approveFromAnotherDeviceWithCompletion:(void *)arg1; // needs 1 arg types, found 7: id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, NSError *, void*
- (void)cancelApproveFromAnotherDevice;
- (void)cancelValidationWithError:(NSError *)arg1;
- (void)resetAccountCDPStateWithEscapeOptionsOffered:(unsigned long long)arg1;
- (void)resetAccountCDPStateWithEscapeOptionsOffered:(unsigned long long)arg1 andSetSecret:(CDPLocalSecret *)arg2;
- (void)setSupportedEscapeOfferMask:(unsigned long long)arg1;
- (unsigned long long)supportedEscapeOfferMask;
- (void)supportedEscapeOfferMaskCompletion:(void *)arg1; // needs 1 arg types, found 6: id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, unsigned long long, void*
- (void)validateCustodianRecoveryInfo:(void *)arg1 withCompletion:(void *)arg2; // needs 2 arg types, found 8: CDPCustodianRecoveryInfo *, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, NSError *, void*
- (void)validateRecoveryKey:(void *)arg1 withCompletion:(void *)arg2; // needs 2 arg types, found 9: NSString *, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, bool, NSError *, void*
- (void)validateSecret:(void *)arg1 devices:(void *)arg2 type:(void *)arg3 withCompletion:(void *)arg4; // needs 4 arg types, found 11: NSString *, NSArray *, unsigned long long, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, bool, NSError *, void*

@optional

- (void)attemptToJoinCircleWithPiggybacking:(void *)arg1 withCompletion:(void *)arg2; // needs 2 arg types, found 8: CDPContext *, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, NSError *, void*
- (void)repairEDPStateWithContext:(void *)arg1 completion:(void *)arg2; // needs 2 arg types, found 9: CDPContext *, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, bool, NSError *, void*
- (void)validateEDPRecoveryToken:(void *)arg1 withContext:(void *)arg2 completion:(void *)arg3; // needs 3 arg types, found 9: NSString *, CDPContext *, id /* block */, /* Warning: Unrecognized filer type: '<' using 'void*' */ void*, void, id /* block */, bool, NSError *, void*

@end
